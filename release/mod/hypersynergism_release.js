...
`):n=e.html),S.injectStyle(`#${a} {
            display: flex;
            ${p.objectToCSS(e.styles)}
        }`),`<div class="hs-panel-div ${t}" id="${a}"${o} ${i}>${n}</div>`}static _modal(e){let t=e.class??"",a=e.htmlContent??"",n=e.data??[],o=e.title??"",i=e.id??p.domid(),r=this.#i(e.props),l=this.#t(e.data);return S.injectStyle(`#${i} {
            ${p.objectToCSS(e.styles)}
...
@-webkit-keyframes hs-animtext-1 {
    ...
    `).join(""),n={htmlContent:`
        ...
        `,title:"Configure Corruption Loadout"},o=await s.Modal(n);setTimeout(()=>{document.getElementById("hs-corruption-save-btn")?.addEventListener("click",()=>{Object.keys(fe).forEach(i=>{let r=document.getElementById(`hs-corruption-${i}`),l=Math.max(0,Math.min(16,Number(r?.value)||0));e[i]=l}),S.removeInjectedStyle("hs-corruption-modal-styles"),s.CloseModal(o)})},0)}async function Mt(s,e,t,a){let n="hs-autosing-challenges-modal",o=[...e],i=x=>ie.find(I=>I.value===x)?.label??null,r=x=>x.challengeNumber===200&&x.ifJump!==void 0,l=x=>`${x.toLocaleString()}ms`,u=(x,I)=>`
    ...
    `,g=(x,I)=>{let R=i(x.challengeNumber),B=!!R,D=B?`<strong>${R}</strong>`:`Challenge ${x.challengeNumber}
     (${x.challengeCompletions} completions)`;return`
        ...
        `},m=()=>{let x=[],I=new Map;o.forEach((B,D)=>{if(r(B)){let G=B.ifJump.ifJumpIndex;I.has(G)||I.set(G,[]),I.get(G).push({ifIndex:D,ifId:B.ifJump.id})}}),o.forEach((B,D)=>{I.has(D)&&I.get(D).forEach(G=>{let Z=o[G.ifIndex];x.push(d(G.ifIndex,Z.ifJump.id))}),r(B)?x.push(u(B,D)):x.push(g(B,D))});let R=o.length;return I.has(R)&&I.get(R).forEach(B=>{let D=o[B.ifIndex];x.push(d(B.ifIndex,D.ifJump.id))}),x.join("")},h=()=>{let x=document.getElementById("hs-challenge-list-container");x&&(x.innerHTML=m(),F(),Q())},A={htmlContent:`
    <div id="${n}" class="hs-challenges-modal-container">
        ...
        `,title:o?`Edit Strategy Phase ${n.startPhase}-${n.endPhase}`:"Create Strategy Phase"},h=await s.Modal(m);setTimeout(()=>{let A=document.querySelector(".hs-phase-modal-container");A&&A.addEventListener("click",async M=>{let E=M.target;if(E.id==="hs-autosing-phase-done"){let y=document.getElementById("hs-autosing-end-phase")?.value;if(!y)return;let w={startPhase:n?n.startPhase:i,endPhase:y,corruptions:{...r},strat:[...l]};o&&a?a(w):t(w),S.removeInjectedStyle("hs-phase-modal-styles"),s.CloseModal(h)}E.id==="hs-autosing-phase-corruptions"&&await vt(s,r),E.id==="hs-autosing-phase-challenges"&&await Mt(s,l,n?.startPhase??"error",n?.endPhase??"error")})},0)}var ne=class{static async open(e,t){let a=H.getModule("HSUI");if(!a||!a.uiReady)return;let n=!!e,o=e?{strategyName:e.strategyName,strategy:JSON.parse(JSON.stringify(e.strategy))}:{strategyName:"",strategy:[]},i=()=>{for(let d=1;d<o.strategy.length;d++){let g=o.strategy[d-1],m=o.strategy[d];m.startPhase=g.endPhase}},r=()=>{let d=document.getElementById("hs-autosing-phase-list");if(d){if(o.strategy.length===0){d.innerHTML='<div class="hs-strategy-empty-state">No strategy phases added yet.</div>';return}d.innerHTML=o.strategy.map((g,m)=>`
...
`;var Le=class extends L{#e;#t;#a=null;#r=[];#i=null;#n=null;#o=new Map;#s;#l;#u;#d=new Map;#m=`
...
.hs-export-btn:active {
    transform: translateY(0);
            ...
            <div style="font-size: 11px; color: #888; margin-bottom: 4px;">TIMING</div>`,t!==null&&(h+=`<div style="margin-bottom: 4px;">Last: <span style="color: #2196F3; font-weight: bold;">${t.toFixed(2)}s</span></div>`),a!==null&&(h+=`<div style="margin-bottom: 4px;">Avg (5): <span style="color: #9C27B0; font-weight: bold;">${a.toFixed(2)}s</span></div>`),n!==null&&(h+=`<div style="margin-bottom: 4px;">Avg (10): <span style="color: #19ae11; font-weight: bold;">${n.toFixed(2)}s</span></div>`),o!==null&&(h+=`<div style="margin-bottom: 4px;">Avg (50): <span style="color: #FF9800; font-weight: bold;">${o.toFixed(2)}s</span></div>`),i!==null&&(h+=`<div>Avg (All): <span class="hs-rainbow-text">${i.toFixed(2)}s</span></div>`),h+="</div>",this.phaseHistory.size>0){h+=`<div style="margin-bottom: 8px; padding-bottom: 8px; border-bottom: 1px solid #444;">
