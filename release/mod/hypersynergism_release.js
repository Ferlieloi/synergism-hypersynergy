...
`;var Le=class extends L{#e;#t;#a=null;#r=[];#i=null;#n=null;#o=new Map;#s;#l;#u;#d=new Map;#m=`
    ...
    `;#f="hs-ambrosia-idle-loadout-css";#v="hs-ambrosia-minibar-css";#h=!1;#M;#p;#c;#S=!1;#A;#y;constructor(e){super(e)}async init(){let e=this;c.log("Initializing HSAmbrosia module",this.context),this.#a=await v.HookElement("#blueberryUpgradeContainer"),this.#r=await v.HookElements(".blueberryLoadoutSlot"),this.#i=await v.HookElement("#bbLoadoutContainer"),this.#c=document.querySelector("#hs-panel-debug-gamedata-currentambrosia"),this.loadState(),await this.#k(),c.log("Hooking stuff",this.context);for(let[t,a]of b.HSAmbrosia.ambrosiaLoadoutIcons.entries()){let n=document.querySelector(`#${a.draggableIconId}`);n&&(n.draggable=!0,n.dataset.hsid=t,n.addEventListener("dragstart",o=>{if(!o.dataTransfer)return;let i=o.currentTarget?.dataset.hsid;i&&(c.log(`Drag start ${i}`,this.context),o.dataTransfer.effectAllowed="move",o.dataTransfer.setData("hs-amb-drag",i))}))}if(!this.#i){c.warn("Could not find loadout container",this.context);return}this.#r.forEach(t=>{let a=t.id;if(a){let n=this.#T(a);if(n){let o=this.#o.get(n);o&&this.#x(n,o)}}}),this.#i.delegateEventListener("dragenter",".blueberryLoadoutSlot",t=>{t.dataTransfer&&t.dataTransfer.types.includes("hs-amb-drag")&&(t.preventDefault(),t.stopPropagation(),t.dataTransfer.effectAllowed="move")}),this.#i?.delegateEventListener("dragover",".blueberryLoadoutSlot",t=>{t.dataTransfer&&t.dataTransfer.types.includes("hs-amb-drag")&&(t.preventDefault(),t.stopPropagation(),t.dataTransfer.effectAllowed="move")}),this.#i?.delegateEventListener("drop",".blueberryLoadoutSlot",async t=>{if(t.dataTransfer&&t.dataTransfer.types.includes("hs-amb-drag")){t.preventDefault(),t.stopPropagation();let a=this.#E(t.dataTransfer.getData("hs-amb-drag")),n=t.target,o=n.id;if(!a){c.warn(`Invalid icon ID: ${a}`,this.context);return}if(!b.HSAmbrosia.ambrosiaLoadoutIcons.has(a)){c.warn(`Could not find loadout slot entry for ${a}`,this.context);return}if(!n.classList.contains("blueberryLoadoutSlot")||!o)return;let i=this.#T(o);if(!i){c.warn(`Invalid slot ID: ${o}`,this.context);return}let r=b.HSAmbrosia.ambrosiaLoadoutIcons.get(a);this.#x(i,a),this.#o.set(i,a),this.saveState(),await e.updateQuickBar()}}),this.#i?.delegateEventListener("contextmenu",".blueberryLoadoutSlot",async t=>{t.preventDefault();let a=t.target,n=a.id,o=this.#T(n);if(!o){c.warn(`Invalid slot ID: ${n}`,this.context);return}if(!this.#o.get(o)){c.warn(`No icon found for slot ID: ${n}`,this.context);return}a.classList.remove("hs-ambrosia-slot"),a.style.backgroundImage="",this.#o.delete(o),this.saveState(),await e.updateQuickBar()}),this.#i?.delegateEventListener("click",".blueberryLoadoutSlot",async t=>{let n=t.target.id,o=this.#T(n);if(!o){c.warn(`Invalid slot ID: ${n}`,this.context);return}await e.#w(o)}),this.isInitialized=!0}async#w(e){this.#s=e;let t=C.getSetting("autoLoadoutState");t&&t.setValue(`<green>${e}</green>`),this.#n||(this.#n=await v.HookElement("header"));let a=this.#n.querySelector(`#${b.HSAmbrosia.quickBarId}`);if(a){a.querySelectorAll(".blueberryLoadoutSlot").forEach(i=>{i.classList.remove("hs-ambrosia-active-slot")});let o=a.querySelector(`#${b.HSAmbrosia.quickBarLoadoutIdPrefix}-${e}`);o&&o.classList.add("hs-ambrosia-active-slot")}else c.warn("Could not find quick bar element",this.context);let n=document.querySelector("#bbLoadoutContainer");if(n){n.querySelectorAll(".blueberryLoadoutSlot").forEach(i=>{i.classList.remove("hs-ambrosia-active-slot")});let o=n.querySelector(`#${e}`);o&&o.classList.add("hs-ambrosia-active-slot")}}#E(e){return Object.values(he).find(t=>t===e)}#T(e){return Object.values(me).find(t=>t===e)}#x(e,t){let a=document.querySelector(`#${e}`);if(!a){c.warn(`Could not find slot element for ${e}`,this.context);return}let n=b.HSAmbrosia.ambrosiaLoadoutIcons.get(t);if(!n){c.warn(`Could not find icon for ${t}`,this.context);return}a.classList.add("hs-ambrosia-slot"),a.style.backgroundImage=`url(${n.url})`}async saveState(){let e=H.getModule("HSStorage");if(e){let t=JSON.stringify(Array.from(this.#o.entries()));e.setData(b.HSAmbrosia.storageKey,t)}else c.warn("saveState - Could not find storage module",this.context)}async loadState(){let e=H.getModule("HSStorage");if(e){let a=e.getData(b.HSAmbrosia.storageKey);if(!a){c.warn("loadState - No data found",this.context);return}try{let n=JSON.parse(a);this.#o=new Map(n)}catch{c.warn("loadState - Error parsing data",this.context);return}}else c.warn("loadState - Could not find storage module",this.context);let t=C.getSetting("autoLoadoutState");t&&!this.#s?this.#s=p.removeColorTags(t.getValue()):c.warn("loadState - Could not find autoLoadoutState setting",this.context)}async createQuickBar(){let e=this;if(this.#d.forEach((t,a)=>{a.removeEventListener("click",t)}),this.#d.clear(),this.#i=await v.HookElement("#bbLoadoutContainer"),this.#n=await v.HookElement("header"),this.#i&&this.#n){let t=this.#n.querySelector("nav.navbar"),a=this.#i.cloneNode(!0);a.id=b.HSAmbrosia.quickBarId;let n=a.querySelector(".blueberryLoadoutSetting");a.querySelectorAll(".blueberryLoadoutSlot").forEach(i=>{let r=i.id;i.id=`${b.HSAmbrosia.quickBarLoadoutIdPrefix}-${r}`;let l=async function(u){await e.#P(u,r)};this.#d.set(i,l),i.addEventListener("click",l)}),n&&n.remove(),this.#n.insertBefore(a,t),S.injectStyle(this.#m,this.#g),this.#s&&await this.#w(this.#s)}}async destroyQuickBar(){this.#n||(this.#n=await v.HookElement("header"));let e=this.#n.querySelector(`#${b.HSAmbrosia.quickBarId}`);e?(this.#d.forEach((a,n)=>{n.removeEventListener("click",a)}),this.#d.clear(),e.remove(),S.removeInjectedStyle(this.#g)):c.warn("Could not find quick bar element",this.context);let t=document.querySelector("#bbLoadoutContainer");t&&t.querySelectorAll(".blueberryLoadoutSlot").forEach(a=>{a.classList.remove("hs-ambrosia-active-slot")})}async#k(){let e=await v.HookElement("#importBlueberriesButton");if(!e||document.getElementById("hs-ambrosia-extra-btn"))return;let t=document.createElement("button");t.id="hs-ambrosia-extra-btn",t.className="ambrosiaLoadoutBtn",t.textContent="Quick Import",e.parentElement?.insertBefore(t,e.nextSibling),t.addEventListener("click",()=>this.#R())}async updateQuickBar(){C.getSetting("ambrosiaQuickBar").isEnabled()&&(await this.destroyQuickBar(),await this.createQuickBar(),this.#s&&await this.#w(this.#s))}async enableAutoLoadout(){let e=this,t=C.getSetting("autoLoadoutState");if(t&&t.getValue().includes("Unknown")||!this.#s){let d=C.getSetting("autoLoadout");d&&d.isEnabled()&&d.disable(),c.warn("Could not enable auto loadout - current loadout state is not known!",this.context);return}let a=[v.HookElement("#addCode"),v.HookElement("#addCodeAll"),v.HookElement("#addCodeOne"),v.HookElement("#timeCode")],o=(await Promise.allSettled(a)).map((d,g)=>d.status==="fulfilled"?d.value:null);if(o.some(d=>d===null)){c.warn("Problem with enabling auto loadout",this.context);return}let[i,r,l,u]=o;this.#l||(this.#l=async d=>{await e.#B(d)}),this.#u||(this.#u=async d=>{await e.#N(d)}),i.removeEventListener("click",this.#l,{capture:!0}),i.addEventListener("click",this.#l,{capture:!0}),r.removeEventListener("click",this.#l,{capture:!0}),r.addEventListener("click",this.#l,{capture:!0}),l.removeEventListener("click",this.#l,{capture:!0}),l.addEventListener("click",this.#l,{capture:!0}),u.removeEventListener("click",this.#u,{capture:!0}),u.addEventListener("click",this.#u,{capture:!0}),c.log("Enabled auto loadout",this.context)}async disableAutoLoadout(){let e=this,t=[v.HookElement("#addCode"),v.HookElement("#addCodeAll"),v.HookElement("#addCodeOne"),v.HookElement("#timeCode")],n=(await Promise.allSettled(t)).map((u,d)=>u.status==="fulfilled"?u.value:null);if(n.some(u=>u===null)){c.warn("Problem with disabling auto loadout",this.context);return}let[o,i,r,l]=n;this.#l&&(o.removeEventListener("click",this.#l,{capture:!0}),i.removeEventListener("click",this.#l,{capture:!0}),r.removeEventListener("click",this.#l,{capture:!0})),this.#u&&l.removeEventListener("click",this.#u,{capture:!0}),c.log("Disabled auto loadout",this.context)}async#B(e){let t=this.#s,a=C.getSetting("autoLoadoutAdd");if(t&&a){let n=this.#I(a.getValue()),o=await v.HookElement(`#${n}`);await this.#C(),await p.hiddenAction(async()=>{o.click()})}}async#N(e){let t=this.#s,a=C.getSetting("autoLoadoutTime");if(t&&a){let n=this.#I(a.getValue()),o=await v.HookElement(`#${n}`);await this.#C(),await p.hiddenAction(async()=>{o.click()})}}async#P(e,t){let a=document.querySelector(`#${t}`);a?(await this.#C(),await p.hiddenAction(async()=>{a.click()})):c.warn(`Could not find real button for ${t}`,this.context)}async#C(){let e=await v.HookElement("#blueberryToggleMode");e&&e.innerText.includes("SAVE")&&e.click()}#I(e){let t=Object.values(me).find(a=>a===`blueberryLoadout${e}`);return t||c.warn(`Could not convert loadout ${e} to slot`,this.context),t}subscribeGameDataChanges(){let e=H.getModule("HSGameData");e&&!this.gameDataSubscriptionId&&(this.gameDataSubscriptionId=e.subscribeGameDataChange(this.gameDataCallback.bind(this)),c.debug("Subscribed to game data changes",this.context))}unsubscribeGameDataChanges(){let e=H.getModule("HSGameData");e&&this.gameDataSubscriptionId&&!this.#h&&!this.#S&&(e.unsubscribeGameDataChange(this.gameDataSubscriptionId),this.gameDataSubscriptionId=void 0,c.debug("Unsubscribed from game data changes",this.context))}async gameDataCallback(){let e=H.getModule("HSGameDataAPI");if(!e)return;let t=e.getGameData();if(t&&t.blueberryTime&&t.redAmbrosiaTime){let a=t.blueberryTime,n=t.redAmbrosiaTime,o=e.R_calculateRequiredBlueberryTime(),i=e.R_calculateRequiredRedAmbrosiaTime(),r=a/o*100,l=n/i*100,u=e.calculateAmbrosiaSpeed(),d=e.calculateBlueBerries(),g=u*d,m=t.shopUpgrades.shopAmbrosiaAccelerator,h=e.calculateLuck(),A=t.singularityChallenges.noAmbrosiaUpgrades.completions>0?1:0,M=h.total/100+A,E=(h.total-100*Math.floor(h.total/100))/100,f=0,y=0,w=0,U=g/o*100,P=U+.1,V=1/2+(3/5-1/2)*+(t.singularityChallenges.noAmbrosiaUpgrades.completions>=15)+(2/3-3/5)*+(t.singularityChallenges.noAmbrosiaUpgrades.completions>=19)+(3/4-2/3)*+(t.singularityChallenges.noAmbrosiaUpgrades.completions>=20);if(m>0&&g>0){let Q=o/g;f=Math.min(Q*V,M*.2*m),y=f*g,w=y/o*100}if(this.#h&&this.#M&&this.#p){let Q=C.getSetting("ambrosiaIdleSwapNormalLoadout"),O=C.getSetting("ambrosiaIdleSwap100Loadout");if(Q&&O){let J=Q.getValue(),F=O.getValue();if(!Number.isInteger(parseInt(J,10))||!Number.isInteger(parseInt(F,10))){let j=C.getSetting("ambrosiaIdleSwap");j&&j.disable(),c.log("Idle swap was disabled due to unconfigured loadouts",this.context);return}let z=this.#I(Q.getValue()),Y=this.#I(O.getValue()),x=P+w,I=x+P,R=100-P,B=100,D=b.HSAmbrosia.idleSwapMinRedTreshold,G=D+b.HSAmbrosia.idleSwapMinRedTreshold,Z=b.HSAmbrosia.idleSwapMaxRedTreshold;if(r>=R&&r<=B||l>=Z&&l<=100){if(this.#s!==Y){let j=await v.HookElement(`#${Y}`);await this.#C(),await p.hiddenAction(async()=>{j.click()})}}else if((r>=x&&r<=I||l>=D&&l<=G)&&this.#s!==z){let j=await v.HookElement(`#${z}`);await this.#C(),await p.hiddenAction(async()=>{j.click()})}}if(this.#c&&S.isModPanelOpen()){let J=document.createElement("div");for(J.innerHTML=`
                        ...
                        blue: ${this.#A}, 
                        red: ${this.#y}`,"hs-minibars-undefined"):c.logOnce("HSAmbrosia.gameDataCallback() - berryMinibarsEnabled was false","hs-minibars-false")}}async enableIdleSwap(){let e=this,t=H.getModule("HSGameState");t?(this.#e=t.subscribeGameStateChange("MAIN_VIEW",this.#L.bind(this)),this.#t=t.subscribeGameStateChange("SINGULARITY_VIEW",async(a,n)=>{n.getId()===5?(this.#M=await v.HookElement("#ambrosiaProgressBar"),this.#p=await v.HookElement("#pixelProgressBar"),this.#h=!0,this.#H(),this.subscribeGameDataChanges()):(this.#h=!1,this.#D(),this.unsubscribeGameDataChanges())}),t.getCurrentUIView("SINGULARITY_VIEW").getId()===5&&t.getCurrentUIView("MAIN_VIEW").getId()===13&&(this.#M=await v.HookElement("#ambrosiaProgressBar"),this.#p=await v.HookElement("#pixelProgressBar"),this.#h=!0,this.#H(),this.subscribeGameDataChanges())):c.warn("HSAmbrosia.enableIdleSwap() - gameStateMod==undefined","hs-enable-idleswap-gamestate"),this.#c||(this.#c=document.querySelector("#hs-panel-debug-gamedata-currentambrosia"))}disableIdleSwap(){this.#h=!1,this.unsubscribeGameDataChanges();let e=H.getModule("HSGameState");e?(this.#e&&(e.unsubscribeGameStateChange("MAIN_VIEW",this.#e),this.#e=void 0),this.#t&&(e.unsubscribeGameStateChange("SINGULARITY_VIEW",this.#t),this.#t=void 0)):c.warnOnce("HSAmbrosia.disableIdleSwap() - gameStateMod==undefined","hs-disable-idleswap-gamestate"),this.#D()}#L(e,t){let a=H.getModule("HSGameState");a?e.getId()===13&&t.getId()!==13&&a.getCurrentUIView("SINGULARITY_VIEW").getId()===5&&(this.#h=!1):c.warnOnce("HSAmbrosia.gameStateCallbackMain() - gameStateMod==undefined","hs-amb-gamestate-cb")}#H(){if(document.querySelector(`#${b.HSAmbrosia.idleSwapIndicatorId}`))return;let t=document.createElement("div");t.id=b.HSAmbrosia.idleSwapIndicatorId,t.innerText="IDLE SWAP ENABLED WHILE IN THIS VIEW",S.injectHTMLElement(t,a=>{let n=document.querySelector("#singularityAmbrosia"),o=document.querySelector("#ambrosiaProgressBar");n?.insertBefore(a,o)}),S.injectStyle(this.#b,this.#f)}#D(){let e=document.querySelector(`#${b.HSAmbrosia.idleSwapIndicatorId}`);e&&e.remove(),S.removeInjectedStyle(this.#f)}async enableBerryMinibars(){if(this.#n||(this.#n=await v.HookElement("header")),!this.#n)return;let t=(await v.HookElement("#ambrosiaProgressBar")).cloneNode(!0),a=t.querySelector("#ambrosiaProgress"),n=t.querySelector("#ambrosiaProgressText");t.id=b.HSAmbrosia.blueBarId,a.id=b.HSAmbrosia.blueBarProgressId,n.id=b.HSAmbrosia.blueBarProgressTextId;let i=(await v.HookElement("#pixelProgressBar")).cloneNode(!0),r=i.querySelector("#pixelProgress"),l=i.querySelector("#pixelProgressText");i.id=b.HSAmbrosia.redBarId,r.id=b.HSAmbrosia.redBarProgressId,l.id=b.HSAmbrosia.redBarProgressTextId;let u=document.createElement("div");u.id=b.HSAmbrosia.barWrapperId,u.appendChild(t),u.appendChild(i);let d=C.getSetting("ambrosiaQuickBar"),g;d&&d.isEnabled()?g=this.#n.querySelector("#hs-ambrosia-quick-loadout-container"):g=this.#n.querySelector("nav.navbar"),this.#n.insertBefore(u,g),S.injectStyle(Et,this.#v),this.#A=a,this.#y=r,this.subscribeGameDataChanges(),this.#S=!0}async disableBerryMinibars(){this.#n||(this.#n=await v.HookElement("header"));let e=this.#n.querySelector(`#${b.HSAmbrosia.barWrapperId}`);e?e.remove():c.warn("Could not find bar wrapper element",this.context),S.removeInjectedStyle(this.#v),this.#S=!1,this.unsubscribeGameDataChanges()}async#R(){let e=null;try{e=document.querySelector(".blueberryLoadoutSlot.hs-ambrosia-active-slot");let t=await navigator.clipboard.readText();if(!t||typeof t!="string"){S.Notify("Clipboard does not contain valid loadout data",{notificationType:"warning"});return}let a=t.split(`
