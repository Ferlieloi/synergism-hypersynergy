"use strict";(()=>{var te=(l=>(l.SLOT_1="blueberryLoadout1",l.SLOT_2="blueberryLoadout2",l.SLOT_3="blueberryLoadout3",l.SLOT_4="blueberryLoadout4",l.SLOT_5="blueberryLoadout5",l.SLOT_6="blueberryLoadout6",l.SLOT_7="blueberryLoadout7",l.SLOT_8="blueberryLoadout8",l))(te||{}),ae=(I=>(I.TUTORIAL="ambrosiaTutorial",I.PATREON="ambrosiaPatreon",I.OBTAINIUM="ambrosiaObtainium1",I.OFFERING="ambrosiaOffering1",I.HYPEFLUX="ambrosiaHyperflux",I.RA_TUTORIAL="redAmbrosiaTutorial",I.RA_FREE="redAmbrosiaFreeTutorialLevels",I.RA_CONV1="redAmbrosiaConversionImprovement1",I.RA_BLUEGEN="redAmbrosiaBlueberryGenerationSpeed",I.RA_REGLUCK="redAmbrosiaRegularLuck",I.QUARKS1="ambrosiaQuarks1",I.CUBES1="ambrosiaCubes1",I.LUCK1="ambrosiaLuck1",I.BASE_OBT1="ambrosiaBaseObtainium1",I.BASE_OFF1="ambrosiaBaseOffering1",I.SING_RED1="ambrosiaSingReduction1",I.RA_FREE2="redAmbrosiaFreeLevelsRow2",I.RA_CUBE="redAmbrosiaRedAmbrosiaCube",I.RA_OBT="redAmbrosiaRedAmbrosiaObtainium",I.RA_OFF="redAmbrosiaRedAmbrosiaOffering",I.CUBE_QUARK="ambrosiaCubeQuark1",I.LUCK_QUARK="ambrosiaLuckQuark1",I.LUCK_CUBE="ambrosiaLuckCube1",I.QUARK_CUBE="ambrosiaQuarkCube1",I.CUBE_LUCK="ambrosiaCubeLuck1",I.QUARK_LUCK="ambrosiaQuarkLuck1",I.RA_FREE3="redAmbrosiaFreeLevelsRow3",I.RA_CONV2="redAmbrosiaConversionImprovement2",I.RA_REDGEN="redAmbrosiaRedGenerationSpeed",I.RA_REDLUCK="redAmbrosiaRedLuck",I.QUARKS2="ambrosiaQuarks2",I.CUBES2="ambrosiaCubes2",I.LUCK2="ambrosiaLuck2",I.BASE_OBT2="ambrosiaBaseObtainium2",I.BASE_OFF2="ambrosiaBaseOffering2",I.INF_SHOP1="ambrosiaInfiniteShopUpgrades1",I.RA_FREE4="redAmbrosiaFreeLevelsRow4",I.RA_CUBE_IMPR="redAmbrosiaRedAmbrosiaCubeImprover",I.RA_INF_SHOP="redAmbrosiaInfiniteShopUpgrades",I.RA_ACC="redAmbrosiaRedAmbrosiaAccelerator",I.QUARKS3="ambrosiaQuarks3",I.CUBES3="ambrosiaCubes3",I.LUCK3="ambrosiaLuck3",I.SING_RED2="ambrosiaSingReduction2",I.INF_SHOP2="ambrosiaInfiniteShopUpgrades2",I.RA_VISCOUNT="redAmbrosiaViscount",I.RA_FREE5="redAmbrosiaFreeLevelsRow5",I.RA_CONV3="redAmbrosiaConversionImprovement3",I.RA_BLUEGEN2="redAmbrosiaBlueberryGenerationSpeed2",I.RA_REGLUCK2="redAmbrosiaRegularLuck2",I))(ae||{});var A=class{constructor(){throw new Error("Cannot instantiate a static class")}static{this.Debug={debugMode:!1,performanceDebugMode:!1,gameDataDebugMode:!1,calculationCacheDebugMode:!1}}static{this.PrivateAPI={base:"https://jonah.fi",latestRelease:"/hs-version",checkIntervalMs:9e5}}static{this.General={currentModVersion:"2.9.0",isLatestVersion:!0,modGithubUrl:"https://github.com/ahvonenj/synergism-hypersynergy/",modWikiUrl:"https://github.com/ahvonenj/synergism-hypersynergy/wiki/",modWikiFeaturesUrl:"https://github.com/ahvonenj/synergism-hypersynergy/wiki/Mod-Features",modWebsiteUrl:"https://ahvonenj.github.io/synergism-hypersynergy/",heaterUrl:"https://docs.google.com/spreadsheets/d/1v02fjAeAHtLBMB5-7Spz5OHVb-eEDg7m5ISi5Mk0YAY/edit?usp=sharing"}}static{this.Common={eventAPIUrl:"wss://synergism.cc/consumables/connect",pseudoAPIurl:"https://synergism.cc/stripe/upgrades",meAPIurl:"https://synergism.cc/api/v1/users/me",experimentalGDS:!0}}static{this.HSPrototypes={defaultTransitionTiming:100}}static{this.HSElementHooker={elementHookUpdateMS:10,elementsHookUpdateMS:100,enableHelementHookTimeout:!0,elementHookTimeout:500,watcherThrottlingMS:50,defaultWatcherOptions:{greedy:!1,overrideThrottle:!1,characterData:!0,childList:!0,subtree:!0,attributes:!1,attributeOldValue:!1,attributeFilter:[]}}}static{this.HSLogger={logLevel:1,logSize:100}}static{this.HSStorage={storagePrefix:"hs-"}}static{this.HSUI={injectedStylesDomId:"hs-injected-styles",notifyClassName:"hs-notification",notifyTextClassName:"hs-notification-text"}}static{this.HSUIC={defaultImageWidth:32,defaultImageHeight:32}}static{this.HSSettings={storageKey:"settings",strategiesKey:"strategies",serializationBlackList:["settingDescription","settingHelpText","settingValueMultiplier","defaultValue","settingControl","settingAction","patchConfig","usesGameData"],gameDataRequiredTooltip:"This feature requires Game Data Sniffing to be enabled.",gameDataCheckBlacklist:["useGameData","stopSniffOnError"]}}static{this.HSMouse={autoClickIgnoredElements:["#chronosHepteractCraft","#chronosHepteractCraftMax","#chronosHepteractCap","#chronosHepteractAuto","#hyperrealismHepteractCraft","#hyperrealismHepteractCraftMax","#hyperrealismHepteractCap","#hyperrealismHepteractAuto","#quarkHepteractCraft","#quarkHepteractCraftMax","#quarkHepteractCap","#quarkHepteractAuto","#challengeHepteractCraft","#challengeHepteractCraftMax","#challengeHepteractCap","#challengeHepteractAuto","#abyssHepteractCraft","#abyssHepteractCraftMax","#abyssHepteractCap","#abyssHepteractAuto","#acceleratorHepteractCraft","#acceleratorHepteractCraftMax","#acceleratorHepteractCap","#acceleratorHepteractAuto","#acceleratorBoostHepteractCraft","#acceleratorBoostHepteractCraftMax","#acceleratorBoostHepteractCap","#acceleratorBoostHepteractAuto","#multiplierHepteractCraft","#multiplierHepteractCraftMax","#multiplierHepteractCap","#multiplierHepteractAuto","#hepteractToQuarkTrade","#hepteractToQuarkTradeMax","#hepteractToQuarkTradeAuto","#powderDayWarp","#warpAuto","#hepteractAutoPercentageButton","#toggleautosacrifice","#toggleautoBuyFragments","#toggle36","#toggle37","#toggleresearchbuy","#toggleautoresearch","#toggleautoresearchmode","#toggle38","#toggleAntMax","#toggleAutoChallengeStart","#retryChallenge","#toggleAutoChallengeIgnore","#toggle35","#ok_prompt","#cancel_prompt"]}}static{this.HSAmbrosia={storageKey:"ambrosia-loadouts",quickBarId:"hs-ambrosia-quick-loadout-container",quickBarLoadoutIdPrefix:"hs-ambrosia-quickbar",ambrosiaLoadoutIcons:new Map([["ambrosiaTutorial",{url:"https://synergism.cc/Pictures/Default/BlueberryTutorial.png",draggableIconId:"ambrosiaTutorial"}],["ambrosiaPatreon",{url:"https://synergism.cc/Pictures/Default/BlueberryPatreon.png",draggableIconId:"ambrosiaPatreon"}],["ambrosiaObtainium1",{url:"https://synergism.cc/Pictures/Default/BlueberryObtainium.png",draggableIconId:"ambrosiaObtainium1"}],["ambrosiaOffering1",{url:"https://synergism.cc/Pictures/Default/BlueberryOffering.png",draggableIconId:"ambrosiaOffering1"}],["ambrosiaHyperflux",{url:"https://synergism.cc/Pictures/Default/BlueberryHyperflux.png",draggableIconId:"ambrosiaHyperflux"}],["redAmbrosiaTutorial",{url:"https://synergism.cc/Pictures/RedAmbrosia/RedAmbrosiaTutorial.png",draggableIconId:"redAmbrosiaTutorial"}],["redAmbrosiaFreeTutorialLevels",{url:"https://synergism.cc/Pictures/RedAmbrosia/RedAmbrosiaFreeTutorialLevels.png",draggableIconId:"redAmbrosiaFreeTutorialLevels"}],["redAmbrosiaConversionImprovement1",{url:"https://synergism.cc/Pictures/RedAmbrosia/RedAmbrosiaConversionImprovement1.png",draggableIconId:"redAmbrosiaConversionImprovement1"}],["redAmbrosiaBlueberryGenerationSpeed",{url:"https://synergism.cc/Pictures/RedAmbrosia/RedAmbrosiaBlueberryGenerationSpeed.png",draggableIconId:"redAmbrosiaBlueberryGenerationSpeed"}],["redAmbrosiaRegularLuck",{url:"https://synergism.cc/Pictures/RedAmbrosia/RedAmbrosiaRegularLuck.png",draggableIconId:"redAmbrosiaRegularLuck"}],["ambrosiaQuarks1",{url:"https://synergism.cc/Pictures/Default/BlueberryQuarks.png",draggableIconId:"ambrosiaQuarks1"}],["ambrosiaCubes1",{url:"https://synergism.cc/Pictures/Default/BlueberryCubes.png",draggableIconId:"ambrosiaCubes1"}],["ambrosiaLuck1",{url:"https://synergism.cc/Pictures/Default/BlueberryLuck.png",draggableIconId:"ambrosiaLuck1"}],["ambrosiaBaseObtainium1",{url:"https://synergism.cc/Pictures/Default/BlueberryBaseObtainium1.png",draggableIconId:"ambrosiaBaseObtainium1"}],["ambrosiaBaseOffering1",{url:"https://synergism.cc/Pictures/Default/BlueberryBaseOffering1.png",draggableIconId:"ambrosiaBaseOffering1"}],["ambrosiaSingReduction1",{url:"https://synergism.cc/Pictures/Default/BlueberrySingReduction.png",draggableIconId:"ambrosiaSingReduction1"}],["redAmbrosiaFreeLevelsRow2",{url:"https://synergism.cc/Pictures/RedAmbrosia/RedAmbrosiaFreeLevelsRow2.png",draggableIconId:"redAmbrosiaFreeLevelsRow2"}],["redAmbrosiaRedAmbrosiaCube",{url:"https://synergism.cc/Pictures/RedAmbrosia/RedAmbrosiaRedAmbrosiaCube.png",draggableIconId:"redAmbrosiaRedAmbrosiaCube"}],["redAmbrosiaRedAmbrosiaObtainium",{url:"https://synergism.cc/Pictures/RedAmbrosia/RedAmbrosiaObtainium.png",draggableIconId:"redAmbrosiaRedAmbrosiaObtainium"}],["redAmbrosiaRedAmbrosiaOffering",{url:"https://synergism.cc/Pictures/RedAmbrosia/RedAmbrosiaOffering.png",draggableIconId:"redAmbrosiaRedAmbrosiaOffering"}],["ambrosiaCubeQuark1",{url:"https://synergism.cc/Pictures/Default/BlueberryCubeQuark.png",draggableIconId:"ambrosiaCubeQuark1"}],["ambrosiaLuckQuark1",{url:"https://synergism.cc/Pictures/Default/BlueberryLuckQuark.png",draggableIconId:"ambrosiaLuckQuark1"}],["ambrosiaLuckCube1",{url:"https://synergism.cc/Pictures/Default/BlueberryLuckCube.png",draggableIconId:"ambrosiaLuckCube1"}],["ambrosiaQuarkCube1",{url:"https://synergism.cc/Pictures/Default/BlueberryQuarkCube.png",draggableIconId:"ambrosiaQuarkCube1"}],["ambrosiaCubeLuck1",{url:"https://synergism.cc/Pictures/Default/BlueberryCubeLuck.png",draggableIconId:"ambrosiaCubeLuck1"}],["ambrosiaQuarkLuck1",{url:"https://synergism.cc/Pictures/Default/BlueberryQuarkLuck.png",draggableIconId:"ambrosiaQuarkLuck1"}],["redAmbrosiaFreeLevelsRow3",{url:"https://synergism.cc/Pictures/RedAmbrosia/RedAmbrosiaFreeLevelsRow3.png",draggableIconId:"redAmbrosiaFreeLevelsRow3"}],["redAmbrosiaConversionImprovement2",{url:"https://synergism.cc/Pictures/RedAmbrosia/RedAmbrosiaConversionImprovement2.png",draggableIconId:"redAmbrosiaConversionImprovement2"}],["redAmbrosiaRedGenerationSpeed",{url:"https://synergism.cc/Pictures/RedAmbrosia/RedAmbrosiaRedGenerationSpeed.png",draggableIconId:"redAmbrosiaRedGenerationSpeed"}],["redAmbrosiaRedLuck",{url:"https://synergism.cc/Pictures/RedAmbrosia/RedAmbrosiaRedLuck.png",draggableIconId:"redAmbrosiaRedLuck"}],["ambrosiaQuarks2",{url:"https://synergism.cc/Pictures/Default/BlueberryQuarks2.png",draggableIconId:"ambrosiaQuarks2"}],["ambrosiaCubes2",{url:"https://synergism.cc/Pictures/Default/BlueberryCubes2.png",draggableIconId:"ambrosiaCubes2"}],["ambrosiaLuck2",{url:"https://synergism.cc/Pictures/Default/BlueberryLuck2.png",draggableIconId:"ambrosiaLuck2"}],["ambrosiaBaseObtainium2",{url:"https://synergism.cc/Pictures/Default/BlueberryBaseObtainium2.png",draggableIconId:"ambrosiaBaseObtainium2"}],["ambrosiaBaseOffering2",{url:"https://synergism.cc/Pictures/Default/BlueberryBaseOffering2.png",draggableIconId:"ambrosiaBaseOffering2"}],["ambrosiaInfiniteShopUpgrades1",{url:"https://synergism.cc/Pictures/Default/BlueberryInfiniteShopUpgrades.png",draggableIconId:"ambrosiaInfiniteShopUpgrades1"}],["redAmbrosiaFreeLevelsRow4",{url:"https://synergism.cc/Pictures/RedAmbrosia/RedAmbrosiaFreeLevelsRow4.png",draggableIconId:"redAmbrosiaFreeLevelsRow4"}],["redAmbrosiaRedAmbrosiaCubeImprover",{url:"https://synergism.cc/Pictures/RedAmbrosia/RedAmbrosiaRedAmbrosiaCubeImprover.png",draggableIconId:"redAmbrosiaRedAmbrosiaCubeImprover"}],["redAmbrosiaInfiniteShopUpgrades",{url:"https://synergism.cc/Pictures/RedAmbrosia/RedAmbrosiaInfiniteShopLevels.png",draggableIconId:"redAmbrosiaInfiniteShopUpgrades"}],["redAmbrosiaRedAmbrosiaAccelerator",{url:"https://synergism.cc/Pictures/RedAmbrosia/RedAmbrosiaAccelerator.png",draggableIconId:"redAmbrosiaRedAmbrosiaAccelerator"}],["ambrosiaQuarks3",{url:"https://synergism.cc/Pictures/Default/BlueberryQuarks3.png",draggableIconId:"ambrosiaQuarks3"}],["ambrosiaCubes3",{url:"https://synergism.cc/Pictures/Default/BlueberryCubes3.png",draggableIconId:"ambrosiaCubes3"}],["ambrosiaLuck3",{url:"https://synergism.cc/Pictures/Default/BlueberryLuck3.png",draggableIconId:"ambrosiaLuck3"}],["ambrosiaSingReduction2",{url:"https://synergism.cc/Pictures/Default/BlueberrySingReduction2.png",draggableIconId:"ambrosiaSingReduction2"}],["ambrosiaInfiniteShopUpgrades2",{url:"https://synergism.cc/Pictures/Default/BlueberryInfiniteShopUpgrades2.png",draggableIconId:"ambrosiaInfiniteShopUpgrades2"}],["redAmbrosiaViscount",{url:"https://synergism.cc/Pictures/RedAmbrosia/RedAmbrosiaTutorial.png",draggableIconId:"redAmbrosiaViscount"}],["redAmbrosiaFreeLevelsRow5",{url:"https://synergism.cc/Pictures/RedAmbrosia/RedAmbrosiaFreeLevelsRow5.png",draggableIconId:"redAmbrosiaFreeLevelsRow5"}],["redAmbrosiaConversionImprovement3",{url:"https://synergism.cc/Pictures/RedAmbrosia/RedAmbrosiaConversionImprovement3.png",draggableIconId:"redAmbrosiaConversionImprovement3"}],["redAmbrosiaBlueberryGenerationSpeed2",{url:"https://synergism.cc/Pictures/RedAmbrosia/RedAmbrosiaBlueberryGenerationSpeed.png",draggableIconId:"redAmbrosiaBlueberryGenerationSpeed2"}],["redAmbrosiaRegularLuck2",{url:"https://synergism.cc/Pictures/RedAmbrosia/RedAmbrosiaRegularLuck.png",draggableIconId:"redAmbrosiaRegularLuck2"}]]),idleSwapIndicatorId:"hs-ambrosia-loadout-idle-swap-indicator",idleSwapMaxBlueTreshold:97,idleSwapMinBlueTreshold:3,idleSwapMaxRedTreshold:99,idleSwapMinRedTreshold:1,R_TIME_PER_AMBROSIA:30,R_TIME_PER_RED_AMBROSIA:1e5,R_digitReduction:4,blueBarId:"hs-blue-progress-bar",blueBarProgressId:"hs-blue-progress",blueBarProgressTextId:"hs-blue-progress-text",redBarId:"hs-red-progress-bar",redBarProgressId:"hs-red-progress",redBarProgressTextId:"hs-red-progress-text",barWrapperId:"hs-minibar-wrapper"}}static{this.HSGameState={viewProperties:new Map([[8,{subViewIds:["cubeTab1","cubeTab2","cubeTab3","cubeTab4","cubeTab5","cubeTab6","cubeTab7"],subViewsSelector:".cubeTab",viewClassName:"CubeView"}],[13,{subViewIds:["singularityContainer1","singularityContainer2","singularityContainer3","singularityContainer4","singularityContainer5"],subViewsSelector:["#singularityContainer1","#singularityContainer2","#singularityContainer3","#singularityContainer4","#singularityContainer5"],viewClassName:"SingularityView"}]])}}static{this.HSGameData={fetchedDataRefreshInterval:6e4,campaignTokenRefreshInterval:6e4,turboModeSpeedMs:66,turboCSSId:"hs-game-data-turbo-css"}}};var F=class{static async getLatestRelease(){try{let t=await(await fetch(`${A.PrivateAPI.base}${A.PrivateAPI.latestRelease}`)).json();return t.code===0?t.data:null}catch{return null}}};var S=class{static#e="HSElementHooker";static#t=new Map;static HookElement(e,t){let a=this;return new Promise((o,i)=>{let n,s=t?t.querySelector(e):document.querySelector(e);s&&o(s);let l=setInterval(()=>{let u=t?t.querySelector(e):document.querySelector(e);u&&(clearTimeout(n),clearInterval(l),o(u))},A.HSElementHooker.elementHookUpdateMS);A.HSElementHooker.elementHookTimeout&&(n=setTimeout(()=>{c.warn("Hook timed out",a.#e),clearInterval(l),o(h.nullProxy("ElementHookTimeout"))},A.HSElementHooker.elementHookTimeout))})}static HookElements(e){let t=this;return new Promise((a,o)=>{let i,n=setInterval(()=>{let s=[];if(Array.isArray(e)&&e.length===0||!Array.isArray(e)&&(!e||e==="")){clearInterval(n),a([]);return}if(Array.isArray(e))e.forEach(l=>{s.push(document.querySelector(l))});else{let l=document.querySelectorAll(e),u=Array.from(l);s.push(...u)}!s.includes(null)&&s.length>0&&(clearTimeout(i),clearInterval(n),a(s))},A.HSElementHooker.elementsHookUpdateMS);A.HSElementHooker.elementHookTimeout&&(i=setTimeout(()=>{c.warn("Hook timed out",t.#e),clearInterval(n),a(h.nullProxy("ElementsHookTimeout"))},A.HSElementHooker.elementHookTimeout))})}static watchElement(e,t,a){let o=this,i={...A.HSElementHooker.defaultWatcherOptions,...a};if(!e){c.warn("watchElement - element not found",this.#e);return}let n=h.uuidv4();this.#t.set(n,{element:e,callback:t,value:void 0,parser:i.valueParser?i.valueParser:d=>d.innerText,observer:void 0,lastCall:void 0});let s=new MutationObserver(d=>{let g=o.#t.get(n);if(g){if(!i.overrideThrottle&&g.lastCall&&performance.now()-g.lastCall<A.HSElementHooker.watcherThrottlingMS)return;let m=g.parser,p=g.callback,b=g.value;if(m){let M=m(e,d);(M!==b||i.greedy)&&(g.value=M,p(M))}else c.warn("watchElement - error while observing, wParser is null",this.#e);g.lastCall=performance.now()}else c.warn("watchElement - error while observing, could not get watcher",this.#e)}),l=o.#t.get(n);l?l.observer=s:c.warn("watchElement - error while setting up observer, could not get watcher",this.#e);let u={characterData:i.characterData,childList:i.childList,subtree:i.subtree,attributes:i.attributes};return i.attributes&&(u={...u,attributeOldValue:i.attributeOldValue,attributeFilter:i.attributeFilter}),s.observe(e,u),n}static stopWatching(e){let t=this.#t.get(e);return t?(t.observer?t.observer.disconnect():c.warn("Watcher found, but it's observer is null",this.#e),this.#t.delete(e),!0):(c.warn(`No watcher found for uuid: ${e}`),!1)}static stopWatchers(){c.log("Stopping all watchers",this.#e),this.#t.forEach(({observer:e})=>{e&&e.disconnect()}),this.#t.clear()}};var h=class r{static#e=null;static{this.sleep=e=>new Promise(t=>setTimeout(t,e))}static{this.sleepTime=10}static wait(e){return new Promise(function(t){setTimeout(t,e)})}static uuidv4(){return"10000000-1000-4000-8000-100000000000".replace(/[018]/g,e=>(+e^crypto.getRandomValues(new Uint8Array(1))[0]&15>>+e/4).toString(16))}static domid(){return"hs-rnd-00000000000".replace(/[018]/g,e=>(+e^crypto.getRandomValues(new Uint8Array(1))[0]&15>>+e/4).toString(16))}static hashCode(e){let t=0,a,o;if(e.length===0)return t;for(a=0;a<e.length;a++)o=e.charCodeAt(a),t=(t<<5)-t+o,t|=0;return t}static async computeHash(e){let a=new TextEncoder().encode(e),o=await crypto.subtle.digest("SHA-1",a);return Array.from(new Uint8Array(o)).map(i=>i.toString(16).padStart(2,"0")).join("")}static N(e,t=2,a=2){let o=0,i="";try{typeof e=="string"?o=parseFloat(e):o=e,o>1e6?i=o.toExponential(a).replace("+",""):i=o.toFixed(t)}catch{return console.error(`[HS]: HSUtil.N FAILED FOR ${e}`),i}return i}static getTime(){let e=new Date,t=e.getHours(),a=e.getMinutes(),o=t.toString().padStart(2,"0"),i=a.toString().padStart(2,"0");return`${o}:${i}`}static camelToKebab(e){return e.replace(/^([A-Z])/,t=>t.toLowerCase()).replace(/([A-Z])/g,t=>"-"+t.toLowerCase())}static kebabToCamel(e){return e.replace(/-([a-z])/g,(t,a)=>a.toUpperCase())}static objectToCSS(e){let t="";if(e==null)return"";for(let[a,o]of Object.entries(e))o!=null&&(t+=`${this.camelToKebab(a)}: ${o};
`);return t}static isNumeric(e){return!isNaN(e-parseFloat(e))}static isString(e){return typeof e=="string"||e instanceof String}static isBoolean(e){return typeof e=="boolean"}static parseFloat2(e){if(!e)return NaN;let t=e.indexOf(",");if(t===-1)return parseFloat(e);e=e.replace(/,/g,"");let a=e.indexOf(".");if(a===-1)return parseFloat(e.replace(/\,/g,"."));let o=t<a?e.replace(/\,/g,""):e.replace(/\./g,"").replace(",",".");return parseFloat(o)}static nullProxy(e){let t=new Proxy({},{get:()=>(c.warn(`Get operation intercepted by Null Proxy '${e}', something is not right`,"Proxy"),t),set:()=>(c.warn(`Set operation intercepted by Null Proxy '${e}', something is not right`,"Proxy"),!0)});return t}static parseColorTags(e){let t=/<([a-zA-Z]+|#[0-9A-Fa-f]{3,6})>(.*?)<\/\1>/g;return e.replace(t,(a,o,i)=>`<span style="color: ${o}">${i}</span>`)}static removeColorTags(e){try{let t=/<([a-zA-Z]+|#[0-9A-Fa-f]{3,6})>(.*?)<\/\1>/g;return e.replace(t,(a,o,i)=>`${i}`)}catch(t){return console.warn("Error removing color tags from log message",t),`${e}`}}static unfuckNumericString(e){if(!e)return e;if(e.toLowerCase().includes("e"))return e.replace(/,/g,"");let t=e.replace(/[^0-9.,-]/g,""),a=t.split("."),o="";return a.length>1?o=a[0].replace(/,/g,"")+"."+a[1].replace(/,/g,""):o=t.replace(/,/g,""),o}static#t(e){let t=CSSStyleDeclaration.prototype.setProperty,a;try{a=Object.getOwnPropertyDescriptor(Object.getPrototypeOf(e.style),"display"),a||(a=Object.getOwnPropertyDescriptor(CSSStyleDeclaration.prototype,"display")),a||(a={configurable:!0,enumerable:!0,get:function(){return this.getPropertyValue("display")},set:function(o){this.setProperty("display",o,"")}})}catch{a={configurable:!0,enumerable:!0,get:function(){return this.getPropertyValue("display")},set:function(i){this.setProperty("display",i,"")}}}return Object.defineProperty(e.style,"display",{get:function(){return"none"},set:function(){},configurable:!0}),e.style.setProperty=function(o,i,n){return o==="display"?t.call(this,o,"none"):t.call(this,o,i,n)},{restore:()=>{Object.defineProperty(e.style,"display",a),e.style.setProperty=t}}}static#a=null;static#n=null;static#i=null;static async hiddenAction(e,t="alert",a=!1,o=25){let i=this.#a?this.#a:await S.HookElement("#transparentBG"),n=this.#n?this.#n:await S.HookElement("#confirmationBox"),s=this.#i?this.#i:await S.HookElement("#alertWrapper");this.#a=i,this.#n=n,this.#i=s;let l=document.querySelector("#ok_alert"),u=document.querySelector("#ok_confirm"),d=r.#t(i),g=r.#t(n),m=r.#t(s);await e(),await r.wait(o),a?(u.click(),await r.wait(o),d.restore(),g.restore(),m.restore(),l.click()):(d.restore(),g.restore(),m.restore(),t==="alert"?l.click():u.click())}static async Noop(){}static eventBuffNumToName(e){return{0:"Quark",1:"GoldenQuark",2:"Cubes",3:"PowderConversion",4:"AscensionSpeed",5:"GlobalSpeed",6:"AscensionScore",7:"AntSacrifice",8:"Offering",9:"Obtainium",10:"Octeract",11:"BlueberryTime",12:"AmbrosiaLuck",13:"OneMind"}[e]}static async isLatestVersion(){let e=await F.getLatestRelease();return!(e&&e.version!==A.General.currentModVersion)}static asString(e){return e===null?"":String(e)}static async waitForElement(e,t=5e3){return new Promise((a,o)=>{let i=document.getElementById(e);if(i)return a(i);let n=new MutationObserver(()=>{let l=document.getElementById(e);l&&(n.disconnect(),clearTimeout(s),a(l))}),s=setTimeout(()=>{n.disconnect(),o(new Error(`Element ${e} not found within ${t}ms`))},t);n.observe(document.body,{childList:!0,subtree:!0})})}static async waitForInnerText(e,t=a=>a.trim().length>0){if(!t(e.innerText))return new Promise(a=>{let o=new MutationObserver(()=>{t(e.innerText)&&(o.disconnect(),a())});o.observe(e,{childList:!0,characterData:!0,subtree:!0})})}static async click(e){return e.click(),await r.sleep(r.sleepTime),Promise.resolve()}static async DblClick(e){return e.click(),await new Promise(t=>setTimeout(t,5)),e.click(),await r.sleep(r.sleepTime),e.dispatchEvent(new MouseEvent("dblclick",{bubbles:!0})),Promise.resolve()}static startDialogWatcher(){this.#e===null&&(this.#e=window.setInterval(()=>{let e=document.getElementById("confirmWrapper");if(e&&e.style.display==="block"){let a=document.getElementById("ok_confirm");a&&(a.click(),c.debug("Auto-clicked ok_confirm"))}let t=document.getElementById("alertWrapper");if(t&&t.style.display==="block"){let a=document.getElementById("ok_alert");a&&(a.click(),c.debug("Auto-clicked ok_alert"))}},this.sleepTime))}static async stopDialogWatcher(){return new Promise(e=>{let t=0,a=3,o=window.setInterval(()=>{let i=!1,n=document.getElementById("confirmWrapper");if(n&&n.style.display==="block"){let l=document.getElementById("ok_confirm");l&&(l.click(),c.debug("Auto-clicked ok_confirm during shutdown"),i=!0)}let s=document.getElementById("alertWrapper");if(s&&s.style.display==="block"){let l=document.getElementById("ok_alert");l&&(l.click(),c.debug("Auto-clicked ok_alert during shutdown"),i=!0)}i?t=0:t++,t>=a&&(window.clearInterval(o),this.#e!==null&&(window.clearInterval(this.#e),this.#e=null),c.debug("Dialog watcher stopped after clearing all dialogs"),e())},r.sleepTime)})}static base64WithCRLF(e,t=49e3){let a=[];for(let o=0;o<e.length;o+=t)a.push(e.slice(o,o+t));return a.join(`\r
`)}};var C=class{constructor(e){this.moduleName=e.moduleName,this.context=e.context,this.moduleColor=e.moduleColor,this.moduleType=1,this.isInitialized=!1}getName(){return this.moduleName}setModuleType(e){this.moduleType=e}},oe=class extends C{#e;#t;#a;#n;constructor(e){super(e),this.setModuleType(2),this.#e=e.moduleKind,this.#t=e.moduleScriptUrl,this.#a=e.moduleCSSUrl,this.#n=e.scriptContext}async#i(){if(!this.#t){c.error(`Could not load script for ext module ${this.moduleName} - script url missing`);return}let e=document.createElement("script");return e.type="text/javascript",e.src=this.#t,document.head.appendChild(e),new Promise((t,a)=>{e.onload=()=>t(),e.onerror=()=>a()})}async#o(){if(!this.#a){c.error(`Could not load CSS for ext module ${this.moduleName} - css url missing`);return}let e=document.createElement("link");return e.rel="stylesheet",e.href=this.#a,document.head.appendChild(e),new Promise((t,a)=>{e.onload=()=>t(),e.onerror=()=>a()})}async init(){if(this.#n in window){c.error(`Could not load ext module ${this.getName} - import conflict in window`);return}switch(this.#e){case 1:await this.#i();break;case 2:await this.#o();break;case 3:await this.#o(),await this.#i();break;default:break}this.isInitialized=!0}};var ie=class extends C{#e;#t;#a;#n;#i;constructor(e){super(e),this.#e=document.getElementById("offeringPotionHide"),this.#t=document.getElementById("obtainiumPotionHide"),this.#a={attributes:!1,childList:!0,subtree:!0},this.#n=new MutationObserver((t,a)=>{this.#o(t,a)}),this.#i=new MutationObserver((t,a)=>{this.#s(t,a)})}async init(){c.log("Initialising HSPotions module",this.context),this.observe(),this.isInitialized=!0}observe(){this.#n.observe(this.#e,this.#a),this.#i.observe(this.#t,this.#a)}#o(e,t){if(document.getElementById("offeringPotionMultiUseButton")===null){let o=document.getElementById("useofferingpotion"),i=document.getElementById("buyofferingpotion");if(!o||!i){c.warn("Could not find native buttons for use/buy offering potions",this.context);return}if(o){let n=o.cloneNode(!0);n.id="offeringPotionMultiUseButton",n.textContent="CONSUME 10x",n.addEventListener("click",()=>{for(let s=0;s<10;s++)o.click()}),o.parentNode?.insertBefore(n,o.nextSibling)}if(i){let n=i.cloneNode(!0);n.id="offeringPotionMultiBuyButton",n.textContent="BUY 10x",n.addEventListener("click",()=>{for(let s=0;s<10;s++)i.click(),setTimeout(()=>{document.getElementById("ok_confirm")?.click()},1)}),i.parentNode?.insertBefore(n,i.nextSibling)}this.#n.disconnect(),c.log("Offering potion multi buy / consume buttons injected",this.context)}}#s(e,t){if(document.getElementById("obtainiumPotionMultiUseButton")===null){let o=document.getElementById("useobtainiumpotion"),i=document.getElementById("buyobtainiumpotion");if(!o||!i){c.warn("Could not find native buttons for use/buy obtainium potions",this.context);return}if(o){let n=o.cloneNode(!0);n.id="obtainiumPotionMultiUseButton",n.textContent="CONSUME 10x",n.addEventListener("click",()=>{for(let s=0;s<10;s++)o.click()}),o.parentNode?.insertBefore(n,o.nextSibling)}if(i){let n=i.cloneNode(!0);n.id="obtainiumPotionMultiBuyButton",n.textContent="BUY 10x",n.addEventListener("click",()=>{for(let s=0;s<10;s++)i.click(),setTimeout(()=>{document.getElementById("ok_confirm")?.click()},1)}),i.parentNode?.insertBefore(n,i.nextSibling)}this.#i.disconnect(),c.log("Obtainium potion multi buy / consume buttons injected",this.context)}}};var ne=class extends C{#e;#t;#a;#n;#i="white-space: nowrap; user-select: all; -webkit-user-select: all; -moz-user-select: all; -ms-user-select: all;";constructor(e){super(e),this.#a={attributes:!1,childList:!0,subtree:!0},this.#n=new MutationObserver((t,a)=>{this.#r(t,a)})}async init(){c.log("Initialising HSCodes module",this.context);let e=this;this.#t=await S.HookElement("#promocodes"),this.#t.addEventListener("click",function(t){e.#e=document.querySelector("#promptWrapper > #prompt > label"),e.#e&&(e.#s(),e.#e.innerHTML="",e.#o())},{capture:!0}),this.#e=await S.HookElement("#promptWrapper > #prompt > label"),this.#o(),this.isInitialized=!0}#o(){this.#n?.disconnect(),this.#n?.observe(this.#e,this.#a)}#s(){this.#n?.disconnect()}#r(e,t){let a=this;try{if(this.#s(),this.#e&&this.#e.innerText.includes("synergism2021")){let o=this.#e.innerText;this.#e.innerHTML=`<div id="hs-hijack-codes-wrapper">
                    [HSCodes] Hypersynergism has hijacked this modal to offer you all the reusable codes conveniently (click code to auto input it):</br>
                    <span style="${this.#i}" data-code="synergism2021">synergism2021</span>
                    <span style="${this.#i}" data-code="Khafra">Khafra</span>
                    <span style="${this.#i}" data-code=":unsmith:">:unsmith:</span>
                    <span style="${this.#i}" data-code=":antismith:">:antismith:</span>
                </div>`,document.delegateEventListener("click","#hs-hijack-codes-wrapper > span",function(i){let n=this.dataset.code,s=document.querySelector("#prompt_text");n&&s?s.value=n:c.warn("Could not inject code to code input",a.context)},!0),c.debug("Hijacked code redeem panel",this.context)}}finally{this.#o()}}};var N=["start","prestige","transcend","reincarnate","ant","sacrifice","ascension","challenge10","challenge11","challenge12","challenge13","challenge14","w5x10max","alpha","p2x1x10","p3x1","beta","1e15-expo","omega","singularity","end"],Ne=[{label:"Exit Transcension challenge",value:101},{label:"Exit Reincarnation challenge",value:102},{label:"Exit Ascension challenge",value:103},{label:"Ascend",value:104},{label:"Ambrosia Early Cube loadout",value:105},{label:"Ambrosia Late Cube loadout",value:106},{label:"Ambrosia Quark loadout",value:107}];var Oe=`{
    "expandCostProtection": {
        "settingName": "expandCostProtection",
        "settingDescription": "Hept. expand cost protection",
        "settingHelpText": "When enabled, makes clicking hepteract icons in the hepteract view (quick expand and max feature) not do anything if it would cost >= settingValue percentage hepteracts",
        "settingType": "numeric",
        "enabled": true,
        "settingValue": 0.5,
        "settingControl": {
            "controlId": "hs-setting-expand-cost-protection-value",
            "controlType": "number",
            "controlGroup": "hepteracts",
            "controlPage": "hepteract",
            "controlEnabledId": "hs-setting-expand-cost-protection-btn",
            "controlOptions": {
                "min": 0,
                "max": 1,
                "step": 0.1
            }
        }
    },
    "expandCostProtectionDoubleCap": {
        "settingName": "expandCostProtectionDoubleCap",
        "settingDescription": "Hepteract double capacity unlocked?",
        "settingHelpText": "Check this if you have the double capacity upgrade unlocked for hepteracts. This will make the expand cost protection setting work correctly.",
        "settingType": "boolean",
        "enabled": false,
        "settingControl": {
            "controlType": "switch",
            "controlGroup": "hepteracts",
            "controlPage": "hepteract",
            "controlEnabledId": "hs-setting-hepteract-double-cap-btn"
        }
    },
    "expandCostProtectionNotifications": {
        "settingName": "expandCostProtectionNotifications",
        "settingDescription": "Suppress cost protection logs",
        "settingHelpText": "Check this if you want to suppress the rather spammy cost protection notifications in the log",
        "settingType": "boolean",
        "enabled": false,
        "settingControl": {
            "controlType": "switch",
            "controlGroup": "hepteracts",
            "controlPage": "hepteract",
            "controlEnabledId": "hs-setting-hepteract-cost-protection-notifications-btn"
        }
    },
    "syncNotificationOpacity": {
        "settingName": "syncNotificationOpacity",
        "settingDescription": "Notification opacity",
        "settingHelpText": "Controls in-game notification opacity. Achievement notifications are one example of in-game notifications.",
        "settingType": "numeric",
        "enabled": true,
        "settingValue": 0.6,
        "settingAction": "syncNotificationOpacity",
        "settingControl": {
            "controlId": "hs-setting-notification-opacity-value",
            "controlType": "number",
            "controlGroup": "notifications",
            "controlPage": "ui",
            "controlEnabledId": "hs-setting-notification-opacity-btn",
            "controlOptions": {
                "min": 0,
                "max": 1,
                "step": 0.05
            }
        }
    },
    "logTimestamp": {
        "settingName": "logTimestamp",
        "settingDescription": "Display timestamps in log",
        "settingHelpText": "Controls whether timestamps are visible or not in the mod's log view",
        "settingType": "boolean",
        "enabled": true,
        "settingAction": "logTimestamp",
        "settingControl": {
            "controlType": "switch",
            "controlGroup": "log",
            "controlPage": "log",
            "controlEnabledId": "hs-setting-display-timestamp-btn"
        }
    },
    "showDebugLogs": {
        "settingName": "showDebugLogs",
        "settingDescription": "Show debug logs",
        "settingHelpText": "Controls whether 'debug' type logs are visible or not in the mod's log view.",
        "settingType": "boolean",
        "enabled": false,
        "settingControl": {
            "controlType": "switch",
            "controlGroup": "debug",
            "controlPage": "debug",
            "controlEnabledId": "hs-setting-display-debug-logs-btn"
        }
    },
    "reactiveMouseHover": {
        "settingName": "reactiveMouseHover",
        "settingDescription": "Reactive mouse hover (when SHIFT is held)",
        "settingHelpText": "When enabled, mouse will emit hover events at current mouse position when SHIFT is held. Setting value is event interval in seconds (0.1 = 100ms).",
        "settingType": "numeric",
        "enabled": true,
        "settingValue": 0.2,
        "settingValueMultiplier": 1000,
        "settingAction": "reactiveMouseHover",
        "settingControl": {
            "controlId": "hs-setting-reactive-mouse-hover-value",
            "controlType": "number",
            "controlGroup": "mouse",
            "controlPage": "input",
            "controlEnabledId": "hs-setting-reactive-mouse-hover-btn",
            "controlOptions": {
                "min": 0,
                "max": 1,
                "step": 0.05
            }
        }
    },
    "autoClick": {
        "settingName": "autoClick",
        "settingDescription": "Auto click (when CTRL is held)",
        "settingHelpText": "When enabled, mouse will click automatically at current mouse position when CTRL is held. Setting value is click interval in seconds (0.1 = 100ms)",
        "settingType": "numeric",
        "enabled": true,
        "settingValue": 0.15,
        "settingValueMultiplier": 1000,
        "settingAction": "autoClick",
        "settingControl": {
            "controlId": "hs-setting-auto-click-value",
            "controlType": "number",
            "controlGroup": "mouse",
            "controlPage": "input",
            "controlEnabledId": "hs-setting-auto-click-btn",
            "controlOptions": {
                "min": 0,
                "max": 1,
                "step": 0.05
            }
        }
    },
    "autoClickIgnoreElements": {
        "settingName": "autoClickIgnoreElements",
        "settingDescription": "Auto click ignores certain elements",
        "settingHelpText": "When enabled, auto click will ignore (not click on) certain elements such as some buttons which there should be no need to auto click. This is to prevent accidental clicks.",
        "settingType": "boolean",
        "enabled": true,
        "settingControl": {
            "controlType": "switch",
            "controlGroup": "mouse",
            "controlPage": "input",
            "controlEnabledId": "hs-setting-autoclick-ignore-btn"
        }
    },
    "ambrosiaQuickBar": {
        "settingName": "ambrosiaQuickBar",
        "settingDescription": "Ambrosia quick bar",
        "settingHelpText": "When enabled, displays ambrosia loadout quick bar in the header. This is a quick way to load ambrosia loadouts without having to open the ambrosia menu.",
        "settingType": "boolean",
        "enabled": true,
        "settingAction": "ambrosiaQuickBarAction",
        "settingControl": {
            "controlType": "switch",
            "controlGroup": "ambrosia",
            "controlPage": "ambrosia",
            "controlEnabledId": "hs-setting-ambrosia-quickbar-btn"
        }
    },
    "autoLoadout": {
        "settingName": "autoLoadout",
        "settingDescription": "Auto loadout",
        "settingHelpText": "When enabled, the mod will automatically switch to configured ambrosia loadouts when ADD or TIME buttons are clicked.",
        "settingType": "boolean",
        "enabled": false,
        "settingAction": "autoLoadout",
        "settingControl": {
            "controlType": "switch",
            "controlGroup": "ambrosia",
            "controlPage": "ambrosia",
            "controlEnabledId": "hs-setting-auto-loadout-btn"
        }
    },
    "autoLoadoutState": {
        "settingName": "autoLoadoutState",
        "settingDescription": "Current loadout state",
        "settingHelpText": "This shows if the mod is aware of your current loadout state. If the state is Unknown, auto loadout switch will not work. To fix this, open the ambrosia menu and switch to any loadout.",
        "settingType": "state",
        "settingValue": "<red>Unknown</red>",
        "enabled": true,
        "settingControl": {
            "controlId": "hs-setting-auto-loadout-state-value",
            "controlType": "state",
            "controlGroup": "ambrosia",
            "controlPage": "ambrosia"
        }
    },
    "autoLoadoutAdd": {
        "settingName": "autoLoadoutAdd",
        "settingDescription": "ADD Loadout",
        "settingHelpText": "Configures to loadout to use when ADD button is clicked.",
        "settingType": "selectstring",
        "enabled": true,
        "settingValue": "",
        "settingControl": {
            "controlId": "hs-setting-add-loadout-value",
            "controlType": "select",
            "controlGroup": "ambrosia",
            "controlPage": "ambrosia",
            "selectOptions": [
                {
                    "text": "None",
                    "value": ""
                },
                {
                    "text": "Loadout 1",
                    "value": "1"
                },
                {
                    "text": "Loadout 2",
                    "value": "2"
                },
                {
                    "text": "Loadout 3",
                    "value": "3"
                },
                {
                    "text": "Loadout 4",
                    "value": "4"
                },
                {
                    "text": "Loadout 5",
                    "value": "5"
                },
                {
                    "text": "Loadout 6",
                    "value": "6"
                },
                {
                    "text": "Loadout 7",
                    "value": "7"
                },
                {
                    "text": "Loadout 8",
                    "value": "8"
                }
            ]
        }
    },
    "autoLoadoutTime": {
        "settingName": "autoLoadoutTime",
        "settingDescription": "TIME loadout",
        "settingHelpText": "Configures to loadout to use when TIME button is clicked.",
        "settingType": "selectstring",
        "enabled": true,
        "settingValue": "",
        "settingControl": {
            "controlId": "hs-setting-time-loadout-value",
            "controlType": "select",
            "controlGroup": "ambrosia",
            "controlPage": "ambrosia",
            "selectOptions": [
                {
                    "text": "None",
                    "value": ""
                },
                {
                    "text": "Loadout 1",
                    "value": "1"
                },
                {
                    "text": "Loadout 2",
                    "value": "2"
                },
                {
                    "text": "Loadout 3",
                    "value": "3"
                },
                {
                    "text": "Loadout 4",
                    "value": "4"
                },
                {
                    "text": "Loadout 5",
                    "value": "5"
                },
                {
                    "text": "Loadout 6",
                    "value": "6"
                },
                {
                    "text": "Loadout 7",
                    "value": "7"
                },
                {
                    "text": "Loadout 8",
                    "value": "8"
                }
            ]
        }
    },
    "ambrosiaIdleSwap": {
        "settingName": "ambrosiaIdleSwap",
        "settingDescription": "Ambrosia AFK swapper",
        "settingHelpText": "When enabled, automatically switches ambrosia loadouts depending on the ambrosia bar progress. IMPORTANT: BY DESIGN, THE AUTO SWAPPER WILL ONLY WORK WHEN IN THE AMBROSIA VIEW, SO NO 'SWAPPING IN THE BACKGROUND'",
        "settingType": "boolean",
        "enabled": true,
        "usesGameData": true,
        "settingAction": "ambrosiaIdleSwapAction",
        "settingControl": {
            "controlType": "switch",
            "controlGroup": "ambrosia2",
            "controlPage": "ambrosia",
            "controlEnabledId": "hs-setting-ambrosia-idle-swap-btn"
        }
    },
    "ambrosiaIdleSwapNormalLoadout": {
        "settingName": "ambrosiaIdleSwapNormalLoadout",
        "settingDescription": "Normal loadout",
        "settingHelpText": "Ambrosia AFK swapper will switch to this loadout when ambrosia bar is not full.",
        "settingType": "selectstring",
        "enabled": true,
        "settingValue": "",
        "settingControl": {
            "controlId": "hs-setting-idle-swap-normal-loadout-value",
            "controlType": "select",
            "controlGroup": "ambrosia2",
            "controlPage": "ambrosia",
            "selectOptions": [
                {
                    "text": "None",
                    "value": ""
                },
                {
                    "text": "Loadout 1",
                    "value": "1"
                },
                {
                    "text": "Loadout 2",
                    "value": "2"
                },
                {
                    "text": "Loadout 3",
                    "value": "3"
                },
                {
                    "text": "Loadout 4",
                    "value": "4"
                },
                {
                    "text": "Loadout 5",
                    "value": "5"
                },
                {
                    "text": "Loadout 6",
                    "value": "6"
                },
                {
                    "text": "Loadout 7",
                    "value": "7"
                },
                {
                    "text": "Loadout 8",
                    "value": "8"
                }
            ]
        }
    },
    "ambrosiaIdleSwap100Loadout": {
        "settingName": "ambrosiaIdleSwap100Loadout",
        "settingDescription": "100% loadout",
        "settingHelpText": "Ambrosia AFK swapper will switch to this loadout when ambrosia bar is about to hit 100%.",
        "settingType": "selectstring",
        "enabled": true,
        "settingValue": "",
        "settingControl": {
            "controlId": "hs-setting-idle-swap-100-loadout-value",
            "controlType": "select",
            "controlGroup": "ambrosia2",
            "controlPage": "ambrosia",
            "selectOptions": [
                {
                    "text": "None",
                    "value": ""
                },
                {
                    "text": "Loadout 1",
                    "value": "1"
                },
                {
                    "text": "Loadout 2",
                    "value": "2"
                },
                {
                    "text": "Loadout 3",
                    "value": "3"
                },
                {
                    "text": "Loadout 4",
                    "value": "4"
                },
                {
                    "text": "Loadout 5",
                    "value": "5"
                },
                {
                    "text": "Loadout 6",
                    "value": "6"
                },
                {
                    "text": "Loadout 7",
                    "value": "7"
                },
                {
                    "text": "Loadout 8",
                    "value": "8"
                }
            ]
        }
    },
    "ambrosiaMinibars": {
        "settingName": "ambrosiaMinibars",
        "settingDescription": "Ambrosia minibars",
        "settingHelpText": "Displays small blue- and red ambrosia progress bars in the page header",
        "settingType": "boolean",
        "enabled": true,
        "usesGameData": true,
        "settingAction": "ambrosiaMinibarAction",
        "settingControl": {
            "controlType": "switch",
            "controlGroup": "ambrosia3",
            "controlPage": "ambrosia",
            "controlEnabledId": "hs-setting-ambrosia-minibar-btn"
        }
    },
    "patch_ambrosiaViewOverflow": {
        "settingName": "patch_ambrosiaViewOverflow",
        "settingDescription": "Fix ambrosia view overflow",
        "settingHelpText": "This patch fixes an overflow issue in the ambrosia view which makes the page jump around when hovering over ambrosia upgrades.",
        "settingType": "boolean",
        "enabled": false,
        "settingAction": "patch",
        "patchConfig": {
            "patchName": "AmbrosiaViewOverflow"
        },
        "settingControl": {
            "controlType": "switch",
            "controlGroup": "patch",
            "controlPage": "patch",
            "controlEnabledId": "hs-setting-patch-ambrosia-view-overflow-btn"
        }
    },
    "patch_testPatch": {
        "settingName": "patch_testPatch",
        "settingDescription": "Test patch",
        "settingHelpText": "Test patch (makes the Buildings button color red)",
        "settingType": "boolean",
        "enabled": false,
        "settingAction": "patch",
        "patchConfig": {
            "patchName": "TestPatch"
        },
        "settingControl": {
            "controlType": "switch",
            "controlGroup": "patch",
            "controlPage": "patch",
            "controlEnabledId": "hs-setting-patch-test-btn"
        }
    },
    "patch_shopItemNameMapping": {
        "settingName": "patch_shopItemNameMapping",
        "settingDescription": "Display shop item names",
        "settingHelpText": "Populates the shop items with their names.",
        "settingType": "boolean",
        "enabled": true,
        "settingAction": "patch",
        "patchConfig": {
            "patchName": "ShopItemNameMapping"
        },
        "settingControl": {
            "controlType": "switch",
            "controlGroup": "patch",
            "controlPage": "patch",
            "controlEnabledId": "hs-setting-patch-shopnames-btn"
        }
    },
    "patch_iconSetCaching": {
        "settingName": "patch_iconSetCaching",
        "settingDescription": "IconSet Caching",
        "settingHelpText": "Use an alternative method of caching for certain icons to prevent excessive fetching when using DevTools. This probably doesn't matter for most players. Leave it off if you don't know what it does.",
        "settingType": "boolean",
        "enabled": false,
        "settingAction": "patch",
        "patchConfig": {
            "patchName": "IconSetCaching"
        },
        "settingControl": {
            "controlType": "switch",
            "controlGroup": "patch",
            "controlPage": "patch",
            "controlEnabledId": "hs-setting-patch-iconsets-btn"
        }
    },
    "useGameData": {
        "settingBlockId": "hs-setting-block-gamedata",
        "settingName": "useGameData",
        "settingDescription": "Game Data Sniffing (GDS)",
        "settingHelpText": "Enables game data sniffing by continuously scanning localStorage save data in the background. Enabling this gives the mod vastly enhanced functionality, but may cause performance issues on some systems.",
        "settingType": "boolean",
        "enabled": true,
        "settingAction": "useGameData",
        "settingControl": {
            "controlType": "switch",
            "controlGroup": "gamedata",
            "controlPage": "gamedata",
            "controlEnabledId": "hs-setting-use-game-data-btn"
        }
    },
    "stopSniffOnError": {
        "settingName": "stopSniffOnError",
        "settingDescription": "Auto disable on error",
        "settingHelpText": "When enabled, the mod will automatically disable game data sniffing if an error occurs. Recommended to be enabled as reading game data is not always reliable.",
        "settingType": "boolean",
        "enabled": true,
        "settingControl": {
            "controlType": "switch",
            "controlGroup": "gamedata",
            "controlPage": "gamedata",
            "controlEnabledId": "hs-setting-stop-sniff-on-error-btn"
        }
    },
    "startAutosing": {
        "settingName": "startAutosing",
        "settingDescription": "Start auto-sing",
        "settingHelpText": "when enabled, the mod will automatically perform singularities.",
        "settingType": "boolean",
        "enabled": false,
        "usesGameData": true,
        "settingAction": "startAutosingAction",
        "settingControl": {
            "controlType": "switch",
            "controlGroup": "auto-sing",
            "controlPage": "auto-sing",
            "controlEnabledId": "hs-setting-auto-sing-enabled"
        }
    },
    "singularityNumber": {
        "settingName": "singularityNumber",
        "settingDescription": "Singularity",
        "settingHelpText": "Controls at what singularity you wil auto-sing.",
        "settingType": "numeric",
        "enabled": true,
        "settingValue": 1,
        "settingControl": {
            "controlId": "hs-setting-auto-sing-number",
            "controlType": "number",
            "controlGroup": "auto-sing",
            "controlPage": "auto-sing",
            "controlOptions": {
                "min": 1,
                "max": 300,
                "step": 1
            }
        }
    },
    "autosingEarlyCubeLoadout": {
        "settingName": "autosingEarlyCubeLoadout",
        "settingDescription": "Pre-AOAG ambrosia cube loadout",
        "settingHelpText": "This is the ambrosia loadout that gives you the most cubes in the early game",
        "settingType": "selectstring",
        "enabled": true,
        "settingValue": "",
        "settingControl": {
            "controlId": "hs-setting-auto-sing-early-cube-loadout-value",
            "controlType": "select",
            "controlGroup": "auto-sing",
            "controlPage": "auto-sing",
            "selectOptions": [
                {
                    "text": "None",
                    "value": ""
                },
                {
                    "text": "Loadout 1",
                    "value": "1"
                },
                {
                    "text": "Loadout 2",
                    "value": "2"
                },
                {
                    "text": "Loadout 3",
                    "value": "3"
                },
                {
                    "text": "Loadout 4",
                    "value": "4"
                },
                {
                    "text": "Loadout 5",
                    "value": "5"
                },
                {
                    "text": "Loadout 6",
                    "value": "6"
                },
                {
                    "text": "Loadout 7",
                    "value": "7"
                },
                {
                    "text": "Loadout 8",
                    "value": "8"
                }
            ]
        }
    },
    "autosingLateCubeLoadout": {
        "settingName": "autosingLateCubeLoadout",
        "settingDescription": "Post-AOAG ambrosia cube loadout",
        "settingHelpText": "This is the ambrosia loadout that gives you the most cubes in the late game",
        "settingType": "selectstring",
        "enabled": true,
        "settingValue": "",
        "settingControl": {
            "controlId": "hs-setting-auto-sing-late-cube-loadout-value",
            "controlType": "select",
            "controlGroup": "auto-sing",
            "controlPage": "auto-sing",
            "selectOptions": [
                {
                    "text": "None",
                    "value": ""
                },
                {
                    "text": "Loadout 1",
                    "value": "1"
                },
                {
                    "text": "Loadout 2",
                    "value": "2"
                },
                {
                    "text": "Loadout 3",
                    "value": "3"
                },
                {
                    "text": "Loadout 4",
                    "value": "4"
                },
                {
                    "text": "Loadout 5",
                    "value": "5"
                },
                {
                    "text": "Loadout 6",
                    "value": "6"
                },
                {
                    "text": "Loadout 7",
                    "value": "7"
                },
                {
                    "text": "Loadout 8",
                    "value": "8"
                }
            ]
        }
    },
    "autosingQuarkLoadout": {
        "settingName": "autosingEarlyQuarkLoadout",
        "settingDescription": "Quark loadout",
        "settingHelpText": "This is the ambrosia loadout that gives you the most quarks. Note: this loadout is automatically selected right before the autosing presses the singularity button, the ascension button is also clicked so you gain the extra quarks.",
        "settingType": "selectstring",
        "enabled": true,
        "settingValue": "",
        "settingControl": {
            "controlId": "hs-setting-auto-sing-quark-loadout-value",
            "controlType": "select",
            "controlGroup": "auto-sing",
            "controlPage": "auto-sing",
            "selectOptions": [
                {
                    "text": "None",
                    "value": ""
                },
                {
                    "text": "Loadout 1",
                    "value": "1"
                },
                {
                    "text": "Loadout 2",
                    "value": "2"
                },
                {
                    "text": "Loadout 3",
                    "value": "3"
                },
                {
                    "text": "Loadout 4",
                    "value": "4"
                },
                {
                    "text": "Loadout 5",
                    "value": "5"
                },
                {
                    "text": "Loadout 6",
                    "value": "6"
                },
                {
                    "text": "Loadout 7",
                    "value": "7"
                },
                {
                    "text": "Loadout 8",
                    "value": "8"
                }
            ]
        }
    },
    "autosingStrategy": {
        "settingName": "autosingStrategy",
        "settingDescription": "autosing strategy",
        "settingHelpText": "This is the strategy that the autosinger will follow",
        "settingType": "selectstring",
        "enabled": true,
        "settingValue": "default_strategy",
        "settingControl": {
            "controlId": "hs-setting-auto-sing-strategy",
            "controlType": "select",
            "controlGroup": "auto-sing-strategy",
            "controlPage": "auto-sing",
            "selectOptions": []
        }
    },
    "editAutosingStrategy": {
        "settingName": "editAutosingStrategy",
        "settingDescription": "edit strategy",
        "settingHelpText": "Opens a new window where you can edit the selected autosing strategy",
        "settingType": "button",
        "enabled": true,
        "settingValue": "",
        "settingAction": "editAutosingStrategy",
        "settingControl": {
            "controlId": "hs-setting-auto-sing-strategy-editor-button",
            "controlType": "button",
            "controlGroup": "auto-sing-strategy",
            "controlPage": "auto-sing"
        }
    },
    "deleteAutosingStrategy": {
        "settingName": "deleteAutosingStrategy",
        "settingDescription": "delete strategy",
        "settingHelpText": "deletes the selected strategy",
        "settingType": "button",
        "enabled": true,
        "settingValue": "",
        "settingAction": "deleteAutosingStrategy",
        "settingControl": {
            "controlId": "hs-setting-auto-sing-strategy-deleter-button",
            "controlType": "button",
            "controlGroup": "auto-sing-strategy",
            "controlPage": "auto-sing"
        }
    },
    "createAutosingStrategy": {
        "settingName": "createAutosingStrategy",
        "settingDescription": "create strategy",
        "settingHelpText": "Opens a new window where you can create your own autosing strategy",
        "settingType": "button",
        "enabled": true,
        "settingValue": "",
        "settingAction": "createAutosingStrategy",
        "settingControl": {
            "controlId": "hs-setting-auto-sing-strategy-maker-button",
            "controlType": "button",
            "controlGroup": "auto-sing-strategy",
            "controlPage": "auto-sing"
        }
    },
    "exportAutosingStrategy": {
        "settingName": "exportAutosingStrategy",
        "settingDescription": "export strategy",
        "settingHelpText": "exports the selected strategy to your clipboard",
        "settingType": "button",
        "enabled": true,
        "settingValue": "",
        "settingAction": "exportAutosingStrategy",
        "settingControl": {
            "controlId": "hs-setting-auto-sing-strategy-exporter-button",
            "controlType": "button",
            "controlGroup": "auto-sing-strategy",
            "controlPage": "auto-sing"
        }
    },
    "importAutosingStrategy": {
        "settingName": "importAutosingStrategy",
        "settingDescription": "import strategy",
        "settingHelpText": "Opens a Modal where you can paste a strategy that you want to import",
        "settingType": "button",
        "enabled": true,
        "settingValue": "",
        "settingAction": "importAutosingStrategy",
        "settingControl": {
            "controlId": "hs-setting-auto-sing-strategy-importer-button",
            "controlType": "button",
            "controlGroup": "auto-sing-strategy",
            "controlPage": "auto-sing"
        }
    }
}`;var Ye=`{
    "hepteracts": {
        "groupName": "Hepteract settings",
        "order": 1
    },
    "notifications": {
        "groupName": "Notification settings",
        "order": 2
    },
    "log": {
        "groupName": "Log settings",
        "order": 3
    },
    "mouse": {
        "groupName": "Mouse settings",
        "order": 4
    },
    "ambrosia": {
        "groupName": "Ambrosia quickbar settings",
        "order": 5
    },
    "ambrosia2": {
        "groupName": "Idle Swapper settings",
        "order": 6
    },
    "ambrosia3": {
        "groupName": "Minibar settings",
        "order": 7
    },
    "patch": {
        "groupName": "Patch settings",
        "order": 8
    },
    "gamedata": {
        "groupName": "Game data settings",
        "order": 9
    },
    "shop": {
        "groupName": "Shop settings",
        "order": 10
    },
    "debug": {
        "groupName": "Debugging settings",
        "order": 11
    },
    "auto-sing": {
        "groupName": "Auto-Sing settings",
        "order": 12
    },
    "auto-sing-strategy": {
        "groupName": "Auto-Sing strategy settings",
        "order": 13
    }
}`;var Je=`{
    "hepteract": {
        "page": "hepteract",
        "pageName": "Hepteracts",
        "pageColor": "mediumpurple",
        "order": 1
    },
    "ui": {
        "page": "ui",
        "pageName": "UI",
        "pageColor": "cyan",
        "order": 2
    },
    "log": {
        "page": "log",
        "pageName": "Logging",
        "pageColor": "orange",
        "order": 3
    },
    "input": {
        "page": "input",
        "pageName": "Input",
        "pageColor": "plum",
        "order": 4
    },
    "ambrosia": {
        "page": "ambrosia",
        "pageName": "Ambrosia",
        "pageColor": "gold",
        "order": 5
    },
    "patch": {
        "page": "patch",
        "pageName": "Patch",
        "pageColor": "fuchsia",
        "order": 6
    },
    "gamedata": {
        "page": "gamedata",
        "pageName": "Game Data",
        "pageColor": "crimson",
        "order": 7
    },
    "shop": {
        "page": "shop",
        "pageName": "Shop",
        "pageColor": "#b300b2",
        "order": 8
    },
    "debug": {
        "page": "debug",
        "pageName": "Debugging",
        "pageColor": "maroon",
        "order": 9
    },
    "auto-sing": {
        "page": "auto-sing",
        "pageName": "auto-sing",
        "pageColor": "grey",
        "order": 10
    },
    "misc": {
        "page": "misc",
        "pageName": "Misc.",
        "pageColor": "white",
        "order": 11
    }
}`;var ze=`[
    {
        "strategyName": "default_strategy",
        "strategy": [
            {
                "startPhase": "start",
                "endPhase": "challenge14",
                "corruptions": {
                    "viscosity": 0,
                    "drought": 0,
                    "deflation": 0,
                    "extinction": 0,
                    "illiteracy": 0,
                    "recession": 0,
                    "dilation": 0,
                    "hyperchallenged": 0
                },
                "strat": [
                    {
                        "challengeNumber": 105,
                        "challengeCompletions": 0,
                        "challengeWaitTime": 0,
                        "challengeMaxTime": 0
                    },
                    {
                        "challengeNumber": 6,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 10000
                    },
                    {
                        "challengeNumber": 7,
                        "challengeCompletions": 0,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 8,
                        "challengeCompletions": 0,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 9,
                        "challengeCompletions": 0,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 10,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 0,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 11,
                        "challengeCompletions": 0,
                        "challengeWaitTime": 0,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 6,
                        "challengeCompletions": 0,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 7,
                        "challengeCompletions": 0,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 8,
                        "challengeCompletions": 0,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 9,
                        "challengeCompletions": 0,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 10,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 100,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 12,
                        "challengeCompletions": 0,
                        "challengeWaitTime": 0,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 6,
                        "challengeCompletions": 0,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 7,
                        "challengeCompletions": 0,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 8,
                        "challengeCompletions": 0,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 9,
                        "challengeCompletions": 0,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 10,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 100,
                        "challengeMaxTime": 10000
                    },
                    {
                        "challengeNumber": 13,
                        "challengeCompletions": 0,
                        "challengeWaitTime": 0,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 10,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 100,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 14,
                        "challengeCompletions": 0,
                        "challengeWaitTime": 0,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 10,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 100,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 15,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 104,
                        "challengeCompletions": 0,
                        "challengeWaitTime": 0,
                        "challengeMaxTime": 0
                    },
                    {
                        "challengeNumber": 103,
                        "challengeCompletions": 0,
                        "challengeWaitTime": 0,
                        "challengeMaxTime": 0
                    }
                ]
            },
            {
                "startPhase": "challenge14",
                "endPhase": "alpha",
                "corruptions": {
                    "viscosity": 0,
                    "drought": 6,
                    "deflation": 4,
                    "extinction": 11,
                    "illiteracy": 0,
                    "recession": 14,
                    "dilation": 4,
                    "hyperchallenged": 1
                },
                "strat": [
                    {
                        "challengeNumber": 5,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 10000
                    },
                    {
                        "challengeNumber": 6,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 7,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 8,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 9,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 10,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 15,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 104,
                        "challengeCompletions": 0,
                        "challengeWaitTime": 0,
                        "challengeMaxTime": 0
                    },
                    {
                        "challengeNumber": 103,
                        "challengeCompletions": 0,
                        "challengeWaitTime": 0,
                        "challengeMaxTime": 0
                    }
                ]
            },
            {
                "startPhase": "alpha",
                "endPhase": "1e15-expo",
                "corruptions": {
                    "viscosity": 3,
                    "drought": 16,
                    "deflation": 1,
                    "extinction": 12,
                    "illiteracy": 16,
                    "recession": 16,
                    "dilation": 7,
                    "hyperchallenged": 7
                },
                "strat": [
                    {
                        "challengeNumber": 5,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 10000
                    },
                    {
                        "challengeNumber": 6,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 7,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 8,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 9,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 10,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 15,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 104,
                        "challengeCompletions": 0,
                        "challengeWaitTime": 0,
                        "challengeMaxTime": 0
                    },
                    {
                        "challengeNumber": 103,
                        "challengeCompletions": 0,
                        "challengeWaitTime": 0,
                        "challengeMaxTime": 0
                    }
                ]
            },
            {
                "startPhase": "1e15-expo",
                "endPhase": "omega",
                "corruptions": {
                    "viscosity": 6,
                    "drought": 16,
                    "deflation": 16,
                    "extinction": 13,
                    "illiteracy": 16,
                    "recession": 16,
                    "dilation": 11,
                    "hyperchallenged": 10
                },
                "strat": [
                    {
                        "challengeNumber": 5,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 6,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 7,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 8,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 9,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 10,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 15,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 104,
                        "challengeCompletions": 0,
                        "challengeWaitTime": 0,
                        "challengeMaxTime": 0
                    },
                    {
                        "challengeNumber": 103,
                        "challengeCompletions": 0,
                        "challengeWaitTime": 0,
                        "challengeMaxTime": 0
                    }
                ]
            },
            {
                "startPhase": "omega",
                "endPhase": "singularity",
                "corruptions": {
                    "viscosity": 10,
                    "drought": 16,
                    "deflation": 16,
                    "extinction": 16,
                    "illiteracy": 16,
                    "recession": 14,
                    "dilation": 14,
                    "hyperchallenged": 13
                },
                "strat": [
                    {
                        "challengeNumber": 106,
                        "challengeCompletions": 0,
                        "challengeWaitTime": 0,
                        "challengeMaxTime": 0
                    },
                    {
                        "challengeNumber": 5,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 10000
                    },
                    {
                        "challengeNumber": 6,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 7,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 8,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 9,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 10,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 15,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 104,
                        "challengeCompletions": 0,
                        "challengeWaitTime": 0,
                        "challengeMaxTime": 0
                    },
                    {
                        "challengeNumber": 103,
                        "challengeCompletions": 0,
                        "challengeWaitTime": 0,
                        "challengeMaxTime": 0
                    }
                ]
            },
            {
                "startPhase": "singularity",
                "endPhase": "end",
                "corruptions": {
                    "viscosity": 16,
                    "drought": 16,
                    "deflation": 16,
                    "extinction": 16,
                    "illiteracy": 16,
                    "recession": 16,
                    "dilation": 16,
                    "hyperchallenged": 16
                },
                "strat": [
                    {
                        "challengeNumber": 5,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 10000
                    },
                    {
                        "challengeNumber": 6,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 7,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 8,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 9,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 10,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 11,
                        "challengeCompletions": 0,
                        "challengeWaitTime": 0,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 10,
                        "challengeCompletions": 70,
                        "challengeWaitTime": 500,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 12,
                        "challengeCompletions": 0,
                        "challengeWaitTime": 0,
                        "challengeMaxTime": 10000
                    },
                    {
                        "challengeNumber": 10,
                        "challengeCompletions": 70,
                        "challengeWaitTime": 100,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 13,
                        "challengeCompletions": 0,
                        "challengeWaitTime": 0,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 10,
                        "challengeCompletions": 70,
                        "challengeWaitTime": 100,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 14,
                        "challengeCompletions": 0,
                        "challengeWaitTime": 0,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 10,
                        "challengeCompletions": 70,
                        "challengeWaitTime": 100,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 15,
                        "challengeCompletions": 0,
                        "challengeWaitTime": 0,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 5,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 100,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 6,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 100,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 7,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 100,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 8,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 100,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 9,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 100,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 10,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 100,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 104,
                        "challengeCompletions": 0,
                        "challengeWaitTime": 0,
                        "challengeMaxTime": 0
                    },
                    {
                        "challengeNumber": 103,
                        "challengeCompletions": 0,
                        "challengeWaitTime": 0,
                        "challengeMaxTime": 0
                    },
                    {
                        "challengeNumber": 5,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 100,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 6,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 100,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 7,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 100,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 8,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 100,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 9,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 100,
                        "challengeMaxTime": 1000000
                    },
                    {
                        "challengeNumber": 10,
                        "challengeCompletions": 1,
                        "challengeWaitTime": 100,
                        "challengeMaxTime": 1000000
                    }
                ]
            }
        ]
    }
]`;var w=class{static#e="HSUIC";static#t(e){if(!e)return"";let t="";for(let[a,o]of e.entries())t+=`data-${a}="${o}" `;return t}static#a(e){switch(e){case 1:return"checkbox";case 2:return"color";case 3:return"number";case 4:return"text";case 5:return"select";case 6:return"state"}}static#n(e){switch(e){case 1:return"hs-panel-input-checkbox";case 2:return"hs-panel-input-color";case 3:return"hs-panel-input-number";case 4:return"hs-panel-input-text";case 5:return"hs-panel-input-select";case 6:return"hs-panel-input-state"}}static#i(e){if(!e)return"";let t="";for(let[a,o]of Object.entries(e))typeof o!==void 0&&(t+=` ${a}="${o}"`);return t}static Button(e){let t=e.class??"",a=e.text??"",o=e.id??h.domid(),i=this.#i(e.props),n=this.#t(e.data);return f.injectStyle(`#${o} {
            ${h.objectToCSS(e.styles)}
        }`),`<div class="hs-panel-btn ${t}" id="${o}"${i} ${n}>${a}</div>`}static Input(e){let t=e.class??"",a=this.#a(e.type),o=this.#n(e.type),i=e.id??h.domid(),n=this.#i(e.props),s=this.#t(e.data);return f.injectStyle(`#${i} {
            ${h.objectToCSS(e.styles)}
        }`),`<input type="${a}" class="${o} ${t}" id="${i}"${n} ${s}></input>`}static Select(e,t){let a=e.class??"",o=this.#a(e.type),i=this.#n(e.type),n=e.id??h.domid(),s=this.#i(e.props),l=this.#t(e.data);f.injectStyle(`#${n} {
            ${h.objectToCSS(e.styles)}
        }`);let u=t.map(d=>`<option value="${d.value}" ${d.selected?"selected":""}>${d.text}</option>`).join(`
`);return`<select class="${i} ${a}" id="${n}"${s} ${l}>${u}</select>`}static Div(e){let t=e.class??"",a=e.id??h.domid(),o="",i=this.#i(e.props),n=this.#t(e.data);return e.html&&(Array.isArray(e.html)?o=e.html.join(`
`):o=e.html),f.injectStyle(`#${a} {
            ${h.objectToCSS(e.styles)}
        }`),`<div class="hs-panel-div ${t}" ${a?`id="${a}"`:""}${i} ${n}>${o}</div>`}static P(e){let t=e.class??"",a=e.id??h.domid(),o="",i=this.#i(e.props),n=this.#t(e.data);return e.text&&(Array.isArray(e.text)?o=e.text.join(`
`):o=e.text),f.injectStyle(`#${a} {
            ${h.objectToCSS(e.styles)}
        }`),`<p class="hs-panel-p ${t}" ${a?`id="${a}"`:""}${i} ${n}>${o}</p>`}static Image(e){let t=e.class??"",a=e.id??h.domid(),o=this.#i(e.props),i=this.#t(e.data);if(!e.src)return c.warn("HSUIC.Image: No src provided for image component",this.#e),"";let n=e.width?e.width:A.HSUIC.defaultImageWidth,s=e.height?e.height:A.HSUIC.defaultImageHeight;return f.injectStyle(`#${a} {
            ${h.objectToCSS(e.styles)}
        }`),`<img src="${e.src}" width="${n}" height="${s}" class="hs-panel-img ${t}" ${a?`id="${a}"`:""}${o} ${i}/>`}static Grid(e){let t=e.class??"",a=e.id??h.domid(),o="",i=this.#i(e.props),n=this.#t(e.data);return e.html&&(Array.isArray(e.html)?o=e.html.join(`
`):o=e.html),f.injectStyle(`#${a} {
            display: grid;
            ${h.objectToCSS(e.styles)}
        }`),`<div class="hs-panel-div ${t}" id="${a}"${i} ${n}>${o}</div>`}static Flex(e){let t=e.class??"",a=e.id??h.domid(),o="",i=this.#i(e.props),n=this.#t(e.data);return e.html&&(Array.isArray(e.html)?o=e.html.join(`
`):o=e.html),f.injectStyle(`#${a} {
            display: flex;
            ${h.objectToCSS(e.styles)}
        }`),`<div class="hs-panel-div ${t}" id="${a}"${i} ${n}>${o}</div>`}static _modal(e){let t=e.class??"",a=e.htmlContent??"",o=e.data??[],i=e.title??"",n=e.id??h.domid(),s=this.#i(e.props),l=this.#t(e.data);return f.injectStyle(`#${n} {
            ${h.objectToCSS(e.styles)}
        }`),`<div class="hs-modal ${t}" id="${n}"${s} ${l}>
                    <div class="hs-modal-head">
                        <div class="hs-modal-head-left">
                        <div class="hs-modal-title">${i}</div>
                        </div>
                        <div class="hs-modal-head-right" data-close="${e.id}">x</div>
                    </div>
                    <div class="hs-modal-body">
                        ${a}
                    </div>
                    <div class="hs-modal-resizer"></div>
                </div>`}};var Ze=`.hs-panel-body::-webkit-scrollbar,
#hs-ui-log::-webkit-scrollbar {
    width: 5px;
    height: 5px;
}

.hs-panel-body::-webkit-scrollbar-track,
#hs-ui-log::-webkit-scrollbar-track {
    background: #1c1b22;
    border-radius: 3px;
}

.hs-panel-body::-webkit-scrollbar-thumb,
#hs-ui-log::-webkit-scrollbar-thumb {
    background: #a22a2a;
    border-radius: 3px;
}

.hs-panel-body::-webkit-scrollbar-corner,
#hs-ui-log::-webkit-scrollbar-corner {
    background: #1c1b22;
}

@supports not selector(::-webkit-scrollbar) {

    .hs-panel-body::-webkit-scrollbar,
    #hs-ui-log::-webkit-scrollbar {
        scrollbar-color: #a22a2a #1c1b22;
    }
}


#hs-panel,
.hs-panel-header,
.hs-panel-header-left,
.hs-panel-header-right,
.hs-panel-tabs,
.hs-panel-tab,
.hs-panel-body {
    box-sizing: border-box;
}

.hs-panel-closed {
    display: none;
}

#hs-panel {
    width: 500px;
    height: 400px;
    position: absolute;
    top: 100px;
    left: 100px;
    z-index: 7000;

    background-color: #1c1b22;
    border: 1px solid white;
    border-radius: 3px;

    -webkit-box-shadow: 3px 3px 5px 0px rgba(0, 0, 0, 0.75);
    -moz-box-shadow: 3px 3px 5px 0px rgba(0, 0, 0, 0.75);
    box-shadow: 3px 3px 5px 0px rgba(0, 0, 0, 0.75);

    font-family: -apple-system,
        BlinkMacSystemFont,
        "Segoe UI",
        Roboto,
        Oxygen,
        Ubuntu,
        Cantarell,
        "Open Sans",
        "Helvetica Neue",
        sans-serif;

    opacity: 0.92;
}

.hs-panel-header {
    width: 100%;
    height: 45px;
    line-height: calc(45px - (5px * 2));
    font-size: 14pt;
    color: white;
    background-color: #353439;
    display: flex;
}

.hs-panel-header-left {
    padding: 5px 10px 5px 10px;
    flex-grow: 1;
}

.hs-panel-header-right {
    width: 36px;
    height: 36px;
    margin: 4px;
    border: 1px solid white;
    flex-grow: 0;
    line-height: 35px;
    text-align: center;
    font-size: 14pt;
    font-weight: bold;
}

.hs-panel-header-right:hover {
    background-color: maroon;
    cursor: pointer;
}

.hs-panel-tabs {
    width: 100%;
    height: 40px;
    padding: 0px 0px 0px 0px;
    color: white;

    display: flex;
    flex-direction: row;
    flex-wrap: nowrap;
    justify-content: center;
    align-items: center;
    align-content: space-between;
}

.hs-panel-tab {
    height: 100%;
    flex-grow: 1;
    flex-shrink: 1;
    flex-basis: auto;
    display: flex;
    justify-content: center;
    font-weight: bold;
    line-height: 40px;
    margin: 0px 2px 0px 2px;
    border-radius: 0px;
    font-size: 13pt;
}

.hs-panel-tab:hover {
    background-color: #006;
    cursor: pointer;
}

#hs-panel-tab-1 {
    border-top: 1px solid orange;
    border-left: 1px solid orange;
    border-right: 1px solid orange;
}

#hs-panel-tab-1.hs-tab-selected {
    background-color: orange;
}

#hs-panel-tab-2 {
    border-top: 1px solid cyan;
    border-left: 1px solid cyan;
    border-right: 1px solid cyan;
}

#hs-panel-tab-2.hs-tab-selected {
    background-color: blue;
}

#hs-panel-tab-3 {
    border-top: 1px solid maroon;
    border-left: 1px solid maroon;
    border-right: 1px solid maroon;
}

#hs-panel-tab-3.hs-tab-selected {
    background-color: maroon;
}

#hs-panel-tab-4 {
    border-top: 1px solid plum;
    border-left: 1px solid plum;
    border-right: 1px solid plum;
}

#hs-panel-tab-4.hs-tab-selected {
    background-color: plum;
}

#hs-panel-tab-5 {
    border-top: 1px solid royalblue;
    border-left: 1px solid royalblue;
    border-right: 1px solid royalblue;
}

#hs-panel-tab-5.hs-tab-selected {
    background-color: royalblue;
}

.hs-panel-body {
    width: 100%;
    height: calc(100% - 45px - 40px);
    background-color: #18171c;
    border-top: 1px solid white;
    display: none;
    padding: 5px;
    overflow-y: auto;
}

.hs-panel-body-open-flex {
    display: flex;
}

.hs-panel-body-open-block {
    display: block;
}

#hs-log-panel {
    flex-direction: column;
    flex-wrap: nowrap;
    justify-content: flex-start;
    align-items: flex-start;
    align-content: space-between;

    padding: 0;
    overflow: hidden;
}

#hs-ui-log {
    width: 100%;
    resize: none;
    box-sizing: border-box;

    background-color: #18171c;
    color: white;

    overflow-x: auto;
    overflow-y: scroll;

    flex-grow: 1;

    padding: 5px;
}

.hs-ui-log-line {
    font-family: consolas, courier-new;
    font-size: 9pt;
    white-space: nowrap;
}

.hs-ui-log-line-info {
    color: aqua;
}

.hs-ui-log-line-warn {
    color: yellow;
}

.hs-ui-log-line-error {
    color: red;
}

.hs-ui-log-line-debug {
    font-style: italic;
}

.hs-log-ts-hidden {
    display: none;
}

.hs-log-ctx {}

.hs-log-ts {}

#hs-ui-log-controls {
    width: 100%;
    height: 35px;
    box-sizing: border-box;
    margin: 5px;
    display: flex;
    flex-direction: row;
    flex-wrap: nowrap;
    justify-content: flex-start;
    align-items: center;
    align-content: space-between;
}

#hs-panel-control {
    position: absolute;
    top: 10px;
    right: 10px;
    width: 35px;
    height: 35px;
    background-image: url(https://synergism.cc/Pictures/Default/OcteractCorruptions.png);
    background-repeat: no-repeat;
    background-size: contain;
    transform-origin: 50% 50%;
}

#hs-panel-control:hover {
    cursor: pointer;
    transform: scale(1.05);
}

.hs-panel-btn {
    border: 2px solid white;
    min-height: 30px;
    color: white;
    transition: background-color 0.15s, border-color 0.15s;
    cursor: pointer;
    background-color: #101828;
    width: 130px;
    height: 30px;
    line-height: 30px;
    text-align: center;
}

.hs-panel-btn:hover {
    background-color: #005;
}

.hs-modal {
    width: auto;
    height: auto;
    position: absolute;
    z-index: 7000;

    top: -9001px;
    left: -9001px;

    background-color: #1c1b22;
    border: 1px solid white;
    border-radius: 3px;

    -webkit-box-shadow: 3px 3px 5px 0px rgba(0, 0, 0, 0.75);
    -moz-box-shadow: 3px 3px 5px 0px rgba(0, 0, 0, 0.75);
    box-shadow: 3px 3px 5px 0px rgba(0, 0, 0, 0.75);

    font-family: -apple-system,
        BlinkMacSystemFont,
        "Segoe UI",
        Roboto,
        Oxygen,
        Ubuntu,
        Cantarell,
        "Open Sans",
        "Helvetica Neue",
        sans-serif;

    opacity: 0.97;
}

.hs-modal-head {
    width: 100%;
    height: 45px;
    line-height: calc(45px - (5px * 2));
    font-size: 14pt;
    color: white;
    background-color: #353439;
    display: flex;
}

.hs-modal-head-left {
    padding: 5px 10px 5px 10px;
    flex-grow: 1;
}

.hs-modal-head-right {
    width: 36px;
    height: 36px;
    margin: 4px;
    border: 1px solid white;
    flex-grow: 0;
    line-height: 35px;
    text-align: center;
    font-size: 14pt;
    font-weight: bold;
}

.hs-modal-head-right:hover {
    background-color: maroon;
    cursor: pointer;
}

.hs-modal-body {
    width: 100%;
    height: calc(100% - 45px);
    background-color: #18171c;
    border-top: 1px solid white;
    padding: 10px;
    box-sizing: border-box;
    overflow-x: hidden;
    overflow-y: auto;
}

.hs-modal-img {
    image-rendering: auto;
    width: 100%;
    max-width: 40vw;
}

#hs-settings-panel {
    padding: 10px;
    scrollbar-gutter: stable;
}

.hs-panel-setting-block {
    display: grid;
    grid-column: span 2;
    grid-template-columns: 2fr auto;
    /*grid-template-rows: auto auto;*/
    grid-column-gap: 5px;
    grid-row-gap: 5px;
    width: 100%;
    padding-bottom: 10px;
    /*border-bottom: 1px solid white;*/
    box-sizing: border-box;
    padding-left: 10px;
}

#hs-settings-panel>.hs-panel-setting-block:not(:last-child) {
    /*padding-bottom: 10px;
    margin-bottom: 10px;*/
}

.hs-panel-setting-block-text-wrapper {
    grid-column: 1 / span 2;
    box-sizing: border-box;
}

.hs-panel-setting-block-text:hover {
    cursor: help;
}

.hs-modal-resizer {
    position: absolute;
    bottom: 0;
    right: 0;
    width: 16px;
    height: 16px;
    cursor: nwse-resize;
    background: linear-gradient(135deg, transparent 0%, transparent 50%, #888 50%, #888 100%);
}

.hs-modal-resizer:hover {
    background: linear-gradient(135deg, transparent 0%, transparent 50%, #aaa 50%, #aaa 100%);
}

.hs-panel-setting-block-num-input {
    display: block;
    height: 30px;
    border: 1px solid green;
    grid-column: 1 / 2;
    box-sizing: border-box;
}

.hs-panel-setting-block-select-input {
    display: block;
    height: 30px;
    border: 1px solid green;
    grid-column: 1 / 2;
    box-sizing: border-box;
    min-height: 20px;
    color: white;
    padding: 0 0 0 0.2em;
    background-color: #101828;
}

.hs-panel-setting-block-select-input:hover {
    cursor: pointer;
}

.hs-panel-setting-block-state {
    margin: 0;
}

.hs-panel-setting-block-btn {
    width: 30px;
    border: 1px solid green;
    display: block;
    grid-column: 2 / 3;
    height: 30px;
    box-sizing: border-box;
}

.hs-panel-settings-block-btn-standalone {
    grid-column: 1 / 1;
}

.hs-panel-setting-block-gamedata-icon {
    margin-left: 5px;
}

.hs-panel-setting-block-gamedata-icon:hover {
    cursor: help;
    filter: brightness(150%);
}

.hs-disabled {
    border: 1px solid red;
}

.hs-ambrosia-slot {
    background-repeat: no-repeat;
    background-size: 80%;
    background-position: center;
}

.hs-notification {
    position: fixed;
    width: 300px;
    max-width: 300px;
    height: 50px;
    max-height: 50px;
    color: #f5f6fa;
    border: 1px solid #f5f6fa;
    display: flex;
    align-items: center;
    justify-content: center;
    z-index: 7000;
    padding: 5px;
    box-sizing: border-box;
}

.hs-notification-text {
    text-overflow: ellipsis;
    overflow: hidden;
    font-size: 10pt;
    max-height: 36px;
    text-align: center;
    box-sizing: border-box;
}

.hs-panel-info-h1 {
    font-size: 16pt;
    margin-left: 5px;
}

.hs-panel-info-h2 {
    font-size: 13pt;
    margin-left: 10px;
}

.hs-panel-info-p {
    font-size: 11pt;
    margin-left: 15px;
}

.hs-panel-subtabs {
    width: 100%;
    height: auto;
    padding: 0px 0px 0px 0px;
    color: white;
    display: flex;
    flex-direction: row;
    flex-wrap: wrap;
    justify-content: center;
    align-items: center;
    gap: 5px;
}

.hs-panel-subtab {
    height: 25px;
    width: 100px;
    max-width: 100px;
    flex-grow: 1;
    flex-shrink: 1;
    flex-basis: auto;
    display: flex;
    justify-content: center;
    font-weight: bold;
    line-height: 25px;
    margin: 0px 2px 0px 2px;
    border-radius: 0px;
    font-size: 11pt;
}

.hs-panel-subtab:hover {
    background-color: #006;
    cursor: pointer;
}

.hs-panel-settings-grid {
    display: none;
    margin-top: 15px;
}

.hs-panel-settings-grid.open {
    display: block;
}

.hs-resizer {
    position: absolute;
    bottom: 2px;
    right: 2px;
    width: 10px;
    height: 10px;
    cursor: se-resize;
    user-select: none;
    background: linear-gradient(to right, #ee64ff 2px, transparent 2px) 0 0, linear-gradient(to left, #ee64ff 2px, transparent 2px) 100% 100%, linear-gradient(to bottom, #ee64ff 2px, transparent 2px) 0 0, linear-gradient(to top, #ee64ff 2px, transparent 2px) 100% 100%;
    background-repeat: no-repeat;
    background-size: 5px 5px;
}

a.hs-link:visited {
    color: #ff3d00;
}

a.hs-link:hover {
    color: #ff3d00;
    text-decoration: underline;
}

a.hs-link {
    color: #ff3d00;
    text-decoration: none;
}

.hs-rainbowBorder {
    border: 2px solid;
    --angle: 0deg;
    border-image: conic-gradient(from var(--angle), #ff5e00, #ff9a00, #ffcd00, #e5ff00, #a5ff00, #00ffc8, #00c8ff, #00a5ff, #9500ff, #ff00e1, #ff0095, #ff5e00) 1;
    animation: hue-rotate 6s linear infinite;
}

#hs-panel-new-ver {
    background: linear-gradient(to right, #e74949 20%, #f9bb44 30%, #ff982e 70%, #b73636 80%);
    -webkit-background-clip: text;
    background-clip: text;
    -webkit-text-fill-color: transparent;
    background-size: 500% auto;
    animation: hs-animtext-1 3.5s ease-in-out infinite alternate;
}

/* Strategy Modal Styles */
.hs-strategy-modal-container {
    display: flex;
    flex-direction: column;
    gap: 16px;
    padding: 20px;
    width: 100%;
    height: 100%;
    min-width: 500px;
    min-height: 400px;
    background-color: #18171c;
    border-radius: 3px;
    box-sizing: border-box;
}

.hs-strategy-input-section {
    display: flex;
    flex-direction: column;
    gap: 12px;
}

.hs-strategy-input-label {
    font-weight: 600;
    color: white;
    font-size: 14px;
    margin-bottom: 4px;
}

.hs-strategy-name-input {
    padding: 10px 14px;
    background-color: #101828;
    border: 2px solid white;
    border-radius: 3px;
    color: white;
    font-size: 14px;
    transition: all 0.2s;
    box-sizing: border-box;
}

.hs-strategy-name-input:focus {
    border-color: cyan;
    background-color: #005;
    outline: none;
}

.hs-strategy-phase-list {
    flex: 1;
    background-color: #1c1b22;
    border-radius: 3px;
    border: 1px solid #353439;
    padding: 12px;
    overflow-y: auto;
    box-sizing: border-box;
}

.hs-strategy-phase-item {
    display: flex;
    align-items: center;
    gap: 8px;
    padding: 8px 12px;
    background-color: #353439;
    border: 1px solid white;
    border-radius: 3px;
    margin-bottom: 6px;
    transition: all 0.15s;
}

.hs-strategy-phase-item:hover {
    background-color: #005;
    border-color: cyan;
}

.hs-strategy-phase-text {
    flex: 1;
    font-weight: 500;
    color: white;
    font-size: 12px;
}

.hs-strategy-phase-arrow {
    color: cyan;
    font-weight: bold;
}

.hs-strategy-btn-group {
    display: flex;
    gap: 8px;
    justify-content: flex-end;
}

.hs-strategy-btn {
    padding: 10px 20px;
    border-radius: 3px;
    border: 2px solid white;
    font-weight: 600;
    cursor: pointer;
    transition: all 0.15s;
    text-align: center;
    background-color: #101828;
    color: white;
    min-height: 30px;
    line-height: 30px;
}

.hs-strategy-btn:hover {
    background-color: #005;
}

.hs-strategy-btn-primary {
    border-color: green;
}

.hs-strategy-btn-primary:hover {
    background-color: #006;
}

.hs-strategy-btn-secondary {
    border-color: white;
}

.hs-strategy-btn-icon {
    padding: 4px 8px;
    font-size: 13px;
    min-width: 26px;
    width: 26px;
    height: 26px;
    line-height: 16px;
    border: 1px solid white;
}

.hs-strategy-btn-edit {
    border-color: orange;
}

.hs-strategy-btn-edit:hover {
    background-color: #660;
}

.hs-strategy-btn-delete {
    border-color: red;
}

.hs-strategy-btn-delete:hover {
    background-color: maroon;
}

.hs-strategy-empty-state {
    text-align: center;
    color: #888;
    padding: 40px 20px;
    font-style: italic;
}

/* Phase Modal Styles */
.hs-phase-modal-container {
    display: flex;
    flex-direction: column;
    gap: 20px;
    padding: 20px;
    width: 100%;
    height: 100%;
    min-width: 500px;
    min-height: 400px;
    background-color: #18171c;
    border-radius: 3px;
    box-sizing: border-box;
}

.hs-phase-select-group {
    display: flex;
    flex-direction: column;
    gap: 8px;
}

.hs-phase-label {
    font-weight: 600;
    color: white;
    font-size: 14px;
}

.hs-phase-select {
    padding: 10px 14px;
    background-color: #101828;
    border: 2px solid white;
    border-radius: 3px;
    color: white;
    font-size: 14px;
    cursor: pointer;
    transition: all 0.15s;
    box-sizing: border-box;
}

.hs-phase-select:focus {
    border-color: plum;
    background-color: #005;
    outline: none;
}

.hs-phase-select:disabled {
    opacity: 0.5;
    cursor: not-allowed;
    border-color: #666;
}

.hs-phase-config-group {
    display: flex;
    gap: 12px;
}

.hs-phase-config-btn {
    flex: 1;
    padding: 14px 20px;
    background-color: #101828;
    border: 2px solid plum;
    color: white;
    border-radius: 3px;
    font-weight: 600;
    cursor: pointer;
    text-align: center;
    transition: all 0.15s;
}

.hs-phase-config-btn:hover {
    background-color: #006;
}

.hs-phase-footer {
    display: flex;
    justify-content: flex-end;
}

.hs-phase-done-btn {
    padding: 12px 32px;
    background-color: #101828;
    border: 2px solid green;
    color: white;
    border-radius: 3px;
    font-weight: 600;
    cursor: pointer;
    transition: all 0.15s;
}

.hs-phase-done-btn:hover {
    background-color: #060;
}

/* Corruption Modal Styles */
.hs-corruption-modal-container {
    display: flex;
    flex-direction: column;
    gap: 20px;
    padding: 20px;
    width: 100%;
    height: 100%;
    min-width: 550px;
    min-height: 450px;
    background-color: #18171c;
    border-radius: 3px;
    box-sizing: border-box;
}

.hs-corruption-grid {
    display: grid;
    grid-template-columns: repeat(2, 1fr);
    gap: 16px;
    flex: 1;
    overflow-y: auto;
}

.hs-corruption-item {
    display: flex;
    align-items: center;
    gap: 12px;
    padding: 14px;
    background-color: #353439;
    border: 1px solid white;
    border-radius: 3px;
    transition: all 0.15s;
}

.hs-corruption-item:hover {
    background-color: #005;
    border-color: maroon;
}

.hs-corruption-icon {
    width: 40px;
    height: 40px;
    flex-shrink: 0;
}

.hs-corruption-label {
    flex: 1;
    font-weight: 600;
    color: white;
    font-size: 14px;
}

.hs-corruption-input {
    width: 70px;
    padding: 8px 12px;
    background-color: #101828;
    border: 2px solid white;
    border-radius: 3px;
    color: white;
    font-size: 14px;
    font-weight: 600;
    text-align: center;
    transition: all 0.15s;
    box-sizing: border-box;
}

.hs-corruption-input:focus {
    border-color: maroon;
    background-color: #005;
    outline: none;
}

.hs-corruption-footer {
    display: flex;
    justify-content: flex-end;
    padding-top: 10px;
    border-top: 1px solid #353439;
}

.hs-corruption-done-btn {
    padding: 12px 32px;
    background-color: #101828;
    border: 2px solid green;
    color: white;
    border-radius: 3px;
    font-weight: 600;
    cursor: pointer;
    transition: all 0.15s;
}

.hs-corruption-done-btn:hover {
    background-color: #060;
}

/* Challenges Modal Styles */
.hs-challenges-modal-container {
    display: flex;
    flex-direction: column;
    gap: 12px;
    padding: 16px;
    width: 100%;
    height: 100%;
    min-width: 550px;
    min-height: 500px;
    background-color: #18171c;
    border-radius: 3px;
    box-sizing: border-box;
}

.hs-challenges-input-section {
    display: grid;
    grid-template-columns: 1fr 1fr;
    gap: 8px;
    padding: 12px;
    background-color: #353439;
    border: 1px solid white;
    border-radius: 3px;
}

.hs-challenges-input-row {
    display: flex;
    align-items: center;
    gap: 8px;
}

.hs-challenges-input-label {
    width: 90px;
    font-weight: 600;
    color: white;
    font-size: 12px;
    white-space: nowrap;
}

.hs-challenges-input {
    flex: 1;
    padding: 5px 8px;
    background-color: #101828;
    border: 2px solid white;
    border-radius: 3px;
    color: white;
    font-size: 12px;
    transition: all 0.15s;
    box-sizing: border-box;
}

.hs-challenges-input:focus {
    border-color: royalblue;
    background-color: #005;
    outline: none;
}

.hs-challenges-add-btn {
    grid-column: 1 / -1;
    padding: 6px 16px;
    background-color: #101828;
    border: 2px solid green;
    color: white;
    border-radius: 3px;
    font-weight: 600;
    cursor: pointer;
    text-align: center;
    transition: all 0.15s;
    margin-top: 2px;
    font-size: 12px;
}

.hs-challenges-action-row {
    display: flex;
    align-items: center;
    gap: 8px;
    margin-bottom: 4px;
}

.hs-challenges-action-label {
    width: 80px;
    font-weight: 600;
    color: white;
    font-size: 13px;
}

.hs-challenges-action-select {
    flex: 1;
    padding: 6px 10px;
    background-color: #101828;
    border: 2px solid white;
    border-radius: 3px;
    color: white;
    font-size: 13px;
    transition: all 0.15s;
    box-sizing: border-box;
}

.hs-challenges-add-btn:hover {
    background-color: #060;
}

.hs-challenges-list-container {
    flex: 1;
    overflow-y: auto;
    padding: 10px;
    background-color: #1c1b22;
    border: 1px solid #353439;
    border-radius: 3px;
    box-sizing: border-box;
}

.hs-challenge-item {
    display: flex;
    align-items: center;
    gap: 6px;
    padding: 6px 8px;
    background-color: #353439;
    border: 1px solid white;
    border-radius: 3px;
    margin-bottom: 4px;
    transition: all 0.15s;
}

.hs-challenge-item:hover {
    background-color: #005;
    border-color: royalblue;
}

.hs-challenge-item-text {
    flex: 1;
    font-weight: 500;
    color: white;
    font-size: 12px;
}

.hs-challenge-item-text>div {
    font-size: 11px;
    color: #4db8ca;
    margin-top: 2px;
}

.hs-challenge-controls {
    display: grid;
    grid-template-columns: 1fr 1fr;
    grid-template-rows: 1fr 1fr;
    gap: 3px;
}

.hs-challenge-btn {
    padding: 3px 6px;
    min-width: 24px;
    width: 24px;
    height: 24px;
    cursor: pointer;
    background-color: #101828;
    border: 1px solid white;
    border-radius: 3px;
    text-align: center;
    font-weight: 600;
    line-height: 16px;
    font-size: 11px;
    transition: all 0.15s;
}

.hs-challenge-btn:hover {
    background-color: #005;
}

.hs-challenge-btn-edit {
    border-color: orange;
}

.hs-challenge-btn-edit:hover {
    background-color: #660;
}

.hs-challenge-btn-delete {
    border-color: red;
}

.hs-challenge-btn-delete:hover {
    background-color: maroon;
}

.hs-challenges-footer {
    display: flex;
    justify-content: flex-end;
    gap: 10px;
    padding-top: 8px;
    border-top: 1px solid #353439;
}

.hs-challenges-footer-btn {
    padding: 10px 24px;
    border-radius: 3px;
    border: 2px solid white;
    font-weight: 600;
    cursor: pointer;
    transition: all 0.15s;
    background-color: #101828;
    color: white;
    font-size: 13px;
}

.hs-challenges-cancel-btn {
    border-color: white;
}

.hs-challenges-cancel-btn:hover {
    background-color: #005;
}

.hs-challenges-save-btn {
    border-color: green;
}

.hs-challenges-save-btn:hover {
    background-color: #060;
}

.hs-challenges-empty-state {
    text-align: center;
    color: #888;
    padding: 40px 20px;
    font-style: italic;
}

.hs-strategy-phase-list::-webkit-scrollbar,
.hs-corruption-grid::-webkit-scrollbar,
.hs-challenges-list-container::-webkit-scrollbar {
    width: 5px;
    height: 5px;
}

.hs-strategy-phase-list::-webkit-scrollbar-track,
.hs-corruption-grid::-webkit-scrollbar-track,
.hs-challenges-list-container::-webkit-scrollbar-track {
    background: #1c1b22;
    border-radius: 3px;
}

.hs-strategy-phase-list::-webkit-scrollbar-thumb,
.hs-corruption-grid::-webkit-scrollbar-thumb,
.hs-challenges-list-container::-webkit-scrollbar-thumb {
    background: #a22a2a;
    border-radius: 3px;
}

.hs-strategy-error {
    margin-top: 8px;
    padding: 6px 10px;
    background: rgba(255, 0, 0, 0.12);
    border-left: 3px solid #c33;
    color: #900;
    font-size: 0.9em;
}

@keyframes hs-animtext-1 {
    0% {
        background-position: 0% 50%;
    }

    100% {
        background-position: 100% 50%;
    }
}

@-webkit-keyframes hs-animtext-1 {
    0% {
        background-position: 0% 50%;
    }

    100% {
        background-position: 100% 50%;
    }
}`;var We=`<div id="hs-panel" class="hs-panel-closed">
    <div class="hs-panel-header">
        <div class="hs-panel-header-left" id="hs-panel-version">Hypersynergism</div>
        <div class="hs-panel-header-right">X</div>
    </div>
    <div class="hs-panel-tabs">
        <div class="hs-panel-tab hs-tab-selected" id="hs-panel-tab-1" data-tab="1">Log</div>
        <div class="hs-panel-tab" id="hs-panel-tab-5" data-tab="5">Info</div>
        <div class="hs-panel-tab" id="hs-panel-tab-2" data-tab="2">Tools</div>
        <div class="hs-panel-tab" id="hs-panel-tab-3" data-tab="3">Settings</div>
        <div class="hs-panel-tab" id="hs-panel-tab-4" data-tab="4">Debug</div>
    </div>
    <div class="hs-panel-body hs-panel-body-1 hs-panel-body-open-flex" id="hs-log-panel">
        <div id="hs-ui-log"></div>
        <div id="hs-ui-log-controls">
            <button class="hs-panel-btn" id="hs-ui-log-clear">Clear log</button>
        </div>
    </div>
    <div class="hs-panel-body hs-panel-body-2" id="hs-tools-panel">Tools</div>
    <div class="hs-panel-body hs-panel-body-3" id="hs-settings-panel">Setting</div>
    <div class="hs-panel-body hs-panel-body-4" id="hs-debug-panel"></div>
    <div class="hs-panel-body hs-panel-body-5" id="hs-info-panel">
        <h1 class="hs-panel-info-h1">Important information about the mod can be found here</h1>

        <h2 class="hs-panel-info-h2">12.5.2025 - Game Data Sniffing (GDS) and Singularity</h2>
        <p class="hs-panel-info-p">
            The mod will automatically disable GDS when the player enters a new Singularity or exits or leaves Singularity challenge.</br></br>
            This is because GDS reads and writes to localStorage heavily, which is fine otherwise,
            but results in stuttering and other issues if it is enabled when starting a new singularity.</br></br>
            <span style = "color: chartreuse">After a few seconds, the mod will automatically re-enable GDS for the new Singularity.</span>
        </p>
    </div>
    <div class="hs-resizer"></div>
</div>`;var f=class r extends C{constructor(t){super(t);this.uiReady=!1;this.#d=new Set;this.#b=[{tabId:1,tabBodySel:".hs-panel-body-1",tabSel:"#hs-panel-tab-1",panelDisplayType:"flex"},{tabId:2,tabBodySel:".hs-panel-body-2",tabSel:"#hs-panel-tab-2",panelDisplayType:"block"},{tabId:3,tabBodySel:".hs-panel-body-3",tabSel:"#hs-panel-tab-3",panelDisplayType:"block"},{tabId:4,tabBodySel:".hs-panel-body-4",tabSel:"#hs-panel-tab-4",panelDisplayType:"block"},{tabId:5,tabBodySel:".hs-panel-body-5",tabSel:"#hs-panel-tab-5",panelDisplayType:"block"}];this.#a=Ze,this.#t=We}static#e="HSUI";#t;#a;#n;#i;#o;#s;#r;#l;static#u=!1;#d;static#m=new Map;static#g;#b;async init(){c.log("Initialising HSUI module",this.context);let t=this;r.#g=document.createElement("style"),r.#g.id=A.HSUI.injectedStylesDomId,document.head.appendChild(r.#g),r.injectStyle(this.#a,"hs-panel-css"),r.injectHTMLString(this.#t),this.#n=await S.HookElement("#hs-panel"),this.#i=await S.HookElement("#hs-panel-version"),this.#o=await S.HookElement(".hs-panel-header-right"),this.#r=await S.HookElement("#hs-ui-log"),this.#l=await S.HookElement("#hs-ui-log-clear");let a=await S.HookElement(".hs-panel-header"),o=await S.HookElement(".hs-resizer");this.#f(this.#n,a),this.#M(this.#n,o),this.#o.addEventListener("click",async()=>{r.#u&&t.#n&&(await t.#n.transition({opacity:0}),r.#u=!1,t.#n?.classList.add("hs-panel-closed"))}),this.#l.addEventListener("click",()=>{c.clear()});let i=document.querySelectorAll(".hs-panel-tab");i.forEach(n=>{n.addEventListener("click",s=>{let l=s.target,u=l.dataset.tab?parseInt(l.dataset.tab,10):null;if(!l.classList.contains("hs-tab-selected"))if(u){let d=t.#b.find(m=>m.tabId===u);if(!d){c.error(`Could not find tab config for tabId ${u}`,t.context);return}i.forEach(m=>{m.classList.remove("hs-tab-selected")}),l.classList.add("hs-tab-selected"),document.querySelectorAll(".hs-panel-body").forEach(m=>{m.classList.remove("hs-panel-body-open-flex"),m.classList.remove("hs-panel-body-open-block")});let g=document.querySelector(d.tabBodySel);if(g){switch(d.panelDisplayType){case"flex":g.classList.add("hs-panel-body-open-flex");break;case"block":g.classList.add("hs-panel-body-open-block");break}u===1&&c.scrollToBottom()}}else c.error("tabId is null",t.context)})}),this.#s=document.createElement("div"),this.#s.id="hs-panel-control",this.#s.addEventListener("click",async()=>{if(!r.#u&&t.#n){r.#u=!0,t.#n.style.opacity="0",t.#n.classList.remove("hs-panel-closed");let n=t.#v(1,t.#n);t.#n.style.left=`${n.x}px`,t.#n.style.top=`${n.y}px`,c.scrollToBottom(),await t.#n.transition({opacity:.92})}}),document.body.appendChild(this.#s),this.uiReady=!0,this.isInitialized=!0}static isModPanelOpen(){return r.#u}#f(t,a){let o=0,i=0,n=0,s=0;a.onmousedown=l;function l(g){g.preventDefault(),n=g.clientX,s=g.clientY,document.onmouseup=d,document.onmousemove=u}function u(g){g.preventDefault(),o=n-g.clientX,i=s-g.clientY,n=g.clientX,s=g.clientY;let m=t.offsetTop-i,p=t.offsetLeft-o,b=t.getBoundingClientRect(),M=window.innerWidth,E=window.innerHeight,y=45,T=10;p=Math.max(-(b.width-y),Math.min(p,M-y)),m=Math.max(T,Math.min(m,E-y)),t.style.top=m+"px",t.style.left=p+"px"}function d(){document.onmouseup=null,document.onmousemove=null}}#M(t,a){let o=t,i=a,n=!1,s,l,u,d;i.addEventListener("mousedown",p=>{n=!0,s=p.clientX,l=p.clientY,u=o.offsetWidth,d=o.offsetHeight,document.addEventListener("mousemove",g),document.addEventListener("mouseup",m)});function g(p){if(!n)return;let b=u+(p.clientX-s),M=d+(p.clientY-l);b<=500&&(b=500),M<=400&&(M=400),b>=1e3&&(b=1e3),M>=700&&(M=700),o.style.width=b+"px",o.style.height=M+"px"}function m(){n=!1,document.removeEventListener("mousemove",g),document.removeEventListener("mouseup",m)}}async getLogElement(){if(this.#r)return this.#r;{let t=await S.HookElement("#hs-ui-log");return this.#r=t,t}}replaceTabContents(t,a){let o=this.#b.find(n=>n.tabId===t);if(!o){c.warn("Could not find tab to replace contents",this.context);return}let i=document.querySelector(o.tabBodySel);i&&(i.innerHTML=a,c.log(`Replaced tab ${o.tabId} content`,this.context))}updateTitle(t){this.#i?this.#i.innerText=t:c.warn("Could not update panel title",this.context)}static#p(t){let a=t.indexOf("{"),o=t.indexOf("}");if(a>-1&&o>-1){a++;let i=t.substring(a,o);return h.isString(i)?i.replace(/\s+/g,"").length===0:!0}else return!0}static injectStyle(t,a){if(t&&!this.#p(t)){let o=a||"hs-injected-style-"+h.domid();this.#m.has(o)||this.#m.set(o,t),this.updateInjectedStyleBlock(),c.debug("Injected new CSS",this.#e)}}static removeInjectedStyle(t){this.#m.has(t)?(this.#m.delete(t),this.updateInjectedStyleBlock(),c.debug("Removed injected CSS",this.#e)):c.debug(`<yellow>Could not find style with id ${t}</yellow>`,this.#e)}static updateInjectedStyleBlock(){let t=document.querySelector(`#${A.HSUI.injectedStylesDomId}`);this.#g||(this.#g=document.createElement("style"),this.#g.id=A.HSUI.injectedStylesDomId,document.head.appendChild(this.#g)),this.#g.innerHTML="",this.#m.forEach((a,o)=>{r.#g.innerHTML+=a})}static injectHTMLString(t,a){let o=document.createElement("div");for(o.innerHTML=t;o.firstChild;)a?a(o.firstChild):document.body.appendChild(o.firstChild);c.debug("Injected new HTML",this.#e)}static injectHTMLElement(t,a){a(t),c.debug("Injected new HTML",this.#e)}renameTab(t,a){let o=this.#b.find(n=>n.tabId===t);if(!o){c.warn("Could not find tab to rename",this.context);return}let i=document.querySelector(o.tabSel);i&&(i.innerHTML=a)}#v(t=1,a){let o={x:0,y:0},i=window.innerWidth/2,n=window.innerHeight/2,s=0,l=0;if(a){let u=a.getBoundingClientRect();s=u.width,l=u.height}if(Number.isInteger(t))switch(t){case 1:o={x:i-s/2,y:n-l/2};break;case 2:o={x:window.innerWidth-25-s,y:n-l/2};break;case 3:o={x:25+s,y:n-l/2};break;default:o={x:i-s/2,y:n-l/2};break}else o=t;return o}async Modal(t){let a=`hs-dom-${h.uuidv4()}`,o=w._modal({...t,id:a,title:t.title||"",styles:{opacity:0}});r.injectHTMLString(o);let i=document.querySelector(`#${a}`),n=document.querySelector(`#${a} > .hs-modal-head`);if(this.#d.add(i),t.needsToLoad&&t.needsToLoad===!0){let s=document.querySelectorAll(`#${a} > .hs-modal-body img`),l=Array.from(s).map(u=>new Promise(d=>{u.complete?d():(u.addEventListener("load",()=>d()),u.addEventListener("error",()=>{d()}))}));await Promise.all(l)}if(i){let s=this.#v(t.position,i),l=i.getBoundingClientRect(),u=window.innerWidth,d=window.innerHeight,g=s.x,m=s.y;g+l.width>u-10&&(g=u-l.width-10),g<10&&(g=10),m+l.height>d-10&&(m=d-l.height-10),m<10&&(m=10),i.style.left=`${g}px`,i.style.top=`${m}px`,await i.transition({opacity:1}),this.#f(i,n);let p=i.querySelector(".hs-modal-resizer");p&&this.#M(i,p),i.addEventListener("click",async function(b){let M=b.target.dataset.close;if(M){let E=document.querySelector(`#${M}`);E&&(await E.transition({opacity:0}),E.parentElement?.removeChild(E))}})}return a}CloseModal(t){if(t){let a=document.getElementById(t);a&&(a.remove(),this.#d.delete(a))}else this.#d.forEach(a=>a.remove()),this.#d.clear()}static async Notify(t,a){let o={position:a?.position??"bottomRight",popDuration:a?.popDuration??400,displayDuration:a?.displayDuration??4e3,hideDuration:a?.hideDuration??2300,notificationType:a?.notificationType??"default"},i=document.createElement("div"),n=document.createElement("div");i.className=A.HSUI.notifyClassName,n.className=A.HSUI.notifyTextClassName;let s=300,l=50,u={default:"#192a56",warning:"#cd6133",error:"#b33939",success:"#009432"},d={topLeft:{top:`-${l}px`,left:"15px"},top:{top:`-${l}px`,left:`calc(50vw - ${s/2}px)`},topRight:{top:`-${l}px`,right:"15px"},right:{top:`calc(50vh - ${l/2}px)`,right:`-${s}px`},bottomRight:{bottom:`-${l}px`,right:"15px"},bottom:{bottom:`-${l}px`,left:`calc(50vw - ${s/2}px)`},bottomLeft:{bottom:`-${l}px`,left:"15px"},left:{top:`calc(50vh - ${l/2}px)`,left:`-${s}px`}},g={topLeft:{top:"15px"},top:{top:"15px"},topRight:{top:"15px"},right:{right:"15px"},bottomRight:{bottom:"15px"},bottom:{bottom:"15px"},bottomLeft:{bottom:"15px"},left:{left:"15px"}};i.style=h.objectToCSS({...d[o.position],opacity:"1",backgroundColor:u[o.notificationType]}),n.innerText=t,i.appendChild(n),document.body.querySelectorAll(`.${A.HSUI.notifyClassName}`).forEach(m=>{m.clearTransitions(),m.remove()}),document.body.appendChild(i),await i.transition({...g[o.position]},o.popDuration,"linear(0, 0.408 26.7%, 0.882 50.9%, 0.999 57.7%, 0.913 65.3%, 0.893 68.8%, 0.886 72.4%, 0.903 78.5%, 0.986 92.3%, 1)"),await h.wait(o.displayDuration),await i.transition({opacity:"0"},o.hideDuration,"linear"),n.remove(),i.remove(),n=null,i=null}};var Y=class r extends C{static#e="";static#t;static#a;static#n;static#i;static#o=[];constructor(e){super(e),r.#e=e.context}async init(){let e=this;c.log("Capturing mouse events",this.context),document.addEventListener("mousemove",r.#s.bind(r)),document.addEventListener("keydown",function(t){let a=v.getSetting("reactiveMouseHover"),o=v.getSetting("autoClick");t.code==="ShiftLeft"&&!r.#n&&a.isEnabled()&&(r.#n=setInterval(()=>{r.#r("mouseover")},a.getCalculatedValue())),t.code==="ControlLeft"&&!r.#i&&o.isEnabled()&&(r.#i=setInterval(()=>{r.#r("click")},o.getCalculatedValue()))}),document.addEventListener("keyup",function(t){t.code==="ShiftLeft"&&r.clearInterval("hover"),t.code==="ControlLeft"&&r.clearInterval("click")}),this.isInitialized=!0}static#s(e){this.#t={x:e.clientX,y:e.clientY},this.#l()}static#r(e){let{x:t,y:a}=this.#t,o=document.elementFromPoint(t,a);if(o){if(e==="click"&&v.getSetting("autoClickIgnoreElements").getValue()){if(o.id&&o.id!==""&&A.HSMouse.autoClickIgnoredElements.includes(`#${o.id}`))return;o.classList.length>0&&o.classList.forEach(s=>{A.HSMouse.autoClickIgnoredElements.includes(`.${s}`)})}let i=new MouseEvent(e,{view:window,bubbles:!0,cancelable:!0});o.dispatchEvent(i)}}static#l(){if(f.isModPanelOpen())if(!this.#a||this.#a===void 0){let e=document.querySelector("#hs-panel-debug-mousepos");e&&(this.#a=e,this.#a.innerHTML=`Mouse: (X: ${this.#t.x}, Y: ${this.#t.y})`)}else this.#a.innerHTML=`Mouse: (X: ${this.#t.x}, Y: ${this.#t.y})`}static clearInterval(e){e==="hover"&&r.#n?(clearInterval(r.#n),r.#n=null):e==="click"&&r.#i&&(clearInterval(r.#i),r.#i=null)}static getPosition(){return this.#t}};var Ve="data:image/png;base64,";var je="data:image/png;base64,";var Ke="data:image/png;base64,";var Xe="data:image/png;base64,";var qe="data:image/png;base64,";var _e="data:image/png;base64,";var $e="data:image/png;base64,";var et="data:image/png;base64,";var Bt={viscosity:Ve,drought:je,deflation:Ke,extinction:Xe,illiteracy:qe,recession:_e,dilation:$e,hyperchallenged:et},se={viscosity:{label:"Viscosity"},drought:{label:"Drought"},deflation:{label:"Deflation"},extinction:{label:"Extinction"},illiteracy:{label:"Illiteracy"},recession:{label:"Recession"},dilation:{label:"Dilation"},hyperchallenged:{label:"Hyperchallenged"}};async function tt(r,e){let t="hs-autosing-corruption-modal",a=Object.keys(se).map(n=>`
        <div class="hs-corruption-item">
            <img src="${Bt[n]}" class="hs-corruption-icon" alt="${se[n].label}" />
            <div class="hs-corruption-label">${se[n].label}</div>
            <input 
                type="number" 
                id="hs-corruption-${n}" 
                class="hs-corruption-input"
                min="0" 
                max="16" 
                value="${e[n]??0}"
            />
        </div>
    `).join(""),o={htmlContent:`
            <div id="${t}" class="hs-corruption-modal-container">
                <div class="hs-corruption-grid">
                    ${a}
                </div>
                <div class="hs-corruption-footer">
                    <div class="hs-corruption-done-btn" id="hs-corruption-save-btn">
                        Done
                    </div>
                </div>
            </div>
        `,title:"Configure Corruption Loadout"},i=await r.Modal(o);setTimeout(()=>{document.getElementById("hs-corruption-save-btn")?.addEventListener("click",()=>{Object.keys(se).forEach(n=>{let s=document.getElementById(`hs-corruption-${n}`),l=Math.max(0,Math.min(16,Number(s?.value)||0));e[n]=l}),f.removeInjectedStyle("hs-corruption-modal-styles"),r.CloseModal(i)})},0)}async function at(r,e){let t=m=>Ne.find(p=>p.value===m)?.label??null,a="hs-autosing-challenges-modal",o=[...e],i=()=>o.length===0?'<div class="hs-challenges-empty-state">No challenges added yet.</div>':o.map((m,p)=>{let b=t(m.challengeNumber),M=!!b;return`
                <div class="hs-challenge-item">
                    <div class="hs-challenge-item-text">
                        ${M?`<strong>${b}</strong>`:`Challenge ${m.challengeNumber} (${m.challengeCompletions} completions)`}
                        <div>
                            Wait after goal reached: ${m.challengeWaitTime}ms ${M?"":`| Max: ${m.challengeMaxTime}ms`}
                        </div>
                    </div>
                    <div class="hs-challenge-controls">
                        <div class="hs-challenge-btn hs-challenge-btn-edit" id="hs-challenge-edit-${p}" data-index="${p}">\u270E</div>
                        <div class="hs-challenge-btn" id="hs-challenge-up-${p}" data-index="${p}">\u2191</div>
                        <div class="hs-challenge-btn hs-challenge-btn-delete" id="hs-challenge-delete-${p}" data-index="${p}">\xD7</div>
                        <div class="hs-challenge-btn" id="hs-challenge-down-${p}" data-index="${p}">\u2193</div>
                    </div>
                </div>`}).join(""),n=()=>{let m=document.getElementById("hs-challenge-list-container");m&&(m.innerHTML=i())},s={htmlContent:`
    <div id="${a}" class="hs-challenges-modal-container">
        <div class="hs-challenges-input-section">
            <div class="hs-challenges-input-row" style="grid-column: 1 / -1;">
                <div class="hs-challenges-input-label">Special Action:</div>
                <select id="hs-challenge-action-select" class="hs-challenges-input">
                    <option value="">None (Standard Challenge)</option>
                    ${Ne.map(m=>`<option value="${m.value}">${m.label}</option>`).join("")}
                </select>
            </div>
            <div class="hs-challenges-input-row">
                <div class="hs-challenges-input-label">Challenge #:</div>
                <input type="number" id="hs-challenge-num-input" class="hs-challenges-input" min="1" max="15" value="1" />
            </div>
            <div class="hs-challenges-input-row">
                <div class="hs-challenges-input-label">Min Completions:</div>
                <input type="number" id="hs-challenge-completions-input" class="hs-challenges-input" min="1" value="1" />
            </div>
            <div class="hs-challenges-input-row">
                <div class="hs-challenges-input-label">Wait (ms):</div>
                <input type="number" id="hs-challenge-wait-input" class="hs-challenges-input" min="0" value="0" />
            </div>
            <div class="hs-challenges-input-row">
                <div class="hs-challenges-input-label">Max Time (ms):</div>
                <input type="number" id="hs-challenge-max-time-input" class="hs-challenges-input" min="100" value="10000" />
            </div>
            <div class="hs-challenges-add-btn" id="hs-challenge-add-btn">Add Action/Challenge</div>
        </div>

        <div class="hs-challenges-list-container">
            <div id="hs-challenge-list-container">
                ${i()}
            </div>
        </div>

        <div class="hs-challenges-footer">
            <div class="hs-challenges-footer-btn hs-challenges-cancel-btn" id="hs-challenges-cancel-btn">Cancel</div>
            <div class="hs-challenges-footer-btn hs-challenges-save-btn" id="hs-challenges-save-btn">Save Strategy</div>
        </div>
    </div>`,title:"Configure Strategy Actions"},l=await r.Modal(s),u=null,d=()=>{let p=!!document.getElementById("hs-challenge-action-select")?.value,b=document.getElementById("hs-challenge-num-input"),M=document.getElementById("hs-challenge-completions-input"),E=document.getElementById("hs-challenge-max-time-input");[b,M,E].forEach(y=>{y&&(y.disabled=p,y.parentElement.style.opacity=p?"0.4":"1")})},g=()=>{document.getElementById("hs-challenge-num-input").value="1",document.getElementById("hs-challenge-completions-input").value="0",document.getElementById("hs-challenge-wait-input").value="0",document.getElementById("hs-challenge-max-time-input").value="1000000",document.getElementById("hs-challenge-action-select").value="",document.getElementById("hs-challenge-add-btn").textContent="Add Action/Challenge",u=null,d()};setTimeout(()=>{let m=document.getElementById(a),p=document.getElementById("hs-challenge-action-select");p?.addEventListener("change",d),m?.addEventListener("click",b=>{let M=b.target,E=M.id;if(E==="hs-challenge-add-btn"){let y=!!p.value,T={challengeNumber:Number(y?p.value:document.getElementById("hs-challenge-num-input").value),challengeCompletions:y?0:Number(document.getElementById("hs-challenge-completions-input").value),challengeWaitTime:Number(document.getElementById("hs-challenge-wait-input").value),challengeMaxTime:y?0:Number(document.getElementById("hs-challenge-max-time-input").value)};u!==null?o[u]=T:o.push(T),n(),g()}if(E.startsWith("hs-challenge-edit-")){u=Number(M.dataset.index);let y=o[u];t(y.challengeNumber)?p.value=String(y.challengeNumber):(p.value="",document.getElementById("hs-challenge-num-input").value=String(y.challengeNumber),document.getElementById("hs-challenge-completions-input").value=String(y.challengeCompletions),document.getElementById("hs-challenge-max-time-input").value=String(y.challengeMaxTime)),document.getElementById("hs-challenge-wait-input").value=String(y.challengeWaitTime),document.getElementById("hs-challenge-add-btn").textContent="Update Action",d()}if(E.startsWith("hs-challenge-delete-")&&(o.splice(Number(M.dataset.index),1),n(),g()),E.startsWith("hs-challenge-up-")){let y=Number(M.dataset.index);y>0&&([o[y],o[y-1]]=[o[y-1],o[y]],n())}if(E.startsWith("hs-challenge-down-")){let y=Number(M.dataset.index);y<o.length-1&&([o[y],o[y+1]]=[o[y+1],o[y]],n())}E==="hs-challenges-save-btn"&&(e.length=0,e.push(...o),r.CloseModal(l)),E==="hs-challenges-cancel-btn"&&r.CloseModal(l)})},0)}async function Ue(r,e,t,a,o){let i=!!o,n=e.length?e[e.length-1].endPhase:"start",s=o?.corruptions||{viscosity:0,drought:0,deflation:0,extinction:0,illiteracy:0,recession:0,dilation:0,hyperchallenged:0},l=o?.strat||[],u=N.indexOf(n),g=N.slice(u+1).map((b,M)=>`<option value="${b}" ${o?.endPhase===b||M===0?"selected":""}>${b}</option>`).join(""),m={htmlContent:`
        <div class="hs-phase-modal-container">
            <div class="hs-phase-select-group">
                <label class="hs-phase-label">Starting Phase</label>
                <select class="hs-phase-select" disabled>
                    <option selected>
                        ${o?.startPhase??n}
                    </option>
                </select>
            </div>

            <div class="hs-phase-select-group">
                <label class="hs-phase-label">Ending Phase</label>
                <select
                    id="hs-autosing-end-phase"
                    class="hs-phase-select"
                >
                    ${g}
                </select>
            </div>

            <div class="hs-phase-config-group">
                <div class="hs-phase-config-btn" id="hs-autosing-phase-corruptions">
                    Configure Corruptions
                </div>
                <div class="hs-phase-config-btn" id="hs-autosing-phase-challenges">
                    Configure Challenges
                </div>
            </div>

            <div class="hs-phase-footer">
                <div class="hs-phase-done-btn" id="hs-autosing-phase-done">
                    ${i?"Save":"Create Phase"}
                </div>
                <div class="hs-phase-error" id="hs-phase-error" style="display:none;"></div>
            </div>
        </div>
        `,title:i?"Edit Strategy Phase":"Create Strategy Phase"},p=await r.Modal(m);setTimeout(()=>{let b=document.querySelector(".hs-phase-modal-container");b&&b.addEventListener("click",async M=>{let E=M.target;if(E.id==="hs-autosing-phase-done"){let T=document.getElementById("hs-autosing-end-phase")?.value;if(!T)return;let H={startPhase:o?o.startPhase:n,endPhase:T,corruptions:{...s},strat:[...l]};i&&a?a(H):t(H),f.removeInjectedStyle("hs-phase-modal-styles"),r.CloseModal(p)}E.id==="hs-autosing-phase-corruptions"&&await tt(r,s),E.id==="hs-autosing-phase-challenges"&&await at(r,l)})},0)}var Z=class{static async open(e,t){let a=k.getModule("HSUI");if(!a||!a.uiReady)return;let o=!!e,i=e?{strategyName:e.strategyName,strategy:JSON.parse(JSON.stringify(e.strategy))}:{strategyName:"",strategy:[]},n=()=>{for(let d=1;d<i.strategy.length;d++){let g=i.strategy[d-1],m=i.strategy[d];m.startPhase=g.endPhase}},s=()=>{let d=document.getElementById("hs-autosing-phase-list");if(d){if(i.strategy.length===0){d.innerHTML='<div class="hs-strategy-empty-state">No strategy phases added yet.</div>';return}d.innerHTML=i.strategy.map((g,m)=>`
                    <div class="hs-strategy-phase-item">
                        <div class="hs-strategy-phase-text">
                            Phase ${m+1}: ${g.startPhase} <span class="hs-strategy-phase-arrow">\u2192</span> <strong>${g.endPhase}</strong>
                        </div>
                        <div class="hs-strategy-btn hs-strategy-btn-icon hs-strategy-btn-edit" data-phase-index="${m}" data-action="edit">
                            \u270E
                        </div>
                        <div class="hs-strategy-btn hs-strategy-btn-icon hs-strategy-btn-delete" data-phase-index="${m}" data-action="delete">
                            \xD7
                        </div>
                    </div>
                `).join("")}},l={htmlContent:`
                <div class="hs-strategy-modal-container" id="hs-strategy-modal-root">
                    <div class="hs-strategy-input-section">
                        <div class="hs-strategy-input-label">Strategy Name</div>
                        <input 
                            type="text" 
                            id="hs-autosing-strategy-name" 
                            class="hs-strategy-name-input"
                            placeholder="Enter strategy name..."
                            value="${i.strategyName}"
                        />
                    </div>

                    <div class="hs-strategy-input-section">
                        <div class="hs-strategy-input-label">Strategy Phases</div>
                        <div id="hs-autosing-phase-list" class="hs-strategy-phase-list">
                            <div class="hs-strategy-empty-state">No strategy phases added yet.</div>
                        </div>
                    </div>

                    <div class="hs-strategy-error" id="hs-strategy-error" style="display: none; color: #ef5350; padding: 10px; background: rgba(239, 83, 80, 0.1); border: 1px solid #ef5350; border-radius: 3px; margin-top: 10px;"></div>

                    <div class="hs-strategy-btn-group">
                        <div class="hs-strategy-btn hs-strategy-btn-secondary" id="hs-autosing-add-phase-btn">
                            + Add Phase
                        </div>
                        <div class="hs-strategy-btn hs-strategy-btn-primary" id="hs-autosing-create-btn">
                            ${o?"Update Strategy":"Create Strategy"}
                        </div>
                    </div>
                </div>
            `,title:o?"Edit Autosing Strategy":"Create Autosing Strategy"},u=await a.Modal(l);setTimeout(()=>{let d=document.querySelector(".hs-strategy-modal-container");d&&(s(),d.addEventListener("click",async g=>{let m=g.target,p=m.dataset.action,b=m.dataset.phaseIndex;if(m.id==="hs-autosing-add-phase-btn")await Ue(a,i.strategy,M=>{i.strategy.push(M),n(),s()});else if(m.id==="hs-autosing-create-btn"){let M=document.getElementById("hs-strategy-error"),E=document.getElementById("hs-autosing-strategy-name");i.strategyName=E?.value||"Unnamed Strategy";try{o?(v.saveStrategiesToStorage(i,e.strategyName),f.Notify(`Strategy "${i.strategyName}" updated`,{notificationType:"success"})):(v.saveStrategiesToStorage(i),f.Notify(`Strategy "${i.strategyName}" created`,{notificationType:"success"})),a.CloseModal(u)}catch(y){M&&(M.textContent=y.message,M.style.display="block")}}else if(p==="edit"&&b!==void 0){let M=parseInt(b),E=i.strategy[M];await Ue(a,i.strategy.slice(0,M),()=>{},y=>{i.strategy[M]=y,n(),s()},E)}else if(p==="delete"&&b!==void 0){let M=parseInt(b);i.strategy.splice(M,1),n(),s()}}))},0)}};var re=class{#e={syncNotificationOpacity:async e=>{let t=document.querySelector("#notification"),a=e.contextName??"HSSettings";if(e.disable&&e.disable===!0)t.style.removeProperty("opacity");else{let o=e.value;t&&o&&o>=0&&o<=1&&(t.style.opacity=o.toString())}},logTimestamp:async e=>{e.disable&&e.disable===!0?c.setTimestampDisplay(!1):c.setTimestampDisplay(!0)},reactiveMouseHover:async e=>{let t=e.contextName??"HSSettings";e.disable&&e.disable===!0&&Y.clearInterval("hover")},autoClick:async e=>{let t=e.contextName??"HSSettings";e.disable&&e.disable===!0&&Y.clearInterval("click")},ambrosiaQuickBarAction:async e=>{let t=e.contextName??"HSSettings",a=k.getModule("HSAmbrosia");a&&(e.disable&&e.disable===!0?await a.destroyQuickBar():await a.createQuickBar())},ambrosiaMinibarAction:async e=>{let t=e.contextName??"HSSettings",a=k.getModule("HSAmbrosia");a&&(e.disable&&e.disable===!0?await a.disableBerryMinibars():await a.enableBerryMinibars())},patch:async e=>{let t=e.contextName??"HSSettings";if(!e.patchConfig||!e.patchConfig.patchName){c.error("No patch config provided for setting action",t);return}let a=k.getModule("HSPatches");a&&(e.disable&&e.disable===!0?(console.log("Disabling patch",e.patchConfig.patchName,t),await a.revertPatch(e.patchConfig.patchName)):(console.log("Enabling patch",e.patchConfig.patchName,t),await a.applyPatch(e.patchConfig.patchName)))},useGameData:async e=>{let t=e.contextName??"HSSettings",a=k.getModule("HSGameData");a&&(e.disable&&e.disable===!0?a.disableGDS():a.enableGDS())},autoLoadout:async e=>{let t=e.contextName??"HSSettings",a=k.getModule("HSAmbrosia");a&&(e.disable&&e.disable===!0?await a.disableAutoLoadout():await a.enableAutoLoadout())},ambrosiaIdleSwapAction:async e=>{let t=e.contextName??"HSSettings",a=k.getModule("HSAmbrosia");a&&(e.disable&&e.disable===!0?await a.disableIdleSwap():await a.enableIdleSwap())},startAutosingAction:async e=>{let t=e.contextName??"HSSettings",a=k.getModule("HSAutosing");a&&(e.disable&&e.disable===!0?await a.disableAutoSing():await a.enableAutoSing())},createAutosingStrategy:async e=>{let t=e.contextName??"HSSettings";await Z.open()},editAutosingStrategy:async e=>{let t=e.contextName??"HSSettings";await v.editSelectedStrategy()},deleteAutosingStrategy:async e=>{let t=e.contextName??"HSSettings";await v.deleteSelectedStrategy()},exportAutosingStrategy:async e=>{let t=e.contextName??"HSSettings";await v.exportSelectedStrategy()},importAutosingStrategy:async e=>{let t=e.contextName??"HSSettings";await v.importStrategy()}};constructor(){}getAction(e){let t=this;return e in this.#e?a=>{t.#e[e](a)}:null}};var U=class{constructor(e,t,a,o){this.context="HSSetting";this.definition=e,this.settingAction=t,this.#e=a,this.#t=o,this.definition.defaultValue=this.definition.settingValue}#e;#t;getDefinition(){return this.definition}setDefinition(e){this.definition=e}enable(){this.#a(!0)}disable(){this.#a(!1)}async handleToggle(e){let t=!this.definition.enabled,a=this.definition.enabled!==t,o=v.getSetting("useGameData");if(!A.HSSettings.gameDataCheckBlacklist.includes(this.definition.settingName)&&o&&this.definition.usesGameData&&a&&t&&!o.isEnabled()){c.warn(`Enable GDS before enabling ${this.definition.settingDescription}!`);return}if(this.definition.settingName==="useGameData"&&a&&!t){let n=v.getSettings();for(let[s,l]of Object.entries(n)){let u=l.getDefinition();A.HSSettings.gameDataCheckBlacklist.includes(s)||"usesGameData"in u&&u.usesGameData===!0&&l.isEnabled()&&l.definition.settingName!=="startAutosing"&&l.disable()}}if(c.log(`${this.definition.settingName}: ${this.definition.enabled} -> ${!this.definition.enabled}`,this.context),!a)return;this.definition.enabled=t,this.definition.settingType==="boolean"&&(this.definition.settingValue=t,this.definition.calculatedSettingValue=t);let i=e.target;t?(i.innerText=this.#e,i.classList.remove("hs-disabled")):(i.innerText=this.#t,i.classList.add("hs-disabled")),this.handleSettingAction("state",t)}#a(e){let t=this.definition.enabled!==e;if(!t)return;if(this.definition.settingName==="useGameData"&&t&&!e){let o=v.getSettings();for(let[i,n]of Object.entries(o)){let s=n.getDefinition();A.HSSettings.gameDataCheckBlacklist.includes(i)||"usesGameData"in s&&s.usesGameData===!0&&n.isEnabled()&&n.definition.settingName!=="startAutosing"&&n.disable()}}c.log(`${this.definition.settingName}: ${this.definition.enabled} -> ${e}`,this.context),this.definition.enabled=e;let a=document.querySelector(`#${this.definition.settingControl?.controlEnabledId}`);e&&a?(a.innerText=this.#e,a.classList.remove("hs-disabled")):(a.innerText=this.#t,a.classList.add("hs-disabled")),this.handleSettingAction("state",e),v.saveSettingsToStorage()}async initialAction(e,t){await this.handleSettingAction(e,t)}async handleSettingAction(e,t){if(this.settingAction){let a=this.settingAction,o={contextName:this.context,value:this.definition.calculatedSettingValue??null};this.definition.patchConfig&&this.definition.patchConfig.patchName&&(o.patchConfig=this.definition.patchConfig),a&&a instanceof Function&&(e==="state"?t?await a({...o,disable:!1}):await a({...o,disable:!0}):await a({...o,disable:!1}))}}hasControls(){return"settingControl"in this.definition}isEnabled(){return this.definition.enabled}getCalculatedValue(){return this.definition.calculatedSettingValue}toString(){return JSON.stringify(this.definition)}},le=class extends U{constructor(e,t,a,o){super(e,t,a,o),this.definition.enabled=!0,this.definition.settingValue=null,this.definition.calculatedSettingValue=null}getValue(){return null}setValue(e){}async handleChange(e){c.log(`Button pressed: ${this.definition.settingName}`,this.context),await this.handleSettingAction("value")}async initialAction(e,t){}enable(){}disable(){}},ce=class extends U{constructor(e,t,a,o){super(e,t,a,o),this.definition.settingValueMultiplier&&(this.definition.calculatedSettingValue=this.definition.settingValue*this.definition.settingValueMultiplier)}getValue(){return this.definition.settingValue}setValue(e){return this.definition.settingValue=e}async handleChange(e){let t=parseFloat(e.target.value);c.log(`${this.definition.settingName}: ${this.definition.settingValue} -> ${t}`,this.context),this.definition.settingValue=t,this.definition.calculatedSettingValue=t*this.definition.settingValueMultiplier,await super.handleSettingAction("value")}},ue=class extends U{constructor(e,t,a,o){super(e,t,a,o)}getValue(){return this.definition.settingValue}setValue(e){return this.definition.settingValue=e}async handleChange(e){let t=e.target.value;c.log(`${this.definition.settingName}: ${this.definition.settingValue} -> ${t}`,this.context),this.definition.settingValue=t,this.definition.calculatedSettingValue=t,await super.handleSettingAction("value")}},de=class extends U{constructor(e,t,a,o){super(e,t,a,o)}getValue(){return this.definition.enabled}setValue(e){this.definition.settingValue=e,this.definition.calculatedSettingValue=e,this.definition.enabled=e}async handleChange(e){}},ge=class extends U{constructor(e,t,a,o){super(e,t,a,o),this.definition.settingValueMultiplier&&(this.definition.calculatedSettingValue=this.definition.settingValue*this.definition.settingValueMultiplier)}getValue(){return this.definition.settingValue}setValue(e){}async handleChange(e){let t=parseFloat(e.target.value);c.log(`${this.definition.settingName}: ${this.definition.settingValue} -> ${t}`,this.context),this.definition.settingValue=t,this.definition.calculatedSettingValue=t*this.definition.settingValueMultiplier,await super.handleSettingAction("value")}},he=class extends U{constructor(e,t,a,o){super(e,t,a,o)}getValue(){return this.definition.settingValue}setValue(e){}async handleChange(e){let t=e.target.value;c.log(`${this.definition.settingName}: ${this.definition.settingValue} -> ${t}`,this.context),this.definition.settingValue=t,this.definition.calculatedSettingValue=t,await super.handleSettingAction("value")}},me=class extends U{constructor(e,t,a,o){super(e,t,a,o)}getValue(){return h.removeColorTags(this.definition.settingValue)}getDisplayValue(){return h.parseColorTags(this.definition.settingValue)}setValue(e){this.definition.settingValue=e,this.definition.calculatedSettingValue=e;let t=document.querySelector(`#${this.definition.settingControl?.controlId}`);t&&(t.innerHTML=this.getDisplayValue()),v.saveSettingsToStorage()}async handleChange(e){}};var ot="data:image/png;base64,";var v=class r extends C{static#e="";static#t={};static#a;static#n;static#i=!1;static#o=!1;static#s="\u2713";static#r="\u2717";static#l=[];#u;constructor(e){super(e),r.#e=this.context,this.#u=new re,c.log("Parsing mod settings",this.context);try{c.log("Parsing control groups",this.context),r.#a=JSON.parse(Ye)}catch(t){c.error(`Error parsing control groups ${t}`,this.context),r.#i=!1}try{c.log("Parsing control pages",this.context),r.#n=JSON.parse(Je)}catch(t){c.error(`Error parsing control pages ${t}`,this.context),r.#i=!1}try{c.log("Parsing settings.json",this.context);let t=this.#c(),a;"useGameData"in t?a=t.useGameData.enabled:a=!1;for(let[o,i]of Object.typedEntries(t)){(i.settingType==="boolean"||h.isBoolean(i.settingValue))&&(i.settingValue=!1),i.usesGameData&&i.enabled&&!a&&(A.HSSettings.gameDataCheckBlacklist.includes(o)||(c.info(`Disabled ${i.settingDescription} on load because GDS is not on`,this.context),i.enabled=!1)),this.#d(i,r.#a);let n="settingAction"in i?i.settingAction:void 0,s=n?this.#u.getAction(n):null;if(i.settingType==="numeric")"settingValueMultiplier"in i||(i.settingValueMultiplier=1),r.#t[o]=new ce(i,s,r.#s,r.#r);else if(i.settingType==="string")r.#t[o]=new ue(i,s,r.#s,r.#r);else if(i.settingType==="boolean")r.#t[o]=new de(i,s,r.#s,r.#r);else if(i.settingType==="selectnumeric")"settingValueMultiplier"in i||(i.settingValueMultiplier=1),r.#t[o]=new ge(i,s,r.#s,r.#r);else if(i.settingType==="selectstring")r.#t[o]=new he(i,s,r.#s,r.#r);else if(i.settingType==="state")r.#t[o]=new me(i,s,r.#s,r.#r);else if(i.settingType==="button")r.#t[o]=new le(i,s,r.#s,r.#r);else throw new Error(`Could not parse setting ${o.toString()} (settingType: ${i.settingType}, settingValue: ${i.settingValue})`)}r.saveSettingsToStorage(),r.#i=!0}catch(t){c.error(`Error parsing mod settings ${t}`,this.context),r.#i=!1}}async init(){this.#p(r.#l),this.isInitialized=!0}static async syncSettings(){if(c.log("Syncing mod settings",r.#e),!r.#i){c.error("Could not sync settings - settings not parsed yet",r.#e);return}for(let[e,t]of Object.typedEntries(r.#t)){c.debug(`Syncing ${e} settings`,r.#e);let a=t.getDefinition(),o=t.hasControls()?a.settingControl:void 0;if(o){let i=o.controlType,n=o.controlOptions;if(i==="text"||i==="number"){let s=document.querySelector(`#${o.controlId}`);s&&(i==="number"&&n?("min"in n&&s.setAttribute("min",n.min.toString()),"max"in n&&s.setAttribute("max",n.max.toString()),"step"in n&&s.setAttribute("step",n.step.toString())):i==="text"&&n&&"placeholder"in n&&s.setAttribute("placeholder",n.placeholder),s.value=h.asString(a.settingValue),s.addEventListener("change",async l=>{await this.#S(l,t)}))}else if(i==="select"){let s=h.asString(a.settingValue),l=document.querySelector(`#${o.controlId}`);l&&(Array.from(l.options).some(d=>d.value===s)?l.value=s:(l.value="",c.warn(`Setting value ${s} does not exist in select options for setting ${e}`,r.#e)),l.addEventListener("change",async d=>{await this.#S(d,t)}))}else if(i==="state"){let s=h.parseColorTags(h.asString(a.settingValue)),l=document.querySelector(`#${o.controlId}`);l&&(l.innerHTML=s)}else if(i==="button"){let s=document.querySelector(`#${o.controlId}`);s&&s.addEventListener("click",async l=>{await this.#S(l,t)})}if(o.controlEnabledId){let s=document.querySelector(`#${o.controlEnabledId}`);s&&(a.enabled?(s.innerText=r.#s,s.classList.remove("hs-disabled")):(s.innerText=r.#r,s.classList.add("hs-disabled")),s.addEventListener("click",async l=>{await this.#A(l,t)}))}await t.initialAction("state",a.enabled)}}c.log("Finished syncing mod settings",r.#e),this.#o=!0}static autoBuildSettingsUI(){let e=this;if(!r.#i)return c.error("Could not sync settings - settings not parsed yet",r.#e),{didBuild:!1,navHTML:"",pagesHTML:""};let t=[],a=!0,o=Object.entries(r.#t).sort((g,m)=>{let p=g[1].getDefinition().settingControl?.controlGroup,b=m[1].getDefinition().settingControl?.controlGroup;return p&&b?(r.#a[p].order||0)-(r.#a[b].order||0):p?-1:b?1:0}),i=Object.entries(r.#n).sort((g,m)=>{let p=g[1].order,b=m[1].order;return p&&b?(p||0)-(b||0):p?-1:b?1:0}),n=[];for(let[g,m]of i)o.some(b=>b[1].getDefinition().settingControl?.controlPage===g)&&n.push(w.Div({class:"hs-panel-subtab",id:`hs-panel-settings-subtab-${g}`,data:new Map([["subtab",g],["color",m.pageColor||""]]),styles:{border:m.pageColor?`1px solid ${m.pageColor}`:"1px solid gray"},html:m.pageName}));let s=w.Div({class:"hs-panel-subtabs",html:n}),l=new Map,u=null;for(let[g,m]of o){let p=m.getDefinition(),b=p.settingControl,M=p.settingBlockId||void 0,E="";if(p.usesGameData&&(E=w.Image({class:"hs-panel-setting-block-gamedata-icon",src:ot,width:18,height:18,props:{title:A.HSSettings.gameDataRequiredTooltip}})),b){let y=l.get(b.controlPage)||[],T=[];if(!u||u!==b.controlGroup){u=b.controlGroup;let H=r.#a[u];y.push(w.Div({html:H.groupName,styles:{borderBottom:"1px solid limegreen",gridColumn:"span 2",marginBottom:"15px"}}))}if(b.controlType==="switch")T=[w.Div({class:"hs-panel-setting-block-text-wrapper",styles:{display:"flex",flexDirection:"row",alignItems:"center"},html:[w.P({class:"hs-panel-setting-block-text",props:{title:p.settingHelpText},text:p.settingDescription,styles:{margin:"0"}}),E]})],b.controlEnabledId&&T.push(w.Button({class:"hs-panel-setting-block-btn hs-panel-settings-block-btn-standalone",id:b.controlEnabledId,text:""}));else if(b.controlType==="button")T=[w.Button({id:b.controlId,text:p.settingDescription||"Error: No button text"})];else{let H=null;switch(b.controlType){case"text":H=4;break;case"number":H=3;break;case"select":H=5;break;case"state":H=6;break;default:H=null}if(H){if(T=[w.Div({class:"hs-panel-setting-block-text-wrapper",styles:{display:"flex",flexDirection:"row",alignItems:"center"},html:[w.P({class:"hs-panel-setting-block-text",props:{title:p.settingHelpText},text:p.settingDescription,styles:{margin:"0"}}),E]})],H===3||H===4)T.push(w.Input({class:"hs-panel-setting-block-num-input",id:b.controlId,type:H}));else if(H===5)if(b.selectOptions)T.push(w.Select({class:"hs-panel-setting-block-select-input",id:b.controlId,type:H},b.selectOptions));else{c.error(`Error autobuilding settings UI, ${p.settingName} does not have selectOptions defined`,e.#e),a=!1;break}else H===6&&T.push(w.P({class:"hs-panel-setting-block-state",id:b.controlId,text:""}));b.controlEnabledId&&T.push(w.Button({class:"hs-panel-setting-block-btn",id:b.controlEnabledId,text:""}))}else{c.error("Error autobuilding settings UI, control type resolution failed (how??)",e.#e),a=!1;break}}y.push(w.Div({id:M,class:"hs-panel-setting-block",html:T})),l.set(b.controlPage,y)}else{c.error(`Error autobuilding settings UI, controls not defined for setting ${g}`,e.#e),a=!1;break}}for(let[g,m]of l.entries())l.set(g,[w.Div({class:"hs-panel-settings-grid",id:`settings-grid-${g}`,html:m})]);let d=[];for(let[g,m]of l.entries())d.push(m.join(""));return{didBuild:a,navHTML:s,pagesHTML:d.join("")}}static validateStrategy(e){if(!e)throw new Error("Strategy is undefined");if(!("strategyName"in e))throw new Error("Strategy is missing strategyName property");if(!("strategy"in e))throw new Error("Strategy is missing strategy property");if(!Array.isArray(e.strategy))throw new Error("Strategy.strategy must be an array");let t=e.strategy;if(t.length===0)throw new Error("Strategy has no components");let a=[...N];for(let o=0;o<t.length;o++){let i=t[o],{startPhase:n,endPhase:s,corruptions:l,strat:u}=i;if(!l)throw new Error(`Component ${o+1} (${n} \u2192 ${s}) has no corruptions defined`);if(!Array.isArray(u)||u.length===0)throw new Error(`Component ${o+1} (${n} \u2192 ${s}) must have at least one challenge`);if(a[0]!==n)throw new Error(`Component ${o+1} must start at "${a[0]}", got "${n}"`);let d=a.indexOf(s);if(d===-1)throw new Error(`Component ${o+1} ends at "${s}", which is not valid or already consumed`);a=a.slice(d)}if(a.length>1)throw new Error(`Uncovered phases: ${a.join(", ")}`)}#d(e,t){if(!e)throw new Error("Setting is undefined (wtf)");let a=["text","number","switch","select","state","button"],o=["numeric","string","boolean","selectnumeric","selectstring","state","button"];if(!("settingName"in e))throw new Error("Setting is missing settingName property");let i=e.settingName;if(!("enabled"in e))throw new Error(`Setting '${i}' is missing enabled property`);if(!("settingDescription"in e))throw new Error(`Setting '${i}' is missing settingDescription property`);if(!("settingValue"in e))throw new Error(`Setting '${i}' is missing settingValue property`);if(!("settingType"in e))throw new Error(`Setting '${i}' is missing settingType property`);if(!o.includes(e.settingType))throw new Error(`Setting '${i}' has invalid settingType property`);let n=e.settingType;if(n==="numeric"){if(!h.isNumeric(e.settingValue))throw new Error(`Setting '${i}' has invalid settingValue property for settingType ${n}`)}else if(n==="string"){if(!h.isString(e.settingValue))throw new Error(`Setting '${i}' has invalid settingValue property for settingType ${n}`)}else if(n==="boolean"){if(!h.isBoolean(e.settingValue))throw new Error(`Setting '${i}' has invalid settingValue property for settingType ${n}`)}else if(n==="selectnumeric"){if(!h.isString(e.settingValue)&&!h.isNumeric(e.settingValue))throw new Error(`Setting '${i}' has invalid settingValue property for settingType ${n}`)}else if(n==="selectstring"&&!h.isString(e.settingValue)&&!h.isNumeric(e.settingValue))throw new Error(`Setting '${i}' has invalid settingValue property for settingType ${n}`);if("settingControl"in e&&e.settingControl){let s=e.settingControl;if(s.controlType!=="switch"&&!("controlId"in s))throw new Error(`Setting '${i}' has settingControl defined and it is not type'switch', but it is missing controlId property`);if(!("controlType"in s))throw new Error(`Setting '${i}' has settingControl defined, but it is missing controlType property`);if(!("controlGroup"in s))throw new Error(`Setting '${i}' has settingControl defined, but it is missing controlGroup property`);if(!a.includes(s.controlType))throw new Error(`Setting '${i}' has invalid controlType property`);if(!(s.controlGroup in t))throw new Error(`Setting '${i}' has invalid controlGroup property`)}}static getSetting(e){return this.#t[e]}static getSettings(){return this.#t}static getStrategies(){return this.#l}static#m(){let e={};for(let[t,a]of Object.typedEntries(this.#t)){let o={...a.getDefinition()},i=A.HSSettings.serializationBlackList;for(let n of i)o[n]&&delete o[n];e[t]=o}return JSON.stringify(e)}static saveStrategiesToStorage(e,t){let a=k.getModule("HSStorage");if(!a)throw new Error("Could not find Storage Module");let o=a.getData(A.HSSettings.strategiesKey);if(Array.isArray(o)||(o=[]),!e&&t){let i=o.filter(n=>n.strategyName!==t);r.#l=r.#l.filter(n=>n.strategyName!==t),a.setData(A.HSSettings.strategiesKey,i),this.#v(t),c.debug("<green>Strategy removed</green>",this.#e);return}if(e){this.validateStrategy(e);let i=!!t;if(o.some(u=>u.strategyName!==e.strategyName?!1:i?u.strategyName!==t:!0))throw new Error(`Strategy with name "${e.strategyName}" already exists.`);let s=o;i&&(s=o.filter(u=>u.strategyName!==t),this.#v(t),r.#l=r.#l.filter(u=>u.strategyName!==t)),s=s.concat(e),r.#l.push(e);let l=a.setData(A.HSSettings.strategiesKey,s.filter(u=>u.strategyName!=="default_strategy"));this.#h(e),l?c.debug(`<green>Strategy ${i?"updated":"saved"} to localStorage</green>`,this.#e):c.warn("Could not save Strategy to localStorage",this.#e)}}static async deleteSelectedStrategy(){let e=r.getSetting("autosingStrategy"),t=e.getValue();if(!t||t===""){f.Notify("Please select a strategy to delete",{notificationType:"warning"});return}let a=e.getDefinition().settingControl;if(!a?.selectOptions)return;let o=a.selectOptions.find(n=>n.value.toString()===t);if(!o)return;let i=o.text;if(i=="default_strategy"){f.Notify("cannot delete default strategy");return}confirm(`Are you sure you want to delete strategy "${i}"?`)&&(r.saveStrategiesToStorage(void 0,i),f.Notify(`Strategy "${i}" deleted`,{notificationType:"success"}))}static async exportSelectedStrategy(){let e=r.getSetting("autosingStrategy"),t=e.getValue();if(!t||t===""){f.Notify("Please select a strategy to export",{notificationType:"warning"});return}let a=e.getDefinition().settingControl;if(!a?.selectOptions)return;let o=a.selectOptions.find(l=>l.value.toString()===t);if(!o)return;let i=o.text,s=r.getStrategies().find(l=>l.strategyName===i);if(!s){f.Notify("Strategy not found",{notificationType:"error"});return}try{let l=JSON.stringify(s,null,2);await navigator.clipboard.writeText(l),f.Notify(`Strategy "${i}" copied to clipboard`,{notificationType:"success"})}catch(l){f.Notify("Failed to copy strategy to clipboard",{notificationType:"error"}),c.log(`Export failed: ${l}`,"HSAutosing")}}static async importStrategy(){let e=k.getModule("HSUI");if(e){let t=await e.Modal({title:"Import Strategy",htmlContent:`
            <div style="display: flex; flex-direction: column; gap: 15px; padding: 10px;">
                <div>
                    <label for="import-strategy-name" style="display: block; margin-bottom: 5px; font-weight: bold;">Strategy Name:</label>
                    <input type="text" id="import-strategy-name" placeholder="Enter strategy name" style="width: 100%; padding: 8px; box-sizing: border-box;" />
                </div>
                <div>
                    <label for="import-strategy-json" style="display: block; margin-bottom: 5px; font-weight: bold;">Strategy JSON:</label>
                    <textarea id="import-strategy-json" placeholder="Paste strategy JSON here" rows="10" style="width: 100%; padding: 8px; box-sizing: border-box; font-family: monospace;"></textarea>
                </div>
                <div style="display: flex; gap: 10px; justify-content: flex-end;">
                    <button id="import-strategy-cancel" data-close="modal-will-be-replaced" style="padding: 8px 16px; cursor: pointer;">Cancel</button>
                    <button id="import-strategy-submit" style="padding: 8px 16px; cursor: pointer; background-color: #4CAF50; color: white; border: none;">Import</button>
                </div>
            </div>
        `}),a=document.querySelector(`#${t} #import-strategy-cancel`);a&&(a.dataset.close=t);let o=document.querySelector(`#${t} #import-strategy-submit`),i=document.querySelector(`#${t} #import-strategy-name`),n=document.querySelector(`#${t} #import-strategy-json`);if(!o||!i||!n){f.Notify("Failed to create import modal",{notificationType:"error"});return}o.addEventListener("click",async()=>{let s=i.value.trim(),l=n.value.trim();if(!s){f.Notify("Please enter a strategy name",{notificationType:"warning"});return}if(r.getStrategies().some(g=>g.strategyName===s)){f.Notify(`Strategy "${s}" already exists`,{notificationType:"warning"});return}let d;try{d=JSON.parse(l)}catch{f.Notify("Invalid JSON format",{notificationType:"error"});return}this.validateStrategy(d),d.strategyName=s;try{r.saveStrategiesToStorage(d),f.Notify(`Strategy "${s}" imported successfully`,{notificationType:"success"});let g=document.querySelector(`#${t}`);g&&(await g.transition({opacity:0}),g.parentElement?.removeChild(g))}catch(g){f.Notify("Failed to save strategy",{notificationType:"error"}),c.log(`Import failed: ${g}`,"HSAutosing")}})}else f.Notify("Failed to find HSUI",{notificationType:"error"})}static async editSelectedStrategy(){let e=r.getSetting("autosingStrategy"),t=e.getValue();if(!t||t===""){f.Notify("Please select a strategy to edit",{notificationType:"warning"});return}let a=e.getDefinition().settingControl;if(!a?.selectOptions)return;let o=a.selectOptions.find(s=>s.value.toString()===t);if(!o)return;let n=r.getStrategies().find(s=>s.strategyName===o.text);if(!n){f.Notify("Cannot edit selected strategy",{notificationType:"error"});return}if(n.strategyName=="default_strategy"){f.Notify("cannot edit default strategy");return}await Z.open(n,t)}static saveSettingsToStorage(){let e=k.getModule("HSStorage");if(e){let t=this.#m();e.setData(A.HSSettings.storageKey,t)?c.debug("<green>Settings saved to localStorage</green>",this.#e):c.warn("Could not save settings to localStorage",this.#e)}}#g(){let e=JSON.parse(ze);for(let t of e)t&&r.validateStrategy(t);return e}#b(){let e=JSON.parse(Oe);for(let[t,a]of Object.typedEntries(e))if(a){if((a.settingType==="boolean"||h.isBoolean(a.settingValue))&&(a.settingValue=!1),(a.settingType==="selectnumeric"||a.settingType==="selectstring")&&("settingValue"in a||(a.settingValue=""),"settingControl"in a&&a.settingControl)){let o=a.settingControl;"selectOptions"in o&&o.selectOptions&&(o.selectOptions.find(n=>n.value==="")||o.selectOptions.unshift({text:"None",value:""}))}(a.settingType==="numeric"||a.settingType==="selectnumeric"||h.isNumeric(a.settingValue))&&("settingValueMultiplier"in a||(a.settingValueMultiplier=1)),a.settingType==="state"&&("settingValue"in a||(a.settingValue="<red>null</red>")),this.#d(a,r.#a)}return e}#f(){let e=k.getModule("HSStorage");if(!e)return c.warn("Could not find HSStorage module",this.context),null;let t=e.getData(A.HSSettings.strategiesKey);return t?Array.isArray(t)?t:[t]:null}#M(){let e=k.getModule("HSStorage");if(e){let t=e.getData(A.HSSettings.storageKey);return t?JSON.parse(t):(c.warn("Could not load settings from localStorage",this.context),null)}else return c.warn("Could not find HSStorage module",this.context),null}#p(e){for(let t of e)if(t.strategyName===void 0)throw new Error("Strategy name is missing. Cannot add strategy to options.");for(let t of e)r.#h(t)}static#v(e){let a=this.getSetting("autosingStrategy").getDefinition().settingControl;if(!a?.selectOptions)return;let o=a.selectOptions.findIndex(i=>i.text===e);if(o!==-1){let i=a.selectOptions[o].value;a.selectOptions.splice(o,1);let n=document.querySelector(`#${a.controlId}`);if(n){let s=n.querySelector(`option[value="${i}"]`);s&&s.remove()}}}static#h(e){let a=this.getSetting("autosingStrategy").getDefinition().settingControl;if(!a?.selectOptions)return;let o=a.selectOptions.length>0?Math.max(...a.selectOptions.map(n=>Number(n.value)))+1:1;a.selectOptions.push({text:e.strategyName,value:o});let i=document.querySelector(`#${a.controlId}`);if(i){let n=document.createElement("option");n.value=String(o),n.textContent=e.strategyName,i.appendChild(n)}}#c(){let e=this.#b(),t=this.#g();r.#l.push(...t);try{let a=this.#M(),o=this.#f();o&&r.#l.push(...o);let i=JSON.parse(JSON.stringify(e));return a?(c.log("<green>Found settings from localStorage!</green>",this.context),Object.keys(e).forEach(n=>{n in a&&Object.keys(a[n]).forEach(s=>{if(s in e[n]){let l=a[n][s];if(l!==null&&typeof l=="object"&&!Array.isArray(l)&&typeof e[n][s]=="object"&&!Array.isArray(e[n][s])){let u={...e[n][s]};Object.keys(l).forEach(d=>{d in u&&(u[d]=l[d])}),i[n][s]=u}else i[n][s]=l}})}),i):e}catch(a){return c.error("Error while resolving settings",this.context),console.log(a),e}}static async#S(e,t){await t.handleChange(e),this.saveSettingsToStorage()}static async#A(e,t){await t.handleToggle(e),this.saveSettingsToStorage()}static dumpToConsole(){console.log("------------------ HYPERSYNERGISM CURRENT SETTINGS DUMP START ------------------"),this.#t?console.log(this.#t):console.log("NO SETTINGS FOUND (wtf)"),console.log("------------------ HYPERSYNERGISM CURRENT SETTINGS DUMP END ------------------")}};var pe=class extends C{#e;#t;#a=["chronos","hyperrealism","quark","challenge","abyss","accelerator","acceleratorBoost","multiplier"];#n=[];#i=[];#o={chronosHepteract:0,hyperrealismHepteract:0,quarkHepteract:0,challengeHepteract:0,abyssHepteract:0,acceleratorHepteract:0,acceleratorBoostHepteract:0,multiplierHepteract:0};#s={chronosHepteract:null,hyperrealismHepteract:null,quarkHepteract:100,challengeHepteract:null,abyssHepteract:null,acceleratorHepteract:null,acceleratorBoostHepteract:null,multiplierHepteract:null};#r=0;#l=0;#u=0;#d=0;#m=0;#g=`
        <div id="hs-ratio-container">
            <div class="hs-ratio" id="hs-ratio-a">CHR/HYP/CHL: 1 / 123 / 123</div>
            <div class="hs-ratio" id="hs-ratio-b">ACC/BST/MLT: 1 / 123 / 123</div>
            <div class="hs-ratio" id="hs-ratio-c">CHR/ACC: 1 / 123</div>
        </div>`;#b=`
        #hs-ratio-container {
            width: 100%;
            display: grid;
            justify-items: center;
            grid-template-columns: repeat(3, 1fr);
            grid-template-rows: 1fr;
            grid-column-gap: 0px;
            grid-row-gap: 0px;
        }`;#f;#M;#p;#v;#h;#c;#S;#A=!1;#y=!1;constructor(e){super(e),this.#n=this.#a.map(t=>`${t}Hepteract`),this.#i=this.#a.map(t=>`${t}ProgressBarText`)}async init(){let e=this;c.log("Initialising HSHepteracts module",this.context);let t=k.getModule("HSGameState");t&&(t.subscribeGameStateChange("MAIN_VIEW",(a,o)=>{a.getId()===8&&o.getId()!==8&&t.getCurrentUIView("CUBE_VIEW").getId()===7&&e.#S&&(c.debug("Hepteract forge view closed, stopping watch",this.context),S.stopWatching(e.#S))}),t.subscribeGameStateChange("CUBE_VIEW",async(a,o)=>{o.getId()===7?(c.debug("Hepteract forge view opened, starting watch",this.context),e.#h=await S.HookElement("#hepteractQuantity"),e.#S=S.watchElement(e.#h,i=>{try{let n=parseFloat(h.unfuckNumericString(i));e.#c=n}catch{c.error("Failed to parse owned hepteracts",e.context)}e.#y=!1},{greedy:!0,overrideThrottle:!0,valueParser:i=>i.querySelector("span")?.innerText})):a.getId()===7&&e.#S&&(c.debug("Hepteract forge view closed, stopping watch",this.context),S.stopWatching(e.#S))})),this.#e=await S.HookElement("#heptGrid"),this.#t=await S.HookElement("#hepteractCraftTexts"),this.#e.childNodes.forEach(a=>{if(a.nodeType===1){let o=a,i=o.id;if(e.#n.includes(i)){let n=document.querySelector(`#${i}CraftMax`),s=document.querySelector(`#${i}Cap`),l=document.querySelector(`#${i}Image`);o.addEventListener("mouseenter",async u=>{if(i in e.#s&&i==="quarkHepteract")return;let d=document.querySelector("#hepteractCostText");if(d){let m=d.innerText.match(/you\s+(.*?)\s+Hepteracts/i);if(m){let p=m[1];try{if(i in e.#s){let b=h.parseFloat2(p);e.#s[i]=b}}catch{c.warn(`Error while parsing hepteract cost for ${i}`,e.context)}}}}),n&&s&&l&&(l.addEventListener("mouseenter",async u=>{let m=u.target.id.toLowerCase().includes("quark");if(e.#c!==null&&e.#c!==void 0){let p=e.#o[i],b=e.#s[i];if(p===null||b===null)return;let M=v.getSetting("expandCostProtectionDoubleCap"),E=null;if(M.getValue(),E=p*2*b,e.#c===0)e.#w(E,"\u221E",m);else{let y=e.#c>0?E/e.#c:1;e.#w(E,y,m)}}}),l.addEventListener("click",async u=>{let g=u.target.id,m=g.toLowerCase().includes("quark");if(!g||m)return;if(e.#A||e.#y){c.debug(`Quick expand cancelled, another expand was still pending (exp ${e.#A}, wtch: ${e.#y})`,e.context);return}e.#A=!0;let p=null,b=null;if(e.#c!==null&&e.#c!==void 0){if(e.#c===0){c.info("Owned hepteracts is 0",this.context),e.#A=!1;return}let y=e.#o[i],T=e.#s[i];(y===null||T===null)&&c.warn(`Hepteract cost for ${i} not parsed yet`,e.context);let H=v.getSetting("expandCostProtectionDoubleCap"),D=null;H.getValue(),D=y*2,p=y*2*T,b=e.#c>0?p/e.#c:1,c.debug(`
                                    Current max: ${y},
                                    Cube cost: ${h.N(T)},
                                    Next hepts: ${h.N(D)},
                                    Buy cost: ${h.N(p)},
                                    Percent owned: ${h.N(b)},
                                    Double Cap: ${H.getValue()}`,this.context);let x=v.getSetting("expandCostProtection"),J=v.getSetting("expandCostProtectionNotifications"),R=!(J&&J.getValue()===!0);if(x.isEnabled()){let L=x.getCalculatedValue();if(L&&b>=L){R&&c.info(`Hept. cost protection: Cost owned ${h.N(b*100)}% >= ${L*100}%`,this.context),e.#A=!1;return}}}else{c.warn("Owned hepteracts not parsed yet",this.context),e.#y=!1,e.#A=!1;return}let M=await S.HookElement("#toggle35");if(M&&M.innerText.includes("ON")&&(c.info("Turned hepteract notification toggle OFF",this.context),M.click()),await h.hiddenAction(async()=>{s.click()},"confirm",!1,25),await h.wait(25),n.click(),p&&b&&e.#w(p,b),await h.wait(5),i!=="quarkHepteract"){let y=document.querySelector("#hepteractCostText");if(y){let H=y.innerText.match(/you\s+(.*?)\s+Hepteracts/i);if(H){let D=H[1];try{if(i in e.#s){let x=h.parseFloat2(D);e.#s[i]=x}}catch{c.warn(`Error while parsing NEW hepteract cost for ${i}`,e.context)}}}}let E=e.#h?e.#h:document.querySelector("#hepteractQuantity");if(E){let y=E.querySelector("span");if(y){let T=y.innerText,H=parseFloat(T);e.#c=H}}e.#A=!1}))}}}),document.querySelectorAll(".heptTypeImage").length>0&&f.injectStyle(`
                .heptTypeImage:not(#quarkHepteractImage):not(#hepteractToQuarkImage):not(#overfluxPowderImage) {
                    transform: scale(1);
                    transform-origin: 50% 50%;
                }

                .heptTypeImage:not(#quarkHepteractImage):not(#hepteractToQuarkImage):not(#overfluxPowderImage):hover {
                    transform: scale(1.05);
                    cursor: pointer;
                }

                .heptTypeImage:not(#quarkHepteractImage):not(#hepteractToQuarkImage):not(#overfluxPowderImage):active {
                    transform: scale(0.98);
                }
            `),c.log("Hepteract images now serve as 'quick expand and max' buttons",this.context),c.log("Setting up hepteract ratio watch",this.context),f.injectStyle(this.#b),f.injectHTMLString(this.#g,a=>{e.#e?.parentNode?.insertBefore(a,e.#e)}),this.#f=document.querySelector("#hs-ratio-a"),this.#M=document.querySelector("#hs-ratio-b"),this.#p=document.querySelector("#hs-ratio-c"),this.#i.forEach(a=>{let o=document.querySelector(`#${a}`),i=a.substring(0,a.indexOf("ProgressBar"))+"Hepteract";o&&i?S.watchElement(o,n=>{i in e.#o?(e.#o[i]=n,Object.values(e.#o).every(s=>s>0)&&(e.#l=Math.round(e.#o.chronosHepteract/e.#o.challengeHepteract),e.#r=Math.round(e.#o.hyperrealismHepteract/e.#o.challengeHepteract),e.#d=Math.round(e.#o.acceleratorHepteract/e.#o.multiplierHepteract),e.#u=Math.round(e.#o.acceleratorBoostHepteract/e.#o.multiplierHepteract),e.#m=Math.round(e.#o.chronosHepteract/e.#o.acceleratorHepteract),this.#f&&this.#M&&this.#p&&(this.#f.innerText=`CHR/HYP/CHL: ${h.N(e.#l,0)} / ${h.N(e.#r,0)} / 1`,this.#M.innerText=`ACC/BST/MLT: ${h.N(e.#d,0)} / ${h.N(e.#u,0)} / 1`,this.#p.innerText=`CHR/ACC: ${h.N(e.#m,0)} / 1`))):c.warn(`Key ${i} not found in #boxCounts`,e.context)},{valueParser:n=>{let s=n.innerText;if(typeof s=="string"){let l=s.split("/");try{if(l&&l[1])return parseFloat(h.unfuckNumericString(l[1]))}catch{return c.warn(`Parsing failed for ${l}`,e.context),""}}return""}}):c.warn("Invalid meter or boxName",e.context)}),this.isInitialized=!0}#E(e){let t={cost:0,percentOwned:0};if(this.#c!==null&&this.#c!==void 0){if(this.#c===0){c.info("Owned hepteracts is 0",this.context);return}let a=this.#o[e],o=this.#s[e];if(a===null||o===null){c.warn(`Hepteract cost for ${e} not parsed yet`,this.context);return}let i=v.getSetting("expandCostProtectionDoubleCap"),n=null;i.getValue()?n=a*o:n=a*2*o;let s=this.#c>0?n/this.#c:1;return{cost:n,percentOwned:s}}else{c.warn("Owned hepteracts not parsed yet",this.context);return}}#w(e,t,a=!1){if(this.#t){let o=this.#t.querySelector("#hs-costText"),i;if(h.isNumeric(t)?i=h.N(t*100):i=t,o)a?o.innerText=`[${this.context}]: Total QUARK cost to max after next expand: ${h.N(e)} (ESTIMATE!)`:o.innerText=`[${this.context}]: Total HEPT cost to max after next expand: ${h.N(e)} (${i}% of owned)`;else{let n=document.createElement("div");n.id="hs-costText",a?n.innerText=`[${this.context}]: Total QUARK cost to max after next expand: ${h.N(e)} (ESTIMATE!)`:n.innerText=`[${this.context}]: Total HEPT cost to max after next expand: ${h.N(e)} (${i}% of owned)`,this.#t.appendChild(n)}}}};var Ae=class extends C{#e=[];#t;#a=3;#n=null;#i=3e3;constructor(e){super(e)}async init(){let e=this;c.log("Initialising HSTalismans module",this.context),this.#t=await S.HookElement("#buyTalismanAll"),this.#e=await S.HookElements(".fragmentBtn");let t=this.#t.cloneNode(!0);this.#t.replaceWith(t),this.#t=await S.HookElement("#buyTalismanAll"),this.#t.addEventListener("click",a=>{e.#n&&clearTimeout(e.#n),e.#e.length!==0&&(e.#e[e.#a].click(),e.#a++,e.#a>e.#e.length-1&&(e.#a=0),e.#n=setTimeout(()=>{e.#a=3},e.#i))}),c.log("Talisman BUY ALL button is now more functional",this.context),this.isInitialized=!0}};var be=class extends C{constructor(e){super(e)}async init(){c.log("Extending native prototypes with extra functionality",this.context),Element.prototype.delegateEventListener=this.#t(),Document.prototype.delegateEventListener=this.#t(),Element.prototype.removeDelegateEventListener=this.#a(),Document.prototype.removeDelegateEventListener=this.#a(),HTMLElement.prototype.transition=this.#n(),HTMLElement.prototype.clearTransitions=this.#i(),Object.typedEntries=this.#o,String.prototype.colorTag=function(e){return`<${e}>${this}</${e}>`},HTMLElement.prototype.textNodes=function(){return Array.from(this.childNodes).filter(e=>e.nodeType===Node.TEXT_NODE).map(e=>e.textContent??null).filter(e=>e!==null)},this.isInitialized=!0}#e(e,t){return`${e}:${t}`}#t(){let e=this;return function(t,a,o,i){let n=this;n._delegateListeners||(n._delegateListeners=new Map);let s=e.#e(t,a);if(i!==void 0&&i===!0&&n._delegateListeners.has(s)&&n._delegateListeners.get(s).size>0)return this;n._delegateListeners.has(s)||n._delegateListeners.set(s,new Map);let l=function(u){let d=u.target;if(d.matches(a))o.call(d,u);else{let g=n.querySelectorAll(a);for(let m of Array.from(g))if(m.contains(d)){o.call(m,u);break}}};return n._delegateListeners.get(s).set(o,{delegateHandler:l}),this.addEventListener(t,l),this}}#a(){let e=this;return function(t,a,o,i){let n=this;if(!n._delegateListeners)return this;let s=e.#e(t,a),l=n._delegateListeners.get(s);if(!l)return this;let u=l.get(o);return u&&(n.removeEventListener(t,u.delegateHandler),l.delete(o),l.size===0&&n._delegateListeners.delete(s),n._delegateListeners.size===0&&delete n._delegateListeners),this}}#n(){return function(e,t,a="linear"){let o=this,i=o.style;return t=t??A.HSPrototypes.defaultTransitionTiming,new Promise(n=>{if(Object.keys(e).length===0){n();return}let s=i.transition,l=Object.keys(e).join(", ");i.transition=`${l} ${t}ms ${a}`;let u=new Set(Object.keys(e)),d=!1,g=()=>{d||(o.removeEventListener("transitionend",p),delete o._hsTransitionEndHandler,i.transition=s,d=!0,n())},m=()=>{!d&&u.size===0&&g()},p=y=>{if(!(y instanceof TransitionEvent))return;let T=h.kebabToCamel(y.propertyName);u.has(T)&&(u.delete(T),u.size===0&&g())};this._hsTransitionEndHandler=p,o.addEventListener("transitionend",p);let b=setTimeout(()=>{d||g()},t+50);o.offsetHeight;let M=window.getComputedStyle(o),E=!1;if(Object.entries(e).forEach(([y,T])=>{let H=M.getPropertyValue(y.replace(/([A-Z])/g,"-$1").toLowerCase()),D=typeof T=="number"&&y!=="zIndex"&&y!=="opacity"?`${T}px`:String(T);H&&H!=="none"&&H!=="auto"&&H!==D?E=!0:u.delete(y),i[y]=T}),!E||u.size===0){clearTimeout(b),g();return}u.size===0&&g(),m()})}}#i(){return function(){this.style.transition="none",this._hsTransitionEndHandler&&(this.removeEventListener("transitionend",this._hsTransitionEndHandler),delete this._hsTransitionEndHandler)}}#o(e){return Object.entries(e)}};var fe=class extends C{#e;constructor(e){super(e),this.#e=new Map}async init(){}createShadow(e,t,a=!1){let o=t??h.domid(),i=this.#t(e);if(i)return i;let n=new Se(e,o,this).create(a);return n&&this.#e.set(o,n),n}getShadow(e){return this.#e.get(e)}destroyShadow(e){let t=e instanceof Se?e.name:e,a=this.getShadow(t);a&&(a._destroy(),this.#e.delete(t))}#t(e){for(let[t,a]of this.#e.entries())if(e===a.getElement())return a;return null}},Se=class{#e="HSShadow";#t;#a;#n;#i;#o;#s;#r={position:"absolute",width:"80vw",height:"80vh",left:"1vw",top:"1vh",overflowX:"auto",overflowY:"auto",zIndex:"1",backgroundColor:"rgba(125, 125, 125, 0.8)"};constructor(e,t,a){this.#a=e,this.name=t,this.#t=a,this.#s=document.createElement("div")}getElement(){return this.#a}getContainer(){return this.#s}hide(){this.#a.style.display="none",this.#s.style.display="none"}show(){this.#a.style.display="block",this.#s.style.display="block"}create(e=!1){this.#o=this.#a.style.cssText;let t=this.#a.parentNode;if(this.#i=this.#a.nextSibling,!t)return c.warn("Could not create shadow, parent is null",this.#e),null;this.#n=t;for(let[a,o]of Object.entries(this.#r))this.#s.style[a]=o;return e||this.hide(),document.body.appendChild(this.#s),this.#a.parentNode?.removeChild(this.#a),this.#s.appendChild(this.#a),this}destroySelf(){}_destroy(){this.#n&&(this.#i?this.#n.insertBefore(this.#a,this.#i):this.#n.appendChild(this.#a),this.#o&&(this.#a.style.cssText=this.#o),this.#s?.remove())}};var ye=class extends C{constructor(e){super(e)}async init(){}setData(e,t){try{let a=JSON.stringify(t);return localStorage.setItem(`${A.HSStorage.storagePrefix}${e}`,a),!0}catch(a){return a instanceof TypeError?c.warn("Data serialization error",this.context):a instanceof DOMException?c.warn("localStorage quota exceeded",this.context):c.warn(`Error saving to localStorage: ${a}`,this.context),!1}}getData(e){try{let t=localStorage.getItem(`${A.HSStorage.storagePrefix}${e}`);return t===null?(c.warn(`localStorage[${e}] is null`,this.context),null):JSON.parse(t)}catch(t){return c.warn(`Error retrieving from localStorage: ${t}`,this.context),null}}clearData(e){try{localStorage.removeItem(`${A.HSStorage.storagePrefix}${e}`)}catch(t){c.warn(`Error clearing localStorage: ${t}`,this.context)}}};var it=`#hs-minibar-wrapper {
    width: 281px;
    display: flex;
    padding: 0px;
    gap: 4px;
    flex-direction: column;
    align-self: flex-end;
    margin-right: 4%;
    margin-bottom: 5px;
}

#hs-blue-progress-bar,
#hs-red-progress-bar {
    width: 100%;
    height: 4px;
    background-color: black;
    border: 1px solid white;
    border-radius: 0px;
    box-shadow: 0 1px 3px rgb(0 0 0 / 20%);
    position: relative;
}

#hs-blue-progress {
    height: 100%;
    background: linear-gradient(to right, navy, darkturquoise);
    border-radius: 0px;
    transition: width 0.4s ease;
}

#hs-red-progress {
    height: 100%;
    background: linear-gradient(to right, maroon, red);
    border-radius: 0px;
    transition: width 0.4s ease;
}

#hs-blue-progress-text,
#hs-red-progress-text {
    display: none;
}
`;var Me=class extends C{#e;#t;#a=null;#n=[];#i=null;#o=null;#s=new Map;#r;#l;#u;#d=new Map;#m=`
        #${A.HSAmbrosia.quickBarId} > .blueberryLoadoutSlot:hover {
            filter: brightness(150%);
        }
        
        .hs-ambrosia-active-slot {
            --angle: 0deg;
            border-image: conic-gradient(
                from var(--angle), 
                #ff5e00, 
                #ff9a00, 
                #ffcd00, 
                #e5ff00, 
                #a5ff00, 
                #00ffc8, 
                #00c8ff, 
                #00a5ff, 
                #9500ff, 
                #ff00e1, 
                #ff0095, 
                #ff5e00
            ) 1;
            
            animation: hue-rotate 6s linear infinite;
        }

        @keyframes hue-rotate {
            to {
                --angle: 360deg;
            }
        }

        @property --angle {
            syntax: '<angle>';
            initial-value: 0deg;
            inherits: false;
        }
    `;#g="hs-ambrosia-quickbar-css";#b=`
        #hs-ambrosia-loadout-idle-swap-indicator {
            margin-bottom: 10px;
            font-family: fantasy;
            letter-spacing: 3px;

            background: linear-gradient(to right, #774ed1 20%, #00affa 30%, #0190cd 70%, #774ed1 80%);
            -webkit-background-clip: text;
            background-clip: text;
            -webkit-text-fill-color: transparent;
            background-size: 500% auto;
            animation: hs-loadout-ind-glow 3.5s ease-in-out infinite alternate;
        }

        @keyframes hs-loadout-ind-glow {
            0% {
                background-position: 0% 50%;
            }

            100% {
                background-position: 100% 50%;
            }
        }

        @-webkit-keyframes hs-loadout-ind-glow {
            0% {
                background-position: 0% 50%;
            }

            100% {
                background-position: 100% 50%;
            }
        }
    `;#f="hs-ambrosia-idle-loadout-css";#M="hs-ambrosia-minibar-css";#p=!1;#v;#h;#c;#S=!1;#A;#y;constructor(e){super(e)}async init(){let e=this;c.log("Initializing HSAmbrosia module",this.context),this.#a=await S.HookElement("#blueberryUpgradeContainer"),this.#n=await S.HookElements(".blueberryLoadoutSlot"),this.#i=await S.HookElement("#bbLoadoutContainer"),this.#c=document.querySelector("#hs-panel-debug-gamedata-currentambrosia"),this.loadState(),await this.#H(),c.log("Hooking stuff",this.context);for(let[t,a]of A.HSAmbrosia.ambrosiaLoadoutIcons.entries()){let o=document.querySelector(`#${a.draggableIconId}`);o&&(o.draggable=!0,o.dataset.hsid=t,o.addEventListener("dragstart",i=>{if(!i.dataTransfer)return;let n=i.currentTarget?.dataset.hsid;n&&(c.log(`Drag start ${n}`,this.context),i.dataTransfer.effectAllowed="move",i.dataTransfer.setData("hs-amb-drag",n))}))}if(!this.#i){c.warn("Could not find loadout container",this.context);return}this.#n.forEach(t=>{let a=t.id;if(a){let o=this.#C(a);if(o){let i=this.#s.get(o);i&&this.#T(o,i)}}}),this.#i.delegateEventListener("dragenter",".blueberryLoadoutSlot",t=>{t.dataTransfer&&t.dataTransfer.types.includes("hs-amb-drag")&&(t.preventDefault(),t.stopPropagation(),t.dataTransfer.effectAllowed="move")}),this.#i?.delegateEventListener("dragover",".blueberryLoadoutSlot",t=>{t.dataTransfer&&t.dataTransfer.types.includes("hs-amb-drag")&&(t.preventDefault(),t.stopPropagation(),t.dataTransfer.effectAllowed="move")}),this.#i?.delegateEventListener("drop",".blueberryLoadoutSlot",async t=>{if(t.dataTransfer&&t.dataTransfer.types.includes("hs-amb-drag")){t.preventDefault(),t.stopPropagation();let a=this.#w(t.dataTransfer.getData("hs-amb-drag")),o=t.target,i=o.id;if(!a){c.warn(`Invalid icon ID: ${a}`,this.context);return}if(!A.HSAmbrosia.ambrosiaLoadoutIcons.has(a)){c.warn(`Could not find loadout slot entry for ${a}`,this.context);return}if(!o.classList.contains("blueberryLoadoutSlot")||!i)return;let n=this.#C(i);if(!n){c.warn(`Invalid slot ID: ${i}`,this.context);return}let s=A.HSAmbrosia.ambrosiaLoadoutIcons.get(a);this.#T(n,a),this.#s.set(n,a),this.saveState(),await e.updateQuickBar()}}),this.#i?.delegateEventListener("contextmenu",".blueberryLoadoutSlot",async t=>{t.preventDefault();let a=t.target,o=a.id,i=this.#C(o);if(!i){c.warn(`Invalid slot ID: ${o}`,this.context);return}if(!this.#s.get(i)){c.warn(`No icon found for slot ID: ${o}`,this.context);return}a.classList.remove("hs-ambrosia-slot"),a.style.backgroundImage="",this.#s.delete(i),this.saveState(),await e.updateQuickBar()}),this.#i?.delegateEventListener("click",".blueberryLoadoutSlot",async t=>{let o=t.target.id,i=this.#C(o);if(!i){c.warn(`Invalid slot ID: ${o}`,this.context);return}await e.#E(i)}),this.isInitialized=!0}async#E(e){this.#r=e;let t=v.getSetting("autoLoadoutState");t&&t.setValue(`<green>${e}</green>`),this.#o||(this.#o=await S.HookElement("header"));let a=this.#o.querySelector(`#${A.HSAmbrosia.quickBarId}`);if(a){a.querySelectorAll(".blueberryLoadoutSlot").forEach(n=>{n.classList.remove("hs-ambrosia-active-slot")});let i=a.querySelector(`#${A.HSAmbrosia.quickBarLoadoutIdPrefix}-${e}`);i&&i.classList.add("hs-ambrosia-active-slot")}else c.warn("Could not find quick bar element",this.context);let o=document.querySelector("#bbLoadoutContainer");if(o){o.querySelectorAll(".blueberryLoadoutSlot").forEach(n=>{n.classList.remove("hs-ambrosia-active-slot")});let i=o.querySelector(`#${e}`);i&&i.classList.add("hs-ambrosia-active-slot")}}#w(e){return Object.values(ae).find(t=>t===e)}#C(e){return Object.values(te).find(t=>t===e)}#T(e,t){let a=document.querySelector(`#${e}`);if(!a){c.warn(`Could not find slot element for ${e}`,this.context);return}let o=A.HSAmbrosia.ambrosiaLoadoutIcons.get(t);if(!o){c.warn(`Could not find icon for ${t}`,this.context);return}a.classList.add("hs-ambrosia-slot"),a.style.backgroundImage=`url(${o.url})`}async saveState(){let e=k.getModule("HSStorage");if(e){let t=JSON.stringify(Array.from(this.#s.entries()));e.setData(A.HSAmbrosia.storageKey,t)}else c.warn("saveState - Could not find storage module",this.context)}async loadState(){let e=k.getModule("HSStorage");if(e){let a=e.getData(A.HSAmbrosia.storageKey);if(!a){c.warn("loadState - No data found",this.context);return}try{let o=JSON.parse(a);this.#s=new Map(o)}catch{c.warn("loadState - Error parsing data",this.context);return}}else c.warn("loadState - Could not find storage module",this.context);let t=v.getSetting("autoLoadoutState");t&&!this.#r?this.#r=h.removeColorTags(t.getValue()):c.warn("loadState - Could not find autoLoadoutState setting",this.context)}async createQuickBar(){let e=this;if(this.#d.forEach((t,a)=>{a.removeEventListener("click",t)}),this.#d.clear(),this.#i=await S.HookElement("#bbLoadoutContainer"),this.#o=await S.HookElement("header"),this.#i&&this.#o){let t=this.#o.querySelector("nav.navbar"),a=this.#i.cloneNode(!0);a.id=A.HSAmbrosia.quickBarId;let o=a.querySelector(".blueberryLoadoutSetting");a.querySelectorAll(".blueberryLoadoutSlot").forEach(n=>{let s=n.id;n.id=`${A.HSAmbrosia.quickBarLoadoutIdPrefix}-${s}`;let l=async function(u){await e.#U(u,s)};this.#d.set(n,l),n.addEventListener("click",l)}),o&&o.remove(),this.#o.insertBefore(a,t),f.injectStyle(this.#m,this.#g),this.#r&&await this.#E(this.#r)}}async destroyQuickBar(){this.#o||(this.#o=await S.HookElement("header"));let e=this.#o.querySelector(`#${A.HSAmbrosia.quickBarId}`);e?(this.#d.forEach((a,o)=>{o.removeEventListener("click",a)}),this.#d.clear(),e.remove(),f.removeInjectedStyle(this.#g)):c.warn("Could not find quick bar element",this.context);let t=document.querySelector("#bbLoadoutContainer");t&&t.querySelectorAll(".blueberryLoadoutSlot").forEach(a=>{a.classList.remove("hs-ambrosia-active-slot")})}async#H(){let e=await S.HookElement("#importBlueberriesButton");if(!e||document.getElementById("hs-ambrosia-extra-btn"))return;let t=document.createElement("button");t.id="hs-ambrosia-extra-btn",t.className="ambrosiaLoadoutBtn",t.textContent="Quick Import",e.parentElement?.insertBefore(t,e.nextSibling),t.addEventListener("click",()=>this.#L())}async updateQuickBar(){v.getSetting("ambrosiaQuickBar").isEnabled()&&(await this.destroyQuickBar(),await this.createQuickBar(),this.#r&&await this.#E(this.#r))}async enableAutoLoadout(){let e=this,t=v.getSetting("autoLoadoutState");if(t&&t.getValue().includes("Unknown")||!this.#r){let d=v.getSetting("autoLoadout");d&&d.isEnabled()&&d.disable(),c.warn("Could not enable auto loadout - current loadout state is not known!",this.context);return}let a=[S.HookElement("#addCode"),S.HookElement("#addCodeAll"),S.HookElement("#addCodeOne"),S.HookElement("#timeCode")],i=(await Promise.allSettled(a)).map((d,g)=>d.status==="fulfilled"?d.value:null);if(i.some(d=>d===null)){c.warn("Problem with enabling auto loadout",this.context);return}let[n,s,l,u]=i;this.#l||(this.#l=async d=>{await e.#B(d)}),this.#u||(this.#u=async d=>{await e.#N(d)}),n.removeEventListener("click",this.#l,{capture:!0}),n.addEventListener("click",this.#l,{capture:!0}),s.removeEventListener("click",this.#l,{capture:!0}),s.addEventListener("click",this.#l,{capture:!0}),l.removeEventListener("click",this.#l,{capture:!0}),l.addEventListener("click",this.#l,{capture:!0}),u.removeEventListener("click",this.#u,{capture:!0}),u.addEventListener("click",this.#u,{capture:!0}),c.log("Enabled auto loadout",this.context)}async disableAutoLoadout(){let e=this,t=[S.HookElement("#addCode"),S.HookElement("#addCodeAll"),S.HookElement("#addCodeOne"),S.HookElement("#timeCode")],o=(await Promise.allSettled(t)).map((u,d)=>u.status==="fulfilled"?u.value:null);if(o.some(u=>u===null)){c.warn("Problem with disabling auto loadout",this.context);return}let[i,n,s,l]=o;this.#l&&(i.removeEventListener("click",this.#l,{capture:!0}),n.removeEventListener("click",this.#l,{capture:!0}),s.removeEventListener("click",this.#l,{capture:!0})),this.#u&&l.removeEventListener("click",this.#u,{capture:!0}),c.log("Disabled auto loadout",this.context)}async#B(e){let t=this.#r,a=v.getSetting("autoLoadoutAdd");if(t&&a){let o=this.#k(a.getValue()),i=await S.HookElement(`#${o}`);await this.#I(),await h.hiddenAction(async()=>{i.click()})}}async#N(e){let t=this.#r,a=v.getSetting("autoLoadoutTime");if(t&&a){let o=this.#k(a.getValue()),i=await S.HookElement(`#${o}`);await this.#I(),await h.hiddenAction(async()=>{i.click()})}}async#U(e,t){let a=document.querySelector(`#${t}`);a?(await this.#I(),await h.hiddenAction(async()=>{a.click()})):c.warn(`Could not find real button for ${t}`,this.context)}async#I(){let e=await S.HookElement("#blueberryToggleMode");e&&e.innerText.includes("SAVE")&&e.click()}#k(e){let t=Object.values(te).find(a=>a===`blueberryLoadout${e}`);return t||c.warn(`Could not convert loadout ${e} to slot`,this.context),t}subscribeGameDataChanges(){let e=k.getModule("HSGameData");e&&!this.gameDataSubscriptionId&&(this.gameDataSubscriptionId=e.subscribeGameDataChange(this.gameDataCallback.bind(this)),c.debug("Subscribed to game data changes",this.context))}unsubscribeGameDataChanges(){let e=k.getModule("HSGameData");e&&this.gameDataSubscriptionId&&!this.#p&&!this.#S&&(e.unsubscribeGameDataChange(this.gameDataSubscriptionId),this.gameDataSubscriptionId=void 0,c.debug("Unsubscribed from game data changes",this.context))}async gameDataCallback(){let e=k.getModule("HSGameDataAPI");if(!e)return;let t=e.getGameData();if(t&&t.blueberryTime&&t.redAmbrosiaTime){let a=t.blueberryTime,o=t.redAmbrosiaTime,i=e.R_calculateRequiredBlueberryTime(),n=e.R_calculateRequiredRedAmbrosiaTime(),s=a/i*100,l=o/n*100,u=e.calculateAmbrosiaSpeed(),d=e.calculateBlueBerries(),g=u*d,m=t.shopUpgrades.shopAmbrosiaAccelerator,p=e.calculateLuck(),b=t.singularityChallenges.noAmbrosiaUpgrades.completions>0?1:0,M=p.total/100+b,E=(p.total-100*Math.floor(p.total/100))/100,y=0,T=0,H=0,D=g/i*100,x=D+.1,J=1/2+(3/5-1/2)*+(t.singularityChallenges.noAmbrosiaUpgrades.completions>=15)+(2/3-3/5)*+(t.singularityChallenges.noAmbrosiaUpgrades.completions>=19)+(3/4-2/3)*+(t.singularityChallenges.noAmbrosiaUpgrades.completions>=20);if(m>0&&g>0){let R=i/g;y=Math.min(R*J,M*.2*m),T=y*g,H=T/i*100}if(this.#p&&this.#v&&this.#h){let R=v.getSetting("ambrosiaIdleSwapNormalLoadout"),L=v.getSetting("ambrosiaIdleSwap100Loadout");if(R&&L){let P=R.getValue(),$=L.getValue();if(!Number.isInteger(parseInt(P,10))||!Number.isInteger(parseInt($,10))){let z=v.getSetting("ambrosiaIdleSwap");z&&z.disable(),c.log("Idle swap was disabled due to unconfigured loadouts",this.context);return}let ee=this.#k(R.getValue()),Q=this.#k(L.getValue()),G=x+H,O=G+x,V=100-x,j=100,Pe=A.HSAmbrosia.idleSwapMinRedTreshold,mt=Pe+A.HSAmbrosia.idleSwapMinRedTreshold,pt=A.HSAmbrosia.idleSwapMaxRedTreshold;if(s>=V&&s<=j||l>=pt&&l<=100){if(this.#r!==Q){let z=await S.HookElement(`#${Q}`);await this.#I(),await h.hiddenAction(async()=>{z.click()})}}else if((s>=G&&s<=O||l>=Pe&&l<=mt)&&this.#r!==ee){let z=await S.HookElement(`#${ee}`);await this.#I(),await h.hiddenAction(async()=>{z.click()})}}if(this.#c&&f.isModPanelOpen()){let P=document.createElement("div");for(P.innerHTML=`
                        BLUE - Value: ${a.toFixed(2)}, Max: ${i}, Percent: ${s.toFixed(2)}<br>
                        RED - Value: ${o.toFixed(2)}, Max: ${n}, Percent: ${l.toFixed(2)}<br>
                        BLUE SPD MLT: ${u.toFixed(2)}<br>
                        BLUE SPD %: ${D.toFixed(2)}<br>
                        BERRY: ${d}</br>
                        TOT BLU: ${(u*d).toFixed(2)}</br>
                        ------------------------</br>
                        ADD LUK: ${p.additive.toFixed(2)}</br>
                        RAW LUK: ${p.raw.toFixed(2)}</br>
                        TOT LUK: ${p.total.toFixed(2)}</br>
                        ------------------------</br>
                        ACC CNT: ${m}</br>
                        ACCEL AMOUNT: ${T.toFixed(2)}</br>
                        ACCEL %: ${H.toFixed(2)}</br>
                        `,this.#c.innerHTML="";P.firstChild;)this.#c.appendChild(P.firstChild)}}this.#S?this.#A&&this.#y?(this.#A.style.width=`${s}%`,this.#y.style.width=`${l}%`):c.warnOnce(`
                        HSAmbrosia.gameDataCallback() - minibar element(s) undefined. 
                        blue: ${this.#A}, 
                        red: ${this.#y}`,"hs-minibars-undefined"):c.logOnce("HSAmbrosia.gameDataCallback() - berryMinibarsEnabled was false","hs-minibars-false")}}async enableIdleSwap(){let e=this,t=k.getModule("HSGameState");t?(this.#e=t.subscribeGameStateChange("MAIN_VIEW",this.#R.bind(this)),this.#t=t.subscribeGameStateChange("SINGULARITY_VIEW",async(a,o)=>{o.getId()===5?(this.#v=await S.HookElement("#ambrosiaProgressBar"),this.#h=await S.HookElement("#pixelProgressBar"),this.#p=!0,this.#D(),this.subscribeGameDataChanges()):(this.#p=!1,this.#x(),this.unsubscribeGameDataChanges())}),t.getCurrentUIView("SINGULARITY_VIEW").getId()===5&&t.getCurrentUIView("MAIN_VIEW").getId()===13&&(this.#v=await S.HookElement("#ambrosiaProgressBar"),this.#h=await S.HookElement("#pixelProgressBar"),this.#p=!0,this.#D(),this.subscribeGameDataChanges())):c.warn("HSAmbrosia.enableIdleSwap() - gameStateMod==undefined","hs-enable-idleswap-gamestate"),this.#c||(this.#c=document.querySelector("#hs-panel-debug-gamedata-currentambrosia"))}disableIdleSwap(){this.#p=!1,this.unsubscribeGameDataChanges();let e=k.getModule("HSGameState");e?(this.#e&&(e.unsubscribeGameStateChange("MAIN_VIEW",this.#e),this.#e=void 0),this.#t&&(e.unsubscribeGameStateChange("SINGULARITY_VIEW",this.#t),this.#t=void 0)):c.warnOnce("HSAmbrosia.disableIdleSwap() - gameStateMod==undefined","hs-disable-idleswap-gamestate"),this.#x()}#R(e,t){let a=k.getModule("HSGameState");a?e.getId()===13&&t.getId()!==13&&a.getCurrentUIView("SINGULARITY_VIEW").getId()===5&&(this.#p=!1):c.warnOnce("HSAmbrosia.gameStateCallbackMain() - gameStateMod==undefined","hs-amb-gamestate-cb")}#D(){if(document.querySelector(`#${A.HSAmbrosia.idleSwapIndicatorId}`))return;let t=document.createElement("div");t.id=A.HSAmbrosia.idleSwapIndicatorId,t.innerText="IDLE SWAP ENABLED WHILE IN THIS VIEW",f.injectHTMLElement(t,a=>{let o=document.querySelector("#singularityAmbrosia"),i=document.querySelector("#ambrosiaProgressBar");o?.insertBefore(a,i)}),f.injectStyle(this.#b,this.#f)}#x(){let e=document.querySelector(`#${A.HSAmbrosia.idleSwapIndicatorId}`);e&&e.remove(),f.removeInjectedStyle(this.#f)}async enableBerryMinibars(){if(this.#o||(this.#o=await S.HookElement("header")),!this.#o)return;let t=(await S.HookElement("#ambrosiaProgressBar")).cloneNode(!0),a=t.querySelector("#ambrosiaProgress"),o=t.querySelector("#ambrosiaProgressText");t.id=A.HSAmbrosia.blueBarId,a.id=A.HSAmbrosia.blueBarProgressId,o.id=A.HSAmbrosia.blueBarProgressTextId;let n=(await S.HookElement("#pixelProgressBar")).cloneNode(!0),s=n.querySelector("#pixelProgress"),l=n.querySelector("#pixelProgressText");n.id=A.HSAmbrosia.redBarId,s.id=A.HSAmbrosia.redBarProgressId,l.id=A.HSAmbrosia.redBarProgressTextId;let u=document.createElement("div");u.id=A.HSAmbrosia.barWrapperId,u.appendChild(t),u.appendChild(n);let d=v.getSetting("ambrosiaQuickBar"),g;d&&d.isEnabled()?g=this.#o.querySelector("#hs-ambrosia-quick-loadout-container"):g=this.#o.querySelector("nav.navbar"),this.#o.insertBefore(u,g),f.injectStyle(it,this.#M),this.#A=a,this.#y=s,this.subscribeGameDataChanges(),this.#S=!0}async disableBerryMinibars(){this.#o||(this.#o=await S.HookElement("header"));let e=this.#o.querySelector(`#${A.HSAmbrosia.barWrapperId}`);e?e.remove():c.warn("Could not find bar wrapper element",this.context),f.removeInjectedStyle(this.#M),this.#S=!1,this.unsubscribeGameDataChanges()}async#L(){let e=null;try{e=document.querySelector(".blueberryLoadoutSlot.hs-ambrosia-active-slot");let t=await navigator.clipboard.readText();if(!t||typeof t!="string"){f.Notify("Clipboard does not contain valid loadout data",{notificationType:"warning"});return}let a=t.split(`
`).map(u=>u.trim());if(a.length===0||a.length>16){f.Notify(`Invalid number of loadouts: ${a.length}. Expected 1-16.`,{notificationType:"warning"});return}let o=document.getElementById("importBlueberries"),i=await S.HookElement("#blueberryToggleMode");if(!o)throw new Error("Import input element not found");if(!i)throw new Error("Mode toggle button not found");i.innerText.includes("LOAD ")&&i.click();let s=0,l=0;h.startDialogWatcher();for(let u=0;u<a.length;u++){let d=a[u],g=u+1;if(!d){l++;continue}let m=document.getElementById(`blueberryLoadout${g}`);if(!m){console.warn(`Loadout button ${g} not found`);continue}let p=new Blob([d],{type:"application/json"}),b=new File([p],"quick-import.json",{type:"application/json"}),M=new DataTransfer;M.items.add(b),o.files=M.files;let E=new Event("change",{bubbles:!0});o.dispatchEvent(E),await h.sleep(50),m.click(),await h.sleep(50),s++}i.click(),f.Notify(`Imported ${s} loadout(s), skipped ${l} empty slot(s)`,{notificationType:"success"})}catch(t){let a=t instanceof Error?t.message:typeof t=="string"?t:"Unknown error";console.error("Quick Import error:",t),c.error(`Quick Import failed: ${a}`,this.context,!0),f.Notify("Quick Import failed",{notificationType:"error"})}finally{await h.stopDialogWatcher(),e&&e.click()}}};var ve=class extends C{constructor(e){super(e)}async init(){c.log("Initializing HSStats module",this.context),this.isInitialized=!0}};var Ee=class extends C{#e=-1;#t={CubeView:X,SingularityView:q};#a={MAIN_VIEW:{currentView:new K("unknown"),previousView:new K("unknown"),viewChangeSubscribers:new Map},CUBE_VIEW:{currentView:new X("unknown"),previousView:new X("unknown"),viewChangeSubscribers:new Map},SINGULARITY_VIEW:{currentView:new q("unknown"),previousView:new q("unknown"),viewChangeSubscribers:new Map}};#n=["buildings","upgrades","statistics","runes","challenges","research","ants","cubes","campaigns","traits","settings","shop","singularity","event","pseudoCoins"];constructor(e){super(e)}async init(){let e=this;c.log("Initializing HSGameState module",this.context);for(let t of this.#n){let a=document.querySelector(`#${t}`);S.watchElement(a,async o=>{let{view:i,state:n}=o;if(n!=="none"){let s=new K(i);if(s.getId()!==-1)e.#a.MAIN_VIEW.previousView=e.#a.MAIN_VIEW.currentView,e.#a.MAIN_VIEW.currentView=s,c.debug(`Main UI view changed ${e.#a.MAIN_VIEW.previousView.getName()} -> ${e.#a.MAIN_VIEW.currentView.getName()}`,e.context);else{c.warn(`Main UI view ${i} not found`,e.context);return}e.#a.MAIN_VIEW.viewChangeSubscribers.forEach(l=>{try{l(e.#a.MAIN_VIEW.previousView,e.#a.MAIN_VIEW.currentView)}catch(u){c.error(`Error when trying to call MAIN VIEW change subscriber callback: ${u}`,e.context)}}),this.#i(s.getId())}},{characterData:!1,childList:!1,subtree:!1,attributes:!0,attributeOldValue:!1,attributeFilter:["style"],valueParser:(o,i)=>{for(let n of i)if(n.type==="attributes"&&n.attributeName==="style"){let s=n.target,l=s.style.getPropertyValue("display");return{view:s.id,state:l}}}})}A.HSGameState.viewProperties.forEach(async(t,a)=>{for(let o of t.subViewIds){let i=document.querySelector(`#${o}`);S.watchElement(i,async n=>{let{view:s,state:l}=n;if(l!=="none"){let u;try{let g=this.#t[t.viewClassName];if(!g)throw new Error(`Class "${t.viewClassName}" not found in viewClasses for mainViewId ${a}`);u=new g(o)}catch(g){c.warn(`Failed to instantiate sub-view ${t.viewClassName}: ${g}`,e.context);return}let d=u.getViewKey();if(u.getId()!==e.#e){e.#a[d].previousView=e.#a[d].currentView,e.#a[d].currentView=u;let g=e.#a[d].previousView,m=e.#a[d].currentView;e.#a[d].viewChangeSubscribers.forEach(p=>{try{p(g,m)}catch(b){c.error(`Error when trying to call CUBE VIEW change subscriber callback: ${b}`,e.context)}}),c.debug(`Subview changed ${g.getName()} -> ${m.getName()}`,e.context)}else{c.warn(`Subview ${s} not found`,e.context);return}}},{characterData:!1,childList:!1,subtree:!1,attributes:!0,attributeOldValue:!1,attributeFilter:["style"],valueParser:(n,s)=>{for(let l of s)if(l.type==="attributes"&&l.attributeName==="style"){let u=l.target,d=u.style.getPropertyValue("display");return{view:u.id,state:d}}}})}}),this.isInitialized=!0}subscribeGameStateChange(e,t){let a=h.uuidv4();return this.#a[e].viewChangeSubscribers.set(a,t),a}unsubscribeGameStateChange(e,t){this.#a[e].viewChangeSubscribers.has(t)?this.#a[e].viewChangeSubscribers.delete(t):c.warn(`Subscription ID ${t} not found for view key ${e}`,this.context)}async#i(e){let t=this,a=A.HSGameState.viewProperties.get(e);if(!a){c.debug(`No view properties found for main view ID ${e}`,this.context);return}(await S.HookElements(a.subViewsSelector)).forEach(async i=>{let n=i.id;if(i.style.getPropertyValue("display")!=="none"){let l;try{let d=this.#t[a.viewClassName];if(!d)throw new Error(`Class "${a.viewClassName}" not found in viewClasses for mainViewId ${e}`);l=new d(n)}catch(d){c.warn(`Failed to instantiate sub-view ${a.viewClassName} for tab ${n}: ${d}`,t.context);return}let u=l.getViewKey();if(l&&l.getId()!==this.#e){t.#a[u].previousView=t.#a[u].currentView,t.#a[u].currentView=l;let d=t.#a[u].previousView,g=t.#a[u].currentView;t.#a[u].viewChangeSubscribers.forEach(m=>{try{m(d,g)}catch(p){c.error(`Error when trying to call VIEW change subscriber callback: ${p}`,t.context)}})}else c.warn(`Sub-view tab ${n} (for ${a.viewClassName}) resolved to UNKNOWN or failed to initialize.`,t.context)}})}getCurrentUIView(e){return this.#a[e].currentView}},_=class{#e;#t;constructor(e,t){this.#e=e,this.#t=t}getName(){return this.#e}getViewKey(){return this.#t}},K=class extends _{#e;constructor(e){super(e,"MAIN_VIEW"),this.#e=this.getViewEnum(e)}getViewEnum(e){switch(e){case"buildings":return 1;case"upgrades":return 2;case"statistics":return 3;case"runes":return 4;case"challenges":return 5;case"research":return 6;case"ants":return 7;case"cubes":return 8;case"campaigns":return 9;case"traits":return 10;case"settings":return 11;case"shop":return 12;case"singularity":return 13;case"event":return 14;case"pseudoCoins":return 15}return-1}getId(){return this.#e}},X=class extends _{#e;constructor(e){super(e,"CUBE_VIEW"),this.#e=this.getViewEnum(e)}getViewEnum(e){switch(e){case"cubeTab1":return 1;case"cubeTab2":return 2;case"cubeTab3":return 3;case"cubeTab4":return 4;case"cubeTab5":return 5;case"cubeTab6":return 6;case"cubeTab7":return 7}return-1}getId(){return this.#e}},q=class extends _{#e;constructor(e){super(e,"SINGULARITY_VIEW"),this.#e=this.getViewEnum(e)}getViewEnum(e){switch(e){case"singularitycontainer1":return 1;case"singularityContainer2":return 2;case"singularityContainer3":return 3;case"singularityContainer4":return 4;case"singularityContainer5":return 5}return-1}getId(){return this.#e}};var B=class{#e;constructor(e){this.#e=e}get patchName(){return this.#e}};var we=class extends B{#e=`
        #ambrosiaUpgradeValues {
            overflow-y: auto;
            height: 25vh;
        }

        #ambrosiaUpgradeValues::-webkit-scrollbar {
            width: 5px;
            height: 5px;
        }

        #ambrosiaUpgradeValues::-webkit-scrollbar-track {
            background: #1c1b22;
            border-radius: 3px;
        }

        #ambrosiaUpgradeValues::-webkit-scrollbar-thumb {
            background: #a22a2a;
            border-radius: 3px;
        }

        #ambrosiaUpgradeValues::-webkit-scrollbar-corner {
            background: #1c1b22;
        }

        @supports not selector(::-webkit-scrollbar) {
            #ambrosiaUpgradeValues::-webkit-scrollbar {
                scrollbar-color: #a22a2a #1c1b22;
            }
        }`;#t="ambrosiaViewOverflowPatch";async applyPatch(){f.injectStyle(this.#e,this.#t)}async revertPatch(){f.removeInjectedStyle(this.#t)}};var Ie=class extends B{async applyPatch(){}async revertPatch(){}};var nt=`{
    "offeringPotion": "Offering Potion",
    "obtainiumPotion": "Obtainium Potion",
    "offeringEX": "Offering EX",
    "offeringAuto": "Offering Auto",
    "obtainiumEX": "Obtainium EX",
    "obtainiumAuto": "Obtainium Auto",
    "instantChallenge": "Instant Challenge Completions",
    "antSpeed": "Ant Speed",
    "cashGrab": "Cash Grab",
    "shopTalisman": "the Plastic talisman",
    "seasonPass": "Season Pass",
    "challengeExtension": "Reincarnation Challenge EX",
    "challengeTome": "Challenge 10 Requirement Reduce",
    "cubeToQuark": "Cube Quarks +50%",
    "tesseractToQuark": "Tesseract Quarks +50%",
    "hypercubeToQuark": "Hypercube Quarks +50%",
    "seasonPass2": "Season Pass 2",
    "seasonPass3": "Season Pass 3",
    "chronometer": "Chronometer 1",
    "infiniteAscent": "Infinite Ascent",
    "calculator": "PL-AT calculator",
    "calculator2": "PL-AT X calculator",
    "calculator3": "PL-AT \\u03A9 calculator",
    "calculator4": "PL-AT \\u03B4 calculator",
    "calculator5": "PL-AT \\u0393 calculator",
    "calculator6": "QUAAA-T calculator",
    "calculator7": "PL-AT \\u03A9\\u03A9 calculator",
    "constantEX": "Constant EX",
    "powderEX": "Powder EX",
    "chronometer2": "Chronometer 2",
    "chronometer3": "Chronometer 3",
    "seasonPassY": "Season Pass Y",
    "seasonPassZ": "Season Pass Z",
    "challengeTome2": "Challenge 10 Requirement Reduction 2",
    "instantChallenge2": "Instant Challenge Completions 2",
    "cubeToQuarkAll": "Quark Gain Cube Improvement 2",
    "cashGrab2": "Cash Grab 2",
    "chronometerZ": "Chronometer Z",
    "obtainiumEX2": "Obtainium EX 2",
    "offeringEX2": "Offering EX 2",
    "powderAuto": "Automated Powder",
    "seasonPassLost": "Season Pass LOST",
    "challenge15Auto": "Challenge 15 Automation",
    "extraWarp": "Extra Warp",
    "autoWarp": "a quack powered Warps?",
    "improveQuarkHept": "Quark Hepteract 1",
    "improveQuarkHept2": "Quark Hepteract 2",
    "improveQuarkHept3": "Quark Hepteract 3",
    "improveQuarkHept4": "Quack Hepteract 4",
    "shopImprovedDaily": "Improved Daily Code 1",
    "shopImprovedDaily2": "Improved Daily Code 2",
    "shopImprovedDaily3": "Improved Daily Code 3",
    "shopImprovedDaily4": "Improved Daily Code 4",
    "offeringEX3": "The final Offering Upgrade",
    "obtainiumEX3": "The final Obtainium Upgrade",
    "improveQuarkHept5": "The final Quark Hepteract Improver",
    "chronometerInfinity": "The final Chronometer",
    "seasonPassInfinity": "The final Season pass",
    "shopSingularityPenaltyDebuff": "A Singularity Tenderizer",
    "shopAmbrosiaLuckMultiplier4": "The Fourth Multiplicative Ambrosia Luck Multiplier",
    "shopOcteractAmbrosiaLuck": "Octeract-Based Ambrosia Luck Amplifier",
    "shopAmbrosiaGeneration1": "Ambrosia Generation Speedup",
    "shopAmbrosiaGeneration2": "Another Ambrosia Generation Speedup",
    "shopAmbrosiaGeneration3": "A better Ambrosia Generation Speedup",
    "shopAmbrosiaGeneration4": "A FINAL Ambrosia Generation Speedup",
    "shopAmbrosiaLuck1": "Ambrosia Luck Increaser",
    "shopAmbrosiaLuck2": "Another Ambrosia Luck Increaser",
    "shopAmbrosiaLuck3": "A better Ambrosia Generation Speedup",
    "shopAmbrosiaLuck4": "A FINAL Ambrosia Luck Increaser",
    "shopRedLuck1": "Low Class Dice of Asmodeus",
    "shopRedLuck2": "Dice of Asmodeus",
    "shopRedLuck3": "High Class Dice of Asmodeus",
    "shopCashGrabUltra": "It's the FINAL CASHGRAB!",
    "shopAmbrosiaAccelerator": "An Ambrosial Accelerator!",
    "shopEXUltra": "It's the FINAL E X!",
    "shopChronometerS": "The FINAL Chronometer",
    "shopAmbrosiaUltra": "The FINAL Ambrosia Exaltation... I don't flippin know!",
    "shopSingularitySpeedup": "Singularity Timed-Perks Speedup",
    "shopSingularityPotency": "Singularity Passives Potency",
    "shopSadisticRune": "Sadistic Rune Unlock! Or does it?",
    "shopInfiniteShopUpgrades": "Blue Infinity Shop Voucher"
}`;var ke=class extends B{#e;#t;async applyPatch(){let e=this;try{this.#e=JSON.parse(nt);let t=await S.HookElement("#actualShop");t&&(this.#t||(this.#t=a=>{e.#a(a)}),t.removeDelegateEventListener("mouseover","div",this.#t),t.delegateEventListener("mouseover","div",this.#t))}catch{c.error("Failed to parse shop item map"),this.#e=void 0}}#a(e){let t;if(e.target instanceof HTMLDivElement)t=e.target;else if(e.target instanceof HTMLImageElement||e.target instanceof HTMLParagraphElement||e.target instanceof HTMLButtonElement)t=e.target.parentElement;else return;if(t.id==="actualShop")return;let a=t.querySelector("img");if(a){let o=a.id,i=this.#e?.[o],n=document.querySelector("#shopHovers"),s=n.querySelector("#hs-shopHoverName");if(s)s.innerText=i||"";else{let l=document.createElement("div");l.id="hs-shopHoverName",l.innerText=i||"",n.insertBefore(l,n.querySelector("p:nth-child(2)"))}}}async revertPatch(){if(this.#t){let t=await S.HookElement("#actualShop");t&&t.removeDelegateEventListener("mouseover","div",this.#t)}let e=document.querySelector("#hs-shopHoverName");e&&e.remove()}};var Te=class extends B{async applyPatch(){let e=await S.HookElement("#buildingstab");e.style.color="red"}async revertPatch(){let e=await S.HookElement("#buildingstab");e.style.color=""}};var Ce=class extends C{#e={AmbrosiaViewOverflow:we,TestPatch:Te,ShopItemNameMapping:ke,IconSetCaching:Ie};#t={};constructor(e){super(e)}async init(){c.log("Initializing HSPatches module",this.context),this.isInitialized=!0}async applyPatch(e){let t=this.#e[e];if(!t){c.warn(`Patch "${e}" not found`,this.context);return}let a;this.#t[e]?(c.debug(`Patch "${e}" is already instantiated`,this.context),a=this.#t[e]):(a=new t(e),this.#t[e]=a),await a.applyPatch()}async revertPatch(e){let t=this.#e[e];if(!t){c.warn(`Patch "${e}" not found`,this.context);return}let a;this.#t[e]?a=this.#t[e]:(a=new t(e),this.#t[e]=a),await a.revertPatch()}};var He=class extends C{#e="Synergysave2";#t;#a;#n=!1;#i;#o;#s=`
        #savegame {
            display: none !important;
        }

        #saveinfo {
            display: none !important;
        }
    `;#r;#l;#u=new Map;#d;#m;#g;#b;#f;#M=!1;#p;#v;#h;#c;#S;#A;#y;#E={tokens:0,maxTokens:0,isAtMaxTokens:!1};#w={HAPPY_HOUR_BELL:{amount:0,ends:[],displayName:"Happy Hour Bell"}};#C=!1;#T;#H;#B=0;#N=5e3;constructor(e){super(e),this.#A=document.querySelector("#campaignTokenCount"),this.#H=new Event("click")}async init(){let e=this;c.log("Initializing HSGameData module",this.context),this.#d=document.querySelector("#singularitybtn"),this.#g=Array.from(document.querySelectorAll("#singularityChallenges > div.singularityChallenges > div")),this.#m=document.querySelector("#importFileButton");try{let a=await(await fetch(A.Common.pseudoAPIurl)).json();this.#p=a,this.#Q()}catch{c.error("Could not fetch pseudo data",this.context)}try{let a=await(await fetch(A.Common.meAPIurl)).json();this.#v=a,this.#O()}catch{c.error("Could not fetch me data",this.context)}this.#h=k.getModule("HSGameDataAPI"),this.#U(),this.isInitialized=!0}async forceUpdateAllData(){let e=Date.now();if(e-this.#B<this.#N){c.warn("Forced data refresh on cooldown",this.context);return}this.#B=e,await this.#k(),await this.#Y(),await this.#k(),this.#L();let t=await S.HookElement("#savegame");t&&t.dispatchEvent(this.#H),this.#T&&(this.#a=JSON.parse(this.#T)),this.#Q(),this.#O(),this.#J(),this.#P()}#U(){let e=this,t=k.getModule("HSWebSocket");t&&t.registerWebSocket("consumable-event-socket",{url:A.Common.eventAPIUrl,onMessage:async a=>{if(a?.type==="info-all")if(e.#I(),a.active&&a.active.length>0){c.debug(`Caught WS event: ${a.type} - event count: ${a.active.length}}`,"WebSocket");for(let{internalName:o,endsAt:i,name:n}of a.active){let s=e.#w[o];s.ends.push(i),s.amount++,s.displayName=n}e.#G()}else c.debug("Caught INFO_ALL, but no active events",this.context);else if(a?.type==="error")c.debug("Caught ERROR",this.context),e.#I();else if(a?.type==="consumable-ended"){c.debug("Caught EVENT_ENDED",this.context);let o=e.#w[a?.consumable];o.ends.shift(),o.amount--,e.#G()}else a?.type==="join"&&c.debug("Caught JOIN (connection established)",this.context)},onRetriesFailed:async()=>{e.#I(),e.#G()}})}#I(){for(let e of Object.keys(this.#w))this.#w[e]={amount:0,ends:[],displayName:""};this.#G()}async#k(){c.debug("Refreshing fetched data",this.context);try{let t=await(await fetch("https://synergism.cc/stripe/upgrades")).json();this.#p=t,this.#Q()}catch{c.error("Could not fetch pseudo data",this.context)}try{let t=await(await fetch("https://synergism.cc/api/v1/users/me")).json();this.#v=t,this.#O()}catch{c.error("Could not fetch me data",this.context)}}#R(){if(!this.#n)return;let e=localStorage.getItem(this.#e);if(e&&e!==this.#S){this.#S=e;try{this.#a=JSON.parse(atob(e)),this.#P()}catch(t){c.debug(`<red>Error processing save data:</red> ${t}`,this.context),this.#x()}}requestAnimationFrame(this.#R.bind(this))}#D(){if(this.#n){if(this.#T)try{this.#a=JSON.parse(this.#T),this.#P()}catch(e){c.debug(`<red>Error processing save data:</red> ${e}`,this.context),this.#x()}requestAnimationFrame(this.#D.bind(this))}}#x(){if(!this.#t)return;let e=v.getSetting("useGameData"),t=v.getSetting("stopSniffOnError");e&&t?t.isEnabled()&&(c.debug("Stopped game data sniffing on error",this.context),e.disable()):c.debug(`maybeStopSniffOnError() - Issue with fetching settings: ${e}, ${t}`,this.context)}async enableGDS(){let e=this;this.#n||(f.injectStyle(this.#s,A.HSGameData.turboCSSId),this.#r&&clearInterval(this.#r),await this.#k(),this.#c&&clearInterval(this.#c),this.#c=setInterval(()=>{e.#k()},A.HSGameData.fetchedDataRefreshInterval),this.#Y(),this.#y&&clearInterval(this.#y),this.#E.isAtMaxTokens||(this.#y=setInterval(()=>{e.#Y()},A.HSGameData.campaignTokenRefreshInterval)),this.#i||(this.#i=await S.HookElement("#savegame")),this.#o||(this.#o=await S.HookElement("#saveinfo")),this.#r=setInterval(()=>{this.#i&&this.#o&&this.#H&&this.#i.dispatchEvent(this.#H)},A.HSGameData.turboModeSpeedMs),this.#d||(this.#d=await S.HookElement("#singularitybtn")),this.#g||(this.#g=Array.from(document.querySelectorAll("#singularityChallenges > div.singularityChallenges > div"))),this.#m||(this.#m=await S.HookElement("#importFileButton")),this.#b||(this.#b=async t=>{e.#F(t)}),this.#f||(this.#f=async t=>{e.#z(t)}),this.#d.addEventListener("click",this.#b,{capture:!0}),this.#g.forEach(t=>{t.addEventListener("click",e.#b,{capture:!0})}),this.#m.addEventListener("click",this.#f,{capture:!0}),c.info("GDS = ON",this.context),this.#n=!0,A.Common.experimentalGDS?(this.#L(),this.#D()):this.#R())}#L(){if(this.#C)return;let e=this,t=window.btoa;window.btoa=function(a){return a&&a.length>0&&a[0]==="{"&&(e.#T=a),t(a)},this.#C=!0}async#z(e){let t=v.getSetting("useGameData");t&&t.isEnabled()&&(t.disable(),await f.Notify("GDS has been disabled for save file import",{position:"top",notificationType:"warning"}))}async#F(e){let t=e.target,a=["noSingularityUpgrades","oneChallengeCap","limitedAscensions","noOcteracts","noAmbrosiaUpgrades","limitedTime","sadisticPrequel"];if(t){let o,i=t.getAttribute("style");if(t.id&&a.includes(t.id)?i?.includes("orchid")?o=!0:a.map(s=>document.querySelector(`#${s}`)?.getAttribute("style")?.includes("orchid")).some((s=>s===!0))?o=!1:o=!0:i?.toLowerCase().includes("grayscale")?o=!1:o=!0,o){let n=v.getSetting("useGameData");if(n&&n.isEnabled()){this.#M=!0,n.disable(),await h.wait(4e3);let s=v.getSetting("useGameData");s&&this.#M&&!s.isEnabled()?(c.debug("Re-enabled GDS",this.context),s.enable()):c.debug("GDS was already enabled (WoW fast!)",this.context),this.#M=!1}}}}async disableGDS(){let e=this;this.#r&&(clearInterval(this.#r),this.#r=void 0),this.#c&&clearInterval(this.#c),this.#y&&clearInterval(this.#y),f.removeInjectedStyle(A.HSGameData.turboCSSId),this.#d||(this.#d=await S.HookElement("#singularitybtn")),this.#g||(this.#g=await S.HookElements("#singularityChallenges > div.singularityChallenges > div")),this.#m||(this.#m=await S.HookElement("#importFileButton")),this.#b&&(this.#d.removeEventListener("click",this.#b,{capture:!0}),this.#g.forEach(t=>{t.removeEventListener("click",e.#b,{capture:!0})}),this.#b=void 0),this.#f&&(this.#m.removeEventListener("click",this.#f,{capture:!0}),this.#f=void 0),c.info("GDS turbo = OFF",this.context),this.#n=!1}subscribeGameDataChange(e){let t=h.uuidv4();return this.#u.set(t,e),t}unsubscribeGameDataChange(e){this.#u.has(e)?this.#u.delete(e):c.warn(`Could not unsubscribe from game data change. ID ${e} not found`,this.context)}#P(){this.#h&&this.#a&&this.#h._updateGameData(this.#a),this.#u.forEach(e=>{this.#a?e(this.#a):c.debug("Could not call game data change callback. No save data found",this.context)})}#Q(){this.#h&&this.#p&&this.#h._updatePseudoData(this.#p)}#O(){this.#h&&this.#v&&this.#h._updateMeData(this.#v)}#J(){this.#h&&this.#E&&this.#h._updateCampaignData(this.#E)}#G(){this.#h&&this.#w&&this.#h._updateEventData(this.#w)}#Y(){if(c.debug("Refreshing campaign data",this.context),!this.#A){let t=document.querySelector("#campaignTokenCount");if(t)this.#A=t;else return}let e=this.#A;if(e){let t=e.innerText.match(/^You have (\d+) \/ (\d+) .+$/);if(t&&t[1]&&t[2]){let a=parseInt(t[1],10),o=parseInt(t[2],10);this.#E.tokens=a,this.#E.maxTokens=o,this.#E.isAtMaxTokens=a>0&&o>0&&a===o,this.#J()}}this.#E.isAtMaxTokens&&this.#y&&(c.debug("Dynamic clear of campaign token refresh interval, player is at max",this.context),clearInterval(this.#y))}};var st=[{calculationName:"Consumable Event Buff",fnName:"R_calculateConsumableEventBuff",fnParams:[{paramName:"buff",paramType:"EventBuffType"}],supportsReduce:!1,toolingSupport:!1},{calculationName:"Ambrosia Generation Shop Upgrade",fnName:"R_calculateAmbrosiaGenerationShopUpgrade",fnParams:[{paramName:"reduce_vals",paramType:"boolean",defaultValue:!0}],supportsReduce:!0,toolingSupport:!0},{calculationName:"Ambrosia Generation Singularity Upgrade",fnName:"R_calculateAmbrosiaGenerationSingularityUpgrade",fnParams:[{paramName:"reduce_vals",paramType:"boolean",defaultValue:!0}],supportsReduce:!0,toolingSupport:!0},{calculationName:"Ambrosia Generation Octeract Upgrade",fnName:"R_calculateAmbrosiaGenerationOcteractUpgrade",fnParams:[{paramName:"reduce_vals",paramType:"boolean",defaultValue:!0}],supportsReduce:!0,toolingSupport:!0},{calculationName:"Singularity Milestone Blueberries",fnName:"R_calculateSingularityMilestoneBlueberries",fnParams:[],supportsReduce:!1,toolingSupport:!0},{calculationName:"Dilated Five Leaf Bonus",fnName:"R_calculateDilatedFiveLeafBonus",fnParams:[],supportsReduce:!1,toolingSupport:!0},{calculationName:"Singularity Ambrosia Luck Milestone Bonus",fnName:"R_calculateSingularityAmbrosiaLuckMilestoneBonus",fnParams:[],supportsReduce:!1,toolingSupport:!0},{calculationName:"Ambrosia Luck Shop Upgrade",fnName:"R_calculateAmbrosiaLuckShopUpgrade",fnParams:[{paramName:"reduce_vals",paramType:"boolean",defaultValue:!0}],supportsReduce:!0,toolingSupport:!0},{calculationName:"Ambrosia Luck Singularity Upgrade",fnName:"R_calculateAmbrosiaLuckSingularityUpgrade",fnParams:[{paramName:"reduce_vals",paramType:"boolean",defaultValue:!0}],supportsReduce:!0,toolingSupport:!0},{calculationName:"Ambrosia Luck Octeract Upgrade",fnName:"R_calculateAmbrosiaLuckOcteractUpgrade",fnParams:[{paramName:"reduce_vals",paramType:"boolean",defaultValue:!0}],supportsReduce:!0,toolingSupport:!0},{calculationName:"Total Cubes",fnName:"R_calculateTotalCubes",fnParams:[],supportsReduce:!1,toolingSupport:!0},{calculationName:"Red Ambrosia Upgrade Value",fnName:"R_calculateRedAmbrosiaUpgradeValue",fnParams:[{paramName:"upgradeName",paramType:"keyof RedAmbrosiaUpgrades"}],supportsReduce:!1,toolingSupport:!1},{calculationName:"Campaign Ambrosia Speed Bonus",fnName:"R_calculateCampaignAmbrosiaSpeedBonus",fnParams:[],supportsReduce:!1,toolingSupport:!0},{calculationName:"Campaign Luck Bonus",fnName:"R_calculateCampaignLuckBonus",fnParams:[],supportsReduce:!1,toolingSupport:!0},{calculationName:"Cookie Upgrade 29 Luck",fnName:"R_calculateCookieUpgrade29Luck",fnParams:[],supportsReduce:!1,toolingSupport:!0},{calculationName:"Sum Of Exalt Completions",fnName:"R_calculateSumOfExaltCompletions",fnParams:[],supportsReduce:!1,toolingSupport:!0},{calculationName:"Number Of Thresholds",fnName:"R_calculateNumberOfThresholds",fnParams:[],supportsReduce:!1,toolingSupport:!0},{calculationName:"To Next Threshold",fnName:"R_calculateToNextThreshold",fnParams:[],supportsReduce:!1,toolingSupport:!0},{calculationName:"Required Blueberry Time",fnName:"R_calculateRequiredBlueberryTime",fnParams:[],supportsReduce:!1,toolingSupport:!0},{calculationName:"Required Red Ambrosia Time",fnName:"R_calculateRequiredRedAmbrosiaTime",fnParams:[],supportsReduce:!1,toolingSupport:!0},{calculationName:"Corruption Total Level",fnName:"getCorruptionTotalLevel",fnParams:[],supportsReduce:!1,toolingSupport:!0},{calculationName:"Hepteract Effective",fnName:"R_calculateHepteractEffective",fnParams:[{paramName:"heptType",paramType:"HepteractType"}],supportsReduce:!1,toolingSupport:!1},{calculationName:"Free Shop Infinity Upgrades",fnName:"R_calculateFreeShopInfinityUpgrades",fnParams:[{paramName:"reduce_vals",paramType:"boolean",defaultValue:!0}],supportsReduce:!0,toolingSupport:!0},{calculationName:"All Shop Tablets",fnName:"R_calculateAllShopTablets",fnParams:[{paramName:"reduce_vals",paramType:"boolean",defaultValue:!0}],supportsReduce:!0,toolingSupport:!0},{calculationName:"Limited Ascensions Debuff",fnName:"R_calculateLimitedAscensionsDebuff",fnParams:[],supportsReduce:!1,toolingSupport:!0},{calculationName:"Singularity Reductions",fnName:"R_calculateSingularityReductions",fnParams:[{paramName:"reduce_vals",paramType:"boolean",defaultValue:!0}],supportsReduce:!0,toolingSupport:!0},{calculationName:"Effective Singularities",fnName:"R_calculateEffectiveSingularities",fnParams:[{paramName:"singularityCount",paramType:"number",defaultValue:-1}],supportsReduce:!1,toolingSupport:!0},{calculationName:"Singularity Debuff",fnName:"R_calculateSingularityDebuff",fnParams:[{paramName:"debuff",paramType:"SingularityDebuffs"},{paramName:"singularityCount",paramType:"number",defaultValue:-1}],supportsReduce:!1,toolingSupport:!1},{calculationName:"Ascension Speed Exponent Spread",fnName:"R_calculateAscensionSpeedExponentSpread",fnParams:[{paramName:"reduce_vals",paramType:"boolean",defaultValue:!0}],supportsReduce:!0,toolingSupport:!0},{calculationName:"Challenge 15 Reward",fnName:"calculateChallenge15Reward",fnParams:[{paramName:"rewardName",paramType:"keyof typeof challenge15Rewards"}],supportsReduce:!1,toolingSupport:!1},{calculationName:"Raw Ascension Speed Mult",fnName:"R_calculateRawAscensionSpeedMult",fnParams:[{paramName:"reduce_vals",paramType:"boolean",defaultValue:!0}],supportsReduce:!0,toolingSupport:!0},{calculationName:"Ascension Speed Mult",fnName:"R_calculateAscensionSpeedMult",fnParams:[],supportsReduce:!1,toolingSupport:!0},{calculationName:"Ambrosia Speed",fnName:"calculateAmbrosiaSpeed",fnParams:[{paramName:"reduce_vals",paramType:"boolean",defaultValue:!0}],supportsReduce:!0,toolingSupport:!0},{calculationName:"Blueberries",fnName:"calculateBlueBerries",fnParams:[{paramName:"reduce_vals",paramType:"boolean",defaultValue:!0}],supportsReduce:!0,toolingSupport:!0},{calculationName:"Luck",fnName:"calculateLuck",fnParams:[{paramName:"reduce_vals",paramType:"boolean",defaultValue:!0},{paramName:"true_base",paramType:"boolean",defaultValue:!1}],supportsReduce:!0,toolingSupport:!0},{calculationName:"Luck Conversion",fnName:"R_calculateLuckConversion",fnParams:[{paramName:"reduce_vals",paramType:"boolean",defaultValue:!0}],supportsReduce:!0,toolingSupport:!0},{calculationName:"Red Ambrosia Luck",fnName:"R_calculateRedAmbrosiaLuck",fnParams:[{paramName:"reduce_vals",paramType:"boolean",defaultValue:!0}],supportsReduce:!0,toolingSupport:!0},{calculationName:"Golden Revolution",fnName:"calculateGoldenRevolution",fnParams:[],supportsReduce:!1,toolingSupport:!0},{calculationName:"Dump Data For Heater",fnName:"dumpDataForHeater",fnParams:[],supportsReduce:!1,toolingSupport:!1}];var De=class extends C{constructor(t){super(t);this.isEvent=!1}static{this.Calculations=st}async init(){let t=this;c.log("Initializing HSGameDataAPI module",this.context),this.isInitialized=!0}_updateGameData(t){this.gameData=t}_updateMeData(t){this.meData=t}_updatePseudoData(t){this.pseudoData=t}_updateCampaignData(t){this.campaignData=t}_updateEventData(t){this.eventData=t,this.eventData&&"HAPPY_HOUR_BELL"in this.eventData&&(this.isEvent=this.eventData.HAPPY_HOUR_BELL.amount>0)}getCampaignData(){return this.campaignData}getGameData(){return this.gameData}getMeData(){return this.meData?this.meData:{bonus:{quarkBonus:0},globalBonus:0,personalBonus:0}}getPseudoData(){return this.pseudoData}getEventData(){return this.eventData}static getCalculationDefinitions(t){let a=s=>{if(s)switch(s){case"true":return l=>l;case"false":return l=>!l;case"both":return(...l)=>!0}return(...l)=>!0},o=a(t?.supportsReduce),i=a(t?.toolingSupport);return this.Calculations.filter(s=>o(s.supportsReduce)&&i(s.toolingSupport))}};var rt=.15;var lt={blueberryGenerationSpeed:{costPerLevel:1,maxLevel:100,costFunction:(r,e)=>e*(r+1),levelFunction:r=>1+r/500},blueberryGenerationSpeed2:{costPerLevel:2e3,maxLevel:500,costFunction:(r,e)=>e+0*r,levelFunction:r=>1+r/1e3},freeLevelsRow2:{costPerLevel:10,maxLevel:5,costFunction:(r,e)=>e*Math.pow(2,r),levelFunction:r=>r},freeLevelsRow3:{costPerLevel:250,maxLevel:5,costFunction:(r,e)=>e*Math.pow(2,r),levelFunction:r=>r},freeLevelsRow4:{costPerLevel:5e3,maxLevel:5,costFunction:(r,e)=>e*Math.pow(2,r),levelFunction:r=>r},freeLevelsRow5:{costPerLevel:5e4,maxLevel:5,costFunction:(r,e)=>e*Math.pow(2,r),levelFunction:r=>r},regularLuck:{costPerLevel:1,maxLevel:100,costFunction:(r,e)=>e*(r+1),levelFunction:r=>2*r},regularLuck2:{costPerLevel:2e3,maxLevel:500,costFunction:(r,e)=>e+0*r,levelFunction:r=>2*r},viscount:{costPerLevel:99999,maxLevel:1,costFunction:(r,e)=>e*(r+1),levelFunction:r=>125*r},tutorial:{costFunction:(r,e)=>e+0*r,levelFunction:r=>Math.pow(1.01,r),maxLevel:100,costPerLevel:1},conversionImprovement1:{costFunction:(r,e)=>e*Math.pow(2,r),levelFunction:r=>-r,maxLevel:5,costPerLevel:5},conversionImprovement2:{costFunction:(r,e)=>e*Math.pow(4,r),levelFunction:r=>-r,maxLevel:3,costPerLevel:200},conversionImprovement3:{costFunction:(r,e)=>e*Math.pow(10,r),levelFunction:r=>-r,maxLevel:2,costPerLevel:1e4},freeTutorialLevels:{costFunction:(r,e)=>e+r,levelFunction:r=>r,maxLevel:5,costPerLevel:1},redGenerationSpeed:{costFunction:(r,e)=>e*(r+1),levelFunction:r=>1+3*r/1e3,maxLevel:100,costPerLevel:12},redLuck:{costFunction:(r,e)=>e*(r+1),levelFunction:r=>r,maxLevel:100,costPerLevel:4},redAmbrosiaCube:{costFunction:(r,e)=>e*(r+1),levelFunction:r=>r,maxLevel:1,costPerLevel:500},redAmbrosiaObtainium:{costFunction:(r,e)=>e*(r+1),levelFunction:r=>r,maxLevel:1,costPerLevel:1250},redAmbrosiaOffering:{costFunction:(r,e)=>e*(r+1),levelFunction:r=>r,maxLevel:1,costPerLevel:4e3},redAmbrosiaCubeImprover:{costFunction:(r,e)=>e*(r+1),levelFunction:r=>.01*r,maxLevel:20,costPerLevel:100},infiniteShopUpgrades:{costFunction:(r,e)=>e+100*r,levelFunction:r=>r,maxLevel:40,costPerLevel:200},redAmbrosiaAccelerator:{costFunction:(r,e)=>e+r*0,levelFunction:r=>.02*r+(r>0?1:0),maxLevel:100,costPerLevel:1e3}},ct={chronos:{LIMIT:1e3,DR:1/6},hyperrealism:{LIMIT:1e3,DR:.33},quark:{LIMIT:1e3,DR:.5},challenge:{LIMIT:1e3,DR:1/6},abyss:{LIMIT:1,DR:0},accelerator:{LIMIT:1e3,DR:.2},acceleratorBoost:{LIMIT:1e3,DR:.2},multiplier:{LIMIT:1e3,DR:.2}};var Pt=(r,e,t)=>1+(r-1)*(1-Math.pow(2,-e/t)),ut={cube1:r=>1+1/50*Math.log2(r/175),ascensions:r=>1+1/20*Math.log2(r/375),coinExponent:r=>1+1/150*Math.log2(r/750),taxes:r=>Math.pow(.98,Math.log(r/1250)/Math.log(2)),obtainium:r=>1+1/4*Math.pow(r/7500,.6),offering:r=>1+1/4*Math.pow(r/7500,.8),accelerator:r=>1+1/20*Math.log(r/2500)/Math.log(2),multiplier:r=>1+1/20*Math.log(r/2500)/Math.log(2),runeExp:r=>1+Math.pow(r/2e4,1.5),runeBonus:r=>1+1/33*Math.log(r/1e4)/Math.log(2),cube2:r=>1+1/100*Math.log(r/15e3)/Math.log(2),transcendChallengeReduction:r=>Math.pow(.98,Math.log(r/25e3)/Math.log(2)),reincarnationChallengeReduction:r=>Math.pow(.98,Math.log(r/25e3)/Math.log(2)),antSpeed:r=>Math.pow(1+Math.log(r/2e5)/Math.log(2),4),bonusAntLevel:r=>1+1/20*Math.log(r/15e4)/Math.log(2),cube3:r=>1+1/150*Math.log(r/25e4)/Math.log(2),talismanBonus:r=>1+1/20*Math.log(r/75e4)/Math.log(2),globalSpeed:r=>1+1/20*Math.log(r/25e5)/Math.log(2),blessingBonus:r=>1+1/5*Math.pow(r/3e7,1/4),constantBonus:r=>1+1/5*Math.pow(r/1e8,2/3),cube4:r=>1+1/200*Math.log(r/125e6)/Math.log(2),spiritBonus:r=>1+1/5*Math.pow(r/2e9,1/4),score:r=>r>=1e20?1+1/4*Math.pow(r/1e10,1/8)*Math.pow(1e10,1/8):1+1/4*Math.pow(r/1e10,1/4),quarks:r=>1+3/400*Math.log2(r*32/1e11),hepteractsUnlocked:r=>r>=1e15?1:0,challengeHepteractUnlocked:r=>r>=2e15?1:0,cube5:r=>1+1/300*Math.log2(r/(4e15/1024)),powder:r=>1+1/50*Math.log2(r/(7e15/32)),abyssHepteractUnlocked:r=>r>=1e16?1:0,exponent:r=>Pt(1.05,r,1e18),acceleratorHepteractUnlocked:r=>r>=333e14?1:0,acceleratorBoostHepteractUnlocked:r=>r>=333e14?1:0,multiplierHepteractUnlocked:r=>r>=333e14?1:0,freeOrbs:r=>Math.floor(200*Math.pow(r/2e17,.5)),ascensionSpeed:r=>1+5/100+2*Math.log2(r/15e17)/100};var W=class extends De{constructor(t){super(t);this.#e={R_AmbrosiaGenerationShopUpgrade:{value:void 0,cachedBy:[]},R_AmbrosiaGenerationSingularityUpgrade:{value:void 0,cachedBy:[]},R_AmbrosiaGenerationOcteractUpgrade:{value:void 0,cachedBy:[]},R_SingularityMilestoneBlueberries:{value:void 0,cachedBy:[]},R_DilatedFiveLeafBonus:{value:void 0,cachedBy:[]},R_SingularityAmbrosiaLuckMilestoneBonus:{value:void 0,cachedBy:[]},R_AmbrosiaLuckShopUpgrade:{value:void 0,cachedBy:[]},R_AmbrosiaLuckSingularityUpgrade:{value:void 0,cachedBy:[]},R_AmbrosiaLuckOcteractUpgrade:{value:void 0,cachedBy:[]},R_TotalCubes:{value:void 0,cachedBy:[]},REDAMB_blueberryGenerationSpeed:{value:void 0,cachedBy:[]},REDAMB_blueberryGenerationSpeed2:{value:void 0,cachedBy:[]},REDAMB_freeLevelsRow2:{value:void 0,cachedBy:[]},REDAMB_freeLevelsRow3:{value:void 0,cachedBy:[]},REDAMB_freeLevelsRow4:{value:void 0,cachedBy:[]},REDAMB_freeLevelsRow5:{value:void 0,cachedBy:[]},REDAMB_regularLuck:{value:void 0,cachedBy:[]},REDAMB_regularLuck2:{value:void 0,cachedBy:[]},REDAMB_viscount:{value:void 0,cachedBy:[]},R_CampaignAmbrosiaSpeedBonus:{value:void 0,cachedBy:[]},R_CampaignLuckBonus:{value:void 0,cachedBy:[]},R_CookieUpgrade29Luck:{value:void 0,cachedBy:[]},R_SumOfExaltCompletions:{value:void 0,cachedBy:[]},R_NumberOfThresholds:{value:void 0,cachedBy:[]},R_ToNextThreshold:{value:void 0,cachedBy:[]},R_RequiredBlueberryTime:{value:void 0,cachedBy:[]},R_RequiredRedAmbrosiaTime:{value:void 0,cachedBy:[]},EVENTBUFF_Quark:{value:void 0,cachedBy:[]},EVENTBUFF_GoldenQuark:{value:void 0,cachedBy:[]},EVENTBUFF_Cubes:{value:void 0,cachedBy:[]},EVENTBUFF_PowderConversion:{value:void 0,cachedBy:[]},EVENTBUFF_AscensionSpeed:{value:void 0,cachedBy:[]},EVENTBUFF_GlobalSpeed:{value:void 0,cachedBy:[]},EVENTBUFF_AscensionScore:{value:void 0,cachedBy:[]},EVENTBUFF_AntSacrifice:{value:void 0,cachedBy:[]},EVENTBUFF_Offering:{value:void 0,cachedBy:[]},EVENTBUFF_Obtainium:{value:void 0,cachedBy:[]},EVENTBUFF_Octeract:{value:void 0,cachedBy:[]},EVENTBUFF_BlueberryTime:{value:void 0,cachedBy:[]},EVENTBUFF_AmbrosiaLuck:{value:void 0,cachedBy:[]},EVENTBUFF_OneMind:{value:void 0,cachedBy:[]},R_RawAscensionSpeedMult:{value:void 0,cachedBy:[]},R_HepteractEffective:{value:void 0,cachedBy:[]},R_AllShopTablets:{value:void 0,cachedBy:[]},R_LimitedAscensionsDebuff:{value:void 0,cachedBy:[]},R_SingularityDebuff:{value:void 0,cachedBy:[]},R_SingularityReductions:{value:void 0,cachedBy:[]},R_EffectiveSingularities:{value:void 0,cachedBy:[]},R_AscensionSpeedExponentSpread:{value:void 0,cachedBy:[]},R_RedAmbrosiaLuck:{value:void 0,cachedBy:[]},R_LuckConversion:{value:void 0,cachedBy:[]}};this.#a=lt;this.#n=ct;this.R_calculateNumberOfThresholds=()=>{if(!this.gameData)return 0;let t=this.gameData,a="R_NumberOfThresholds",o=A.HSAmbrosia.R_digitReduction,i=[t.lifetimeAmbrosia],n=this.#i(a,i);if(n)return n;let s=t.lifetimeAmbrosia>0?1+Math.floor(Math.log10(t.lifetimeAmbrosia)):0,u=Math.floor(t.lifetimeAmbrosia/Math.pow(10,s-1))>=3?1:0,d=Math.max(0,2*(s-o)-1+u);return this.#o(a,{value:d,cachedBy:i}),d};this.R_calculateToNextThreshold=()=>{if(!this.gameData)return 0;let t=this.gameData,a="R_ToNextThreshold",o=A.HSAmbrosia.R_digitReduction,i=[t.lifetimeAmbrosia],n=this.#i(a,i);if(n)return n;let s=this.R_calculateNumberOfThresholds(),l;s===0?l=1e4-t.lifetimeAmbrosia:s%2===0?l=Math.pow(10,s/2+o)-t.lifetimeAmbrosia:l=3*Math.pow(10,(s-1)/2+o)-t.lifetimeAmbrosia;let u=l;return this.#o(a,{value:u,cachedBy:i}),u};this.R_calculateRequiredBlueberryTime=()=>{if(!this.gameData)return 0;let t=this.gameData,a="R_RequiredBlueberryTime",o=A.HSAmbrosia.R_TIME_PER_AMBROSIA,i=[t.lifetimeAmbrosia],n=this.#i(a,i);if(n)return n;let s=o;s+=Math.floor(t.lifetimeAmbrosia/500);let l=this.R_calculateNumberOfThresholds(),d=Math.pow(2,l)*s;return this.#o(a,{value:d,cachedBy:i}),d};this.R_calculateRequiredRedAmbrosiaTime=()=>{if(!this.gameData)return 0;let t=this.gameData,a="R_RequiredRedAmbrosiaTime",o=A.HSAmbrosia.R_TIME_PER_RED_AMBROSIA,i=[t.lifetimeRedAmbrosia,t.singularityChallenges.limitedTime.completions],n=this.#i(a,i);if(n)return n;let s=1-.01*t.singularityChallenges.limitedTime.completions,l=o;l+=200*t.lifetimeRedAmbrosia;let u=1e6*+s;l*=+s;let d=Math.min(u,l);return this.#o(a,{value:d,cachedBy:i}),d};this.R_calculateHepteractEffective=t=>{if(!this.gameData)return 0;let a=this.gameData,o="R_HepteractEffective",i=[a.hepteracts[t].BAL,a.platonicUpgrades[19],a.goldenQuarkUpgrades.singQuarkHepteract.level,a.goldenQuarkUpgrades.singQuarkHepteract2.level,a.goldenQuarkUpgrades.singQuarkHepteract3.level,a.shopUpgrades.improveQuarkHept,a.shopUpgrades.improveQuarkHept2,a.shopUpgrades.improveQuarkHept3,a.shopUpgrades.improveQuarkHept4,a.shopUpgrades.improveQuarkHept5],n=this.#i(o,i);if(n)return n;let s=Math.min(a.hepteracts[t].BAL,this.#n[t].LIMIT),l=0;if(t==="chronos"&&(l+=1/750*a.platonicUpgrades[19]),t==="quark"){l+=+a.goldenQuarkUpgrades.singQuarkHepteract.level/100,l+=+a.goldenQuarkUpgrades.singQuarkHepteract2.level/100,l+=+a.goldenQuarkUpgrades.singQuarkHepteract3.level/100,l+=+a.octUpgrades.octeractImprovedQuarkHept.level/100,l+=a.shopUpgrades.improveQuarkHept/100,l+=a.shopUpgrades.improveQuarkHept2/100,l+=a.shopUpgrades.improveQuarkHept3/100,l+=a.shopUpgrades.improveQuarkHept4/100,l+=a.shopUpgrades.improveQuarkHept5/5e3;let u=a.hepteracts[t].BAL,d;return 1e3<u&&u<=1e3*Math.pow(2,10)?d=s*Math.pow(u/1e3,1/2+l):1e3*Math.pow(2,10)<u&&u<=1e3*Math.pow(2,18)?d=s*Math.pow(Math.pow(2,10),1/2+l)*Math.pow(u/(1e3*Math.pow(2,10)),1/4+l/2):1e3*Math.pow(2,18)<u&&u<=1e3*Math.pow(2,44)?d=s*Math.pow(Math.pow(2,10),1/2+l)*Math.pow(Math.pow(2,8),1/4+l/2)*Math.pow(u/(1e3*Math.pow(2,18)),1/6+l/3):1e3*Math.pow(2,44)<u?d=s*Math.pow(Math.pow(2,10),1/2+l)*Math.pow(Math.pow(2,8),1/4+l/2)*Math.pow(Math.pow(2,26),1/6+l/3)*Math.pow(u/(1e3*Math.pow(2,44)),1/12+l/6):d=0,this.#o(o,{value:d,cachedBy:i}),d}return a.hepteracts[t].BAL>this.#n[t].LIMIT&&(s*=Math.pow(a.hepteracts[t].BAL/this.#n[t].LIMIT,this.#n[t].DR+l)),this.#o(o,{value:s,cachedBy:i}),s};this.#t={...this.#e}}#e;#t;#a;#n;#i(t,a){if(!(t in this.#e))return c.debug(`Could not find cache for '${t}'`),!1;let o=this.#e[t];if(o.value===void 0||o.cachedBy.length===0)return A.Debug.calculationCacheDebugMode&&console.log(`Cache missed (reason: null value or empty cache) for ${t} with value ${o.value}`),!1;if(o.cachedBy.length!==a.length)return A.Debug.calculationCacheDebugMode&&console.warn(`Cache missed (reason: cache length mismatch) for ${t} with value ${o.value}`),!1;for(let i=0;i<o.cachedBy.length;i++)if(!a.includes(o.cachedBy[i]))return A.Debug.calculationCacheDebugMode&&console.log(`Cache missed (reason: calc var mismatch) for ${t} (${o.cachedBy[i]})`),!1;return A.Debug.calculationCacheDebugMode&&console.log(`Hit cache for ${t} with value ${o.value}`),o.value}#o(t,a){if(a.cachedBy.length===0||a.value===null||a.value===void 0){A.Debug.calculationCacheDebugMode&&console.warn(`Rejected cache update for ${t} (value: ${a.value}, cachedBy: ${a.cachedBy.length})`);return}this.#e[t]=a}clearCache(){this.#e={...this.#t}}dumpCache(){console.table(this.#e)}#s(t,a,o,i,n){let s=0,l=i(s,a);for(;t>=l&&(t-=l,s+=1,l=i(s,a),!(s>=o)););return n(s)}R_calculateConsumableEventBuff(t){if(!this.eventData)return 0;let a=this.eventData,o=`EVENTBUFF_${h.eventBuffNumToName(t)}`,i=[a.HAPPY_HOUR_BELL.amount,t],n=this.#i(o,i);if(n)return n;let{HAPPY_HOUR_BELL:s}=this.eventData,l=s.amount-1;if(s.amount===0)return this.#o(o,{value:0,cachedBy:i}),0;let u=0;switch(t){case 0:u=s?.25+.025*l:0;break;case 1:u=0;break;case 2:u=s?.5+.05*l:0;break;case 3:u=0;break;case 4:u=0;break;case 5:u=0;break;case 6:u=0;break;case 7:u=0;break;case 8:u=s?.5+.05*l:0;break;case 9:u=s?.5+.05*l:0;break;case 10:u=0;break;case 13:u=0;break;case 11:u=s?.1+.01*l:0;break;case 12:u=s?.1+.01*l:0;break}return this.#o(o,{value:u,cachedBy:i}),u}R_calculateAmbrosiaGenerationShopUpgrade(t=!0){if(!this.gameData)return 0;let a=this.gameData,o="R_AmbrosiaGenerationShopUpgrade",i=[a.shopUpgrades.shopAmbrosiaGeneration1,a.shopUpgrades.shopAmbrosiaGeneration2,a.shopUpgrades.shopAmbrosiaGeneration3,a.shopUpgrades.shopAmbrosiaGeneration4],n=this.#i(o,i);if(n)return n;let s=[1+a.shopUpgrades.shopAmbrosiaGeneration1/100,1+a.shopUpgrades.shopAmbrosiaGeneration2/100,1+a.shopUpgrades.shopAmbrosiaGeneration3/100,1+a.shopUpgrades.shopAmbrosiaGeneration4/1e3],l=s.reduce((u,d)=>u*d);return this.#o(o,{value:l,cachedBy:i}),t?l:s}R_calculateAmbrosiaGenerationSingularityUpgrade(t=!0){if(!this.gameData)return 0;let a=this.gameData,o="R_AmbrosiaGenerationSingularityUpgrade",i=[a.goldenQuarkUpgrades.singAmbrosiaGeneration.level,a.goldenQuarkUpgrades.singAmbrosiaGeneration2.level,a.goldenQuarkUpgrades.singAmbrosiaGeneration3.level,a.goldenQuarkUpgrades.singAmbrosiaGeneration4.level],n=this.#i(o,i);if(n)return n;let s=[1+a.goldenQuarkUpgrades.singAmbrosiaGeneration.level/100,1+a.goldenQuarkUpgrades.singAmbrosiaGeneration2.level/100,1+a.goldenQuarkUpgrades.singAmbrosiaGeneration3.level/100,1+2*a.goldenQuarkUpgrades.singAmbrosiaGeneration4.level/100],l=s.reduce((u,d)=>u*d);return this.#o(o,{value:l,cachedBy:i}),t?l:s}R_calculateAmbrosiaGenerationOcteractUpgrade(t=!0){if(!this.gameData)return 0;let a=this.gameData,o="R_AmbrosiaGenerationOcteractUpgrade",i=[a.octUpgrades.octeractAmbrosiaGeneration.level,a.octUpgrades.octeractAmbrosiaGeneration2.level,a.octUpgrades.octeractAmbrosiaGeneration3.level,a.octUpgrades.octeractAmbrosiaGeneration4.level],n=this.#i(o,i);if(n)return n;let s=[1+a.octUpgrades.octeractAmbrosiaGeneration.level/100,1+a.octUpgrades.octeractAmbrosiaGeneration2.level/100,1+a.octUpgrades.octeractAmbrosiaGeneration3.level/100,1+2*a.octUpgrades.octeractAmbrosiaGeneration4.level/100],l=s.reduce((u,d)=>u*d);return this.#o(o,{value:l,cachedBy:i}),t?l:s}R_calculateSingularityMilestoneBlueberries(){if(!this.gameData)return 0;let t=this.gameData,a="R_SingularityMilestoneBlueberries",o=[t.highestSingularityCount],i=this.#i(a,o);if(i)return i;let n=0;t.highestSingularityCount>=270?n=5:t.highestSingularityCount>=256?n=4:t.highestSingularityCount>=192?n=3:t.highestSingularityCount>=128?n=2:t.highestSingularityCount>=64&&(n=1);let s=n;return this.#o(a,{value:s,cachedBy:o}),s}R_calculateDilatedFiveLeafBonus(){if(!this.gameData)return 0;let t=this.gameData,a="R_DilatedFiveLeafBonus",o=[t.highestSingularityCount],i=this.#i(a,o);if(i)return i;let n=[100,150,200,225,250,255,260,265,269,272],s=n.length/100;for(let u=0;u<n.length;u++)if(t.highestSingularityCount<n[u]){s=u/100;break}let l=s;return this.#o(a,{value:l,cachedBy:o}),l}R_calculateSingularityAmbrosiaLuckMilestoneBonus(){if(!this.gameData)return 0;let t=this.gameData,a="R_SingularityAmbrosiaLuckMilestoneBonus",o=[t.highestSingularityCount],i=this.#i(a,o);if(i)return i;let n=0,s=[35,42,49,56,63,70,77],l=[135,142,149,156,163,170,177];for(let d of s)t.highestSingularityCount>=d&&(n+=5);for(let d of l)t.highestSingularityCount>=d&&(n+=6);let u=n;return this.#o(a,{value:u,cachedBy:o}),u}R_calculateAmbrosiaLuckShopUpgrade(t=!0){if(!this.gameData)return 0;let a=this.gameData,o="R_AmbrosiaLuckShopUpgrade",i=[a.shopUpgrades.shopAmbrosiaLuck1,a.shopUpgrades.shopAmbrosiaLuck2,a.shopUpgrades.shopAmbrosiaLuck3,a.shopUpgrades.shopAmbrosiaLuck4],n=this.#i(o,i);if(n)return n;let s=[2*a.shopUpgrades.shopAmbrosiaLuck1,2*a.shopUpgrades.shopAmbrosiaLuck2,2*a.shopUpgrades.shopAmbrosiaLuck3,.6*a.shopUpgrades.shopAmbrosiaLuck4],l=s.reduce((u,d)=>u+d,0);return this.#o(o,{value:l,cachedBy:i}),t?l:s}R_calculateAmbrosiaLuckSingularityUpgrade(t=!0){if(!this.gameData)return 0;let a=this.gameData,o="R_AmbrosiaLuckSingularityUpgrade",i=[a.goldenQuarkUpgrades.singAmbrosiaLuck.level,a.goldenQuarkUpgrades.singAmbrosiaLuck2.level,a.goldenQuarkUpgrades.singAmbrosiaLuck3.level,a.goldenQuarkUpgrades.singAmbrosiaLuck4.level],n=this.#i(o,i);if(n)return n;let s=[+a.goldenQuarkUpgrades.singAmbrosiaLuck.level*4,+a.goldenQuarkUpgrades.singAmbrosiaLuck2.level*2,+a.goldenQuarkUpgrades.singAmbrosiaLuck3.level*3,+a.goldenQuarkUpgrades.singAmbrosiaLuck4.level*5],l=s.reduce((u,d)=>u+d,0);return this.#o(o,{value:l,cachedBy:i}),t?l:s}R_calculateAmbrosiaLuckOcteractUpgrade(t=!0){if(!this.gameData)return 0;let a=this.gameData,o="R_AmbrosiaLuckOcteractUpgrade",i=[a.octUpgrades.octeractAmbrosiaLuck.level,a.octUpgrades.octeractAmbrosiaLuck2.level,a.octUpgrades.octeractAmbrosiaLuck3.level,a.octUpgrades.octeractAmbrosiaLuck4.level],n=this.#i(o,i);if(n)return n;let s=[+a.octUpgrades.octeractAmbrosiaLuck.level*4,+a.octUpgrades.octeractAmbrosiaLuck2.level*2,+a.octUpgrades.octeractAmbrosiaLuck3.level*3,+a.octUpgrades.octeractAmbrosiaLuck4.level*5],l=s.reduce((u,d)=>u+d,0);return this.#o(o,{value:l,cachedBy:i}),t?l:s}R_calculateTotalCubes(){if(!this.gameData)return 0;let t=this.gameData,a="R_TotalCubes",o=[t.wowCubes,t.wowTesseracts,t.wowHypercubes,t.wowPlatonicCubes,t.wowAbyssals,t.wowOcteracts],i=this.#i(a,o);if(i)return i;let n=Math.floor(Math.log10(Number(t.wowCubes)+1))+Math.floor(Math.log10(Number(t.wowTesseracts)+1))+Math.floor(Math.log10(Number(t.wowHypercubes)+1))+Math.floor(Math.log10(Number(t.wowPlatonicCubes)+1))+Math.floor(Math.log10(t.wowAbyssals+1))+Math.floor(Math.log10(t.wowOcteracts+1))+6;return this.#o(a,{value:n,cachedBy:o}),n}R_calculateRedAmbrosiaUpgradeValue(t){if(!this.gameData)return 0;let a=this.gameData,o=`REDAMB_${t}`;if(!(t in a.redAmbrosiaUpgrades)||!(t in this.#a))return 0;let i=[a.redAmbrosiaUpgrades[t]],n=this.#i(o,i);if(n)return n;let s=this.#a[t],u=this.#s(a.redAmbrosiaUpgrades[t],s.costPerLevel,s.maxLevel,s.costFunction,s.levelFunction);return this.#o(o,{value:u,cachedBy:i}),u}R_calculateCampaignAmbrosiaSpeedBonus(){let t="R_CampaignAmbrosiaSpeedBonus",a=this.campaignData?.tokens??0,o=[a],i=this.#i(t,o);if(i)return i;let n;a<2e3?n=1:n=1+.05*1/2e3*Math.min(a-2e3,2e3)+.05*(1-Math.exp(-Math.max(a-4e3,0)/2e3));let s=n;return this.#o(t,{value:s,cachedBy:o}),s}R_calculateCampaignLuckBonus(){let t="R_CampaignLuckBonus",a=this.campaignData?.tokens??0,o=[a],i=this.#i(t,o);if(i)return i;let n;a<2e3?n=0:n=10+40/2e3*Math.min(a-2e3,2e3)+50*(1-Math.exp(-Math.max(a-4e3,0)/2500));let s=n;return this.#o(t,{value:s,cachedBy:o}),s}R_calculateCookieUpgrade29Luck(){if(!this.gameData)return 0;let t=this.gameData,a="R_CookieUpgrade29Luck",i=[t.cubeUpgrades[79]??0,t.lifetimeRedAmbrosia],n=this.#i(a,i);if(n)return n;let s;t.cubeUpgrades[79]===0||t.lifetimeRedAmbrosia===0?s=0:s=10*Math.pow(Math.log10(t.lifetimeRedAmbrosia),2);let l=s;return this.#o(a,{value:l,cachedBy:i}),l}R_calculateSumOfExaltCompletions(){if(!this.gameData)return 0;let t=this.gameData,a="R_SumOfExaltCompletions",o=[...Object.values(t.singularityChallenges).map(l=>l.completions)],i=this.#i(a,o);if(i)return i;let n=0;for(let l of Object.values(t.singularityChallenges))n+=l.completions;let s=n;return this.#o(a,{value:s,cachedBy:o}),s}getCorruptionTotalLevel(){if(!this.gameData)return 0;let a=this.gameData.corruptions.used;return Object.values(a).reduce((i,n)=>i+n,0)}R_calculateFreeShopInfinityUpgrades(t=!0){return this.R_calculateAllShopTablets(t)}R_calculateAllShopTablets(t=!0){if(!this.gameData)return 0;let a=this.gameData,o="R_AllShopTablets",i=[a.highestSingularityCount,a.goldenQuarkUpgrades.singInfiniteShopUpgrades.level,a.octUpgrades.octeractInfiniteShopUpgrades.level,a.shopUpgrades.shopInfiniteShopUpgrades,a.ambrosiaUpgrades.ambrosiaInfiniteShopUpgrades1.freeLevels,a.ambrosiaUpgrades.ambrosiaInfiniteShopUpgrades2.freeLevels],n=this.#i(o,i);if(n)return n;let s=()=>a.highestSingularityCount>=280?Math.floor(.8*(a.highestSingularityCount-200)):a.highestSingularityCount>=250?Math.floor(.5*(a.highestSingularityCount-200)):0,l=[this.R_calculateRedAmbrosiaUpgradeValue("infiniteShopUpgrades"),s(),+a.goldenQuarkUpgrades.singInfiniteShopUpgrades.level,+a.octUpgrades.octeractInfiniteShopUpgrades.level,Math.floor(.005*a.shopUpgrades.shopInfiniteShopUpgrades*this.R_calculateSumOfExaltCompletions()),+a.ambrosiaUpgrades.ambrosiaInfiniteShopUpgrades1.freeLevels,+a.ambrosiaUpgrades.ambrosiaInfiniteShopUpgrades2.freeLevels],u=l.reduce((d,g)=>d+g,0);return this.#o(o,{value:u,cachedBy:i}),t?u:l}R_calculateLimitedAscensionsDebuff(){if(!this.gameData)return 0;let t=this.gameData,a="R_LimitedAscensionsDebuff";if(!t.singularityChallenges.limitedAscensions.enabled)return 1;let o=[t.ascensionCount,t.singularityChallenges.limitedAscensions.completions],i=this.#i(a,o);if(i)return i;let n=t.ascensionCount-Math.max(0,20-t.singularityChallenges.limitedAscensions.completions);n=Math.max(0,n);let s=Math.pow(2,n);return this.#o(a,{value:s,cachedBy:o}),s}R_calculateSingularityReductions(t=!0){if(!this.gameData)return 0;let a=this.gameData,o="R_SingularityReductions",i=[a.insideSingularityChallenge?1:0,a.ambrosiaUpgrades.ambrosiaSingReduction2.level,a.ambrosiaUpgrades.ambrosiaSingReduction1.level,a.shopUpgrades.shopSingularityPenaltyDebuff],n=this.#i(o,i);if(n)return n;let s;a.insideSingularityChallenge?s=a.ambrosiaUpgrades.ambrosiaSingReduction2.level:s=a.ambrosiaUpgrades.ambrosiaSingReduction1.level;let l=[a.shopUpgrades.shopSingularityPenaltyDebuff,s],u=l.reduce((d,g)=>d+g,0);return this.#o(o,{value:u,cachedBy:i}),t?u:l}R_calculateEffectiveSingularities(t=-1){if(!this.gameData)return 0;let a=this.gameData,o="R_EffectiveSingularities",i=[t,a.insideSingularityChallenge?1:0,a.singularityChallenges.noOcteracts.completions],n=this.#i(o,i);if(n)return n;let s=t===-1?a.singularityCount:t;return s*=Math.min(4.75,.75*t/10+1),a.insideSingularityChallenge&&a.singularityChallenges.noOcteracts.enabled&&(s*=Math.pow(a.singularityChallenges.noOcteracts.completions+1,3)),t>10&&(s*=1.5,s*=Math.min(4,1.25*t/10-.25)),t>25&&(s*=2.5,s*=Math.min(6,1.5*t/25-.5)),t>36&&(s*=4,s*=Math.min(5,t/18-1),s*=Math.pow(1.1,Math.min(t-36,64))),t>50&&(s*=5,s*=Math.min(8,2*t/50-1),s*=Math.pow(1.1,Math.min(t-50,50))),t>100&&(s*=2,s*=t/25,s*=Math.pow(1.1,t-100)),t>150&&(s*=2,s*=Math.pow(1.05,t-150)),t>200&&(s*=1.5,s*=Math.pow(1.275,t-200)),t>215&&(s*=1.25,s*=Math.pow(1.2,t-215)),t>230&&(s*=2),t>269&&(s*=3,s*=Math.pow(3,t-269)),this.#o(o,{value:s,cachedBy:i}),s}R_calculateSingularityDebuff(t,a=-1){if(!this.gameData)return 1;let o=this.gameData;if(a===-1&&(a=o.singularityCount),a===0||o.runes.antiquities>0)return 1;let i=a-this.R_calculateSingularityReductions();if(i<1)return 1;let n=this.R_calculateEffectiveSingularities(i),s;if(t==="Offering")s=i<150?Math.sqrt(n)+1:Math.pow(n,2/3)/400;else if(t==="Global Speed")s=1+Math.sqrt(n)/4;else if(t==="Obtainium")s=i<150?Math.sqrt(n)+1:Math.pow(n,2/3)/400;else if(t==="Researches")s=1+Math.sqrt(n)/2;else if(t==="Ascension Speed")s=i<150?1+Math.sqrt(n)/5:1+Math.pow(n,.75)/1e4;else if(t==="Cubes"){let l=i>100?Math.pow(1.02,i-100):1;s=i<150?1+Math.sqrt(n)*l/4:1+Math.pow(n,.75)*l/1e3}else t==="Platonic Costs"?s=i>36?1+Math.pow(n,3/10)/12:1:t==="Hepteract Costs"?s=i>50?1+Math.pow(n,11/50)/25:1:s=Math.cbrt(n+1);return s}R_calculateAscensionSpeedExponentSpread(t=!0){if(!this.gameData)return 0;let a=this.gameData,o="R_AscensionSpeedExponentSpread",i=[a.goldenQuarkUpgrades.singAscensionSpeed.level,a.goldenQuarkUpgrades.singAscensionSpeed2.level,a.shopUpgrades.chronometerInfinity],n=this.#i(o,i);if(n)return n;let s=[a.goldenQuarkUpgrades.singAscensionSpeed.level>0?.03:0,a.goldenQuarkUpgrades.singAscensionSpeed2.level*.001,.001*Math.floor((a.shopUpgrades.chronometerInfinity+this.R_calculateFreeShopInfinityUpgrades())/40)],l=s.reduce((u,d)=>u+d,0);return this.#o(o,{value:l,cachedBy:i}),t?l:s}calculateChallenge15Reward(t){if(!this.gameData)return c.errorOnce("<red>calculateChallenge15Reward() GAMEDATA WAS NULL</red>",this.context),0;let a=this.gameData,o=a.challenge15Exponent?a.challenge15Exponent:a.highestChallenge15Exponent?a.highestChallenge15Exponent:0;return ut[t](o)}R_calculateRawAscensionSpeedMult(t=!0){if(!this.gameData)return 0;let a=this.gameData,o="R_RawAscensionSpeedMult",i=a.cubeUpgrades[59]??0,n=[a.shopUpgrades.chronometer,a.shopUpgrades.chronometer2,a.shopUpgrades.chronometer3,a.achievements[262],a.achievements[263],a.platonicUpgrades[15],a.singularityCount,a.shopUpgrades.chronometerZ,a.octUpgrades.octeractImprovedAscensionSpeed.level,a.octUpgrades.octeractImprovedAscensionSpeed2.level,a.singularityChallenges.limitedAscensions.completions,a.singularityChallenges.limitedTime.completions,a.shopUpgrades.shopChronometerS,i,a.insideSingularityChallenge?1:0],s=this.#i(o,n);if(s)return s;let l=[1+1.2/100*a.shopUpgrades.chronometer,1+.6/100*a.shopUpgrades.chronometer2,1+1.5/100*a.shopUpgrades.chronometer3,1+.6/1e3*this.R_calculateHepteractEffective("chronos"),1+Math.min(.1,1/100*Math.log10(a.ascensionCount+1))*a.achievements[262],1+Math.min(.1,1/100*Math.log10(a.ascensionCount+1))*a.achievements[263],1+.002*this.getCorruptionTotalLevel()*a.platonicUpgrades[15],this.calculateChallenge15Reward("ascensionSpeed"),1+1/400*i,1+.5*(a.goldenQuarkUpgrades.intermediatePack.level>0?1:0),1+1/1e3*a.singularityCount*a.shopUpgrades.chronometerZ,1+ +a.octUpgrades.octeractImprovedAscensionSpeed.level/2e3*a.singularityCount,1+ +a.octUpgrades.octeractImprovedAscensionSpeed2.level/2e3*a.singularityCount,Math.pow(1.006,a.shopUpgrades.chronometerInfinity+this.R_calculateFreeShopInfinityUpgrades()),Math.pow(1+.1*a.singularityChallenges.limitedAscensions.completions/100,1+Math.max(0,Math.floor(Math.log10(a.ascensionCount)))),1+.06*a.singularityChallenges.limitedTime.completions,Math.max(Math.pow(1.01,(a.singularityCount-200)*a.shopUpgrades.shopChronometerS),1),1/this.R_calculateLimitedAscensionsDebuff(),1/this.R_calculateSingularityDebuff("Ascension Speed"),1+this.R_calculateConsumableEventBuff(4)],u=l.reduce((d,g)=>d*g,1);return this.#o(o,{value:u,cachedBy:n}),t?u:l}R_calculateAscensionSpeedMult(){let t=this.R_calculateRawAscensionSpeedMult(),a=this.R_calculateAscensionSpeedExponentSpread();return t<1?t=Math.pow(t,1-a):t=Math.pow(t,1+a),t}R_calculateCashGrabBonus(t){if(!this.gameData)return 0;let a=this.gameData;return 1+a.shopUpgrades.shopCashGrabUltra*t*Math.min(1,Math.pow(a.lifetimeAmbrosia/1e7,1/3))}R_calculateEXUltraBonus(t){if(!this.gameData)return 0;let a=this.gameData;return 1+t*Math.min(a.shopUpgrades.shopEXUltra,Math.floor(a.lifetimeAmbrosia/1e3)/125)}calculateAmbrosiaSpeed(t=!0){if(!this.gameData)return 0;let a=this.gameData;if(!this.pseudoData)return 0;let o=this.pseudoData;if(!this.meData)return 0;let i=this.meData,n=o?.playerUpgrades.find(M=>M.internalName==="AMBROSIA_GENERATION_BUFF")?.level,s=n?1+n*.05:1,l=this.R_calculateCampaignAmbrosiaSpeedBonus(),u=100*(1+i.globalBonus/100)*(1+i.personalBonus/100)-100,d=this.R_calculateRedAmbrosiaUpgradeValue("blueberryGenerationSpeed"),g=this.R_calculateRedAmbrosiaUpgradeValue("blueberryGenerationSpeed2"),m=a.cubeUpgrades[76]??1,p=[+a.visitedAmbrosiaSubtab,s,l,this.R_calculateAmbrosiaGenerationShopUpgrade(),this.R_calculateAmbrosiaGenerationSingularityUpgrade(),this.R_calculateAmbrosiaGenerationOcteractUpgrade(),1+a.ambrosiaUpgrades.ambrosiaPatreon.level*u/100,1+a.singularityChallenges.oneChallengeCap.completions/100,1+a.singularityChallenges.noAmbrosiaUpgrades.completions/50,d,g,1+.01*m*this.R_calculateNumberOfThresholds(),this.R_calculateCashGrabBonus(rt),this.isEvent?1+this.R_calculateConsumableEventBuff(11):1],b=p.reduce((M,E)=>M*E,1);return t?b:p}calculateBlueBerries(t=!0){let a=this.getGameData();if(!a)return 0;let o=0;a.singularityChallenges.noAmbrosiaUpgrades.completions>=10?o=2:a.singularityChallenges.noAmbrosiaUpgrades.completions>0&&(o=1);let i=[+(a.singularityChallenges.noSingularityUpgrades.completions>0),+a.goldenQuarkUpgrades.blueberries.level,+a.octUpgrades.octeractBlueberries.level,this.R_calculateSingularityMilestoneBlueberries(),o],n=i.reduce((s,l)=>s+l,0);return t?n:i}calculateLuck(t=!0,a=!1){let o=this.getGameData(),i=this.getPseudoData();if(!o)return{additive:0,raw:0,total:0};if(!i)return{additive:0,raw:0,total:0};let n=o.cubeUpgrades[77]??0,s=[1,o.singularityChallenges.noSingularityUpgrades.completions>=30?.05:0,this.R_calculateDilatedFiveLeafBonus(),o.shopUpgrades.shopAmbrosiaLuckMultiplier4/100,o.singularityChallenges.noAmbrosiaUpgrades.completions/200,.001*n,this.isEvent?this.R_calculateConsumableEventBuff(12):0],l=i.playerUpgrades.find(G=>G.internalName==="AMBROSIA_LUCK_BUFF")?.level,u=l?l*20:0,d=this.R_calculateCampaignLuckBonus(),g=this.R_calculateRedAmbrosiaUpgradeValue("freeLevelsRow2"),m=this.R_calculateRedAmbrosiaUpgradeValue("freeLevelsRow3"),p=this.R_calculateRedAmbrosiaUpgradeValue("freeLevelsRow4"),b=this.R_calculateRedAmbrosiaUpgradeValue("freeLevelsRow5"),M={2:g,3:m,4:p,5:b},E=(G,O)=>a?M[O]:G+M[O],y=o.ambrosiaUpgrades.ambrosiaLuck1.level,T=o.ambrosiaUpgrades.ambrosiaLuck2.level,H=o.ambrosiaUpgrades.ambrosiaLuck3.level,D=this.R_calculateTotalCubes(),x=o.ambrosiaUpgrades.ambrosiaCubeLuck1.level,J=o.ambrosiaUpgrades.ambrosiaQuarkLuck1.level,R=this.R_calculateRedAmbrosiaUpgradeValue("regularLuck"),L=this.R_calculateRedAmbrosiaUpgradeValue("regularLuck2"),P=this.R_calculateRedAmbrosiaUpgradeValue("viscount"),$=[100,u,d,this.R_calculateSingularityAmbrosiaLuckMilestoneBonus(),this.R_calculateAmbrosiaLuckShopUpgrade(),this.R_calculateAmbrosiaLuckSingularityUpgrade(),this.R_calculateAmbrosiaLuckOcteractUpgrade(),2*E(y,2)+12*Math.floor(E(y,2)/10),(3+.3*Math.floor(E(y,2)/10))*E(T,4)+40*Math.floor(E(T,4)/10),this.calculateBlueBerries()*E(H,5),this.R_calculateTotalCubes()*.02*E(x,3),.02*E(J,3)*Math.floor(Math.pow(Math.log10(Number(o.worlds)+1)+1,2)),o.highestSingularityCount>=131?131:0,o.highestSingularityCount>=269?269:0,o.shopUpgrades.shopOcteractAmbrosiaLuck*(1+Math.floor(Math.log10(o.totalWowOcteracts+1))),o.singularityChallenges.noAmbrosiaUpgrades.completions*15,R,L,P,2*n,this.R_calculateCookieUpgrade29Luck(),o.shopUpgrades.shopAmbrosiaUltra*this.R_calculateSumOfExaltCompletions()],ee=[0],Q=[];if(a?Q=$:Q=[...$,...ee],t){let G=s.reduce((V,j)=>V+j,0),O=Q.reduce((V,j)=>V+j,0);return{additive:G,raw:O,total:G*O}}else return{additive:s,raw:Q}}R_calculateLuckConversion(t=!0){if(!this.gameData)return 0;let a=this.gameData,o="R_LuckConversion",i=this.R_calculateRedAmbrosiaUpgradeValue("conversionImprovement1"),n=this.R_calculateRedAmbrosiaUpgradeValue("conversionImprovement2"),s=this.R_calculateRedAmbrosiaUpgradeValue("conversionImprovement3"),l=[a.shopUpgrades.shopRedLuck1,a.shopUpgrades.shopRedLuck2,a.shopUpgrades.shopRedLuck3,i,n,s],u=this.#i(o,l);if(u)return u;let d=[20,i,n,s,-.01*Math.floor(a.shopUpgrades.shopRedLuck1/20),-.01*Math.floor(a.shopUpgrades.shopRedLuck2/20),-.01*Math.floor(a.shopUpgrades.shopRedLuck3/20)],g=d.reduce((m,p)=>m+p,0);return this.#o(o,{value:g,cachedBy:l}),t?g:d}R_calculateRedAmbrosiaLuck(t=!0){if(!this.gameData||!this.pseudoData)return 0;let a=this.gameData,o=this.pseudoData,i="R_RedAmbrosiaLuck",n=o.playerUpgrades.find(b=>b.internalName==="RED_LUCK_BUFF")?.level??0,s=this.calculateLuck(),l=this.R_calculateRedAmbrosiaUpgradeValue("redLuck"),u=this.R_calculateRedAmbrosiaUpgradeValue("viscount"),d=[n,s.total,l,u,a.singularityChallenges.noAmbrosiaUpgrades.completions,a.shopUpgrades.shopRedLuck1,a.shopUpgrades.shopRedLuck2,a.shopUpgrades.shopRedLuck3],g=this.#i(i,d);if(g)return g;let m=[100,n,Math.floor((s.total-100)/this.R_calculateLuckConversion()),l,a.singularityChallenges.noAmbrosiaUpgrades.completions*4,a.shopUpgrades.shopRedLuck1*.05,a.shopUpgrades.shopRedLuck2*.075,a.shopUpgrades.shopRedLuck3*.1,u],p=m.reduce((b,M)=>b+M,0);return this.#o(i,{value:p,cachedBy:d}),t?p:m}calculateGoldenRevolution(){if(!this.gameData)return 0;let t=this.gameData,a=t.highestSingularityCount>=100?1-.5*t.highestSingularityCount/250:1}async dumpDataForHeater(){let t=k.getModule("HSGameData");if(t)await t.forceUpdateAllData();else{c.error("Failed to acquire game data for heater export",this.context),f.Notify("Failed to acquire game data for heater export",{position:"top",notificationType:"error"});return}if(!this.gameData)return 0;let a=this.gameData;try{let{additive:o,raw:i,total:n}=this.calculateLuck(!0),s=this.calculateLuck(!0,!0),l=(n-100*Math.floor(n/100))/100,u=(s.total-100*Math.floor(s.total/100))/100,d=this.calculateBlueBerries(),g=this.calculateAmbrosiaSpeed(),m=d*g;return{...this.gameData,hs_data:{lifeTimeAmbrosia:a.lifetimeAmbrosia,lifeTimeRedAmbrosia:a.lifetimeRedAmbrosia,quarks:a.worlds,platonic4x4:a.platonicUpgrades[19],baseLuck:i,luckMult:o,totalLuck:n,trueBaseLuck:s.raw,redAmbrosiaLuck:this.R_calculateRedAmbrosiaLuck(),luckConversion:this.R_calculateLuckConversion(),totalCubes:this.R_calculateTotalCubes(),effectiveSingularity:a.highestSingularityCount,transcription:.55+a.octUpgrades.octeractOneMindImprover.level/150,ascSpeed:this.R_calculateAscensionSpeedMult(),blueberries:d,bonusRow2:this.R_calculateRedAmbrosiaUpgradeValue("freeLevelsRow2"),bonusRow3:this.R_calculateRedAmbrosiaUpgradeValue("freeLevelsRow3"),bonusRow4:this.R_calculateRedAmbrosiaUpgradeValue("freeLevelsRow4"),bonusRow5:this.R_calculateRedAmbrosiaUpgradeValue("freeLevelsRow5"),spread:this.R_calculateAscensionSpeedExponentSpread(),totalInfinityVouchers:this.R_calculateAllShopTablets(),tokens:this.campaignData?.tokens,maxTokens:this.campaignData?.maxTokens,isAtMaxTokens:this.campaignData?.isAtMaxTokens,isEvent:this.isEvent,bellStacks:this.eventData?.HAPPY_HOUR_BELL.amount,personalQuarkBonus:this.meData?.bonus.quarkBonus,blueAmbrosiaBarValue:a.blueberryTime,redAmbrosiaBarValue:a.redAmbrosiaTime,blueAmbrosiaBarMax:this.R_calculateRequiredBlueberryTime(),redAmbrosiaBarMax:this.R_calculateRequiredRedAmbrosiaTime(),ambrosiaSpeedMult:g,ambrosiaSpeed:m,ambrosiaGainChance:l,trueAmbrosiaGainChance:u,ambrosiaAcceleratorCount:a.shopUpgrades.shopAmbrosiaAccelerator,pseudoCoinUpgrades:{ambrosiaGenerationBuffLevel:this.pseudoData?.playerUpgrades.find(b=>b.internalName==="AMBROSIA_GENERATION_BUFF")?.level,ambrosiaLuckBuffLevel:this.pseudoData?.playerUpgrades.find(b=>b.internalName==="AMBROSIA_LUCK_BUFF")?.level,baseObtainiumBuffLevel:this.pseudoData?.playerUpgrades.find(b=>b.internalName==="BASE_OBTAINIUM_BUFF")?.level,baseOfferingBuffLevel:this.pseudoData?.playerUpgrades.find(b=>b.internalName==="BASE_OFFERING_BUFF")?.level,cubeBuffLevel:this.pseudoData?.playerUpgrades.find(b=>b.internalName==="CUBE_BUFF")?.level,redAmbrosiaGenerationBuffLevel:this.pseudoData?.playerUpgrades.find(b=>b.internalName==="RED_GENERATION_BUFF")?.level,redAmbrosiaLuckBuffLevel:this.pseudoData?.playerUpgrades.find(b=>b.internalName==="RED_LUCK_BUFF")?.level},redAmbrosiaUpgrades:{tutorial:this.R_calculateRedAmbrosiaUpgradeValue("tutorial"),conversionImprovement1:this.R_calculateRedAmbrosiaUpgradeValue("conversionImprovement1"),conversionImprovement2:this.R_calculateRedAmbrosiaUpgradeValue("conversionImprovement2"),conversionImprovement3:this.R_calculateRedAmbrosiaUpgradeValue("conversionImprovement3"),freeTutorialLevels:this.R_calculateRedAmbrosiaUpgradeValue("freeTutorialLevels"),freeLevelsRow2:this.R_calculateRedAmbrosiaUpgradeValue("freeLevelsRow2"),freeLevelsRow3:this.R_calculateRedAmbrosiaUpgradeValue("freeLevelsRow3"),freeLevelsRow4:this.R_calculateRedAmbrosiaUpgradeValue("freeLevelsRow4"),freeLevelsRow5:this.R_calculateRedAmbrosiaUpgradeValue("freeLevelsRow5"),blueberryGenerationSpeed:this.R_calculateRedAmbrosiaUpgradeValue("blueberryGenerationSpeed"),blueberryGenerationSpeed2:this.R_calculateRedAmbrosiaUpgradeValue("blueberryGenerationSpeed2"),regularLuck:this.R_calculateRedAmbrosiaUpgradeValue("regularLuck"),regularLuck2:this.R_calculateRedAmbrosiaUpgradeValue("regularLuck2"),redGenerationSpeed:this.R_calculateRedAmbrosiaUpgradeValue("redGenerationSpeed"),redLuck:this.R_calculateRedAmbrosiaUpgradeValue("redLuck"),redAmbrosiaCube:this.R_calculateRedAmbrosiaUpgradeValue("redAmbrosiaCube"),redAmbrosiaObtainium:this.R_calculateRedAmbrosiaUpgradeValue("redAmbrosiaObtainium"),redAmbrosiaOffering:this.R_calculateRedAmbrosiaUpgradeValue("redAmbrosiaOffering"),redAmbrosiaCubeImprover:this.R_calculateRedAmbrosiaUpgradeValue("redAmbrosiaCubeImprover"),viscount:this.R_calculateRedAmbrosiaUpgradeValue("viscount"),infiniteShopUpgrades:this.R_calculateRedAmbrosiaUpgradeValue("infiniteShopUpgrades"),redAmbrosiaAccelerator:this.R_calculateRedAmbrosiaUpgradeValue("redAmbrosiaAccelerator")},isInsideSingularityChallenge:a.insideSingularityChallenge}}}catch(o){let i=o instanceof Error?`${o.message}
${o.stack}`:String(o);c.error(`Failed to calculate game data for heater export
${i}`,this.context),f.Notify("Failed to calculate game data for heater export",{position:"top",notificationType:"error"});return}}};var xe=class extends C{#e=new Map;#t=[5e3,15e3,3e4,6e4];constructor(e){super(e)}async init(){c.log("Initializing HSWebsocket module",this.context),this.isInitialized=!0}#a(e){let t=this.#e.get(e);if(!t){c.warn(`Tried to reconnect websocket ${e} but it doesn't exist`,this.context);return}this.#e.delete(e),this.registerWebSocket(e,t.regParams)}registerWebSocket(e,t){let a=this;if(this.#e.has(e)){c.warn(`Tried to register websocket ${e} again`,this.context);return}if(!t.url){c.error(`Tried to register websocket ${e} without a URL`,this.context);return}let o={socket:new WebSocket(t.url),reconnectionTries:0,onClose:t.onClose??h.Noop,onOpen:t.onOpen??h.Noop,onMessage:t.onMessage??h.Noop,onRetriesFailed:t.onRetriesFailed??h.Noop,regParams:t},i=async l=>{let u=a.#e.get(e);console.log("WS CLOSED",e,{code:l.code,reason:l.reason,wasClean:l.wasClean,tries:u?.reconnectionTries});let d=a.#t[++o.reconnectionTries];d!==void 0?(c.log(`Reconnecting ${e} in ${d}ms (attempt ${o.reconnectionTries})`,a.context),setTimeout(()=>{a.#a(e)},d)):(c.warn(`WebSocket ${e} failed to reconnect after ${o.reconnectionTries} tries`),await(o.onRetriesFailed??h.Noop)()),await(o.onClose??h.Noop)(l)},n=async l=>{let u=a.#e.get(e);if(!u){c.warnOnce(`wsOnOpen(): Socket ${e} not found`,a.context);return}let d;try{d=JSON.parse(l.data)}catch(g){c.warn(`Failed to parse WebSocket message for ${e}: ${g}`,this.context),d=void 0}await(u.onMessage??h.Noop)(d)},s=async l=>{let u=a.#e.get(e);if(!u){c.warnOnce(`wsOnOpen(): Socket ${e} not found`,a.context);return}u.reconnectionTries=0,c.log(`WebSocket ${e} connected successfully`,a.context),await(u.onOpen??h.Noop)(l)};o.socket.onclose=i,o.socket.onopen=s,o.socket.onmessage=n,this.#e.set(e,o),c.log(`Registered websocket ${e}`,this.context)}unregisterWebSocket(e){let t=this.#e.get(e);t?((t.socket.readyState===WebSocket.OPEN||t.socket.readyState===WebSocket.CONNECTING)&&t.socket.close(),this.#e.delete(e),c.log(`Unregistered websocket ${e}`,this.context)):c.debug(`Could not unregister websocket (Maybe you're not logged in?) ${e}`,this.context)}getWebSocket(e){return this.#e.get(e)}};var Be=class r extends C{static#e;constructor(e){super(e),r.#e=e.context}async init(){let e=this;this.isInitialized=!0}};var Re=class extends C{constructor(){super(...arguments);this.autosingEnabled=!1;this.targetSingularity=0;this.sleepTime=50;this.challengeButtons={};this.ambrosia_early_cube=null;this.ambrosia_late_cube=null;this.ambrosia_quark=null;this.C11Unlocked=!1;this.C12Unlocked=!1;this.C13Unlocked=!1;this.C14Unlocked=!1;this.C15Unlocked=!1}init(){c.log("Initializing HSAutosing module",this.context),this.autosingEnabled=!1,this.targetSingularity=0,this.buildingsTab=document.getElementById("buildingstab"),this.challengeTab=document.getElementById("challengetab"),this.runesTab=document.getElementById("runestab"),this.corruptionsTab=document.getElementById("traitstab"),this.settingsTab=document.getElementById("settingstab"),this.singularityTab=document.getElementById("singularitytab");for(let t=1;t<=15;t++){let a=document.getElementById(`challenge${t}`);a&&(this.challengeButtons[t]=a)}return this.exitTranscBtn=document.getElementById("challengebtn"),this.exitReincBtn=document.getElementById("reincarnatechallengebtn"),this.exitAscBtn=document.getElementById("ascendChallengeBtn"),this.ascendBtn=document.getElementById("ascendbtn"),!this.buildingsTab||!this.challengeTab||!this.settingsTab||!this.singularityTab||!this.challengeButtons||!this.exitAscBtn||!this.exitReincBtn?(this.showError("Error during autosing initialization: could not find main tabs"),Promise.resolve()):(c.log("HSAutosing module initialized",this.context),Promise.resolve())}isAutosingEnabled(){return this.autosingEnabled}subscribeGameDataChanges(){let t=k.getModule("HSGameData");t&&!this.gameDataSubscriptionId&&(this.gameDataSubscriptionId=t.subscribeGameDataChange(this.gameDataCallback.bind(this)),c.log("Subscribed to game data changes for autosing",this.context))}unsubscribeGameDataChanges(){let t=k.getModule("HSGameData");t&&this.gameDataSubscriptionId&&(t.unsubscribeGameDataChange(this.gameDataSubscriptionId),this.gameDataSubscriptionId=void 0,c.log("Unsubscribed from game data changes for autosing",this.context))}gameDataCallback(){return this.gameDataResolver&&(this.gameDataResolver(),this.gameDataResolver=void 0),Promise.resolve()}async enableAutoSing(){h.startDialogWatcher(),this.autosingEnabled=!0;let t=v.getSetting("singularityNumber");this.targetSingularity=t.getValue(),this.subscribeGameDataChanges(),new Promise(i=>{this.gameDataResolver=i});let a=k.getModule("HSGameDataAPI");if(!a)return this.showError("Could not find HSGameDataAPI module"),this.stopAutosing(),Promise.resolve();let o=a.getGameData();return o?this.targetSingularity>o.highestSingularityCount?(c.log("target singularity cannot be bigger than highest singularity"),this.stopAutosing(),Promise.resolve()):(this.unsubscribeGameDataChanges(),this.performAutosingLogic(),c.log(`Autosing enabled for target singularity: ${this.targetSingularity}`,this.context),Promise.resolve()):(this.showError("Could not get game data"),this.stopAutosing(),Promise.resolve())}async disableAutoSing(){return this.autosingEnabled=!1,this.stopAutosing(),c.log("Autosing disabled",this.context),Promise.resolve()}async performAutosingLogic(){let t=v.getSetting("autosingStrategy"),a=t.getValue(),o=t.getDefinition().settingControl;if(!o?.selectOptions)return f.Notify("Strategy selector not available",{notificationType:"warning"}),this.stopAutosing(),Promise.resolve();let i=o.selectOptions.find(d=>d.value.toString()===h.asString(a));if(!i)return f.Notify("Selected strategy not found",{notificationType:"warning"}),this.stopAutosing(),Promise.resolve();let n=v.getStrategies();if(this.strategy=n.find(d=>d.strategyName===i.text),!this.strategy)return f.Notify("Could not find strategy",{notificationType:"warning"}),this.stopAutosing(),Promise.resolve();let s=v.getSetting("autosingEarlyCubeLoadout").getValue(),l=v.getSetting("autosingLateCubeLoadout").getValue(),u=v.getSetting("autosingQuarkLoadout").getValue();if((s===l||s===u||l===u)&&f.Notify("Autosing Ambrosia loadout selection contains the same loadout twice",{notificationType:"warning"}),this.ambrosia_early_cube=document.getElementById(`hs-ambrosia-quickbar-blueberryLoadout${s}`),this.ambrosia_late_cube=document.getElementById(`hs-ambrosia-quickbar-blueberryLoadout${l}`),this.ambrosia_quark=document.getElementById(`hs-ambrosia-quickbar-blueberryLoadout${u}`),!this.ambrosia_early_cube||!this.ambrosia_late_cube||!this.ambrosia_quark)return f.Notify("An ambrosia button not found at the start of singularity",{notificationType:"warning"}),this.stopAutosing(),Promise.resolve();try{let d=0;for(;this.isAutosingEnabled();){await h.click(this.ambrosia_quark),await h.click(this.ascendBtn),await this.performSingularity();let g=!1,m=()=>{let p=document.querySelector("#singularitybtn");if(!p)return!1;let b=window.getComputedStyle(p).getPropertyValue("filter");return!b||b==="none"};for(;this.isAutosingEnabled()&&!g;){if(m()){g=!0,c.debug("Singularity button is now ACTIVATED!");break}let p=await this.getStage();await this.matchStageToStrategy(p),await h.sleep(this.sleepTime)}if(this.isAutosingEnabled()){let p=this.getFinalStrategyPhase();p&&(c.debug(`Singularity activated \u2014 performing final phase: ${p.startPhase}-${p.endPhase}`,this.context),await this.matchStageToStrategy("final"))}d+=1}}catch(d){let g=d instanceof Error?d.message:String(d);this.showError(`Error during autosing logic: ${g}`),this.stopAutosing()}}getFinalStrategyPhase(){return!this.strategy||!this.strategy.strategy.length?null:this.strategy.strategy[this.strategy.strategy.length-1]}getPhaseIndex(t){return N.indexOf(t)}isInChallenge(t){return!!document.getElementById(`challenge${t}`)?.classList.contains("challengeActive")}async matchStageToStrategy(t){if(!t||!this.strategy)return;let a=null;if(t.toLowerCase()==="final"){if(a=this.getFinalStrategyPhase(),!a){c.log("No final strategy phase found",this.context);return}}else{let o=null,i=null;for(let l of N){if(!t.startsWith(`${l}-`))continue;let u=t.slice(l.length+1);if(this.isPhaseOption(u)){o=l,i=u;break}}if(!o||!i){console.log(`Invalid stage format ${t}`);return}let n=this.getPhaseIndex(o),s=this.getPhaseIndex(i);if(n===-1||s===-1){console.log(`Unknown stage ${t}`);return}if(a=this.strategy.strategy.find(l=>{let u=this.getPhaseIndex(l.startPhase),d=this.getPhaseIndex(l.endPhase);return u===-1||d===-1?!1:n>=u&&s<=d})??null,!a){console.log(`No strategy phase matched for stage ${t}`);return}}await this.executePhase(a)}isPhaseOption(t){return N.includes(t)}async executePhase(t){await this.setCorruptions(t.corruptions),this.challengeTab.click();for(let a of t.strat){if(a.challengeNumber>=100){c.debug(`Autosing: Perorming special action: ${a.challengeNumber}`,this.context),await this.performSpecialAction(a.challengeNumber);continue}a.challengeNumber>=11&&a.challengeNumber<=15&&await this.ensureChallengeUnlocked(a.challengeNumber),c.debug(`Autosing: waiting for: ${a.challengeCompletions??0} completions of challenge${a.challengeNumber}, after reaching goal waiting ${a.challengeWaitTime}ms 
MAX TIME: ${a.challengeWaitTime}ms`,this.context),await this.waitForCompletion(a.challengeNumber,a.challengeCompletions??0,a.challengeMaxTime,a.challengeWaitTime),await h.sleep(this.sleepTime)}}async performSpecialAction(t){switch(t){case 101:await h.click(this.exitTranscBtn);break;case 102:await h.click(this.exitReincBtn);break;case 103:await h.click(this.exitAscBtn);break;case 104:await h.click(this.ascendBtn);break;case 105:this.ambrosia_early_cube&&await h.click(this.ambrosia_early_cube);break;case 106:this.ambrosia_late_cube&&await h.click(this.ambrosia_late_cube);break;case 107:this.ambrosia_quark&&await h.click(this.ambrosia_quark);break;default:c.log(`Unknown special action ${t}`,this.context)}}async setCorruptions(t){this.corruptionsTab.click();let a=document.getElementById("corrLoadoutsBtn");if(!a)return this.showError("Error: could not access corruptions loadout button"),Promise.resolve();a.click();let o=document.querySelector("#corruptionLoadoutTable button.corrImport");if(!o)return this.showError("Error: could not access corruption import button"),Promise.resolve();o.click();let i=document.getElementById("prompt_text");if(!i)return this.showError("Error: could not access prompt input"),Promise.resolve();let n={viscosity:t.viscosity,drought:t.drought,deflation:t.deflation,extinction:t.extinction,illiteracy:t.illiteracy,recession:t.recession,dilation:t.dilation,hyperchallenged:t.hyperchallenged};i.value=JSON.stringify(n);let s=document.getElementById("ok_prompt");if(!s)return this.showError("Error: could not access prompt OK button"),Promise.resolve();await h.click(s),await h.click(this.ascendBtn)}stopAutosing(){this.autosingEnabled=!1,this.unsubscribeGameDataChanges(),v.getSetting("startAutosing").disable(),h.stopDialogWatcher()}async getStage(){await h.click(this.settingsTab);let t=document.getElementById("switchSettingSubTab4");t||(this.showError("Error during autosing logic: could not access settings to read stage"),this.stopAutosing()),t.click();let a=document.getElementById("kMisc");a||(this.showError("Error during autosing logic: could not access settings to read stage"),this.stopAutosing()),a.click();let o=await h.waitForElement("gameStageStatistic");o||(this.showError("Error during autosing logic: could not access settings to read stage"),this.stopAutosing()),await h.waitForInnerText(o,s=>/Current Game Section:/.test(s));let i=new RegExp("Current Game Section: (.*)"),n=o.innerText.match(i);return n?n[1].trim():(console.error("Could not find game stage"),"")}async performSingularity(){let t=document.getElementById("oneChallengeCap");t||(this.showError("Error during autosing logic: exalt 2 button not found"),this.stopAutosing()),await h.click(t),await h.click(t),h.click(this.singularityTab);let a=document.getElementById("toggleSingularitySubTab1");a||(this.showError("Error during autosing logic: elevator teleport button button not found"),this.stopAutosing()),await h.click(a);let o=document.getElementById("elevatorTargetInput");o||(this.showError("Error during autosing logic: elevator input not found"),this.stopAutosing()),o.value=this.targetSingularity.toString(),o.dispatchEvent(new Event("input",{bubbles:!0}));let i=document.getElementById("elevatorTeleportButton");i||(this.showError("Error during autosing logic: elevator teleport button button not found"),this.stopAutosing()),await h.click(i);let n=document.getElementById("buycoin1");return n||(this.showError("Error during autosing logic: coin button not found"),this.stopAutosing()),c.debug("Buying first coin building: ",this.context),await h.click(n),await h.sleep(this.sleepTime),this.C11Unlocked=!1,this.C12Unlocked=!1,this.C13Unlocked=!1,this.C14Unlocked=!1,this.C15Unlocked=!1,Promise.resolve()}async waitForCompletion(t,a,o=99999999,i=0){let s=0,l=document.getElementById(`challenge${t}level`),u=this.challengeButtons[t];for(;!this.isInChallenge(t)&&s<o;){if(!this.isAutosingEnabled())return this.stopAutosing(),Promise.resolve();if(!u)return c.debug(`Challenge button ${t} not found`,this.context),Promise.resolve();await h.DblClick(u),await h.sleep(100),s+=100}if(!this.isInChallenge(t))return c.debug(`Timeout: Failed to enter challenge ${t}`,this.context),Promise.resolve();for(;s<o;){if(!this.isAutosingEnabled())return this.stopAutosing(),Promise.resolve();if((l?this.parseNumber(l.innerText):0)>=a)return this.markChallengeUnlocked(t),i>0&&await h.sleep(i),Promise.resolve();await h.sleep(10),s+=10}c.debug(`Timeout: Challenge ${t} failed to reach ${a} completions within ${o}ms`)}parseNumber(t){let a=t.replace(/,/g,"").trim();a.includes("/")&&(a=a.split("/")[0]);let o=Number(a);return isNaN(o)?0:o}showError(t){c.log(t,this.context),f.Notify("Failed to acquire game data for heater export",{position:"top",notificationType:"error"})}isChallengeUnlocked(t){switch(t){case 11:return this.C11Unlocked;case 12:return this.C12Unlocked;case 13:return this.C13Unlocked;case 14:return this.C14Unlocked;case 15:return this.C15Unlocked;default:return!0}}markChallengeUnlocked(t){switch(t){case 10:this.C11Unlocked=!0;break;case 11:this.C12Unlocked=!0;break;case 12:this.C13Unlocked=!0;break;case 13:this.C14Unlocked=!0;break;case 14:this.C15Unlocked=!0;break}}async ensureChallengeUnlocked(t){if(this.isChallengeUnlocked(t))return;c.debug(`Ensuring unlock path for c${t}`);let a={11:[10,104],12:[11,10],13:[12,10],14:[13,10],15:[14,10]},o={viscosity:0,drought:0,deflation:0,extinction:0,illiteracy:0,recession:0,dilation:0,hyperchallenged:0};await this.setCorruptions(o);for(let i=11;i<=t;i++){let n=a[i];if(n)for(let s of n)c.debug(`Running prerequisite c${s} for c${i}`,this.context),s===10?(await this.waitForCompletion(s,1,6e4,500),await h.sleep(this.sleepTime)):(await this.waitForCompletion(s,0,6e4,0),await h.sleep(this.sleepTime))}}};var k=class r{#e="HSModuleManager";#t=[];static#a=new Map;#n={HSUI:f,HSPotions:ie,HSCodes:ne,HSHepteracts:pe,HSTalismans:Ae,HSSettings:v,HSPrototypes:be,HSMouse:Y,HSShadowDOM:fe,HSStorage:ye,HSAmbrosia:Me,HSStats:ve,HSGameState:Ee,HSPatches:Ce,HSGameData:He,HSGameDataAPI:W,HSWebSocket:xe,HSDebug:Be,HSAutosing:Re};constructor(e,t){this.#e=e,this.#t=t,c.log("Enabling Hypersynergism modules",this.#e),this.#t.sort((a,o)=>a.loadOrder===void 0?1:o.loadOrder===void 0?-1:a.loadOrder-o.loadOrder)}async preprocessModules(){let e=[];for(let t of this.#t){if(e.includes(t.className)){c.warn(`Module "${t.className}" is already enabled - there is probably a duplicate module in enabledModules (index.ts)!`,this.#e);return}let a=this.addModule(t);if(t.initImmediate!==void 0&&t.initImmediate===!0&&a&&(await a.init(),t.className==="HSUI")){let o=a;await c.integrateToUI(o)}e.push(t.className)}}addModule(e){if(e.moduleType===2)try{let t=e.moduleName||e.context;if(!e.moduleKind)throw new Error(`Failed to add module ${t} because moduleKind is not defined`);if(!e.scriptContext)throw new Error(`Failed to add module ${t} because scriptContext is not defined`);let a=new oe({moduleName:t,context:e.context,moduleColor:e.moduleColor,moduleKind:e.moduleKind,moduleCSSUrl:e.moduleCSSUrl,moduleScriptUrl:e.moduleScriptUrl,scriptContext:e.scriptContext});return r.#a.set(e.className,a),e.moduleColor&&typeof String.prototype.colorTag=="function"?c.log(`Enabled EXTERNAL module '${t.colorTag(e.moduleColor)}'`,this.#e):c.log(`Enabled EXTERNAL module '${t}'`,this.#e),r.#a.get(e.className)}catch(t){c.warn(`Failed to add module ${e.className}:`,this.#e),console.log(t);return}else try{let t=this.#n[e.className];if(!t)throw new Error(`Class "${e.className}" not found in module`);let a=e.moduleName||e.context,o=new t({moduleName:a,context:e.context,moduleColor:e.moduleColor});return r.#a.set(e.className,o),e.moduleColor&&typeof String.prototype.colorTag=="function"?c.log(`Enabled module '${a.colorTag(e.moduleColor)}'`,e.context):c.log(`Enabled module '${a}'`,e.context),r.#a.get(e.className)}catch(t){c.warn(`Failed to add module ${e.className}:`,this.#e),console.log(t);return}}async initModules(){r.#a.forEach;for(let[e,t]of r.#a.entries())t.isInitialized||await t.init()}getModules(){return r.#a}static getModule(e){return r.#a.get(e)}};var c=class r{static#e="HSLogger";static#t=!1;static#a;static#n=-1;static#i=!1;static#o=new Map;static#s;static async integrateToUI(e){let t=await e.getLogElement();t&&(this.#a=t,this.#t=!0,this.log("HSLogger integrated to UI","HSLogger"))}static#r(e,t="HSMain",a=1){if(this.#t){let o=document.createElement("div");o.classList.add("hs-ui-log-line");let i="";switch(a){case 1:i="";break;case 4:o.classList.add("hs-ui-log-line-info"),i="";break;case 2:i="WARN ",o.classList.add("hs-ui-log-line-warn");break;case 3:i="ERROR ",o.classList.add("hs-ui-log-line-error");break;case 5:i="DBG ",o.classList.add("hs-ui-log-line-debug");break;default:i="";break}let n=this.#i?"":"hs-log-ts-hidden",s=k.getModule(t),l=s&&s.moduleColor?h.parseColorTags(t.colorTag(s.moduleColor)):t;o.innerHTML=`${i} [<span class="hs-log-ctx">${l}</span><span class="hs-log-ts ${n}"> (${h.getTime()})</span>]: ${h.parseColorTags(e)}
`;let u=h.hashCode(`${i}${t}${e}`);if(this.#n!==u)this.#a.appendChild(o);else{let g=this.#a.querySelector("div:last-child");if(g)try{let m=g.innerHTML.match(/\(x(\d+)\)/);if(m){let p=m[0],b=parseInt(m[1],10);g.innerHTML=g.innerHTML.replace(p,`(x${b+1})`)}else g.innerHTML+=" (x2)"}catch(m){console.log(m)}}let d=this.#a.querySelectorAll(".hs-ui-log-line");if(d&&d.length>A.HSLogger.logSize){let g=this.#a.querySelector(".hs-ui-log-line:first-child");g&&g.parentElement?.removeChild(g)}r.scrollToBottom(),this.#n=u}}static#l(e,t){let a=A.HSLogger.logLevel;if(a===1||t)return!0;if(a===6)return!1;switch(e){case 1:return a===4||a===5;case 2:return a===2||a===8;case 3:return a===2||a===3;case 4:return a===7||a===5;case 5:return!1}}static scrollToBottom(){this.#t&&this.#a&&(this.#a.scrollTop=this.#a.scrollHeight)}static log(e,t="HSMain",a=!1){this.#l(1,a)&&(console.log(`[${t}]: ${h.removeColorTags(e)}`),this.#r(e,t,1))}static info(e,t="HSMain",a=!1){this.#l(4,a)&&(console.log(`[${t}]: ${h.removeColorTags(e)}`),this.#r(e,t,4))}static warn(e,t="HSMain",a=!1){this.#l(2,a)&&(console.warn(`[${t}]: ${h.removeColorTags(e)}`),this.#r(e,t,2))}static error(e,t="HSMain",a=!1){this.#l(3,a)&&(console.error(`[${t}]: ${h.removeColorTags(e)}`),this.#r(e,t,3))}static debug(e,t="HSMain",a=!1){let o=v.getSetting("showDebugLogs");o&&o.getValue()&&(console.log(`DBG [${t}]: ${h.removeColorTags(e)}`),this.#r(e,t,5))}static clear(){this.#t&&(this.#a.innerHTML="",r.log("Log cleared",this.#e))}static logOnce(e,t){!this.#o.has(t)||!this.#o.get(t)?.logged?(this.#o.set(t,{logged:!0,timestamp:Date.now(),level:1,count:0}),this.log(e,"Once",!0)):this.#o.get(t).count++,this.#u()}static warnOnce(e,t){!this.#o.has(t)||!this.#o.get(t)?.logged?(this.#o.set(t,{logged:!0,timestamp:Date.now(),level:2,count:0}),this.warn(e,"Once",!0)):this.#o.get(t).count++,this.#u()}static errorOnce(e,t){!this.#o.has(t)||!this.#o.get(t)?.logged?(this.#o.set(t,{logged:!0,timestamp:Date.now(),level:3,count:0}),this.error(e,"Once",!0)):this.#o.get(t).count++,this.#u()}static#u(){}static setTimestampDisplay(e){e?this.#i=!0:this.#i=!1;let t=this.#a.querySelectorAll(".hs-ui-log-line");if(t)for(let a of Array.from(t)){let o=a.querySelector(".hs-log-ts");o&&(e?o.classList.remove("hs-log-ts-hidden"):o.classList.add("hs-log-ts-hidden"))}}};var dt="data:image/png;base64,";var gt="data:image/png;base64,";var ht=` #hs-ambrosia-quick-loadout-container {
    display: flex;
    flex-wrap: wrap;
    justify-content: center;
    padding: 0px;
    width: auto;
    gap: 4px;
    flex-direction: row;
    align-self: flex-end;
    margin: 0px 4% 10px 0px;
    border: 0px solid #4f86f7;
 }

 #hs-ambrosia-extra-btn {
    border: 2px solid orange;
 }`;var Le=class{#e="HSMain";#t;#a;#n;constructor(e){this.#t=new k("HSModuleManager",e)}async preprocessModules(){await this.#t.preprocessModules()}async init(){c.log("Initialising Hypersynergism modules",this.#e),await this.#t.initModules(),c.log("Building UI Panel",this.#e),this.#i(),c.log("Injecting style overrides",this.#e),this.#o(),await v.syncSettings(),await f.Notify(`Hypersynergism v${A.General.currentModVersion} loaded`,{position:"top",notificationType:"success"}),this.#a=setInterval(async()=>{let e=await F.getLatestRelease();e&&c.debug(`Latest release: ${e.name} (${e.version})`,this.#e)},A.PrivateAPI.checkIntervalMs)}#i(){let e=k.getModule("HSUI");e&&(e.updateTitle(`Hypersynergism v${A.General.currentModVersion}`),this.#s(e),this.#r(e),this.#l(e),e.renameTab(2,"Tools"),e.renameTab(3,"Settings"),e.renameTab(4,"Debug"))}#o(){f.injectStyle(ht,"hs-override-css")}#s(e){let t=W.getCalculationDefinitions({toolingSupport:"true"}).map(s=>({text:s.supportsReduce?`${s.calculationName} (C)`:s.calculationName,value:s.supportsReduce?`${s.fnName}|c`:s.fnName}));e.replaceTabContents(2,w.Grid({html:[w.Div({html:"Export tools",styles:{borderBottom:"1px solid limegreen",gridColumn:"span 2"}}),w.Div({id:"hs-panel-amb-heater-p",html:`Export an extended save file string for the <a href="${A.General.heaterUrl}" class="hs-link" target="_blank">Ambrosia Heater.</a>`,styles:{gridColumn:"span 2"}}),w.Button({id:"hs-panel-amb-heater-btn",text:"Ambrosia heater"}),w.Div({html:"References",styles:{borderBottom:"1px solid limegreen",gridColumn:"span 2"}}),w.Button({id:"hs-panel-cor-ref-btn",text:"Corruption Ref."}),w.Button({id:"hs-panel-cor-ref-btn-2",text:"Crpt. Onemind"}),w.Div({html:"Mod links",styles:{borderBottom:"1px solid limegreen",gridColumn:"span 2"}}),w.Button({id:"hs-panel-mod-github-btn",text:"Mod Github"}),w.Button({id:"hs-panel-mod-wiki-btn",text:"Mod Wiki"}),w.Button({id:"hs-panel-mod-wiki_features-btn",text:"Mod Features"}),w.Button({id:"hs-panel-mod-website-btn",text:"Mod Website"}),w.Div({html:"Other tools",styles:{borderBottom:"1px solid limegreen",gridColumn:"span 2"}}),w.Button({id:"hs-panel-dump-settings-btn",text:"Dump Settings"}),w.Button({id:"hs-panel-dump-gamedata-btn",text:"Dump Game vars"}),w.Button({id:"hs-panel-clear-settings-btn",text:"CLEAR SETTINGS",styles:{borderColor:"red"}}),w.Button({id:"hs-panel-check-version-btn",text:"CHECK VERSION"}),w.Div({html:"Testing tools",styles:{borderBottom:"1px solid limegreen",gridColumn:"span 2"}}),w.Button({id:"hs-panel-test-notify-btn",text:"Notify test"}),w.Button({id:"hs-panel-test-notify-long-btn",text:"Notify test 2"}),w.Div({html:"Calculation tools",styles:{borderBottom:"1px solid limegreen",gridColumn:"span 2"}}),w.Div({id:"hs-panel-calc-tools-p",html:`Execute supported calculations and see their results. Calculations denoted with "(C)" support "calculating by components",
                        meaning that the calculation results can be output as an array of components that make up the calculations.<br><br>
                        Note that calculating by components always clears the calculation cache first.`,styles:{gridColumn:"span 2",fontSize:"10pt"}}),w.Select({class:"hs-panel-setting-block-select-input",id:"hs-panel-test-calc-sel",type:4,styles:{gridColumn:"span 2"}},t),w.Button({id:"hs-panel-test-calc-redu-btn",text:"Calculate reduced",styles:{width:"auto"}}),w.Button({id:"hs-panel-test-calc-comps-btn",text:"Calculate components",styles:{width:"auto"}}),w.Button({id:"hs-panel-test-calc-cache-clear-btn",text:"Clear cache"}),w.Button({id:"hs-panel-test-calc-cache-dump-btn",text:"Dump cache"}),w.Div({id:"hs-panel-test-calc-latest",styles:{gridColumn:"span 2"}})],styles:{gridTemplateColumns:"repeat(2, 1fr)",gridTemplateRows:"1fr",columnGap:"5px",rowGap:"10px",padding:"5px"}})),document.querySelector("#hs-panel-amb-heater-btn")?.addEventListener("click",async()=>{let s=k.getModule("HSGameDataAPI");if(s){let l=await s.dumpDataForHeater();if(l){let u=JSON.stringify(l),d=btoa(u),g=h.base64WithCRLF(d);await navigator.clipboard.writeText(g),f.Notify("Ambrosia heater data copied to clipboard",{position:"top",notificationType:"success"})}}}),document.querySelector("#hs-panel-cor-ref-btn")?.addEventListener("click",()=>{e.Modal({htmlContent:`<img class="hs-modal-img" src="${dt}" />`,needsToLoad:!0})}),document.querySelector("#hs-panel-cor-ref-btn-2")?.addEventListener("click",()=>{e.Modal({htmlContent:`<img class="hs-modal-img" src="${gt}" />`,needsToLoad:!0})}),document.querySelector("#hs-panel-mod-github-btn")?.addEventListener("click",()=>{window.open(A.General.modGithubUrl,"_blank")}),document.querySelector("#hs-panel-mod-wiki-btn")?.addEventListener("click",()=>{window.open(A.General.modWikiUrl,"_blank")}),document.querySelector("#hs-panel-mod-wiki_features-btn")?.addEventListener("click",()=>{window.open(A.General.modWikiFeaturesUrl,"_blank")}),document.querySelector("#hs-panel-mod-website-btn")?.addEventListener("click",()=>{window.open(A.General.modWebsiteUrl,"_blank")}),document.querySelector("#hs-panel-dump-settings-btn")?.addEventListener("click",()=>{v.dumpToConsole()}),document.querySelector("#hs-panel-dump-gamedata-btn")?.addEventListener("click",()=>{let s=k.getModule("HSGameDataAPI");s&&(console.log("----- GAME DATA -----"),console.log(s.getGameData()),console.log("----- PSEUDO DATA -----"),console.log(s.getPseudoData()),console.log("----- CAMPAIGN DATA -----"),console.log(s.getCampaignData()),console.log("----- ME DATA -----"),console.log(s.getMeData()),console.log("----- EVENT DATA -----"),console.log(s.getEventData()))}),document.querySelector("#hs-panel-clear-settings-btn")?.addEventListener("click",()=>{let s=k.getModule("HSStorage");s&&(s.clearData(A.HSSettings.storageKey),c.info("Stored settings cleared",this.#e))}),document.querySelector("#hs-panel-check-version-btn")?.addEventListener("click",async()=>{await h.isLatestVersion()?f.Notify("You are using the latest version of Hypersynergism!",{position:"top",notificationType:"success"}):f.Notify("You are not using the latest version of Hypersynergism!",{position:"top",notificationType:"warning"})}),document.querySelector("#hs-panel-test-calc-redu-btn")?.addEventListener("click",()=>{let s=k.getModule("HSGameDataAPI"),l=document.querySelector("#hs-panel-test-calc-sel");if(s&&l){let u=l.value.split("|")[0];if(typeof s[u]=="function"){let d=s[u]();console.log(`--- CALCULATED ${u} ---`),console.log(d);let g=document.querySelector("#hs-panel-test-calc-latest");g&&(g.innerText=`Last calc result: ${h.N(d)}`)}else c.warn(`${u} is not a function`,this.#e)}else c.warn("dataModule or calculation select was null",this.#e)}),document.querySelector("#hs-panel-test-calc-comps-btn")?.addEventListener("click",()=>{let s=k.getModule("HSGameDataAPI"),l=document.querySelector("#hs-panel-test-calc-sel");if(s&&l){let u=l.value.split("|"),d=u[0],g=u.includes("c");if(typeof s[d]=="function")if(g){s.clearCache();let m=s[d](!1),p=document.querySelector("#hs-panel-test-calc-latest");p&&(p.innerText=`Last calc result: [${m.toString().split(",").join(", ")}]`),console.log(`--- CALCULATED ${d} ---`),console.log(m)}else c.warn(`${d} cannot be calculated by components`,this.#e);else c.warn(`${d} is not a function`,this.#e)}else c.warn("dataModule or calculation select was null",this.#e)}),document.querySelector("#hs-panel-test-calc-cache-clear-btn")?.addEventListener("click",()=>{let s=k.getModule("HSGameDataAPI");s&&(c.info("Cleared calculation cache",this.#e),s.clearCache())}),document.querySelector("#hs-panel-test-calc-cache-dump-btn")?.addEventListener("click",()=>{let s=k.getModule("HSGameDataAPI");s&&(c.info("Calculation cache dump",this.#e),s.dumpCache())});let a=["topLeft","top","topRight","right","bottomRight","bottom","bottomLeft","left"],o=["default","warning","error","success"],i=-1,n=-1;document.querySelector("#hs-panel-test-notify-btn")?.addEventListener("click",async()=>{i++,n++,i>a.length-1&&(i=0),n>o.length-1&&(n=0),await f.Notify("Test notification",{position:a[i],notificationType:o[n]})}),document.querySelector("#hs-panel-test-notify-long-btn")?.addEventListener("click",async()=>{i++,n++,i>a.length-1&&(i=0),n>o.length-1&&(n=0),await f.Notify("This is a really very extremely long test notification which tests if the notification works with a long notification test notification ",{position:a[i],notificationType:o[n]})})}#r(e){let t=v.autoBuildSettingsUI();t.didBuild&&(e.replaceTabContents(3,[t.navHTML,t.pagesHTML].join("")),document.delegateEventListener("click",".hs-panel-setting-block-gamedata-icon",a=>{let o=document.querySelector("#hs-panel-settings-subtab-gamedata"),i=o.dataset.color,n=document.querySelector("#settings-grid-gamedata"),s=document.querySelectorAll(".hs-panel-settings-grid"),l=document.querySelectorAll(".hs-panel-subtab");o&&n&&s&&l&&(s.forEach(d=>{d.classList.remove("open")}),l.forEach(d=>{d.style.backgroundColor=""}),n.classList.add("open"),i&&i.length>0&&(o.style.backgroundColor=i)),document.querySelector("#hs-setting-block-gamedata").scrollIntoView({block:"start",behavior:"smooth"})}),document.delegateEventListener("click",".hs-panel-subtab",a=>{let o=a.target,i=o.dataset.subtab,n=o.dataset.color;if(i){let s=`#settings-grid-${i}`,l=document.querySelector(s),u=document.querySelectorAll(".hs-panel-settings-grid"),d=document.querySelectorAll(".hs-panel-subtab");l&&u&&d&&(u.forEach(g=>{g.classList.remove("open")}),d.forEach(g=>{g.style.backgroundColor=""}),l.classList.add("open"),n&&n.length>0&&(o.style.backgroundColor=n))}}))}#l(e){e.replaceTabContents(4,w.Grid({html:[w.Div({html:"Mouse",styles:{borderBottom:"1px solid limegreen",gridColumn:"span 2"}}),w.Div({id:"hs-panel-debug-mousepos"}),w.Div({html:"Game Data",styles:{borderBottom:"1px solid limegreen",gridColumn:"span 2"}}),w.Div({id:"hs-panel-debug-gamedata-currentambrosia",styles:{gridColumn:"span 2"}})],styles:{gridTemplateColumns:"repeat(2, 1fr)",gridTemplateRows:"1fr",columnGap:"5px",rowGap:"5px"}}))}};(async()=>{let r=[{className:"HSPrototypes",context:"HSPrototypes",moduleColor:"crimson",loadOrder:1,initImmediate:!0},{className:"HSUI",context:"HSUI",moduleColor:"royalblue",loadOrder:2,initImmediate:!0},{className:"HSStorage",context:"HSStorage",moduleColor:"wheat",loadOrder:3,initImmediate:!0},{className:"HSSettings",context:"HSSettings",moduleColor:"slategray",loadOrder:4},{className:"HSDebug",context:"HSDebug",moduleColor:"#ff2020",loadOrder:5,initImmediate:!0},{className:"HSWebSocket",context:"HSWebSocket",moduleColor:"#FC427B",loadOrder:6},{className:"HSShadowDOM",context:"HSShadowDOM",moduleColor:"hotpink"},{className:"HSPotions",context:"HSPotions",moduleColor:"darkorange"},{className:"HSCodes",context:"HSCodes",moduleColor:"darkgoldenrod"},{className:"HSHepteracts",context:"HSHepteracts",moduleColor:"slateblue"},{className:"HSTalismans",context:"HSTalismans",moduleColor:"cyan"},{className:"HSMouse",context:"HSMouse",moduleColor:"gold"},{className:"HSAmbrosia",context:"HSAmbrosia",moduleColor:"blueviolet"},{className:"HSStats",context:"HSStats",moduleColor:"lawngreen"},{className:"HSGameState",context:"HSGameState",moduleColor:"indianred"},{className:"HSPatches",context:"HSPatches",moduleColor:"#487eb0"},{className:"HSGameData",context:"HSGameData",moduleColor:"#fbc531"},{className:"HSGameDataAPI",context:"HSGameDataAPI",moduleColor:"#fbc531"},{className:"HSAutosing",context:"HSAutosing",moduleColor:"#e87b06ff"},{className:"Chartist",context:"Chartist",moduleColor:"#e8d7bf",moduleType:2,moduleKind:3,moduleScriptUrl:"https://cdn.jsdelivr.net/npm/chartist@1.3.1/dist/index.umd.min.js",moduleCSSUrl:"https://cdn.jsdelivr.net/npm/chartist@1.3.1/dist/index.min.css",scriptContext:"Chartist"}],e=new Le(r);await e.preprocessModules(),window.hypersynergism=e})();})();
