...
`):n=e.text),y.injectStyle(`#${a} {
            ${p.objectToCSS(e.styles)}
        }`),`<p class="hs-panel-p ${t}" ${a?`id="${a}"`:""}${o} ${i}>${n}</p>`}static Image(e){let t=e.class??"",a=e.id??p.domid(),n=this.#i(e.props),o=this.#t(e.data);if(!e.src)return c.warn("HSUIC.Image: No src provided for image component",this.#e),"";let i=e.width?e.width:b.HSUIC.defaultImageWidth,r=e.height?e.height:b.HSUIC.defaultImageHeight;return y.injectStyle(`#${a} {
            ${p.objectToCSS(e.styles)}
...
@-webkit-keyframes hs-animtext-1 {
...
}`;var dt=`<div id="hs-panel" class="hs-panel-closed">
    ...
    `,d=(k,C)=>{let R=i(k.challengeNumber),B=!!R,H=B?`<strong>${R}</strong>`:`Challenge ${k.challengeNumber}
     (${k.challengeCompletions} completions)`;return`
        ...
        `},m=()=>{let k=[],C=new Map;o.forEach((B,H)=>{if(r(B)){let G=B.ifJump.ifJumpIndex;C.has(G)||C.set(G,[]),C.get(G).push({ifIndex:H,ifId:B.ifJump.id})}}),o.forEach((B,H)=>{C.has(H)&&C.get(H).forEach(G=>{let Z=o[G.ifIndex];k.push(g(G.ifIndex,Z.ifJump.id))}),r(B)?k.push(u(B,H)):k.push(d(B,H))});let R=o.length;return C.has(R)&&C.get(R).forEach(B=>{let H=o[B.ifIndex];k.push(g(B.ifIndex,H.ifJump.id))}),k.join("")},A=()=>{let k=document.getElementById("hs-challenge-list-container");k&&(k.innerHTML=m(),F(),Q())},h={htmlContent:`
    <div id="${n}" class="hs-challenges-modal-container">
        ...
        `,title:o?`Edit Strategy Phase ${n.startPhase}-${n.endPhase}`:"Create Strategy Phase"},A=await s.Modal(m);setTimeout(()=>{let h=document.querySelector(".hs-phase-modal-container");h&&h.addEventListener("click",async w=>{let E=w.target;if(E.id==="hs-autosing-phase-done"){let v=document.getElementById("hs-autosing-end-phase")?.value;if(!v)return;let M={startPhase:n?n.startPhase:i,endPhase:v,corruptions:{...r},strat:[...l]};o&&a?a(M):t(M),y.removeInjectedStyle("hs-phase-modal-styles"),s.CloseModal(A)}E.id==="hs-autosing-phase-corruptions"&&await vt(s,r),E.id==="hs-autosing-phase-challenges"&&await Mt(s,l,n?.startPhase??"error",n?.endPhase??"error")})},0)}var ne=class{static async open(e,t){let a=D.getModule("HSUI");if(!a||!a.uiReady)return;let n=!!e,o=e?{strategyName:e.strategyName,strategy:JSON.parse(JSON.stringify(e.strategy))}:{strategyName:"",strategy:[]},i=()=>{for(let g=1;g<o.strategy.length;g++){let d=o.strategy[g-1],m=o.strategy[g];m.startPhase=d.endPhase}},r=()=>{let g=document.getElementById("hs-autosing-phase-list");if(g){if(o.strategy.length===0){g.innerHTML='<div class="hs-strategy-empty-state">No strategy phases added yet.</div>';return}g.innerHTML=o.strategy.map((d,m)=>`
...
`).map(u=>u.trim());if(a.length===0||a.length>16){y.Notify(`Invalid number of loadouts: ${a.length}. Expected 1-16.`,{notificationType:"warning"});return}let n=document.getElementById("importBlueberries"),o=await S.HookElement("#blueberryToggleMode");if(!n)throw new Error("Import input element not found");if(!o)throw new Error("Mode toggle button not found");o.innerText.includes("LOAD ")&&o.click();let r=0,l=0;p.startDialogWatcher();for(let u=0;u<a.length;u++){let g=a[u],d=u+1;if(!g){l++;continue}let m=document.getElementById(`blueberryLoadout${d}`);if(!m){console.warn(`Loadout button ${d} not found`);continue}let A=new Blob([g],{type:"application/json"}),h=new File([A],"quick-import.json",{type:"application/json"}),w=new DataTransfer;w.items.add(h),n.files=w.files;let E=new Event("change",{bubbles:!0});n.dispatchEvent(E),await p.sleep(50),m.click(),await p.sleep(50),r++}o.click(),y.Notify(`Imported ${r} loadout(s), skipped ${l} empty slot(s)`,{notificationType:"success"})}catch(t){let a=t instanceof Error?t.message:typeof t=="string"?t:"Unknown error";console.error("Quick Import error:",t),c.error(`Quick Import failed: ${a}`,this.context,!0),y.Notify("Quick Import failed",{notificationType:"error"})}finally{await p.stopDialogWatcher(),e&&e.click()}}};var Ge=class extends L{constructor(e){super(e)}async init(){c.log("Initializing HSStats module",this.context),this.isInitialized=!0}};var Ne=class extends L{#e=-1;#t={CubeView:le,SingularityView:ce};#a={MAIN_VIEW:{currentView:new se("unknown"),previousView:new se("unknown"),viewChangeSubscribers:new Map},CUBE_VIEW:{currentView:new le("unknown"),previousView:new le("unknown"),viewChangeSubscribers:new Map},SINGULARITY_VIEW:{currentView:new ce("unknown"),previousView:new ce("unknown"),viewChangeSubscribers:new Map}};#r=["buildings","upgrades","statistics","runes","challenges","research","ants","cubes","campaigns","traits","settings","shop","singularity","event","pseudoCoins"];constructor(e){super(e)}async init(){let e=this;c.log("Initializing HSGameState module",this.context);for(let t of this.#r){let a=document.querySelector(`#${t}`);S.watchElement(a,async n=>{let{view:o,state:i}=n;if(i!=="none"){let r=new se(o);if(r.getId()!==-1)e.#a.MAIN_VIEW.previousView=e.#a.MAIN_VIEW.currentView,e.#a.MAIN_VIEW.currentView=r,c.debug(`Main UI view changed ${e.#a.MAIN_VIEW.previousView.getName()} -> ${e.#a.MAIN_VIEW.currentView.getName()}`,e.context);else{c.warn(`Main UI view ${o} not found`,e.context);return}e.#a.MAIN_VIEW.viewChangeSubscribers.forEach(l=>{try{l(e.#a.MAIN_VIEW.previousView,e.#a.MAIN_VIEW.currentView)}catch(u){c.error(`Error when trying to call MAIN VIEW change subscriber callback: ${u}`,e.context)}}),this.#i(r.getId())}},{characterData:!1,childList:!1,subtree:!1,attributes:!0,attributeOldValue:!1,attributeFilter:["style"],valueParser:(n,o)=>{for(let i of o)if(i.type==="attributes"&&i.attributeName==="style"){let r=i.target,l=r.style.getPropertyValue("display");return{view:r.id,state:l}}}})}b.HSGameState.viewProperties.forEach(async(t,a)=>{for(let n of t.subViewIds){let o=document.querySelector(`#${n}`);S.watchElement(o,async i=>{let{view:r,state:l}=i;if(l!=="none"){let u;try{let d=this.#t[t.viewClassName];if(!d)throw new Error(`Class "${t.viewClassName}" not found in viewClasses for mainViewId ${a}`);u=new d(n)}catch(d){c.warn(`Failed to instantiate sub-view ${t.viewClassName}: ${d}`,e.context);return}let g=u.getViewKey();if(u.getId()!==e.#e){e.#a[g].previousView=e.#a[g].currentView,e.#a[g].currentView=u;let d=e.#a[g].previousView,m=e.#a[g].currentView;e.#a[g].viewChangeSubscribers.forEach(A=>{try{A(d,m)}catch(h){c.error(`Error when trying to call CUBE VIEW change subscriber callback: ${h}`,e.context)}}),c.debug(`Subview changed ${d.getName()} -> ${m.getName()}`,e.context)}else{c.warn(`Subview ${r} not found`,e.context);return}}},{characterData:!1,childList:!1,subtree:!1,attributes:!0,attributeOldValue:!1,attributeFilter:["style"],valueParser:(i,r)=>{for(let l of r)if(l.type==="attributes"&&l.attributeName==="style"){let u=l.target,g=u.style.getPropertyValue("display");return{view:u.id,state:g}}}})}}),this.isInitialized=!0}subscribeGameStateChange(e,t){let a=p.uuidv4();return this.#a[e].viewChangeSubscribers.set(a,t),a}unsubscribeGameStateChange(e,t){this.#a[e].viewChangeSubscribers.has(t)?this.#a[e].viewChangeSubscribers.delete(t):c.warn(`Subscription ID ${t} not found for view key ${e}`,this.context)}async#i(e){let t=this,a=b.HSGameState.viewProperties.get(e);if(!a){c.debug(`No view properties found for main view ID ${e}`,this.context);return}(await S.HookElements(a.subViewsSelector)).forEach(async o=>{let i=o.id;if(o.style.getPropertyValue("display")!=="none"){let l;try{let g=this.#t[a.viewClassName];if(!g)throw new Error(`Class "${a.viewClassName}" not found in viewClasses for mainViewId ${e}`);l=new g(i)}catch(g){c.warn(`Failed to instantiate sub-view ${a.viewClassName} for tab ${i}: ${g}`,t.context);return}let u=l.getViewKey();if(l&&l.getId()!==this.#e){t.#a[u].previousView=t.#a[u].currentView,t.#a[u].currentView=l;let g=t.#a[u].previousView,d=t.#a[u].currentView;t.#a[u].viewChangeSubscribers.forEach(m=>{try{m(g,d)}catch(A){c.error(`Error when trying to call VIEW change subscriber callback: ${A}`,t.context)}})}else c.warn(`Sub-view tab ${i} (for ${a.viewClassName}) resolved to UNKNOWN or failed to initialize.`,t.context)}})}getCurrentUIView(e){return this.#a[e].currentView}},ue=class{#e;#t;constructor(e,t){this.#e=e,this.#t=t}getName(){return this.#e}getViewKey(){return this.#t}},se=class extends ue{#e;constructor(e){super(e,"MAIN_VIEW"),this.#e=this.getViewEnum(e)}getViewEnum(e){switch(e){case"buildings":return 1;case"upgrades":return 2;case"statistics":return 3;case"runes":return 4;case"challenges":return 5;case"research":return 6;case"ants":return 7;case"cubes":return 8;case"campaigns":return 9;case"traits":return 10;case"settings":return 11;case"shop":return 12;case"singularity":return 13;case"event":return 14;case"pseudoCoins":return 15}return-1}getId(){return this.#e}},le=class extends ue{#e;constructor(e){super(e,"CUBE_VIEW"),this.#e=this.getViewEnum(e)}getViewEnum(e){switch(e){case"cubeTab1":return 1;case"cubeTab2":return 2;case"cubeTab3":return 3;case"cubeTab4":return 4;case"cubeTab5":return 5;case"cubeTab6":return 6;case"cubeTab7":return 7}return-1}getId(){return this.#e}},ce=class extends ue{#e;constructor(e){super(e,"SINGULARITY_VIEW"),this.#e=this.getViewEnum(e)}getViewEnum(e){switch(e){case"singularitycontainer1":return 1;case"singularityContainer2":return 2;case"singularityContainer3":return 3;case"singularityContainer4":return 4;case"singularityContainer5":return 5}return-1}getId(){return this.#e}};var K=class{#e;constructor(e){this.#e=e}get patchName(){return this.#e}};var Pe=class extends K{#e=`
        ...
        }`;#t="ambrosiaViewOverflowPatch";async applyPatch(){y.injectStyle(this.#e,this.#t)}async revertPatch(){y.removeInjectedStyle(this.#t)}};var Ue=class extends K{async applyPatch(){}async revertPatch(){}};var Ct=`{
...
}`;var Qe=class extends K{#e;#t;async applyPatch(){let e=this;try{this.#e=JSON.parse(Ct);let t=await S.HookElement("#actualShop");t&&(this.#t||(this.#t=a=>{e.#a(a)}),t.removeDelegateEventListener("mouseover","div",this.#t),t.delegateEventListener("mouseover","div",this.#t))}catch{c.error("Failed to parse shop item map"),this.#e=void 0}}#a(e){let t;if(e.target instanceof HTMLDivElement)t=e.target;else if(e.target instanceof HTMLImageElement||e.target instanceof HTMLParagraphElement||e.target instanceof HTMLButtonElement)t=e.target.parentElement;else return;if(t.id==="actualShop")return;let a=t.querySelector("img");if(a){let n=a.id,o=this.#e?.[n],i=document.querySelector("#shopHovers"),r=i.querySelector("#hs-shopHoverName");if(r)r.innerText=o||"";else{let l=document.createElement("div");l.id="hs-shopHoverName",l.innerText=o||"",i.insertBefore(l,i.querySelector("p:nth-child(2)"))}}}async revertPatch(){if(this.#t){let t=await S.HookElement("#actualShop");t&&t.removeDelegateEventListener("mouseover","div",this.#t)}let e=document.querySelector("#hs-shopHoverName");e&&e.remove()}};var Oe=class extends K{async applyPatch(){let e=await S.HookElement("#buildingstab");e.style.color="red"}async revertPatch(){let e=await S.HookElement("#buildingstab");e.style.color=""}};var Ve=class extends L{#e={AmbrosiaViewOverflow:Pe,TestPatch:Oe,ShopItemNameMapping:Qe,IconSetCaching:Ue};#t={};constructor(e){super(e)}async init(){c.log("Initializing HSPatches module",this.context),this.isInitialized=!0}async applyPatch(e){let t=this.#e[e];if(!t){c.warn(`Patch "${e}" not found`,this.context);return}let a;this.#t[e]?(c.debug(`Patch "${e}" is already instantiated`,this.context),a=this.#t[e]):(a=new t(e),this.#t[e]=a),await a.applyPatch()}async revertPatch(e){let t=this.#e[e];if(!t){c.warn(`Patch "${e}" not found`,this.context);return}let a;this.#t[e]?a=this.#t[e]:(a=new t(e),this.#t[e]=a),await a.revertPatch()}};var Ye=class extends L{#e="Synergysave2";#t;#a;#r=!1;#i;#n;#o=`
        #savegame {
...
${o}`,this.context),y.Notify("Failed to calculate game data for heater export",{position:"top",notificationType:"error"});return}}};var Fe=class extends L{#e=new Map;#t=[5e3,15e3,3e4,6e4];constructor(e){super(e)}async init(){c.log("Initializing HSWebsocket module",this.context),this.isInitialized=!0}#a(e){let t=this.#e.get(e);if(!t){c.warn(`Tried to reconnect websocket ${e} but it doesn't exist`,this.context);return}this.#e.delete(e),this.registerWebSocket(e,t.regParams)}registerWebSocket(e,t){let a=this;if(this.#e.has(e)){c.warn(`Tried to register websocket ${e} again`,this.context);return}if(!t.url){c.error(`Tried to register websocket ${e} without a URL`,this.context);return}let n={socket:new WebSocket(t.url),reconnectionTries:0,onClose:t.onClose??p.Noop,onOpen:t.onOpen??p.Noop,onMessage:t.onMessage??p.Noop,onRetriesFailed:t.onRetriesFailed??p.Noop,regParams:t},o=async l=>{let u=a.#e.get(e);console.log("WS CLOSED",e,{code:l.code,reason:l.reason,wasClean:l.wasClean,tries:u?.reconnectionTries});let g=a.#t[++n.reconnectionTries];g!==void 0?(c.log(`Reconnecting ${e} in ${g}ms (attempt ${n.reconnectionTries})`,a.context),setTimeout(()=>{a.#a(e)},g)):(c.warn(`WebSocket ${e} failed to reconnect after ${n.reconnectionTries} tries`),await(n.onRetriesFailed??p.Noop)()),await(n.onClose??p.Noop)(l)},i=async l=>{let u=a.#e.get(e);if(!u){c.warnOnce(`wsOnOpen(): Socket ${e} not found`,a.context);return}let g;try{g=JSON.parse(l.data)}catch(d){c.warn(`Failed to parse WebSocket message for ${e}: ${d}`,this.context),g=void 0}await(u.onMessage??p.Noop)(g)},r=async l=>{let u=a.#e.get(e);if(!u){c.warnOnce(`wsOnOpen(): Socket ${e} not found`,a.context);return}u.reconnectionTries=0,c.log(`WebSocket ${e} connected successfully`,a.context),await(u.onOpen??p.Noop)(l)};n.socket.onclose=o,n.socket.onopen=r,n.socket.onmessage=i,this.#e.set(e,n),c.log(`Registered websocket ${e}`,this.context)}unregisterWebSocket(e){let t=this.#e.get(e);t?((t.socket.readyState===WebSocket.OPEN||t.socket.readyState===WebSocket.CONNECTING)&&t.socket.close(),this.#e.delete(e),c.log(`Unregistered websocket ${e}`,this.context)):c.debug(`Could not unregister websocket (Maybe you're not logged in?) ${e}`,this.context)}getWebSocket(e){return this.#e.get(e)}};var ze=class s extends L{static#e;constructor(e){super(e),s.#e=e.context}async init(){let e=this;this.isInitialized=!0}};var We=class{constructor(){this.timerDisplay=null;this.timerHeader=null;this.timerContent=null;this.isMinimized=!1;this.isDragging=!1;this.isResizing=!1;this.dragOffset={x:0,y:0};this.resizeStart={width:0,height:0,x:0,y:0};this.timestamps=[];this.quarksHistory=[];this.goldenQuarksHistory=[];this.startTime=0;this.createTimerDisplay(),this.setupDragAndResize()}createTimerDisplay(){this.timerDisplay=document.createElement("div"),this.timerDisplay.id="hs-autosing-timer-display",this.timerDisplay.style.cssText=`
        ...
        `,a.onmousedown=n=>this.startResize(n),this.timerDisplay.appendChild(this.timerHeader),this.timerDisplay.appendChild(this.timerContent),this.timerDisplay.appendChild(a),document.body.appendChild(this.timerDisplay)}setupDragAndResize(){!this.timerHeader||!this.timerDisplay||(this.timerHeader.onmousedown=e=>{(e.target===this.timerHeader||e.target.tagName==="SPAN")&&this.startDrag(e)},document.addEventListener("mousemove",e=>{this.isDragging?this.drag(e):this.isResizing&&this.resize(e)}),document.addEventListener("mouseup",()=>{this.isDragging=!1,this.isResizing=!1}))}startDrag(e){if(!this.timerDisplay)return;this.isDragging=!0;let t=this.timerDisplay.getBoundingClientRect();this.dragOffset.x=e.clientX-t.left,this.dragOffset.y=e.clientY-t.top}drag(e){if(!this.timerDisplay||!this.isDragging)return;let t=e.clientX-this.dragOffset.x,a=e.clientY-this.dragOffset.y;this.timerDisplay.style.left=`${t}px`,this.timerDisplay.style.top=`${a}px`,this.timerDisplay.style.right="auto",this.timerDisplay.style.bottom="auto"}startResize(e){if(!this.timerDisplay)return;e.preventDefault(),this.isResizing=!0;let t=this.timerDisplay.getBoundingClientRect();this.resizeStart={width:t.width,height:t.height,x:e.clientX,y:e.clientY}}resize(e){if(!this.timerDisplay||!this.isResizing)return;let t=e.clientX-this.resizeStart.x,a=e.clientY-this.resizeStart.y,n=Math.max(200,this.resizeStart.width+t),o=Math.max(100,this.resizeStart.height+a);this.timerDisplay.style.width=`${n}px`,this.timerDisplay.style.height=`${o}px`}toggleMinimize(){if(!(!this.timerContent||!this.timerDisplay))if(this.isMinimized=!this.isMinimized,this.isMinimized){this.timerContent.style.display="none",this.timerDisplay.style.height="auto";let e=this.timerHeader?.querySelector("button");e&&(e.textContent="+")}else{this.timerContent.style.display="block",this.timerDisplay.style.height="";let e=this.timerHeader?.querySelector("button");e&&(e.textContent="\u2212")}}start(){this.timestamps=[],this.quarksHistory=[],this.goldenQuarksHistory=[],this.startTime=performance.now()}recordSingularity(e,t){let a=performance.now();this.timestamps.push(a),this.quarksHistory.push(e),this.goldenQuarksHistory.push(t),this.updateDisplay()}getSingularityCount(){return Math.max(0,this.timestamps.length)}getLastDuration(){return this.timestamps.length<2?null:(this.timestamps[this.timestamps.length-1]-this.timestamps[this.timestamps.length-2])/1e3}getAverageLast(e){if(e<=0||this.timestamps.length<=e)return null;let t=0;for(let a=1;a<=e;a++)t+=this.timestamps[this.timestamps.length-a]-this.timestamps[this.timestamps.length-(a+1)];return t/e/1e3}getQuarksPerSecond(e){if(e.length<2)return null;let t=e[0],n=e[e.length-1]-t,o=(this.timestamps[this.timestamps.length-1]-this.startTime)/1e3;return o<=0?null:n/o}getLastQuarksGained(e){return e.length<2?null:e[e.length-1]-e[e.length-2]}formatNumber(e){return e.toExponential(2).replace("+","")}updateDisplay(){if(!this.timerContent)return;let e=this.getSingularityCount(),t=this.getLastDuration(),a=this.getAverageLast(5),n=this.getAverageLast(10),o=this.getAverageLast(50),i=this.getAverageLast(e-1),r=this.getQuarksPerSecond(this.goldenQuarksHistory),l=this.getLastQuarksGained(this.goldenQuarksHistory),u=this.goldenQuarksHistory.length>0?this.goldenQuarksHistory[this.goldenQuarksHistory.length-1]:0,g=this.getQuarksPerSecond(this.quarksHistory),d=this.getLastQuarksGained(this.quarksHistory),m=this.quarksHistory.length>0?this.quarksHistory[this.quarksHistory.length-1]:0,A=`<style>
...
</style>`;A+=`<div style="margin-bottom: 8px; padding-bottom: 8px; border-bottom: 1px solid #444;">
