...
]`;var k=class{static#e="HSUIC";static#t(e){if(!e)return"";let t="";for(let[a,n]of e.entries())t+=`data-${a}="${n}" `;return t}static#a(e){switch(e){case 1:return"checkbox";case 2:return"color";case 3:return"number";case 4:return"text";case 5:return"select";case 6:return"state"}}static#r(e){switch(e){case 1:return"hs-panel-input-checkbox";case 2:return"hs-panel-input-color";case 3:return"hs-panel-input-number";case 4:return"hs-panel-input-text";case 5:return"hs-panel-input-select";case 6:return"hs-panel-input-state"}}static#i(e){if(!e)return"";let t="";for(let[a,n]of Object.entries(e))typeof n!==void 0&&(t+=` ${a}="${n}"`);return t}static Button(e){let t=e.class??"",a=e.text??"",n=e.id??p.domid(),o=this.#i(e.props),r=this.#t(e.data);return S.injectStyle(`#${n} {
            ${p.objectToCSS(e.styles)}
        }`),`<div class="hs-panel-btn ${t}" id="${n}"${o} ${r}>${a}</div>`}static Input(e){let t=e.class??"",a=this.#a(e.type),n=this.#r(e.type),o=e.id??p.domid(),r=this.#i(e.props),i=this.#t(e.data);return S.injectStyle(`#${o} {
            ${p.objectToCSS(e.styles)}
        }`),`<input type="${a}" class="${n} ${t}" id="${o}"${r} ${i}></input>`}static Select(e,t){let a=e.class??"",n=this.#a(e.type),o=this.#r(e.type),r=e.id??p.domid(),i=this.#i(e.props),l=this.#t(e.data);S.injectStyle(`#${r} {
            ${p.objectToCSS(e.styles)}
        }`);let u=t.map(d=>`<option value="${d.value}" ${d.selected?"selected":""}>${d.text}</option>`).join(`
`);return`<select class="${o} ${a}" id="${r}"${i} ${l}>${u}</select>`}static Div(e){let t=e.class??"",a=e.id??p.domid(),n="",o=this.#i(e.props),r=this.#t(e.data);return e.html&&(Array.isArray(e.html)?n=e.html.join(`
`):n=e.html),S.injectStyle(`#${a} {
            ${p.objectToCSS(e.styles)}
        }`),`<div class="hs-panel-div ${t}" ${a?`id="${a}"`:""}${o} ${r}>${n}</div>`}static P(e){let t=e.class??"",a=e.id??p.domid(),n="",o=this.#i(e.props),r=this.#t(e.data);return e.text&&(Array.isArray(e.text)?n=e.text.join(`
`):n=e.text),S.injectStyle(`#${a} {
            ${p.objectToCSS(e.styles)}
        }`),`<p class="hs-panel-p ${t}" ${a?`id="${a}"`:""}${o} ${r}>${n}</p>`}static Image(e){let t=e.class??"",a=e.id??p.domid(),n=this.#i(e.props),o=this.#t(e.data);if(!e.src)return c.warn("HSUIC.Image: No src provided for image component",this.#e),"";let r=e.width?e.width:b.HSUIC.defaultImageWidth,i=e.height?e.height:b.HSUIC.defaultImageHeight;return S.injectStyle(`#${a} {
            ${p.objectToCSS(e.styles)}
        }`),`<img src="${e.src}" width="${r}" height="${i}" class="hs-panel-img ${t}" ${a?`id="${a}"`:""}${n} ${o}/>`}static Grid(e){let t=e.class??"",a=e.id??p.domid(),n="",o=this.#i(e.props),r=this.#t(e.data);return e.html&&(Array.isArray(e.html)?n=e.html.join(`
`):n=e.html),S.injectStyle(`#${a} {
            display: grid;
            ${p.objectToCSS(e.styles)}
        }`),`<div class="hs-panel-div ${t}" id="${a}"${o} ${r}>${n}</div>`}static Flex(e){let t=e.class??"",a=e.id??p.domid(),n="",o=this.#i(e.props),r=this.#t(e.data);return e.html&&(Array.isArray(e.html)?n=e.html.join(`
`):n=e.html),S.injectStyle(`#${a} {
            display: flex;
            ${p.objectToCSS(e.styles)}
        }`),`<div class="hs-panel-div ${t}" id="${a}"${o} ${r}>${n}</div>`}static _modal(e){let t=e.class??"",a=e.htmlContent??"",n=e.data??[],o=e.title??"",r=e.id??p.domid(),i=this.#i(e.props),l=this.#t(e.data);return S.injectStyle(`#${r} {
            ${p.objectToCSS(e.styles)}
        }`),`<div class="hs-modal ${t}" id="${r}"${i} ${l}>
                ...
                </div>`}};var dt=`.hs-panel-body::-webkit-scrollbar,
...
@-webkit-keyframes hs-animtext-1 {
    ...
    `).join(""),n={htmlContent:`
        ...
        `,title:"Configure Corruption Loadout"},o=await s.Modal(n);setTimeout(()=>{document.getElementById("hs-corruption-save-btn")?.addEventListener("click",()=>{Object.keys(fe).forEach(r=>{let i=document.getElementById(`hs-corruption-${r}`),l=Math.max(0,Math.min(16,Number(i?.value)||0));e[r]=l}),S.removeInjectedStyle("hs-corruption-modal-styles"),s.CloseModal(o)})},0)}async function Mt(s,e,t,a){let n="hs-autosing-challenges-modal",o=[...e],r=x=>ie.find(I=>I.value===x)?.label??null,i=x=>x.challengeNumber===200&&x.ifJump!==void 0,l=x=>`${x.toLocaleString()}ms`,u=(x,I)=>`
    ...
    `,g=(x,I)=>{let R=r(x.challengeNumber),B=!!R,D=B?`<strong>${R}</strong>`:`Challenge ${x.challengeNumber}
     (${x.challengeCompletions} completions)`;return`
        ...
        `},m=()=>{let x=[],I=new Map;o.forEach((B,D)=>{if(i(B)){let G=B.ifJump.ifJumpIndex;I.has(G)||I.set(G,[]),I.get(G).push({ifIndex:D,ifId:B.ifJump.id})}}),o.forEach((B,D)=>{I.has(D)&&I.get(D).forEach(G=>{let Z=o[G.ifIndex];x.push(d(G.ifIndex,Z.ifJump.id))}),i(B)?x.push(u(B,D)):x.push(g(B,D))});let R=o.length;return I.has(R)&&I.get(R).forEach(B=>{let D=o[B.ifIndex];x.push(d(B.ifIndex,D.ifJump.id))}),x.join("")},A=()=>{let x=document.getElementById("hs-challenge-list-container");x&&(x.innerHTML=m(),F(),Q())},h={htmlContent:`
    <div id="${n}" class="hs-challenges-modal-container">
        <div class="hs-challenges-input-section">
            ...
            <div class="hs-challenges-input-label">Special Action:</div>
                <select id="hs-challenge-action-select" class="hs-challenges-input">
                    ...
                    ${ie.map(x=>`<option value="${x.value}">${x.label}</option>`).join("")}
                </select>
            </div>
    ...
    </div>`,title:`Configure Strategy Actions (${t}-${a})`},M=await s.Modal(h),E=null,f=null,y=null,w=null,U=!1,P=null,V=(x,I)=>{let R;return function(...B){R||(x.apply(this,B),R=!0,setTimeout(()=>R=!1,I))}},Q=()=>{document.querySelectorAll(".hs-challenge-item").forEach(x=>{let I=x,R=I.dataset.ifId;R&&(I.addEventListener("mouseenter",()=>{document.querySelectorAll(`[data-if-id="${R}"]`).forEach(B=>B.classList.add("hs-jump-highlight"))}),I.addEventListener("mouseleave",()=>{document.querySelectorAll(`[data-if-id="${R}"]`).forEach(B=>B.classList.remove("hs-jump-highlight"))}))})},O=x=>{let I=document.getElementById("hs-challenge-list-container");return I?Array.from(I.querySelectorAll(".hs-challenge-item")).indexOf(x):0},J=x=>{let I=document.getElementById("hs-challenge-list-container");if(!I)return 0;let R=Array.from(I.querySelectorAll(".hs-challenge-item")),B=0;for(let D=0;D<=x&&D<R.length;D++)if(!R[D].dataset.jumpFor){if(D===x)return B;B++}return Math.min(B,o.length)},F=()=>{document.querySelectorAll(".hs-challenge-item").forEach(I=>{let R=I.querySelector(".hs-challenge-drag-handle");R.addEventListener("mousedown",B=>{B.preventDefault();let D=I;if(U=D.dataset.jumpFor!==void 0,U)P=Number(D.dataset.jumpFor),y=null;else if(D.dataset.index!==void 0)y=Number(D.dataset.index),P=null;else return;f=D;let G=D.getBoundingClientRect(),Z=B.clientX-G.left,$=B.clientY-G.top;w=D.cloneNode(!0),w.style.opacity="0.3",w.style.pointerEvents="none";let j=D.getBoundingClientRect().width,Nt=D.getBoundingClientRect().height;D.style.position="fixed",D.style.zIndex="1000",D.style.cursor="grabbing",D.style.pointerEvents="none",D.style.width=j+"px",D.style.height=Nt+"px",D.style.boxSizing="border-box",D.style.boxShadow="0 8px 16px rgba(0,0,0,0.3)",D.style.transform="scale(1.03)",D.style.transition="transform 0.15s ease",D.parentNode?.insertBefore(w,D);let de=(W,ge)=>{f&&(f.style.left=W-Z+"px",f.style.top=ge-$+"px")};de(B.clientX,B.clientY);let $e=V(W=>{let ge=document.getElementById("hs-challenge-list-container");if(ge&&(de(W.clientX,W.clientY),f)){f.style.display="none";let tt=document.elementFromPoint(W.clientX,W.clientY);if(f.style.display="",!tt)return;let te=tt.closest(".hs-challenge-item"),Pt=ge.getBoundingClientRect(),at=W.clientY>Pt.bottom-5;if(te&&te!==f&&te!==w||at){let nt=O(te),Ut=O(f);if(nt===Ut)return;if(U&&P!==null){let T=at?o.length:J(nt),pe=o[P];if(i(pe)){pe.ifJump.ifJumpIndex=T;let X=document.getElementById("hs-challenge-list-container");if(X){let Ke=X.scrollTop;X.innerHTML=m(),X.scrollTop=Ke;let N=document.querySelector(`[data-jump-for="${P}"]`);N&&(f=N,w=N.cloneNode(!0),w.style.opacity="0.3",w.style.pointerEvents="none",N.style.position="fixed",N.style.zIndex="1000",N.style.cursor="grabbing",N.style.pointerEvents="none",N.style.width=N.offsetWidth+"px",N.style.boxShadow="0 8px 16px rgba(0,0,0,0.3)",N.style.transform="scale(1.03)",N.parentNode?.insertBefore(w,N),de(W.clientX,W.clientY)),F(),Q()}}}else if(y!==null&&te.dataset.index!==void 0){let T=Number(te.dataset.index);if(T!==y){let pe=o[y];o.splice(y,1),o.splice(T,0,pe),y=T;let X=document.getElementById("hs-challenge-list-container");if(X){let Ke=X.scrollTop;X.innerHTML=m(),X.scrollTop=Ke;let N=document.querySelector(`[data-index="${y}"]`);N&&!N.dataset.jumpFor&&(f=N,w=N.cloneNode(!0),w.style.opacity="0.3",w.style.pointerEvents="none",N.style.position="fixed",N.style.zIndex="1000",N.style.cursor="grabbing",N.style.pointerEvents="none",N.style.width=N.offsetWidth+"px",N.style.boxShadow="0 8px 16px rgba(0,0,0,0.3)",N.style.transform="scale(1.03)",N.parentNode?.insertBefore(w,N),de(W.clientX,W.clientY)),F(),Q()}}}}}},16),et=()=>{f&&(f.style.position="",f.style.zIndex="",f.style.cursor="",f.style.pointerEvents="",f.style.width="",f.style.boxShadow="",f.style.transform="",f.style.left="",f.style.top="",f.style.transition=""),w&&w.parentNode&&w.parentNode.removeChild(w),f=null,y=null,U=!1,P=null,w=null,document.removeEventListener("mousemove",$e),document.removeEventListener("mouseup",et),Y()};document.addEventListener("mousemove",$e),document.addEventListener("mouseup",et)}),R.style.cursor="grab"})},z=()=>{let x=document.getElementById("hs-challenge-action-select"),I=!!x?.value,R=Number(x.value)===200;["hs-challenge-num-input","hs-challenge-completions-input","hs-challenge-max-time-input","hs-challenge-wait-inside-input"].forEach(D=>{let G=document.getElementById(D);G&&(G.disabled=I,G.parentElement.style.opacity=I?"0.4":"1",G.parentElement.style.display=R?"none":"")}),document.querySelectorAll(".hs-if-jump-row").forEach(D=>{D.style.display=R?"":"none"})},Y=()=>{document.getElementById("hs-challenge-num-input").value="1",document.getElementById("hs-challenge-completions-input").value="0",document.getElementById("hs-challenge-wait-inside-input").value="0",document.getElementById("hs-challenge-wait-before-input").value="0",document.getElementById("hs-challenge-max-time-input").value="1000000",document.getElementById("hs-challenge-action-select").value="",document.getElementById("hs-challenge-add-btn").textContent="Add Action/Challenge",E=null,z()};setTimeout(()=>{let x=document.getElementById(n),I=document.getElementById("hs-challenge-action-select");I?.addEventListener("change",z),F(),Q(),x?.addEventListener("click",R=>{let B=R.target,D=B.id;if(D==="hs-challenge-add-btn"){let G=!!I.value,Z=Number(I.value)===200,$;if(Z){let j=E!==null?o[E]:null;$={challengeNumber:200,challengeCompletions:0,challengeWaitTime:0,challengeMaxTime:0,ifJump:{id:p.uuidv4(),ifJumpChallenge:Number(document.getElementById("hs-if-jump-challenge").value),ifJumpOperator:document.getElementById("hs-if-jump-operator").value,ifJumpValue:Number(document.getElementById("hs-if-jump-value").value),ifJumpIndex:j?.ifJump?.ifJumpIndex??o.length+1}}}else $={challengeNumber:Number(G?I.value:document.getElementById("hs-challenge-num-input").value),challengeCompletions:G?0:Number(document.getElementById("hs-challenge-completions-input").value),challengeWaitTime:Number(document.getElementById("hs-challenge-wait-inside-input").value),challengeWaitBefore:Number(document.getElementById("hs-challenge-wait-before-input").value),challengeMaxTime:G?0:Number(document.getElementById("hs-challenge-max-time-input").value)};E!==null?o[E]=$:o.push($),A(),Y()}if(D.startsWith("hs-challenge-edit-")){E=Number(B.dataset.index);let G=o[E],Z=r(G.challengeNumber);if(i(G)){I.value=String(200),document.getElementById("hs-if-jump-challenge").value=String(G.ifJump?.ifJumpChallenge),document.getElementById("hs-if-jump-operator").value=G.ifJump?.ifJumpOperator??">",document.getElementById("hs-if-jump-value").value=String(G.ifJump?.ifJumpValue),document.getElementById("hs-challenge-wait-inside-input").value=String(G.challengeWaitTime),document.getElementById("hs-challenge-wait-before-input").value=String(G.challengeWaitBefore??0),document.getElementById("hs-challenge-add-btn").textContent="Update Action",z();return}Z?I.value=String(G.challengeNumber):(I.value="",document.getElementById("hs-challenge-num-input").value=String(G.challengeNumber),document.getElementById("hs-challenge-completions-input").value=String(G.challengeCompletions),document.getElementById("hs-challenge-max-time-input").value=String(G.challengeMaxTime)),document.getElementById("hs-challenge-wait-inside-input").value=String(G.challengeWaitTime),document.getElementById("hs-challenge-wait-before-input").value=String(G.challengeWaitBefore??0),document.getElementById("hs-challenge-add-btn").textContent="Update Action",z()}D.startsWith("hs-challenge-delete-")&&(o.splice(Number(B.dataset.index),1),A(),Y()),D==="hs-challenges-save-btn"&&(e.length=0,e.push(...o),s.CloseModal(M)),D==="hs-challenges-cancel-btn"&&s.CloseModal(M)})},0)}async function qe(s,e,t,a,n){let o=!!n,r=e.length?e[e.length-1].endPhase:"start",i=n?.corruptions||{viscosity:0,drought:0,deflation:0,extinction:0,illiteracy:0,recession:0,dilation:0,hyperchallenge:0},l=n?.strat||[],u=q.indexOf(r),g=q.slice(u+1).map((h,M)=>`<option value="${h}" ${n?.endPhase===h||M===0?"selected":""}>${h}</option>`).join(""),m={htmlContent:`
        ...
        `,title:o?`Edit Strategy Phase ${n.startPhase}-${n.endPhase}`:"Create Strategy Phase"},A=await s.Modal(m);setTimeout(()=>{let h=document.querySelector(".hs-phase-modal-container");h&&h.addEventListener("click",async M=>{let E=M.target;if(E.id==="hs-autosing-phase-done"){let y=document.getElementById("hs-autosing-end-phase")?.value;if(!y)return;let w={startPhase:n?n.startPhase:r,endPhase:y,corruptions:{...i},strat:[...l]};o&&a?a(w):t(w),S.removeInjectedStyle("hs-phase-modal-styles"),s.CloseModal(A)}E.id==="hs-autosing-phase-corruptions"&&await vt(s,i),E.id==="hs-autosing-phase-challenges"&&await Mt(s,l,n?.startPhase??"error",n?.endPhase??"error")})},0)}var ne=class{static async open(e,t){let a=H.getModule("HSUI");if(!a||!a.uiReady)return;let n=!!e,o=e?{strategyName:e.strategyName,strategy:JSON.parse(JSON.stringify(e.strategy))}:{strategyName:"",strategy:[]},r=()=>{for(let d=1;d<o.strategy.length;d++){let g=o.strategy[d-1],m=o.strategy[d];m.startPhase=g.endPhase}},i=()=>{let d=document.getElementById("hs-autosing-phase-list");if(d){if(o.strategy.length===0){d.innerHTML='<div class="hs-strategy-empty-state">No strategy phases added yet.</div>';return}d.innerHTML=o.strategy.map((g,m)=>`
        ...
        `}),a=document.querySelector(`#${t} #import-strategy-cancel`);a&&(a.dataset.close=t);let n=document.querySelector(`#${t} #import-strategy-submit`),o=document.querySelector(`#${t} #import-strategy-name`),r=document.querySelector(`#${t} #import-strategy-json`);if(r.addEventListener("input",()=>{if(!o.value.trim())try{let i=JSON.parse(r.value);i&&typeof i.strategyName=="string"&&i.strategyName.trim()&&(o.value=i.strategyName.trim())}catch{}}),!n||!o||!r){S.Notify("Failed to create import modal",{notificationType:"error"});return}n.addEventListener("click",async()=>{let i=o.value.trim(),l=r.value.trim();if(!i){S.Notify("Please enter a strategy name",{notificationType:"warning"});return}if(s.getStrategies().some(g=>g.strategyName===i)){S.Notify(`Strategy "${i}" already exists`,{notificationType:"warning"});return}let d;try{d=JSON.parse(l)}catch{S.Notify("Invalid JSON format",{notificationType:"error"});return}this.validateStrategy(d),d.strategyName=i;try{s.saveStrategiesToStorage(d),S.Notify(`Strategy "${i}" imported successfully`,{notificationType:"success"});let g=document.querySelector(`#${t}`);g&&(await g.transition({opacity:0}),g.parentElement?.removeChild(g))}catch(g){S.Notify("Failed to save strategy",{notificationType:"error"}),c.log(`Import failed: ${g}`,"HSAutosing")}})}else S.Notify("Failed to find HSUI",{notificationType:"error"})}static async editSelectedStrategy(){let e=s.getSetting("autosingStrategy"),t=e.getValue();if(!t||t===""){S.Notify("Please select a strategy to edit",{notificationType:"warning"});return}let a=e.getDefinition().settingControl;if(!a?.selectOptions)return;let n=a.selectOptions.find(i=>i.value.toString()===t);if(!n)return;let r=s.getStrategies().find(i=>i.strategyName===n.text);if(!r){S.Notify("Cannot edit selected strategy",{notificationType:"error"});return}if(r.strategyName=="default_strategy"){S.Notify("cannot edit default strategy");return}await ne.open(r,t)}static saveSettingsToStorage(){let e=H.getModule("HSStorage");if(e){let t=this.#g();e.setData(b.HSSettings.storageKey,t)?c.debug("<green>Settings saved to localStorage</green>",this.#e):c.warn("Could not save settings to localStorage",this.#e)}}#d(){let e=JSON.parse(ct);for(let t of e)t&&s.validateStrategy(t);return e}#h(){let e=JSON.parse(rt);for(let[t,a]of Object.typedEntries(e))if(a){if((a.settingType==="boolean"||p.isBoolean(a.settingValue))&&(a.settingValue=!1),(a.settingType==="selectnumeric"||a.settingType==="selectstring")&&("settingValue"in a||(a.settingValue=""),"settingControl"in a&&a.settingControl)){let n=a.settingControl;"selectOptions"in n&&n.selectOptions&&(n.selectOptions.find(r=>r.value==="")||n.selectOptions.unshift({text:"None",value:""}))}(a.settingType==="numeric"||a.settingType==="selectnumeric"||p.isNumeric(a.settingValue))&&("settingValueMultiplier"in a||(a.settingValueMultiplier=1)),a.settingType==="state"&&("settingValue"in a||(a.settingValue="<red>null</red>")),this.#m(a,s.#a)}return e}#f(){let e=H.getModule("HSStorage");if(!e)return c.warn("Could not find HSStorage module",this.context),null;let t=e.getData(b.HSSettings.strategiesKey);return t?Array.isArray(t)?t:[t]:null}#v(){let e=H.getModule("HSStorage");if(e){let t=e.getData(b.HSSettings.storageKey);return t?JSON.parse(t):(c.warn("Could not load settings from localStorage",this.context),null)}else return c.warn("Could not find HSStorage module",this.context),null}#A(e){for(let t of e)if(t.strategyName===void 0)throw new Error("Strategy name is missing. Cannot add strategy to options.");for(let t of e)s.#p(t)}static#M(e){let a=this.getSetting("autosingStrategy").getDefinition().settingControl;if(!a?.selectOptions)return;let n=a.selectOptions.findIndex(o=>o.text===e);if(n!==-1){let o=a.selectOptions[n].value;a.selectOptions.splice(n,1);let r=document.querySelector(`#${a.controlId}`);if(r){let i=r.querySelector(`option[value="${o}"]`);i&&i.remove()}}}static#p(e){let a=this.getSetting("autosingStrategy").getDefinition().settingControl;if(!a?.selectOptions)return;let n=a.selectOptions.length>0?Math.max(...a.selectOptions.map(r=>Number(r.value)))+1:1;a.selectOptions.push({text:e.strategyName,value:n});let o=document.querySelector(`#${a.controlId}`);if(o){let r=document.createElement("option");r.value=String(n),r.textContent=e.strategyName,o.appendChild(r)}}#c(){let e=this.#h(),t=this.#d();s.#l.push(...t);try{let a=this.#v(),n=this.#f();n&&s.#l.push(...n);let o=JSON.parse(JSON.stringify(e));return a?(c.log("<green>Found settings from localStorage!</green>",this.context),Object.keys(e).forEach(r=>{r in a&&Object.keys(a[r]).forEach(i=>{if(i in e[r]){let l=a[r][i];if(l!==null&&typeof l=="object"&&!Array.isArray(l)&&typeof e[r][i]=="object"&&!Array.isArray(e[r][i])){let u={...e[r][i]};Object.keys(l).forEach(d=>{d in u&&(u[d]=l[d])}),o[r][i]=u}else o[r][i]=l}})}),o):e}catch(a){return c.error("Error while resolving settings",this.context),console.log(a),e}}static async#S(e,t){await t.handleChange(e),this.saveSettingsToStorage()}static async#b(e,t){await t.handleToggle(e),this.saveSettingsToStorage()}static dumpToConsole(){console.log("------------------ HYPERSYNERGISM CURRENT SETTINGS DUMP START ------------------"),this.#t?console.log(this.#t):console.log("NO SETTINGS FOUND (wtf)"),console.log("------------------ HYPERSYNERGISM CURRENT SETTINGS DUMP END ------------------")}};var xe=class extends L{#e;#t;#a=["chronos","hyperrealism","quark","challenge","abyss","accelerator","acceleratorBoost","multiplier"];#r=[];#i=[];#n={chronosHepteract:0,hyperrealismHepteract:0,quarkHepteract:0,challengeHepteract:0,abyssHepteract:0,acceleratorHepteract:0,acceleratorBoostHepteract:0,multiplierHepteract:0};#o={chronosHepteract:null,hyperrealismHepteract:null,quarkHepteract:100,challengeHepteract:null,abyssHepteract:null,acceleratorHepteract:null,acceleratorBoostHepteract:null,multiplierHepteract:null};#s=0;#l=0;#u=0;#m=0;#g=0;#d=`
        ...
        #hs-ratio-container {
            ...
            grid-column-gap: 0px;
            grid-row-gap: 0px;
        }`;#f;#v;#A;#M;#p;#c;#S;#b=!1;#y=!1;constructor(e){super(e),this.#r=this.#a.map(t=>`${t}Hepteract`),this.#i=this.#a.map(t=>`${t}ProgressBarText`)}async init(){let e=this;c.log("Initialising HSHepteracts module",this.context);let t=H.getModule("HSGameState");t&&(t.subscribeGameStateChange("MAIN_VIEW",(a,n)=>{a.getId()===8&&n.getId()!==8&&t.getCurrentUIView("CUBE_VIEW").getId()===7&&e.#S&&(c.debug("Hepteract forge view closed, stopping watch",this.context),v.stopWatching(e.#S))}),t.subscribeGameStateChange("CUBE_VIEW",async(a,n)=>{n.getId()===7?(c.debug("Hepteract forge view opened, starting watch",this.context),e.#p=await v.HookElement("#hepteractQuantity"),e.#S=v.watchElement(e.#p,o=>{try{let r=parseFloat(p.unfuckNumericString(o));e.#c=r}catch{c.error("Failed to parse owned hepteracts",e.context)}e.#y=!1},{greedy:!0,overrideThrottle:!0,valueParser:o=>o.querySelector("span")?.innerText})):a.getId()===7&&e.#S&&(c.debug("Hepteract forge view closed, stopping watch",this.context),v.stopWatching(e.#S))})),this.#e=await v.HookElement("#heptGrid"),this.#t=await v.HookElement("#hepteractCraftTexts"),this.#e.childNodes.forEach(a=>{if(a.nodeType===1){let n=a,o=n.id;if(e.#r.includes(o)){let r=document.querySelector(`#${o}CraftMax`),i=document.querySelector(`#${o}Cap`),l=document.querySelector(`#${o}Image`);n.addEventListener("mouseenter",async u=>{if(o in e.#o&&o==="quarkHepteract")return;let d=document.querySelector("#hepteractCostText");if(d){let m=d.innerText.match(/you\s+(.*?)\s+Hepteracts/i);if(m){let A=m[1];try{if(o in e.#o){let h=p.parseFloat2(A);e.#o[o]=h}}catch{c.warn(`Error while parsing hepteract cost for ${o}`,e.context)}}}}),r&&i&&l&&(l.addEventListener("mouseenter",async u=>{let m=u.target.id.toLowerCase().includes("quark");if(e.#c!==null&&e.#c!==void 0){let A=e.#n[o],h=e.#o[o];if(A===null||h===null)return;let M=C.getSetting("expandCostProtectionDoubleCap"),E=null;if(M.getValue(),E=A*2*h,e.#c===0)e.#w(E,"\u221E",m);else{let f=e.#c>0?E/e.#c:1;e.#w(E,f,m)}}}),l.addEventListener("click",async u=>{let g=u.target.id,m=g.toLowerCase().includes("quark");if(!g||m)return;if(e.#b||e.#y){c.debug(`Quick expand cancelled, another expand was still pending (exp ${e.#b}, wtch: ${e.#y})`,e.context);return}e.#b=!0;let A=null,h=null;if(e.#c!==null&&e.#c!==void 0){if(e.#c===0){c.info("Owned hepteracts is 0",this.context),e.#b=!1;return}let f=e.#n[o],y=e.#o[o];(f===null||y===null)&&c.warn(`Hepteract cost for ${o} not parsed yet`,e.context);let w=C.getSetting("expandCostProtectionDoubleCap"),U=null;w.getValue(),U=f*2,A=f*2*y,h=e.#c>0?A/e.#c:1,c.debug(`
                                    Current max: ${f},
                                    Cube cost: ${p.N(y)},
                                    Next hepts: ${p.N(U)},
                                    Buy cost: ${p.N(A)},
                                    Percent owned: ${p.N(h)},
                                    Double Cap: ${w.getValue()}`,this.context);let P=C.getSetting("expandCostProtection"),V=C.getSetting("expandCostProtectionNotifications"),Q=!(V&&V.getValue()===!0);if(P.isEnabled()){let O=P.getCalculatedValue();if(O&&h>=O){Q&&c.info(`Hept. cost protection: Cost owned ${p.N(h*100)}% >= ${O*100}%`,this.context),e.#b=!1;return}}}else{c.warn("Owned hepteracts not parsed yet",this.context),e.#y=!1,e.#b=!1;return}let M=await v.HookElement("#toggle35");if(M&&M.innerText.includes("ON")&&(c.info("Turned hepteract notification toggle OFF",this.context),M.click()),await p.hiddenAction(async()=>{i.click()},"confirm",!1,25),await p.wait(25),r.click(),A&&h&&e.#w(A,h),await p.wait(5),o!=="quarkHepteract"){let f=document.querySelector("#hepteractCostText");if(f){let w=f.innerText.match(/you\s+(.*?)\s+Hepteracts/i);if(w){let U=w[1];try{if(o in e.#o){let P=p.parseFloat2(U);e.#o[o]=P}}catch{c.warn(`Error while parsing NEW hepteract cost for ${o}`,e.context)}}}}let E=e.#p?e.#p:document.querySelector("#hepteractQuantity");if(E){let f=E.querySelector("span");if(f){let y=f.innerText,w=parseFloat(y);e.#c=w}}e.#b=!1}))}}}),document.querySelectorAll(".heptTypeImage").length>0&&S.injectStyle(`
...
`;var Le=class extends L{#e;#t;#a=null;#r=[];#i=null;#n=null;#o=new Map;#s;#l;#u;#m=new Map;#g=`
    ...
    `;#f="hs-ambrosia-idle-loadout-css";#v="hs-ambrosia-minibar-css";#A=!1;#M;#p;#c;#S=!1;#b;#y;constructor(e){super(e)}async init(){let e=this;c.log("Initializing HSAmbrosia module",this.context),this.#a=await v.HookElement("#blueberryUpgradeContainer"),this.#r=await v.HookElements(".blueberryLoadoutSlot"),this.#i=await v.HookElement("#bbLoadoutContainer"),this.#c=document.querySelector("#hs-panel-debug-gamedata-currentambrosia"),this.loadState(),await this.#B(),c.log("Hooking stuff",this.context);for(let[t,a]of b.HSAmbrosia.ambrosiaLoadoutIcons.entries()){let n=document.querySelector(`#${a.draggableIconId}`);n&&(n.draggable=!0,n.dataset.hsid=t,n.addEventListener("dragstart",o=>{if(!o.dataTransfer)return;let r=o.currentTarget?.dataset.hsid;r&&(c.log(`Drag start ${r}`,this.context),o.dataTransfer.effectAllowed="move",o.dataTransfer.setData("hs-amb-drag",r))}))}if(!this.#i){c.warn("Could not find loadout container",this.context);return}this.#r.forEach(t=>{let a=t.id;if(a){let n=this.#x(a);if(n){let o=this.#o.get(n);o&&this.#N(n,o)}}}),this.#i.delegateEventListener("dragenter",".blueberryLoadoutSlot",t=>{t.dataTransfer&&t.dataTransfer.types.includes("hs-amb-drag")&&(t.preventDefault(),t.stopPropagation(),t.dataTransfer.effectAllowed="move")}),this.#i?.delegateEventListener("dragover",".blueberryLoadoutSlot",t=>{t.dataTransfer&&t.dataTransfer.types.includes("hs-amb-drag")&&(t.preventDefault(),t.stopPropagation(),t.dataTransfer.effectAllowed="move")}),this.#i?.delegateEventListener("drop",".blueberryLoadoutSlot",async t=>{if(t.dataTransfer&&t.dataTransfer.types.includes("hs-amb-drag")){t.preventDefault(),t.stopPropagation();let a=this.#G(t.dataTransfer.getData("hs-amb-drag")),n=t.target,o=n.id;if(!a){c.warn(`Invalid icon ID: ${a}`,this.context);return}if(!b.HSAmbrosia.ambrosiaLoadoutIcons.has(a)){c.warn(`Could not find loadout slot entry for ${a}`,this.context);return}if(!n.classList.contains("blueberryLoadoutSlot")||!o)return;let r=this.#x(o);if(!r){c.warn(`Invalid slot ID: ${o}`,this.context);return}let i=b.HSAmbrosia.ambrosiaLoadoutIcons.get(a);this.#N(r,a),this.#o.set(r,a),this.saveState(),await e.updateQuickBar()}}),this.#i?.delegateEventListener("contextmenu",".blueberryLoadoutSlot",async t=>{t.preventDefault();let a=t.target,n=a.id,o=this.#x(n);if(!o){c.warn(`Invalid slot ID: ${n}`,this.context);return}if(!this.#o.get(o)){c.warn(`No icon found for slot ID: ${n}`,this.context);return}a.classList.remove("hs-ambrosia-slot"),a.style.backgroundImage="",this.#o.delete(o),this.saveState(),await e.updateQuickBar()}),this.#i?.delegateEventListener("click",".blueberryLoadoutSlot",async t=>{let n=t.target.id,o=this.#x(n);if(!o){c.warn(`Invalid slot ID: ${n}`,this.context);return}await e.#w(o)}),this.#n=await v.HookElement("header"),this.#n&&(await this.#E(),await this.#R()),this.isInitialized=!0}async disableBerryMinibars(){this.#n||(this.#n=await v.HookElement("header"));let e=this.#n?.querySelector(`#${b.HSAmbrosia.barWrapperId}`);e?(e.style.display="none",S.removeInjectedStyle(this.#v)):c.warn("Could not find bar wrapper element",this.context),this.#S=!1,this.unsubscribeGameDataChanges()}async enableBerryMinibars(){if(this.#n||(this.#n=await v.HookElement("header")),!this.#n)return;let e=this.#n.querySelector(`#${b.HSAmbrosia.barWrapperId}`);e?(e.style.display="block",S.injectStyle(Et,this.#v),this.subscribeGameDataChanges(),this.#S=!0):c.warn("Could not find minibar wrapper",this.context)}async#E(){if(!this.#n)return;if(this.#n.querySelector(`#${b.HSAmbrosia.barWrapperId}`)){c.warn("Minibar wrapper already exists",this.context);return}let t=(await v.HookElement("#ambrosiaProgressBar")).cloneNode(!0),a=t.querySelector("#ambrosiaProgress"),n=t.querySelector("#ambrosiaProgressText");t.id=b.HSAmbrosia.blueBarId,a.id=b.HSAmbrosia.blueBarProgressId,n.id=b.HSAmbrosia.blueBarProgressTextId;let r=(await v.HookElement("#pixelProgressBar")).cloneNode(!0),i=r.querySelector("#pixelProgress"),l=r.querySelector("#pixelProgressText");r.id=b.HSAmbrosia.redBarId,i.id=b.HSAmbrosia.redBarProgressId,l.id=b.HSAmbrosia.redBarProgressTextId;let u=document.createElement("div");u.id=b.HSAmbrosia.barWrapperId,u.style.display="none",u.appendChild(t),u.appendChild(r);let g=this.#n.querySelector("#hs-ambrosia-quickbar-wrapper")||this.#n.querySelector("nav.navbar");this.#n.insertBefore(u,g),this.#b=a,this.#y=i}async#w(e){this.#s=e;let t=C.getSetting("autoLoadoutState");t&&t.setValue(`<green>${e}</green>`),this.#n||(this.#n=await v.HookElement("header"));let a=this.#n.querySelector(`#${b.HSAmbrosia.quickBarId}`);if(a){a.querySelectorAll(".blueberryLoadoutSlot").forEach(r=>{r.classList.remove("hs-ambrosia-active-slot")});let o=a.querySelector(`[data-original-id="${e}"]`);o&&o.classList.add("hs-ambrosia-active-slot")}else c.warn("Could not find quick bar element",this.context);let n=document.querySelector("#bbLoadoutContainer");if(n){n.querySelectorAll(".blueberryLoadoutSlot").forEach(r=>{r.classList.remove("hs-ambrosia-active-slot")});let o=n.querySelector(`#${e}`);o&&o.classList.add("hs-ambrosia-active-slot")}}async#R(){if(!this.#n)return;let e=this;if(this.#n.querySelector(`#${b.HSAmbrosia.quickBarId}`)){c.debug("Quickbar wrapper already exists",this.context);return}if(this.#i){let a=this.#n.querySelector("nav.navbar"),n=this.#i.cloneNode(!0);n.id=b.HSAmbrosia.quickBarId,n.style.display="none";let o=n.querySelector(".blueberryLoadoutSetting");n.querySelectorAll(".blueberryLoadoutSlot").forEach(i=>{let l=i.id;i.dataset.originalId=l,i.id=`${b.HSAmbrosia.quickBarLoadoutIdPrefix}-${l}`;let u=async function(d){await e.#U(d,l)};this.#m.set(i,u),i.addEventListener("click",u)}),o&&o.remove(),this.#n.insertBefore(n,a),S.injectStyle(this.#g,this.#d),await this.#C(),this.#s&&await this.#w(this.#s)}}async showQuickBar(){this.#n||(this.#n=await v.HookElement("header"));let e=this.#n?.querySelector(`#${b.HSAmbrosia.quickBarId}`);e?(e.style.display="",S.injectStyle(this.#g,this.#d),await this.#C(),this.#s&&await this.#w(this.#s)):c.warn("Could not find quickbar wrapper",this.context)}async hideQuickBar(){this.#n||(this.#n=await v.HookElement("header"));let e=this.#n?.querySelector(`#${b.HSAmbrosia.quickBarId}`);e&&(e.style.display="none",S.removeInjectedStyle(this.#d))}async#C(){let e=this.#n?.querySelector(`#${b.HSAmbrosia.quickBarId}`);if(!e)return;e.querySelectorAll(".blueberryLoadoutSlot").forEach(a=>{let n=a.dataset.originalId;if(!n)return;let o=this.#x(n);if(!o)return;let r=this.#o.get(o);if(r){let i=b.HSAmbrosia.ambrosiaLoadoutIcons.get(r);i&&(a.classList.add("hs-ambrosia-slot"),a.style.backgroundImage=`url(${i.url})`)}else a.classList.remove("hs-ambrosia-slot"),a.style.backgroundImage=""}),this.#s&&await this.#w(this.#s)}#D(e,t){let a=`${b.HSAmbrosia.quickBarLoadoutIdPrefix} -${e} `,n=document.querySelector(`#${a} `);if(!n){c.warn(`Could not find quickbar slot element for ${e}`,this.context);return}let o=b.HSAmbrosia.ambrosiaLoadoutIcons.get(t);if(!o){c.warn(`Could not find icon for ${t}`,this.context);return}n.classList.add("hs-ambrosia-slot"),n.style.backgroundImage=`url(${o.url})`}#G(e){return Object.values(he).find(t=>t===e)}#x(e){return Object.values(me).find(t=>t===e)}#N(e,t){let a=document.querySelector(`#${e} `);if(!a){c.warn(`Could not find slot element for ${e}`,this.context);return}let n=b.HSAmbrosia.ambrosiaLoadoutIcons.get(t);if(!n){c.warn(`Could not find icon for ${t}`,this.context);return}a.classList.add("hs-ambrosia-slot"),a.style.backgroundImage=`url(${n.url})`}async saveState(){let e=H.getModule("HSStorage");if(e){let t=JSON.stringify(Array.from(this.#o.entries()));e.setData(b.HSAmbrosia.storageKey,t)}else c.warn("saveState - Could not find storage module",this.context)}async loadState(){let e=H.getModule("HSStorage");if(e){let a=e.getData(b.HSAmbrosia.storageKey);if(!a){c.warn("loadState - No data found",this.context);return}try{let n=JSON.parse(a);this.#o=new Map(n)}catch{c.warn("loadState - Error parsing data",this.context);return}}else c.warn("loadState - Could not find storage module",this.context);let t=C.getSetting("autoLoadoutState");t&&!this.#s?this.#s=p.removeColorTags(t.getValue()):c.warn("loadState - Could not find autoLoadoutState setting",this.context)}async#B(){let e=await v.HookElement("#importBlueberriesButton");if(!e||document.getElementById("hs-ambrosia-extra-btn"))return;let t=document.createElement("button");t.id="hs-ambrosia-extra-btn",t.className="ambrosiaLoadoutBtn",t.textContent="Quick Import",e.parentElement?.insertBefore(t,e.nextSibling),t.addEventListener("click",()=>this.#L())}async updateQuickBar(){C.getSetting("ambrosiaQuickBar").isEnabled()&&await this.#C()}async enableAutoLoadout(){let e=this,t=C.getSetting("autoLoadoutState");if(t&&t.getValue().includes("Unknown")||!this.#s){let d=C.getSetting("autoLoadout");d&&d.isEnabled()&&d.disable(),c.warn("Could not enable auto loadout - current loadout state is not known!",this.context);return}let a=[v.HookElement("#addCode"),v.HookElement("#addCodeAll"),v.HookElement("#addCodeOne"),v.HookElement("#timeCode")],o=(await Promise.allSettled(a)).map((d,g)=>d.status==="fulfilled"?d.value:null);if(o.some(d=>d===null)){c.warn("Problem with enabling auto loadout",this.context);return}let[r,i,l,u]=o;this.#l||(this.#l=async d=>{await e.#k(d)}),this.#u||(this.#u=async d=>{await e.#P(d)}),r.removeEventListener("click",this.#l,{capture:!0}),r.addEventListener("click",this.#l,{capture:!0}),i.removeEventListener("click",this.#l,{capture:!0}),i.addEventListener("click",this.#l,{capture:!0}),l.removeEventListener("click",this.#l,{capture:!0}),l.addEventListener("click",this.#l,{capture:!0}),u.removeEventListener("click",this.#u,{capture:!0}),u.addEventListener("click",this.#u,{capture:!0}),c.log("Enabled auto loadout",this.context)}async disableAutoLoadout(){let e=this,t=[v.HookElement("#addCode"),v.HookElement("#addCodeAll"),v.HookElement("#addCodeOne"),v.HookElement("#timeCode")],n=(await Promise.allSettled(t)).map((u,d)=>u.status==="fulfilled"?u.value:null);if(n.some(u=>u===null)){c.warn("Problem with disabling auto loadout",this.context);return}let[o,r,i,l]=n;this.#l&&(o.removeEventListener("click",this.#l,{capture:!0}),r.removeEventListener("click",this.#l,{capture:!0}),i.removeEventListener("click",this.#l,{capture:!0})),this.#u&&l.removeEventListener("click",this.#u,{capture:!0}),c.log("Disabled auto loadout",this.context)}async#k(e){let t=this.#s,a=C.getSetting("autoLoadoutAdd");if(t&&a){let n=this.#T(a.getValue()),o=await v.HookElement(`#${n} `);await this.#I(),await p.hiddenAction(async()=>{o.click()})}}async#P(e){let t=this.#s,a=C.getSetting("autoLoadoutTime");if(t&&a){let n=this.#T(a.getValue()),o=await v.HookElement(`#${n} `);await this.#I(),await p.hiddenAction(async()=>{o.click()})}}async#U(e,t){let a=document.querySelector(`#${t} `);a?(await this.#I(),await p.hiddenAction(async()=>{a.click()})):c.warn(`Could not find real button for ${t}`,this.context)}async#I(){let e=await v.HookElement("#blueberryToggleMode");e&&e.innerText.includes("SAVE")&&e.click()}#T(e){let t=Object.values(me).find(a=>a===`blueberryLoadout${e}`);return t||c.warn(`Could not convert loadout ${e} to slot`,this.context),t}subscribeGameDataChanges(){let e=H.getModule("HSGameData");e&&!this.gameDataSubscriptionId&&(this.gameDataSubscriptionId=e.subscribeGameDataChange(this.gameDataCallback.bind(this)),c.debug("Subscribed to game data changes",this.context))}unsubscribeGameDataChanges(){let e=H.getModule("HSGameData");e&&this.gameDataSubscriptionId&&!this.#A&&!this.#S&&(e.unsubscribeGameDataChange(this.gameDataSubscriptionId),this.gameDataSubscriptionId=void 0,c.debug("Unsubscribed from game data changes",this.context))}async gameDataCallback(){let e=H.getModule("HSGameDataAPI");if(!e)return;let t=e.getGameData();if(t&&t.blueberryTime&&t.redAmbrosiaTime){let a=t.blueberryTime,n=t.redAmbrosiaTime,o=e.R_calculateRequiredBlueberryTime(),r=e.R_calculateRequiredRedAmbrosiaTime(),i=a/o*100,l=n/r*100,u=e.calculateAmbrosiaSpeed(),d=e.calculateBlueBerries(),g=u*d,m=t.shopUpgrades.shopAmbrosiaAccelerator,A=e.calculateLuck(),h=t.singularityChallenges.noAmbrosiaUpgrades.completions>0?1:0,M=A.total/100+h,E=(A.total-100*Math.floor(A.total/100))/100,f=0,y=0,w=0,U=g/o*100,P=U,V=1/2+(3/5-1/2)*+(t.singularityChallenges.noAmbrosiaUpgrades.completions>=15)+(2/3-3/5)*+(t.singularityChallenges.noAmbrosiaUpgrades.completions>=19)+(3/4-2/3)*+(t.singularityChallenges.noAmbrosiaUpgrades.completions>=20);if(m>0&&g>0){let Q=o/g;f=Math.min(Q*V,M*.2*m),y=1,w=y/o*100}if(this.#A&&this.#M&&this.#p){let Q=C.getSetting("ambrosiaIdleSwapNormalLoadout"),O=C.getSetting("ambrosiaIdleSwap100Loadout");if(Q&&O){let J=Q.getValue(),F=O.getValue();if(!Number.isInteger(parseInt(J,10))||!Number.isInteger(parseInt(F,10))){let j=C.getSetting("ambrosiaIdleSwap");j&&j.disable(),c.log("Idle swap was disabled due to unconfigured loadouts",this.context);return}let z=this.#T(Q.getValue()),Y=this.#T(O.getValue()),x=P+w,I=x+P,R=100-P,B=100,D=b.HSAmbrosia.idleSwapMinRedTreshold,G=D+b.HSAmbrosia.idleSwapMinRedTreshold,Z=b.HSAmbrosia.idleSwapMaxRedTreshold;if(i>=R&&i<=B||l>=Z&&l<=100){if(this.#s!==Y){let j=await v.HookElement(`#${Y} `);await this.#I(),await p.hiddenAction(async()=>{j.click()})}}else if((i>=x&&i<=I||l>=D&&l<=G)&&this.#s!==z){let j=await v.HookElement(`#${z} `);await this.#I(),await p.hiddenAction(async()=>{j.click()})}}if(this.#c&&S.isModPanelOpen()){let J=document.createElement("div");for(J.innerHTML=`
        BLUE - Value: ${a.toFixed(2)}, Max: ${o}, Percent: ${i.toFixed(2)} <br>
        ...
        ACCEL %: ${w.toFixed(2)} </br>
            `,this.#c.innerHTML="";J.firstChild;)this.#c.appendChild(J.firstChild)}}this.#S?this.#b&&this.#y?(this.#b.style.width=`${i}% `,this.#y.style.width=`${l}% `):c.warnOnce(`
        HSAmbrosia.gameDataCallback() - minibar element(s) undefined.
            blue: ${this.#b},
        red: ${this.#y} `,"hs-minibars-undefined"):c.logOnce("HSAmbrosia.gameDataCallback() - berryMinibarsEnabled was false","hs-minibars-false")}}async enableIdleSwap(){let e=this,t=H.getModule("HSGameState");t?(this.#e=t.subscribeGameStateChange("MAIN_VIEW",this.#V.bind(this)),this.#t=t.subscribeGameStateChange("SINGULARITY_VIEW",async(a,n)=>{n.getId()===5?(this.#M=await v.HookElement("#ambrosiaProgressBar"),this.#p=await v.HookElement("#pixelProgressBar"),this.#A=!0,this.#Q(),this.subscribeGameDataChanges()):(this.#A=!1,this.#H(),this.unsubscribeGameDataChanges())}),t.getCurrentUIView("SINGULARITY_VIEW").getId()===5&&t.getCurrentUIView("MAIN_VIEW").getId()===13&&(this.#M=await v.HookElement("#ambrosiaProgressBar"),this.#p=await v.HookElement("#pixelProgressBar"),this.#A=!0,this.#Q(),this.subscribeGameDataChanges())):c.warn("HSAmbrosia.enableIdleSwap() - gameStateMod==undefined","hs-enable-idleswap-gamestate"),this.#c||(this.#c=document.querySelector("#hs-panel-debug-gamedata-currentambrosia"))}disableIdleSwap(){this.#A=!1,this.unsubscribeGameDataChanges();let e=H.getModule("HSGameState");e?(this.#e&&(e.unsubscribeGameStateChange("MAIN_VIEW",this.#e),this.#e=void 0),this.#t&&(e.unsubscribeGameStateChange("SINGULARITY_VIEW",this.#t),this.#t=void 0)):c.warnOnce("HSAmbrosia.disableIdleSwap() - gameStateMod==undefined","hs-disable-idleswap-gamestate"),this.#H()}#V(e,t){let a=H.getModule("HSGameState");a?e.getId()===13&&t.getId()!==13&&a.getCurrentUIView("SINGULARITY_VIEW").getId()===5&&(this.#A=!1):c.warnOnce("HSAmbrosia.gameStateCallbackMain() - gameStateMod==undefined","hs-amb-gamestate-cb")}#Q(){if(document.querySelector(`#${b.HSAmbrosia.idleSwapIndicatorId} `))return;let t=document.createElement("div");t.id=b.HSAmbrosia.idleSwapIndicatorId,t.innerText="IDLE SWAP ENABLED WHILE IN THIS VIEW",S.injectHTMLElement(t,a=>{let n=document.querySelector("#singularityAmbrosia"),o=document.querySelector("#ambrosiaProgressBar");n?.insertBefore(a,o)}),S.injectStyle(this.#h,this.#f)}#H(){let e=document.querySelector(`#${b.HSAmbrosia.idleSwapIndicatorId} `);e&&e.remove(),S.removeInjectedStyle(this.#f)}async#L(){let e=null;try{e=document.querySelector(".blueberryLoadoutSlot.hs-ambrosia-active-slot");let t=await navigator.clipboard.readText();if(!t||typeof t!="string"){S.Notify("Clipboard does not contain valid loadout data",{notificationType:"warning"});return}let a=t.split(`
`).map(u=>u.trim());if(a.length===0||a.length>16){S.Notify(`Invalid number of loadouts: ${a.length}. Expected 1 - 16.`,{notificationType:"warning"});return}let n=document.getElementById("importBlueberries"),o=await v.HookElement("#blueberryToggleMode");if(!n)throw new Error("Import input element not found");if(!o)throw new Error("Mode toggle button not found");o.innerText.includes("LOAD ")&&o.click();let i=0,l=0;p.startDialogWatcher();for(let u=0;u<a.length;u++){let d=a[u],g=u+1;if(!d){l++;continue}let m=document.getElementById(`blueberryLoadout${g}`);if(!m){console.warn(`Loadout button ${g} not found`);continue}let A=new Blob([d],{type:"application/json"}),h=new File([A],"quick-import.json",{type:"application/json"}),M=new DataTransfer;M.items.add(h),n.files=M.files;let E=new Event("change",{bubbles:!0});n.dispatchEvent(E),await p.sleep(50),m.click(),await p.sleep(50),i++}o.click(),S.Notify(`Imported ${i} loadout(s), skipped ${l} empty slot(s)`,{notificationType:"success"})}catch(t){let a=t instanceof Error?t.message:typeof t=="string"?t:"Unknown error";console.error("Quick Import error:",t),c.error(`Quick Import failed: ${a} `,this.context,!0),S.Notify("Quick Import failed",{notificationType:"error"})}finally{await p.stopDialogWatcher(),e&&e.click()}}};var Re=class extends L{constructor(e){super(e)}async init(){c.log("Initializing HSStats module",this.context),this.isInitialized=!0}};var Ge=class extends L{#e=-1;#t={CubeView:le,SingularityView:ce};#a={MAIN_VIEW:{currentView:new se("unknown"),previousView:new se("unknown"),viewChangeSubscribers:new Map},CUBE_VIEW:{currentView:new le("unknown"),previousView:new le("unknown"),viewChangeSubscribers:new Map},SINGULARITY_VIEW:{currentView:new ce("unknown"),previousView:new ce("unknown"),viewChangeSubscribers:new Map}};#r=["buildings","upgrades","statistics","runes","challenges","research","ants","cubes","campaigns","traits","settings","shop","singularity","event","pseudoCoins"];constructor(e){super(e)}async init(){let e=this;c.log("Initializing HSGameState module",this.context);for(let t of this.#r){let a=document.querySelector(`#${t}`);v.watchElement(a,async n=>{let{view:o,state:r}=n;if(r!=="none"){let i=new se(o);if(i.getId()!==-1)e.#a.MAIN_VIEW.previousView=e.#a.MAIN_VIEW.currentView,e.#a.MAIN_VIEW.currentView=i,c.debug(`Main UI view changed ${e.#a.MAIN_VIEW.previousView.getName()} -> ${e.#a.MAIN_VIEW.currentView.getName()}`,e.context);else{c.warn(`Main UI view ${o} not found`,e.context);return}e.#a.MAIN_VIEW.viewChangeSubscribers.forEach(l=>{try{l(e.#a.MAIN_VIEW.previousView,e.#a.MAIN_VIEW.currentView)}catch(u){c.error(`Error when trying to call MAIN VIEW change subscriber callback: ${u}`,e.context)}}),this.#i(i.getId())}},{characterData:!1,childList:!1,subtree:!1,attributes:!0,attributeOldValue:!1,attributeFilter:["style"],valueParser:(n,o)=>{for(let r of o)if(r.type==="attributes"&&r.attributeName==="style"){let i=r.target,l=i.style.getPropertyValue("display");return{view:i.id,state:l}}}})}b.HSGameState.viewProperties.forEach(async(t,a)=>{for(let n of t.subViewIds){let o=document.querySelector(`#${n}`);v.watchElement(o,async r=>{let{view:i,state:l}=r;if(l!=="none"){let u;try{let g=this.#t[t.viewClassName];if(!g)throw new Error(`Class "${t.viewClassName}" not found in viewClasses for mainViewId ${a}`);u=new g(n)}catch(g){c.warn(`Failed to instantiate sub-view ${t.viewClassName}: ${g}`,e.context);return}let d=u.getViewKey();if(u.getId()!==e.#e){e.#a[d].previousView=e.#a[d].currentView,e.#a[d].currentView=u;let g=e.#a[d].previousView,m=e.#a[d].currentView;e.#a[d].viewChangeSubscribers.forEach(A=>{try{A(g,m)}catch(h){c.error(`Error when trying to call CUBE VIEW change subscriber callback: ${h}`,e.context)}}),c.debug(`Subview changed ${g.getName()} -> ${m.getName()}`,e.context)}else{c.warn(`Subview ${i} not found`,e.context);return}}},{characterData:!1,childList:!1,subtree:!1,attributes:!0,attributeOldValue:!1,attributeFilter:["style"],valueParser:(r,i)=>{for(let l of i)if(l.type==="attributes"&&l.attributeName==="style"){let u=l.target,d=u.style.getPropertyValue("display");return{view:u.id,state:d}}}})}}),this.isInitialized=!0}subscribeGameStateChange(e,t){let a=p.uuidv4();return this.#a[e].viewChangeSubscribers.set(a,t),a}unsubscribeGameStateChange(e,t){this.#a[e].viewChangeSubscribers.has(t)?this.#a[e].viewChangeSubscribers.delete(t):c.warn(`Subscription ID ${t} not found for view key ${e}`,this.context)}async#i(e){let t=this,a=b.HSGameState.viewProperties.get(e);if(!a){c.debug(`No view properties found for main view ID ${e}`,this.context);return}(await v.HookElements(a.subViewsSelector)).forEach(async o=>{let r=o.id;if(o.style.getPropertyValue("display")!=="none"){let l;try{let d=this.#t[a.viewClassName];if(!d)throw new Error(`Class "${a.viewClassName}" not found in viewClasses for mainViewId ${e}`);l=new d(r)}catch(d){c.warn(`Failed to instantiate sub-view ${a.viewClassName} for tab ${r}: ${d}`,t.context);return}let u=l.getViewKey();if(l&&l.getId()!==this.#e){t.#a[u].previousView=t.#a[u].currentView,t.#a[u].currentView=l;let d=t.#a[u].previousView,g=t.#a[u].currentView;t.#a[u].viewChangeSubscribers.forEach(m=>{try{m(d,g)}catch(A){c.error(`Error when trying to call VIEW change subscriber callback: ${A}`,t.context)}})}else c.warn(`Sub-view tab ${r} (for ${a.viewClassName}) resolved to UNKNOWN or failed to initialize.`,t.context)}})}getCurrentUIView(e){return this.#a[e].currentView}},ue=class{#e;#t;constructor(e,t){this.#e=e,this.#t=t}getName(){return this.#e}getViewKey(){return this.#t}},se=class extends ue{#e;constructor(e){super(e,"MAIN_VIEW"),this.#e=this.getViewEnum(e)}getViewEnum(e){switch(e){case"buildings":return 1;case"upgrades":return 2;case"statistics":return 3;case"runes":return 4;case"challenges":return 5;case"research":return 6;case"ants":return 7;case"cubes":return 8;case"campaigns":return 9;case"traits":return 10;case"settings":return 11;case"shop":return 12;case"singularity":return 13;case"event":return 14;case"pseudoCoins":return 15}return-1}getId(){return this.#e}},le=class extends ue{#e;constructor(e){super(e,"CUBE_VIEW"),this.#e=this.getViewEnum(e)}getViewEnum(e){switch(e){case"cubeTab1":return 1;case"cubeTab2":return 2;case"cubeTab3":return 3;case"cubeTab4":return 4;case"cubeTab5":return 5;case"cubeTab6":return 6;case"cubeTab7":return 7}return-1}getId(){return this.#e}},ce=class extends ue{#e;constructor(e){super(e,"SINGULARITY_VIEW"),this.#e=this.getViewEnum(e)}getViewEnum(e){switch(e){case"singularitycontainer1":return 1;case"singularityContainer2":return 2;case"singularityContainer3":return 3;case"singularityContainer4":return 4;case"singularityContainer5":return 5}return-1}getId(){return this.#e}};var K=class{#e;constructor(e){this.#e=e}get patchName(){return this.#e}};var Ne=class extends K{#e=`
        ...
        }`;#t="ambrosiaViewOverflowPatch";async applyPatch(){S.injectStyle(this.#e,this.#t)}async revertPatch(){S.removeInjectedStyle(this.#t)}};var Pe=class extends K{async applyPatch(){}async revertPatch(){}};var It=`{
...
}`;var Ue=class extends K{#e;#t;async applyPatch(){let e=this;try{this.#e=JSON.parse(It);let t=await v.HookElement("#actualShop");t&&(this.#t||(this.#t=a=>{e.#a(a)}),t.removeDelegateEventListener("mouseover","div",this.#t),t.delegateEventListener("mouseover","div",this.#t))}catch{c.error("Failed to parse shop item map"),this.#e=void 0}}#a(e){let t;if(e.target instanceof HTMLDivElement)t=e.target;else if(e.target instanceof HTMLImageElement||e.target instanceof HTMLParagraphElement||e.target instanceof HTMLButtonElement)t=e.target.parentElement;else return;if(t.id==="actualShop")return;let a=t.querySelector("img");if(a){let n=a.id,o=this.#e?.[n],r=document.querySelector("#shopHovers"),i=r.querySelector("#hs-shopHoverName");if(i)i.innerText=o||"";else{let l=document.createElement("div");l.id="hs-shopHoverName",l.innerText=o||"",r.insertBefore(l,r.querySelector("p:nth-child(2)"))}}}async revertPatch(){if(this.#t){let t=await v.HookElement("#actualShop");t&&t.removeDelegateEventListener("mouseover","div",this.#t)}let e=document.querySelector("#hs-shopHoverName");e&&e.remove()}};var Qe=class extends K{async applyPatch(){let e=await v.HookElement("#buildingstab");e.style.color="red"}async revertPatch(){let e=await v.HookElement("#buildingstab");e.style.color=""}};var Oe=class extends L{#e={AmbrosiaViewOverflow:Ne,TestPatch:Qe,ShopItemNameMapping:Ue,IconSetCaching:Pe};#t={};constructor(e){super(e)}async init(){c.log("Initializing HSPatches module",this.context),this.isInitialized=!0}async applyPatch(e){let t=this.#e[e];if(!t){c.warn(`Patch "${e}" not found`,this.context);return}let a;this.#t[e]?(c.debug(`Patch "${e}" is already instantiated`,this.context),a=this.#t[e]):(a=new t(e),this.#t[e]=a),await a.applyPatch()}async revertPatch(e){let t=this.#e[e];if(!t){c.warn(`Patch "${e}" not found`,this.context);return}let a;this.#t[e]?a=this.#t[e]:(a=new t(e),this.#t[e]=a),await a.revertPatch()}};var Ve=class extends L{#e="Synergysave2";#t;#a;#r=!1;#i;#n;#o=`
        #savegame {
...
.hs-export-btn:active {
    transform: translateY(0);
...
</style>`;if(A+=`<div style="margin-bottom: 8px; padding-bottom: 8px; border-bottom: 1px solid #444;">
            <div style="font-size: 11px; color: #888; margin-bottom: 4px;">CURRENT SINGULARITY</div>
            <div style="margin-bottom: 4px;">Time: <span style="color: #00E676; font-weight: bold; font-size: 16px;">${this.currentLiveTime.toFixed(2)}s</span></div>`,this._currentPhaseName){let h=(performance.now()-this.currentPhaseStart)/1e3;A+=`<div>Phase: <span style="color: #FF6B6B; font-weight: bold;">${this._currentPhaseName}</span> (<span style="color: #FFD93D;">${h.toFixed(2)}s</span>)</div>`}if(A+="</div>",A+=`<div style="margin-bottom: 8px; padding-bottom: 8px; border-bottom: 1px solid #444;">
        ...
        </div>`,m>0&&(A+=`<div style="margin-bottom: 8px; padding-bottom: 8px; border-bottom: 1px solid #444;">
